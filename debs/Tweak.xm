...
static BOOL g_lowPowerSimulation    = NO;  // 低电模拟（默认关闭）

// 温度安全阀 — 超过此值不拦截任何保护
static const int64_t kSafetyTempThreshold = 75000;  // 75°C (毫摄氏度)
// 低电模拟时报告的伪温度：58°C — 触发系统约 50% 降频
static const int64_t kLowPowerFakeTemp   = 58000;  // 58°C (毫摄氏度)
...
#define SELECTOR_IS_CPU_MITIGATION(s)  ((s) >= 0x40 && (s) <= 0x5F)
#define SELECTOR_IS_BRIGHTNESS(s)      ((s) == 0x30 || (s) == 0x31)
...
%hookf(kern_return_t, IOConnectCallMethod, mach_port_t connection, uint32_t selector, const uint64_t *input, uint32_t inputCnt, const void *inputStruct, size_t inputStructCnt, uint64_t *output, uint32_t *outputCnt, void *outputStruct, size_t *outputStructCnt) {
    ...
    // ===== CPU 降频控制 (0x40-0x5F) =====
    if (SELECTOR_IS_CPU_MITIGATION(selector)) {
        if (g_lowPowerSimulation) {
            // 低电模拟: 放行 CPU 降频，让系统实际执行
            return %orig;
        }
        if (g_cpuProtection) {
            // 正常模式: 阻止 CPU 降频
    ...
    // ===== 亮度控制 (0x30, 0x31) =====
    if (SELECTOR_IS_BRIGHTNESS(selector) && g_brightnessProtection) {
        // 无论低电模拟与否，始终阻止亮度降低
        return KERN_SUCCESS;
    }
...
static kern_return_t hooked_IOServiceSetProperty(io_service_t service, CFStringRef key, CFTypeRef value) {
    ...
    if (g_cpuProtection && !g_lowPowerSimulation) {
...
// --- IORegistryEntryCreateCFProperty — 按 key 类型分别处理 ---
%hookf(CFTypeRef, IORegistryEntryCreateCFProperty, io_registry_entry_t entry, CFStringRef key, CFAllocatorRef allocator, IOOptionBits options) {
    ...
    if ([ks localizedCaseInsensitiveContainsString:S("temperature")] ||
        ...
        if (g_lowPowerSimulation) {
            // 低电模拟: 报告 58°C 触发自然降频
            int64_t val = kLowPowerFakeTemp;
            return CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt64Type, &val);
        }
        if (g_thermalStateProtection) {
            // 正常模式: 报告 0（正常）
            int zero = 0;
    ...
    // ===== 用户层面热状态 (Nominal/Fair/Serious/Critical) =====
    if ([ks localizedCaseInsensitiveContainsString:S("thermalstate")]) {
        if (g_thermalStateProtection || g_lowPowerSimulation) {
            // 无论低电模拟与否，始终阻止热状态升级
            int zero = 0;
            return CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &zero);
        }
...
- (void)tryTakeAction {
    if (g_enabled && g_cpuProtection && !g_lowPowerSimulation) {
        // 低电模拟时放行热动作，让系统执行降频
        return;
    }
    %orig;
...
static NSDictionary* new_getConfigurationFor(NSString *key) {
    ...
    if (!g_enabled || !g_cpuProtection || g_lowPowerSimulation || !config) return config;
...
%ctor {
    @autoreleasepool {
        ...
        NSLog(@"[CPUthermal] 温控防护已激活 — 安全阀:%d°C CPU性能:%d 亮度:%d 热状态:%d HID:%d CPMS:%d 低电模拟:%d",
              (int)(kSafetyTempThreshold / 1000),
              g_cpuProtection, g_brightnessProtection, g_thermalStateProtection,
              g_blockHidEvents, g_keepCPSMAlive, g_lowPowerSimulation);
