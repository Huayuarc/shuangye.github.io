...
static void EvaluateBattery(void) {
    ...
    BOOL enabled = [prefs[S("smartChargeEnabled")] boolValue] &&
                   ![prefs[S("forceFastChargeIgnoreOptimizedCharging")] boolValue];
    io_service_t service = BatteryService();
    ...
    if (capacity >= 0 && connected) {
        if (capacity >= stopLevel && !currentlyInhibited) SetChargingInhibited(YES);
        else if (capacity <= resumeLevel && currentlyInhibited) SetChargingInhibited(NO);
    }
