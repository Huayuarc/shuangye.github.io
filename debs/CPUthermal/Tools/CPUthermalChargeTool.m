...
static BOOL ApplyStopState(void) {
    ...
    kern_return_t inhibitResult=IORegistryEntrySetCFProperty(service,CFSTR("PredictiveChargingInhibit"),kCFBooleanTrue);
    kern_return_t cutoffResult=IORegistryEntrySetCFProperty(service,CFSTR("ExternalConnected"),kCFBooleanFalse);
    IOObjectRelease(service);
    BOOL ownsInhibit=(inhibitResult==KERN_SUCCESS);
    BOOL ownsInputCutoff=(cutoffResult==KERN_SUCCESS);
    SaveOwnership(ownsInhibit,ownsInputCutoff);
    return ownsInhibit&&ownsInputCutoff;
}
static BOOL ApplyResumeState(void) {
    ...
    kern_return_t cutoffResult=KERN_SUCCESS;
    // 线缆仍实际存在时才恢复连接；已拔线时交由下次系统握手自然建立。
    if(ownsInputCutoff&&AdapterConnected(properties))
        cutoffResult=IORegistryEntrySetCFProperty(service,CFSTR("ExternalConnected"),kCFBooleanTrue);
    kern_return_t inhibitResult=ownsInhibit?IORegistryEntrySetCFProperty(service,CFSTR("PredictiveChargingInhibit"),kCFBooleanFalse):KERN_SUCCESS;
    IOObjectRelease(service);
    if(cutoffResult==KERN_SUCCESS&&inhibitResult==KERN_SUCCESS)SaveOwnership(NO,NO);
    return cutoffResult==KERN_SUCCESS&&inhibitResult==KERN_SUCCESS;
}
