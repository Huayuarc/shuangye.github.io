...
static BOOL ApplyStopState(void) {
    ...
    // 不设置 PredictiveChargingInhibit：部分电池工具会把该系统暂停位误译为“温度过高”。
    // 阈值停充只切断电池侧输入连接，系统底层真实温度保护仍由 BMS 独立处理。
    NSDictionary *values=@{S("PredictiveChargingInhibit"):@NO,S("IsCharging"):@YES,S("ExternalConnected"):@NO};
...
static BOOL ApplyResumeState(void) {
    ...
    // 无条件清除旧版本遗留的系统抑制位，消除“电池温度过高”误报。
    values[S("PredictiveChargingInhibit")]=@NO;
...
static void EvaluateBattery(BOOL preferenceEvent) {
    ...
    BOOL bypassTemperature=[prefs[S("bypassBatteryChargeTemperature")] boolValue];
    BOOL forceFastCharge=[prefs[S("forceFastChargeIgnoreHeat")] boolValue];
    NSInteger stopLevel=MAX(70,MIN(100,IntegerValue(prefs,S("smartChargeStopLevel"),80)));
    // 开启温度检测屏蔽或强制满血快充时立即清除暂停原因并恢复电池输入，由 powerd 重新评估充电。
    if(preferenceEvent&&(bypassTemperature||forceFastCharge))ResetOwnedState();
    NSDictionary *state=OwnedState();
    BOOL stopped=[state[S("ownsInhibit")] boolValue]||[state[S("ownsInputCutoff")] boolValue];
    // 温度屏蔽/强制快充优先于智能停充：清理插件自有断流后保持只读，不再到阈值重新断开输入。
    if(bypassTemperature||forceFastCharge) {
        if(stopped)ApplyResumeState();
        gLastEnabled=enabled; gLastStopLevel=stopLevel; return;
    }
...
static BOOL ResetOwnedState(void) {
    // 安装升级时即使所有权文件丢失，也清理旧版遗留的暂停位并恢复已连接输入。
    io_service_t service=BatteryService(); if(service==IO_OBJECT_NULL)return NO;
    NSDictionary *properties=BatteryProperties(service);
    NSMutableDictionary *values=[@{S("PredictiveChargingInhibit"):@NO,S("IsCharging"):@YES} mutableCopy];
    if(AdapterConnected(properties))values[S("ExternalConnected")]=@YES;
    kern_return_t result=IORegistryEntrySetCFProperties(service,(__bridge CFTypeRef)values);
    IOObjectRelease(service);
    if(result==KERN_SUCCESS)SaveOwnership(NO,NO);
    return result==KERN_SUCCESS;
