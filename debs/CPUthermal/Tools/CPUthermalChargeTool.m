...
static BOOL RestoreCharging(void) {
    if (SetSmartBatteryState(YES, YES) != 0) return NO;
    gLimited = NO;
    return YES;
}
...
static void EvaluateBattery(void) {
    ...
    stopLevel = MAX(70, MIN(100, stopLevel));
    // 面板只保留停充阈值；恢复阈值固定为低 5%，形成稳定回差并避免临界电量反复启停。
    ...
    if (capacity >= stopLevel && connected) {
        ...
        if (SetSmartBatteryState(NO, NO) == 0) gLimited = YES;
...
int main(int argc, char **argv) {
    @autoreleasepool {
        if (argc > 1 && strcmp(argv[1], "reset") == 0) return RestoreCharging() ? 0 : 2;
