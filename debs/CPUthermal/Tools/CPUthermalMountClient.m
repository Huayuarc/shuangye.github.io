...
int main(int argc,char *argv[]){@autoreleasepool{
 ...
 for(int i=0;i<100;i++){ NSDictionary *result=[NSDictionary dictionaryWithContentsOfFile:response]; if(result){int code=[result[S("result")] intValue];NSString *storage=[result[S("storagePath")] isKindOfClass:[NSString class]]?result[S("storagePath")]:nil;if([cmd isEqualToString:S("storage-path")]&&code==0&&storage.length)printf("%s\n",storage.UTF8String);[fm removeItemAtPath:response error:nil];return code;} usleep(50000); }
