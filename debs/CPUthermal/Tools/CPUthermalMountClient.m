...
int main(int argc,char *argv[]){@autoreleasepool{
 ...
 if(![body writeToFile:request atomically:YES])return 121; chmod(request.fileSystemRepresentation,0666);
 notify_post("com.huayuarc.cputhermal/mountRequest");
 for(int i=0;i<100;i++){ NSDictionary *result=[NSDictionary dictionaryWithContentsOfFile:response]; if(result){int code=[result[S("result")] intValue];[fm removeItemAtPath:response error:nil];return code;} usleep(50000); }
 [fm removeItemAtPath:request error:nil]; return 124;
}}
