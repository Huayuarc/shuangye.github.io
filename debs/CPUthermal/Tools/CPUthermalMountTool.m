...
static int BindfsLayerCount(NSString *path) {
    ...
    for(int i=0;i<count;i++)
        if(strcmp(mounts[i].f_mntonname,path.fileSystemRepresentation)==0&&strcmp(mounts[i].f_fstypename,"bindfs")==0)layers++;
    return layers;
}
// 重复挂载会在同一挂载点叠出多层 bindfs（现场出现过 12 层），这里一次清干净。
static int UnmountAllLayers(NSString *path) {
    for(int round=0;round<32;round++){
        if(BindfsLayerCount(path)<=0)return 0;
        int r=UnmountPath(path);
        if(r)return r;
    }
...
static int MountPath(NSString *path) {
    ...
    if (IsBindfsMounted(path)) {
        ...
        if ((current.f_flags&MNT_RDONLY)||!sourceMatches||BindfsLayerCount(path)>1) { int u=UnmountAllLayers(path); if(u)return u; }
...
static int RemovePath(NSString *path) {
    int r=UnmountAllLayers(path); if(r)return r;
    NSFileManager *fm=NSFileManager.defaultManager;
    NSString *storage=StoragePath(path);
...
static int RemountAll(void) { int last=0; for(NSString *p in LoadPaths()){ if(BindfsLayerCount(p)>1){int u=UnmountAllLayers(p); if(u){last=u;continue;}} int r=MountPath(p);if(r)last=r;} return last; }
...
// ============================================================================
// 内置 Disabled 守护进程清单
//   把包内的 disabled.plist 写入系统 launchd 的 disabled.plist，
//   文件属主/属组固定为 root:wheel、权限 0644；写入前备份原文件，可随时恢复。
// ============================================================================
static NSString *DisabledBackupRoot(void){
    NSString *root=MountStorageRoot();
    return root.length?[root stringByAppendingPathComponent:S("var/lib/cputhermal-disabled-backup")]:nil;
}
static NSString *DisabledMountDirectory(void){
    return S("/var/db/com.apple.xpc.launchd");
}
// 真实系统文件路径候选：RootHide 的真实根视图在 /rootfs 下，同时兼容常规路径。
// 优先写“文件已存在”的那一个，避免写到隐根里的假路径。
static NSArray<NSString*> *DisabledSystemTargets(void){
    NSFileManager *fm=NSFileManager.defaultManager;
    NSString *name=S("disabled.plist");
    NSMutableArray *candidates=[NSMutableArray array];
    [candidates addObject:[S("/rootfs/private/var/db/com.apple.xpc.launchd") stringByAppendingPathComponent:name]];
    [candidates addObject:[S("/rootfs/var/db/com.apple.xpc.launchd") stringByAppendingPathComponent:name]];
    [candidates addObject:[S("/private/var/db/com.apple.xpc.launchd") stringByAppendingPathComponent:name]];
    [candidates addObject:[S("/var/db/com.apple.xpc.launchd") stringByAppendingPathComponent:name]];
    NSMutableArray *existing=[NSMutableArray array], *usable=[NSMutableArray array];
    NSMutableSet *seen=[NSMutableSet set];
    for(NSString *candidate in candidates){
        NSString *dir=[candidate stringByDeletingLastPathComponent];
        if(![fm fileExistsAtPath:dir])continue;                  // 目录不存在则跳过
        char resolved[4096]={0};
        NSString *key=realpath(dir.fileSystemRepresentation,resolved)?S(resolved):dir;
        if([seen containsObject:key])continue;                   // 同一实际目录只写一次
        [seen addObject:key];
        [usable addObject:candidate];
        if([fm fileExistsAtPath:candidate])[existing addObject:candidate];
    }
    return existing.count?existing:usable;
}
static NSString *BuiltinDisabledSource(void){
    NSFileManager *fm=NSFileManager.defaultManager;
    NSMutableArray *candidates=[NSMutableArray array];
    NSString *root=MountStorageRoot();
    if(root.length)[candidates addObject:[root stringByAppendingPathComponent:S("usr/local/share/CPUthermal/disabled.plist")]];
    NSString *mapped=CPUthermalJBRootPathForRootFSPath("/usr/local/share/CPUthermal/disabled.plist");
    if(mapped.length)[candidates addObject:mapped];
    [candidates addObjectsFromArray:@[
        S("/usr/local/share/CPUthermal/disabled.plist"),
        S("/var/jb/usr/local/share/CPUthermal/disabled.plist")
    ]];
    NSMutableSet *seen=[NSMutableSet set];
    for(NSString *p in candidates){
        if(!p.length||[seen containsObject:p])continue;
        [seen addObject:p];
...
static void FixDisabledPermissions(NSString *path){
    chown(path.fileSystemRepresentation,0,0);
    chmod(path.fileSystemRepresentation,0644);
}
static int VerifyDisabledFile(NSString *path){
    struct stat st={0};
    if(stat(path.fileSystemRepresentation,&st)!=0)return errno;
    if(st.st_uid!=0||st.st_gid!=0||(st.st_mode&07777)!=0644)return EPERM;
    return 0;
}
// 该目录原本不需要挂载即可修改；这里撤销旧版本建立的 bindfs 挂载，
// 并恢复目录本身的属主/属组与访问权限，解除 Filza 中出现的只读锁定。
static int RemoveDisabledMountIfNeeded(void){
    NSString *dir=DisabledMountDirectory();
    NSFileManager *fm=NSFileManager.defaultManager;
    if(IsMounted(dir,NO)){
        int r=RemovePath(dir);                 // 卸载并删除挂载记录与后备副本
        if(r)return r;
    } else if([LoadPaths() containsObject:dir]){
        NSMutableArray *a=LoadPaths(); [a removeObject:dir]; SavePaths(a);
    }
    if([fm fileExistsAtPath:dir]){
        chown(dir.fileSystemRepresentation,0,0);
        chmod(dir.fileSystemRepresentation,0755);
    }
    return 0;
}
static NSString *DisabledManifestBackup(NSString *backupRoot, NSDictionary *manifest, NSString *target){
    NSString *name=manifest[target];
    if(!name.length&&manifest.count==1)name=[[manifest allValues] firstObject];   // 兼容旧版记录
    return name.length?[backupRoot stringByAppendingPathComponent:name]:nil;
}
static int ApplyDisabledDaemons(void){
    int cleanup=RemoveDisabledMountIfNeeded(); if(cleanup)return cleanup;
    NSString *source=BuiltinDisabledSource(); if(!source.length)return ENOENT;
    NSData *data=[NSData dataWithContentsOfFile:source]; if(!data.length)return EIO;
    NSArray *targets=DisabledSystemTargets(); if(!targets.count)return ENOENT;
    NSFileManager *fm=NSFileManager.defaultManager; NSString *backupRoot=DisabledBackupRoot();
    ...
    int applied=0,last=0;
    for(NSString *target in targets){
        if([fm fileExistsAtPath:target]&&backupRoot.length&&!manifest[target]){
            NSString *name=DisabledBackupName(target);
            if([fm copyItemAtPath:target toPath:[backupRoot stringByAppendingPathComponent:name] error:nil])manifest[target]=name;
        }
        if(![data writeToFile:target atomically:YES]){last=EIO;continue;}
        FixDisabledPermissions(target);
        int check=VerifyDisabledFile(target); if(check){last=check;continue;}
        applied++;
    }
    if(manifestPath.length&&manifest.count)[manifest writeToFile:manifestPath atomically:YES];
    if(!applied)return last?last:ENOENT;
    return last;
}
static int RestoreDisabledDaemons(void){
    int cleanup=RemoveDisabledMountIfNeeded(); if(cleanup)return cleanup;
    NSString *backupRoot=DisabledBackupRoot(); if(!backupRoot.length)return ENOENT;
    NSDictionary *manifest=[NSDictionary dictionaryWithContentsOfFile:[backupRoot stringByAppendingPathComponent:S("manifest.plist")]];
    if(!manifest.count)return ENOENT;
    NSArray *targets=DisabledSystemTargets(); if(!targets.count)return ENOENT;
    NSFileManager *fm=NSFileManager.defaultManager; int restored=0,last=0;
    for(NSString *target in targets){
        NSString *backup=DisabledManifestBackup(backupRoot,manifest,target);
        if(!backup.length||![fm fileExistsAtPath:backup]){last=ENOENT;continue;}
        [fm removeItemAtPath:target error:nil];
        if(![fm copyItemAtPath:backup toPath:target error:nil]){last=EIO;continue;}
        FixDisabledPermissions(target);
        int check=VerifyDisabledFile(target); if(check){last=check;continue;}
        restored++;
    }
    if(!restored)return last?last:ENOENT;
    return last;
}
...
// 取消挂载并恢复原文件：清除该路径全部 bindfs 层，删除记录与后备副本，
// 并把系统原文件与原目录权限恢复回来。
static int UnmountAndRestore(NSString *path){
    int r=UnmountAllLayers(path); if(r)return r;
    if([path isEqualToString:DisabledMountDirectory()]){
        NSFileManager *fm=NSFileManager.defaultManager;
        if([fm fileExistsAtPath:path]){chown(path.fileSystemRepresentation,0,0);chmod(path.fileSystemRepresentation,0755);}
        NSString *backupRoot=DisabledBackupRoot();
        NSDictionary *manifest=backupRoot?[NSDictionary dictionaryWithContentsOfFile:[backupRoot stringByAppendingPathComponent:S("manifest.plist")]]:nil;
        if(manifest.count){
            for(NSString *target in DisabledSystemTargets()){
                NSString *backup=DisabledManifestBackup(backupRoot,manifest,target);
                if(!backup.length||![fm fileExistsAtPath:backup])continue;
                [fm removeItemAtPath:target error:nil];
                if(![fm copyItemAtPath:backup toPath:target error:nil])return EIO;
                FixDisabledPermissions(target);
                int check=VerifyDisabledFile(target); if(check)return check;
            }
        }
    } else if([path isEqualToString:S("/System/Library/ThermalMonitor")]){
        RestoreThermalSchedule();
    }
...
static int ExecuteCommand(NSString *cmd, NSString *rawPath) {
    ...
    if ([cmd isEqualToString:S("apply-disabled-daemons")]) return ApplyDisabledDaemons();
    if ([cmd isEqualToString:S("restore-disabled-daemons")]) return RestoreDisabledDaemons();
    NSString *p=StandardPath(rawPath); if(!p)return EINVAL;
    if([cmd isEqualToString:S("unmount-restore")])return UnmountAndRestore(p);
...
int main(int argc,char *argv[]){ @autoreleasepool {
    ...
    if([cmd isEqualToString:S("remount-all")])return RemountAll(); if([cmd isEqualToString:S("cleanup-all")])return CleanupAllMountData(); if([cmd isEqualToString:S("optimize-thermal")])return OptimizeThermalOneClick(); if([cmd isEqualToString:S("apply-disabled-daemons")])return ApplyDisabledDaemons(); if([cmd isEqualToString:S("restore-disabled-daemons")])return RestoreDisabledDaemons(); if(argc!=3)return 64; return ExecuteCommand(cmd,S(argv[2]));
} }
