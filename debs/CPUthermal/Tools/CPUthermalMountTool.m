...
static NSString *KillallPath(void) {
    NSArray *candidates = @[S("/usr/bin/killall"), S("/var/jb/usr/bin/killall"), S("/var/jb/bin/killall")];
    for (NSString *p in candidates) if ([[NSFileManager defaultManager] isExecutableFileAtPath:p]) return p;
    return nil;
}

static int RestartThermalMonitor(void) {
    NSString *killall = KillallPath();
    if (!killall) return ENOENT;
    pid_t pid = 0;
    char *args[] = {(char *)"killall", (char *)"-q", (char *)"thermalmonitord", NULL};
    int r = posix_spawn(&pid, killall.fileSystemRepresentation, NULL, NULL, args, NULL);
    if (r) return r;
