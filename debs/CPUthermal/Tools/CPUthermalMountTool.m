...
static int MountPath(NSString *path) {
    ...
    if (IsBindfsMounted(path)) {
        ...
        struct statfs current={0}; if(statfs(path.fileSystemRepresentation,&current)==0&&(current.f_flags&MNT_RDONLY)) { int u=UnmountPath(path); if(u)return u; } else return 0;
    }
