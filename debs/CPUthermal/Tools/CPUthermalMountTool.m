...
static dispatch_queue_t gRequestQueue;
static void RequestNotification(CFNotificationCenterRef c,void *o,CFStringRef n,const void *x,CFDictionaryRef u){if(gRequestQueue)dispatch_async(gRequestQueue,^{ProcessRequests();});}
static int Serve(void) {
    ...
    gRequestQueue=dispatch_queue_create("com.huayuarc.cputhermal.mount.requests",DISPATCH_QUEUE_SERIAL);
    // 先注册通知再恢复挂载，消除 RemountAll 较慢时客户端通知丢失并返回124的窗口。
    ...
    dispatch_async(gRequestQueue,^{ProcessRequests();RemountAll();ProcessRequests();});
