...
static void TuneAntiDownclock(id v){
    if([v isKindOfClass:NSMutableDictionary.class]) {
        ...
        for(NSString*k in @[S("target"),S("alternateTarget"),S("THERMAL_TRAP_LOAD"),S("THERMAL_TRAP_SLEEP"),S("ForcedThermalLevelTarget0"),S("ForcedThermalLevelTarget1"),S("ForcedThermalPressureLevelLightTarget")]) SetExisting(d,k,@25);

        // hotspot 的 target 改为 25°C 后必须同时中和 PID；否则负 kp/ki 会在 25°C 提前输出降频力度。
        if(d[S("target")]!=nil && (d[S("kp")]!=nil || d[S("ki")]!=nil)) {
            SetExisting(d,S("kp"),@0);
            SetExisting(d,S("ki"),@0);
        }
        // 决策树保持数组长度和节点结构，只将温度边界统一为 25°C、控制力度归零。
        if([d[S("maxLIs")] isKindOfClass:NSArray.class])
            d[S("maxLIs")]=FilledNumericArray(d[S("maxLIs")],@25,YES);
        if([d[S("controlEfforts")] isKindOfClass:NSArray.class])
            d[S("controlEfforts")]=FilledNumericArray(d[S("controlEfforts")],@0,NO);
        // 背光热控制只保留 100% 输出；up/down 是温度边界，统一为 25°C。
        if(d[S("level")]!=nil && (d[S("up")]!=nil || d[S("down")]!=nil)) {
            SetExisting(d,S("up"),@25); SetExisting(d,S("down"),@25); SetExisting(d,S("level"),@100);
        }
        for(id x in d.allValues) TuneAntiDownclock(x);
