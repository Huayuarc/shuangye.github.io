...
static NSString *DisabledVisiblePath(void){
    return [DisabledMountDirectory() stringByAppendingPathComponent:S("disabled.plist")];
}
static NSString *DisabledStoragePath(void){
    return StoragePath(DisabledMountDirectory());
}
...
static int ApplyDisabledDaemons(void){
    ...
    // 与“一键挂载并优化温控”相同：先把整个系统目录 bindfs 挂到当前动态隐根后备目录。
    int mountResult=AddPath(DisabledMountDirectory());
    if(mountResult)return mountResult;
    NSString *visible=DisabledVisiblePath();
    NSString *storage=DisabledStoragePath();
    if(!storage.length)return ENOENT;
    NSString *backing=[storage stringByAppendingPathComponent:S("disabled.plist")];
    NSFileManager *fm=NSFileManager.defaultManager;
    ...
    if(![data writeToFile:backing atomically:YES])return EIO;
    FixDisabledPermissions(backing);
    int backingCheck=VerifyDisabledFile(backing); if(backingCheck)return backingCheck;
    // bindfs 可见路径复查，确保 Filza 看到的就是后备源内容与权限。
    int visibleCheck=VerifyDisabledFile(visible); if(visibleCheck)return visibleCheck;
...
static int RestoreDisabledDaemons(void){
    ...
    int mountResult=AddPath(DisabledMountDirectory()); if(mountResult)return mountResult;
    NSString *storage=DisabledStoragePath(); if(!storage.length)return ENOENT;
    NSString *backing=[storage stringByAppendingPathComponent:S("disabled.plist")];
    NSString *backupName=manifest[DisabledVisiblePath()];
    if(!backupName.length)backupName=manifest[backing];
    if(!backupName.length)return ENOENT;
    NSString *backup=[backupRoot stringByAppendingPathComponent:backupName];
    if(![NSFileManager.defaultManager fileExistsAtPath:backup])return ENOENT;
    [NSFileManager.defaultManager removeItemAtPath:backing error:nil];
    if(![NSFileManager.defaultManager copyItemAtPath:backup toPath:backing error:nil])return EIO;
    FixDisabledPermissions(backing);
    int check=VerifyDisabledFile(backing); if(check)return check;
    return VerifyDisabledFile(DisabledVisiblePath());
}
