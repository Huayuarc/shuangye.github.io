...
static int MakeFileWritable(NSString *file){struct stat st={0};if(lstat(file.fileSystemRepresentation,&st)!=0)return errno;if(S_ISLNK(st.st_mode))return 0;mode_t mode=(st.st_mode&07777)|S_IWUSR|S_IWGRP|S_IWOTH;return chmod(file.fileSystemRepresentation,mode)==0?0:errno;}
static int PrepareStorage(NSString *path) {
    ...
    // 后备目录首次建立时已统一赋予写权限。守护重启/重复挂载时不再递归 chmod
    // 整个 LaunchDaemons/ThermalMonitor 树，否则 Serve 会在注册 IPC 监听前长期阻塞。
...
static int MountPath(NSString *path) {
    ...
    if (IsBindfsMounted(path)) {
        // 正确来源且非只读的 bindfs 已经工作：直接返回，确保守护启动能立即注册 IPC。
        struct statfs current={0};
        if(statfs(path.fileSystemRepresentation,&current)==0) {
            NSString *source=[NSString stringWithUTF8String:current.f_mntfromname];
            NSString *expected=StoragePath(path);
            BOOL own=source.length&&expected.length&&[[source stringByStandardizingPath] isEqualToString:[expected stringByStandardizingPath]];
            if(own&&!(current.f_flags&MNT_RDONLY))return 0;
            if(current.f_flags&MNT_RDONLY){int u=UnmountPath(path);if(u)return u;}
            else return EBUSY;
        }
    }
...
static int RestartThermalMonitor(void){pid_t pid=0;char *args[]={(char*)"killall",(char*)"-q",(char*)"thermalmonitord",NULL};int r=posix_spawnp(&pid,"killall",NULL,NULL,args,NULL);if(r)return r;int status=0;waitpid(pid,&status,0);return 0;}
