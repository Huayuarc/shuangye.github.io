...
static NSArray<NSString*> *DisabledTargets(void){
    NSFileManager *fm=NSFileManager.defaultManager;
    NSMutableArray *candidates=[NSMutableArray arrayWithObjects:
        S("/var/db/com.apple.xpc.launchd/disabled.plist"),
        S("/rootfs/var/db/com.apple.xpc.launchd/disabled.plist"),
        S("/rootfs/private/var/db/com.apple.xpc.launchd/disabled.plist"), nil];
    for(NSString *root in AllMountRoots())
        [candidates addObject:[root stringByAppendingPathComponent:S("var/db/com.apple.xpc.launchd/disabled.plist")]];
    NSMutableArray *targets=[NSMutableArray array]; NSMutableSet *seen=[NSMutableSet set];
    for(NSString *candidate in candidates){
        NSString *dir=[candidate stringByDeletingLastPathComponent];
        if(![fm fileExistsAtPath:dir])continue;                  // 目标目录不存在则跳过
...
static int ApplyDisabledDaemons(void){
    ...
    NSDictionary *old=[NSDictionary dictionaryWithContentsOfFile:manifestPath];
    NSMutableDictionary *manifest=old?[old mutableCopy]:[NSMutableDictionary dictionary];
    int applied=0,last=0;
    for(NSString *target in DisabledTargets()){
        ...
        struct stat st={0};
        if(stat(target.fileSystemRepresentation,&st)!=0||st.st_uid!=0||st.st_gid!=0||(st.st_mode&07777)!=0644){last=EPERM;continue;}
        applied++;
    }
...
static int RestoreDisabledDaemons(void){
    ...
    for(NSString *target in manifest){
        NSString *backup=[backupRoot stringByAppendingPathComponent:manifest[target]];
        if(![fm fileExistsAtPath:backup]){last=ENOENT;continue;}
