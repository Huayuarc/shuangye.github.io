...
static void TuneAntiDownclock(id v){if([v isKindOfClass:NSMutableDictionary.class]){NSMutableDictionary*d=v;for(NSString*k in @[S("CPUMaxPower"),S("GPUMaxPower"),S("PackageMaxPower")])SetExisting(d,k,@65000);SetExisting(d,S("dtThermalLevel"),@0);for(NSString*k in @[S("target"),S("alternateTarget"),S("THERMAL_TRAP_LOAD"),S("THERMAL_TRAP_SLEEP"),S("ForcedThermalLevelTarget0"),S("ForcedThermalLevelTarget1"),S("ForcedThermalPressureLevelLightTarget")])SetExisting(d,k,@150);for(id x in d.allValues)TuneAntiDownclock(x);}else if([v isKindOfClass:NSArray.class])for(id x in v)TuneAntiDownclock(x);}
...
static int RestartThermalMonitor(void){pid_t p=0;char*a[]={(char*)"killall",(char*)"-q",(char*)"thermalmonitord",NULL};return posix_spawnp(&p,"killall",NULL,NULL,a,NULL);}
static NSString *ModelFile(NSString*t){for(NSString*n in [NSFileManager.defaultManager contentsOfDirectoryAtPath:t error:nil])if([n hasSuffix:S("AP-Info.plist")])return[t stringByAppendingPathComponent:n];return nil;}
static int ApplyAntiDownclockSchedule(void){NSString*t=StoragePath(S("/System/Library/ThermalMonitor"));if(!IsBindfsMounted(S("/System/Library/ThermalMonitor")))return 70;NSString*m=ModelFile(t);if(!m)return 71;if(!BackupOnce(m,S("ThermalModel.original.plist")))return 73;id d=MutableTree([NSDictionary dictionaryWithContentsOfFile:m]);if(![d isKindOfClass:NSMutableDictionary.class])return 74;TuneAntiDownclock(d);if(![d writeToFile:m atomically:YES])return 75;MakeStorageWritable(t);return RestartThermalMonitor();}
