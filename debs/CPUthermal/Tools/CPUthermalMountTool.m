...
static int MountPath(NSString *path) {
    ...
    if (IsBindfsMounted(path)) return 0; if (IsMounted(path,NO)) return EBUSY;
    int official=RunJbctl(S("mount"),path); if(official==0&&IsBindfsMounted(path))return 0;
    NSString *storage=StoragePath(path);
    int direct=WithKernelCredentials(^int{ int p=PrepareStorage(path); if(p)return 100+p; int m=mount("bindfs",path.fileSystemRepresentation,MNT_RDONLY,(void *)storage.fileSystemRepresentation); return m==0?0:200+errno; });
    if(direct==0&&IsBindfsMounted(path))return 0;
    // 某些 RootHide 版本的 jbctl 尚无 internal mount，会返回 -1/255；此时报告直接 bindfs 的真实结果。
    if(official==255||official==64||official==42||official==230)return direct; return official;
}
