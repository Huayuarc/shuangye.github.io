...
static int MountPath(NSString *path) {
    ...
    if (IsBindfsMounted(path)) {
        // 正确的 CPUthermal 可写挂载走幂等快路径，重复点击不再递归 chmod 整个后备目录。
        struct statfs current={0};
        if(statfs(path.fileSystemRepresentation,&current)!=0)return errno;
        if(!(current.f_flags&MNT_RDONLY)&&IsMounted(path,YES))return 0;
        if(!IsMounted(path,YES))return EBUSY;
        int u=UnmountPath(path); if(u)return u;
    }
    if (IsMounted(path,NO)) return EBUSY;
    NSString *storage=StoragePath(path);
    int prepared=PrepareStorage(path); if(prepared)return 100+prepared;
    int direct=WithKernelCredentials(^int{ int m=mount("bindfs",path.fileSystemRepresentation,0,(void *)storage.fileSystemRepresentation); return m==0?0:200+errno; });
    if(direct==0&&IsBindfsMounted(path))return 0;
    return direct==0?299:direct;
}
static int UnmountPath(NSString *path) {
...
static int AddPath(NSString *path) { path=StandardPath(path); if(!path.length)return EINVAL; NSMutableArray *a=LoadPaths(); if(![a containsObject:path])[a addObject:path]; if(!SavePaths(a))return EIO; return MountPath(path); }
static int RemovePath(NSString *path) { path=StandardPath(path); if(!path.length)return EINVAL; int r=UnmountPath(path); if(r)return r; NSMutableArray *a=LoadPaths(); [a removeObject:path]; return SavePaths(a)?0:EIO; }
static int RemountAll(void) { int last=0; for(NSString *raw in LoadPaths()){NSString *p=StandardPath(raw);if(!p.length){last=EINVAL;continue;}int r=MountPath(p);if(r)last=r;} return last; }
...
static NSString *SchedulerBackupRoot(void){NSString *root=CPUthermalCurrentRootHideRoot();return root.length?[root stringByAppendingPathComponent:S("var/lib/cputhermal-scheduler-backup")]:nil;}
