...
static NSString *BundledD64Path(void){NSString *root=CPUthermalCurrentRootHideRoot();if(root.length){NSString *p=[root stringByAppendingPathComponent:S("Library/Application Support/CPUthermal/D64AP-Info.plist")];if([[NSFileManager defaultManager] fileExistsAtPath:p])return p;}for(NSString *p in @[S("/var/jb/Library/Application Support/CPUthermal/D64AP-Info.plist"),S("/Library/Application Support/CPUthermal/D64AP-Info.plist")])if([[NSFileManager defaultManager] fileExistsAtPath:p])return p;return nil;}
