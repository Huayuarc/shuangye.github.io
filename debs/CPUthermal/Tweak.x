...
static const char * const kCPUthermalBacklightLimitKeys[] = {
    ...
    "max-brightness","maxbrightness","MaxBrightness","brightnesscap","BrightnessCap",NULL};
    ...
    while ((entry = IOIteratorNext(iterator)) != IO_OBJECT_NULL) {
        for (int i=0; kCPUthermalBacklightLimitKeys[i]; i++) {
            ...
            if (existing) {
                ...
                if (numeric && current > 0.0 && current < target) {
...
- (BOOL)setServiceProperty:(id)service key:(id)key value:(id)value scaleToFixedPoint:(BOOL)scale {
if (shouldApplyFullCPUProtection() && CPUthermalIsThermalServiceKey(key)) return NO;
return %orig(service, key, value, scale);
}

// 口袋过热保护 — 开启「锁屏模拟口袋过热」且熄屏时强制开启 HIP
- (void)setHiPFeatureEnabled:(BOOL)enabled {
if (g_hipLockedEnabled && CPUthermalScreenIsBlanked()) {
%orig(YES);
return;
}
%orig(enabled);
...
//   - setHiPFeatureEnabled: 仅在「锁屏模拟口袋过热」开启且熄屏时强制 YES，其余放行
// ============================================================================
...
// 口袋过热保护 — 开启「锁屏模拟口袋过热」且屏幕关闭时强制开启 HIP
- (void)setHiPFeatureEnabled:(BOOL)enabled {
if (g_hipLockedEnabled && CPUthermalScreenIsBlanked()) {
%orig(YES);
return;
...
static NSMutableArray *CPUthermalMaximizeBacklightArray(NSArray *source) {
    ...
    for (NSUInteger i = 0; i < source.count; i++) [result addObject:[best copy]];
    return result;
}
