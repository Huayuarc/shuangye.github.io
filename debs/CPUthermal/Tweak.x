...
// 本 dylib 仅在 thermalmonitord 内运行，亮度/亮度上限写入均属于温控路径；
// 不再依赖 display/backlight 前缀，覆盖设备私有键（如 brightnessCap/MaxBrightness）。
return [lower containsString:S("brightness")] || [lower containsString:S("luminance")] ||
       [lower containsString:S("nits")] || [lower containsString:S("brightnesscap")] ||
       ([lower containsString:S("limit")] && [lower containsString:S("iomfb")]);
}
...
// thermalmonitord 内所有匹配键均是温控路径；未知设备最大值时也吞写，
// 防止守护在注销后数秒以非 *-limit 键再次压低亮度。
