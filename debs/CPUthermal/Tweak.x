...
static dispatch_source_t g_batterySimulationTimer = NULL;
...
static void updateBatterySimulationTimer(void);
static void switchToLowPowerForSleep(const char *source);
static void switchToFullPowerAfterWake(const char *source);
...
prefs[S("powerMode")] = S(kCPUthermalLowPowerModeC);
if (!CPUthermalWritePrefs(prefs)) {
NSLog(@"[CPUthermal] 锁屏进入低功耗失败：偏好写入失败");
return;
}
os_unfair_lock_lock(&g_modeLock);
...
CPUthermalPostPowerMode(S(kCPUthermalLowPowerModeC));
NSLog(@"[CPUthermal] %s 状态进入锁屏低功耗", source ?: "sleep");
}
}
...
if (!CPUthermalWritePrefs(prefs)) {
NSLog(@"[CPUthermal] 亮屏恢复解除温控失败：偏好写入失败");
return;
}
os_unfair_lock_lock(&g_modeLock);
...
os_unfair_lock_unlock(&g_modeLock);
applyPowerModeToRuntime(NO);
CPUthermalPostPowerMode(S(kCPUthermalFullPowerModeC));
scheduleThermalMonitorReload();
NSLog(@"[CPUthermal] %s 状态恢复，低功耗会话结束并切换解除温控", source ?: "wake");
}
}
...
if (state == 0) switchToFullPowerAfterWake("unlock");
}

static void handleBlankedScreenToken(int token) {
uint64_t state = UINT64_MAX;
if (token <= 0 || notify_get_state(token, &state) != NOTIFY_STATUS_OK) return;
if (state == 0) switchToFullPowerAfterWake("screen-on");
...
static void updateBatterySimulationTimer(void) {
os_unfair_lock_lock(&g_runtimeLock);
dispatch_source_t oldTimer = g_batterySimulationTimer;
if (!g_simulateMaximumCapacityEnabled) {
g_batterySimulationTimer = NULL;
if (oldTimer) {
dispatch_source_cancel(oldTimer);
#if !OS_OBJECT_USE_OBJC
dispatch_release(oldTimer);
#endif
}
os_unfair_lock_unlock(&g_runtimeLock);
return;
...
g_batterySimulationTimer = timer;
dispatch_source_set_timer(timer, dispatch_time(DISPATCH_TIME_NOW, 10ull * NSEC_PER_SEC),
                          10ull * NSEC_PER_SEC, 1ull * NSEC_PER_SEC);
dispatch_source_set_event_handler(timer, ^{
if (!g_simulateMaximumCapacityEnabled) { updateBatterySimulationTimer(); return; }
applyBatteryCapacitySimulation(YES);
});
...
static NSDictionary *patchThermalPlist(NSDictionary *dict) {
    ...
    NSDictionary *patched = CPUthermalPatchBacklightNode(dict, NO, nil);
...
BOOL wasLowPower = isLowPowerMode();
BOOL lowPower = NO;
if (CPUthermalReadPostedPowerMode(&lowPower)) {
os_unfair_lock_lock(&g_modeLock);
g_powerMode = lowPower ? CPUthermalPowerModeLow : CPUthermalPowerModeFull;
os_unfair_lock_unlock(&g_modeLock);
} else {
loadPrefs();
}
applyPowerModeToRuntime(NO);
if (wasLowPower && isFullPowerMode()) scheduleThermalMonitorReload();
// 手动切换也必须服从当前屏幕状态：熄屏=低功耗，亮屏=解除温控。
handleBlankedScreenToken(g_blankedScreenToken);
NSLog(S("[CPUthermal] 功率模式已切换: %@，内核 DVFS 保持启用，mitigation level:%d"),
isLowPowerMode() ? S("低功耗") : S("解除温控"), targetCPUPerformanceLevel());
};
