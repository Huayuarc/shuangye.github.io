...
// iOS15-17 实际写入常见 key 不一定包含 display/backlight 上下文。
...
BOOL displayContext = [lower containsString:S("backlight")] ||
                      [lower containsString:S("display")] ||
                      [lower containsString:S("iomfb")];
if (!displayContext) return NO;
return [lower containsString:S("brightness")] || [lower containsString:S("luminance")] ||
       [lower containsString:S("nits")] || [lower containsString:S("limit")];
}

static BOOL keyIsExplicitBacklightLimit(NSString *key) {
...
if ([lower isEqualToString:S("iomfb_brightness_limit")] ||
    ...
    [lower isEqualToString:S("maxbrightness")] ||
    [lower isEqualToString:S("brightnesscap")]) return YES;
BOOL brightness = [lower containsString:S("brightness")] ||
                  [lower containsString:S("luminance")] ||
                  [lower containsString:S("nits")];
BOOL limiter = [lower containsString:S("limit")] || [lower containsString:S("cap")] ||
               [lower containsString:S("ceiling")] || [lower containsString:S("thermal")] ||
               [lower containsString:S("mitigation")];
return brightness && limiter;
}
...
changed = YES;
} else if (keyIsExplicitBacklightLimit(key)) {
...
// 已记录设备最大值则恢复最大值；未知时只吞明确限制键，普通亮度设置继续放行。
if (replacement) return orig_IOServiceSetProperty(service, key, (__bridge CFTypeRef)replacement);
return keyIsExplicitBacklightLimit(keyString) ? KERN_SUCCESS : orig_IOServiceSetProperty(service, key, value);
}
...
return keyIsExplicitBacklightLimit(keyString) ? KERN_SUCCESS : %orig(entry, key, value);
}
if (isNetworkThrottleProperty(key)) return KERN_SUCCESS;
if (shouldApplyFullCPUProtection() && keyIsThermalThrottleProperty(keyString)) {
return KERN_SUCCESS;
...
// 直接套用运行状态，不再绕完整 settingsChanged 路径；进入目标 App 时立即施加预算，退出立即恢复用户模式。
