...
static NSNumber *g_maxBacklightPowerValue = nil;
...
return [lower containsString:S("brightness")] || [lower containsString:S("luminance")] ||
       [lower containsString:S("nits")] || [lower containsString:S("power")] ||
       [lower containsString:S("limit")] || [lower containsString:S("cap")] ||
       [lower containsString:S("ceiling")] || [lower containsString:S("target")] ||
       [lower containsString:S("level")];
}

static id maximumBacklightReplacementForKey(NSString *key) {
NSString *lower = [key lowercaseString];
if ([lower containsString:S("power")]) return g_maxBacklightPowerValue ?: g_maxBacklightBrightnessValue;
return g_maxBacklightBrightnessValue ?: g_maxBacklightPowerValue;
}
...
if ([originalValue isKindOfClass:[NSString class]]) return [(NSNumber *)maximum stringValue];
if ([originalValue isKindOfClass:[NSNumber class]]) return maximum;
return nil;
}
...
if (shouldApplyHighPerformanceMode()) protectedComponent = protectedComponent || mentionsGPU || mentionsPackage || mentionsSoC;
...
if (replacement) [filtered setObject:replacement forKey:key]; else [filtered removeObjectForKey:key];
changed = YES;
continue;
}
...
return replacement ? orig_IOServiceSetProperty(service, key, (__bridge CFTypeRef)replacement) : KERN_SUCCESS;
}
...
if (thermalDimmingPreventionEnabled() && keyIsBacklightThermalLimit(keyString)) {
id replacement = backlightReplacementMatchingValue(keyString, (__bridge id)value);
return replacement ? %orig(entry, key, (__bridge CFTypeRef)replacement) : KERN_SUCCESS;
}
if (isNetworkThrottleProperty(key)) return KERN_SUCCESS;
if (shouldApplyFullCPUProtection() && keyIsThermalThrottleProperty(keyString)) {
return KERN_SUCCESS;
...
NSString *lower = [key lowercaseString] ?: S("");
if ([lower containsString:S("power")]) {
if (!g_maxBacklightPowerValue || [value compare:g_maxBacklightPowerValue] == NSOrderedDescending) g_maxBacklightPowerValue = value;
} else {
...
if (selected == CPUthermalPowerModeLow) {
    g_highPerformanceModeEnabled = NO;
    NSMutableDictionary *prefs = CPUthermalReadMutablePrefs() ?: [NSMutableDictionary dictionary];
    if ([prefs[S("highPerformanceModeEnabled")] boolValue]) {
        prefs[S("highPerformanceModeEnabled")] = [NSNumber numberWithBool:NO];
