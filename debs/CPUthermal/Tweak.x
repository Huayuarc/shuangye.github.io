...
static dispatch_source_t g_fullPowerRescheduleTimer = NULL;
...
static void startFullPowerRescheduleTimer(void);
static void stopFullPowerRescheduleTimer(void);
...
static void restartThermalMonitorExternally(void);
...
// 进程内自杀式重启在亮屏期间会造成瞬时黑屏/无背光（需双击电源键唤醒），
...
// 低功耗→解除温控专用：真正从外部重启 thermalmonitord，强制清空内核 CPMS 低功耗状态。
// 仅靠 ObjC setter 软恢复不足以恢复被 A15 等平台内核持有的低频目标；挂载工具的
// RestartThermalMonitor 一直采用这种外部 killall + launchd 自愈方式，是已验证安全路径。
// 与 _getConfigurationFor 钩子配合：重启后 thermalmonitord 组件获得干净 initFastLoop，
// 在解除温控模式下不会再重放低功耗的 Level/预算，从而真正回到满血频率。
static void restartThermalMonitorExternally(void) {
dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
// 先等软恢复应用链写完，避免与本次重启竞态。
usleep(600000);
// killall 优先用绝对路径，避免低 PATH 环境 spawn 找不到。
const char *killallPath = access("/usr/bin/killall", X_OK) == 0 ? "/usr/bin/killall"
                            : access("/bin/killall", X_OK) == 0 ? "/bin/killall" : "killall";
pid_t pid = 0;
char *args[] = {(char*)"killall", (char*)"-q", (char*)"thermalmonitord", NULL};
posix_spawnp(&pid, killallPath, NULL, NULL, args, NULL);
});
}
...
if (previous == CPUthermalPowerModeLow && target == CPUthermalPowerModeFull) {
    scheduleThermalMonitorReload();
    restartThermalMonitorExternally();
...
static void stopFullPowerRescheduleTimer(void) {
os_unfair_lock_lock(&g_runtimeLock);
dispatch_source_t timer = g_fullPowerRescheduleTimer;
g_fullPowerRescheduleTimer = NULL;
if (timer) {
dispatch_source_cancel(timer);
#if !OS_OBJECT_USE_OBJC
dispatch_release(timer);
...
// A15 等平台在切换后可能延迟回写低功耗缓存。解除温控期间每 2 秒重新清除
// powerSave/CPULevel/功率预算，确保 Level 2、2500mW 或低 Ceiling 不会残留。
static void startFullPowerRescheduleTimer(void) {
if (!shouldApplyFullCPUProtection()) { stopFullPowerRescheduleTimer(); return; }
os_unfair_lock_lock(&g_runtimeLock);
if (g_fullPowerRescheduleTimer) { os_unfair_lock_unlock(&g_runtimeLock); return; }
dispatch_source_t timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,
    dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0));
dispatch_source_set_timer(timer, dispatch_time(DISPATCH_TIME_NOW, 2 * NSEC_PER_SEC),
                          2 * NSEC_PER_SEC, 0.2 * NSEC_PER_SEC);
dispatch_source_set_event_handler(timer, ^{
if (!shouldApplyFullCPUProtection()) { stopFullPowerRescheduleTimer(); return; }
CPUthermalForceNominalCombined();
applyFullPowerToCommonProduct();
...
g_fullPowerRescheduleTimer = timer;
dispatch_resume(timer);
os_unfair_lock_unlock(&g_runtimeLock);
}
...
// 关键低功耗状态不可只“吞写”：旧值可能已在内核中生效。切回解除温控时
// 主动把同一次写替换为满血值，确保 MitigationHook 的恢复写不会被本 Hook 吞掉。
int restoreValue = 0; BOOL exact = YES;
if ([keyString isEqualToString:S("p-state-cap")]) restoreValue = 15;
else if ([keyString isEqualToString:S("CPU_Ceiling")]) restoreValue = 100;
else if ([keyString isEqualToString:S("CPU_Floor")]) restoreValue = 0;
else exact = NO;
if (exact) {
CFNumberRef number = CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &restoreValue);
kern_return_t result = orig_IOServiceSetProperty(service, key, number);
CFRelease(number); return result;
}
if (keyIsThermalThrottleProperty(keyString)) return KERN_SUCCESS;
}
return orig_IOServiceSetProperty(service, key, value);
}
...
if (exact) {
CFNumberRef number = CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &restoreValue);
kern_return_t result = %orig(entry, key, number);
CFRelease(number); return result;
}
if (keyIsThermalThrottleProperty(keyString)) return KERN_SUCCESS;
}
...
NSLog(@"[CPUthermal] 启动完成，功率状态将在启动静默期后应用", isLowPowerMode() ? S("低功耗") : S("解除温控"));
}
}
