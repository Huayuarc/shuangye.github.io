...
// 仅当系统公开的最高热传感器值达到 99°C 时交还原生热管理。
// 43–46°C 区间不暂停高性能，也不释放 GPU/Package 调度覆盖。
static const float kSystemProtectionTemperatureC = 99.0f;
...
static BOOL g_systemThermalProtectionActive = NO;
...
static void restoreSystemGPUPackageScheduling(void);
static void observeThermalTemperature(float temperature);
...
return enabled && blockNetwork && isFullPowerMode() && !g_systemThermalProtectionActive;
}
...
return enabled && blockPopup && !g_systemThermalProtectionActive;
}
...
return enabled && preventDimming && !g_systemThermalProtectionActive;
}
...
return runtimeProtectionEnabled() && isFullPowerMode() && !g_systemThermalProtectionActive;
}
...
// 用于禁用插件及达到 99°C 保护阈值后，将 CPU/GPU/Package 控制权交还 thermalmonitord。
static void restoreSystemGPUPackageScheduling(void) {
@autoreleasepool {
@try {
g_restoringFullPower = YES;
for (id controller in trackedPowerControllersSnapshot()) {
// 达到保护阈值时重新启用 CPMS，再让各控制器从系统配置重算目标；
// 不预写 CPU Level/PowerSave，避免覆盖机型自己的紧急策略。
if ([controller respondsToSelector:@selector(setCPMSMitigationsEnabled:)]) {
((void (*)(id, SEL, BOOL))objc_msgSend)(controller, @selector(setCPMSMitigationsEnabled:), YES);
}
if ([controller respondsToSelector:@selector(updateCPU)]) {
...
NSLog(@"[CPUthermal] 恢复系统 GPU/Package 调度失败: %@", exception);
} @finally {
g_restoringFullPower = NO;
}
}
...
// 仅使用 thermalmonitord 发布的最高传感器温度；43–46°C 不参与任何保护分支。
static void observeThermalTemperature(float temperature) {
if (temperature < 0.0f || !runtimeProtectionEnabled() || !isFullPowerMode()) return;
// helper 常规返回摄氏度；个别系统的 notify 值仍多一位比例（如 990°C 表示 99°C）。
// 真实设备传感器不会达到 200°C，按十进制逐级收敛可兼容 centi/milli 差异。
while (temperature > 200.0f) temperature /= 10.0f;
BOOL shouldProtect = temperature >= kSystemProtectionTemperatureC;
os_unfair_lock_lock(&g_stateLock);
BOOL changed = (g_systemThermalProtectionActive != shouldProtect);
g_systemThermalProtectionActive = shouldProtect;
os_unfair_lock_unlock(&g_stateLock);
if (!changed) return;
if (shouldProtect) {
// 放开决策树、CPMS、GPU 与 Package；此后由系统自身的紧急热策略接管。
restoreSystemGPUPackageScheduling();
} else {
// 温度回落后恢复用户选定的解除温控/高性能策略。
...
NSString *bundleID = [prefs[S("foregroundBundleID")] isKindOfClass:[NSString class]] ? prefs[S("foregroundBundleID")] : nil;
NSArray *selected = [prefs[S("lowPowerApps")] isKindOfClass:[NSArray class]] ? prefs[S("lowPowerApps")] : nil;
return bundleID.length > 0 && [selected containsObject:bundleID];
}
...
- (void)handleTemperatureEvent:(int)arg1 service:(id)arg2 {
// maxthermalsensorvalue 由 thermalmonitord 更新，单位为摄氏度；以它作为唯一保护阈值来源。
observeThermalTemperature(CPUthermalGetMaxTriggerTemperature());
if (thermalPopupBlockingEnabled() && !g_systemThermalProtectionActive) {
CPUthermalForceNominalCombined();
return;
}
%orig(arg1, arg2);
...
// 延迟到主队列，避免部分 iOS 15/16 设备在 thermalmonitord 构造阶段同步广播
// 热通知引发启动竞态、短时暗屏或 daemon 重启循环。
if (shouldApplyFullCPUProtection()) {
