...
static Method CPUthermalOwnInstanceMethod(Class cls, SEL selector) {
unsigned int count = 0; Method found = NULL; Method *methods = class_copyMethodList(cls, &count);
for (unsigned int i = 0; i < count; i++) if (method_getName(methods[i]) == selector) { found = methods[i]; break; }
...
Method method = CPUthermalOwnInstanceMethod(cls, selector);
