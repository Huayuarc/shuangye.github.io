...
static void restartThermalMonitorForImmediateLowPower(void);
...
static void restartThermalMonitorForImmediateLowPower(void) {
// 先由当前进程同步写入预算，再异步终止自身；launchd 会按 KeepAlive 立即拉起。
// 使用绝对路径避免 thermalmonitord 的精简 PATH 导致 ENOENT。
dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.08*NSEC_PER_SEC)),dispatch_get_global_queue(QOS_CLASS_USER_INITIATED,0),^{
...
posix_spawn(&pid,tool.fileSystemRepresentation,NULL,NULL,args,NULL);
});
}
...
applyPowerModeToRuntime(NO);
if (previous == CPUthermalPowerModeFull && runtimeMode == CPUthermalPowerModeLow)
    restartThermalMonitorForImmediateLowPower();
else if (previous == CPUthermalPowerModeLow && runtimeMode == CPUthermalPowerModeFull)
    scheduleThermalMonitorReload();
}
...
if (previousMode == CPUthermalPowerModeLow && currentMode == CPUthermalPowerModeFull)
    scheduleThermalMonitorReload();
...
if (previous == CPUthermalPowerModeFull && target == CPUthermalPowerModeLow)
    restartThermalMonitorForImmediateLowPower();
else if (previous == CPUthermalPowerModeLow && target == CPUthermalPowerModeFull)
    scheduleThermalMonitorReload();
}
...
// 解除温控冷启动保留 8 秒传感器健康检查；低功耗模式由用户明确请求，150ms 后立即套用预算。
NSTimeInterval bootSettleDelay=isLowPowerMode()?0.15:8.0;
