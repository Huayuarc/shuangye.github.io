...
CPUthermalPowerMode target = [savedMode isEqualToString:S(kCPUthermalLowPowerModeC)]
...
BOOL backlight = [lower containsString:S("backlight")];
BOOL displayBrightness = [lower containsString:S("display")] &&
    ([lower containsString:S("brightness")] || [lower containsString:S("luminance")] || [lower containsString:S("nits")]);
if (!backlight && !displayBrightness) return NO;
return [lower containsString:S("brightness")] || [lower containsString:S("luminance")] ||
       [lower containsString:S("nits")];
}

static id maximumBacklightReplacementForKey(void) {
...
if (thermalDimmingPreventionEnabled() && keyIsBacklightThermalLimit(key)) {
    ...
    id replacement = nil;
    if ([original isKindOfClass:[NSNumber class]]) replacement = maximumBacklightReplacementForKey();
...
static void (*origIssueChargeInhibit)(id, SEL) = NULL;

static BOOL CPUthermalChargeTemperatureBypassEnabled(void) {
    return [CPUthermalReadPrefs()[S("bypassBatteryChargeTemperature")] boolValue] &&
           !CPUthermalSmartChargeCutoffState();
}

static void HookedIssueChargeInhibit(id self, SEL selector) {
    if (CPUthermalChargeTemperatureBypassEnabled()) return;
    if (origIssueChargeInhibit) origIssueChargeInhibit(self, selector);
}

static void InstallChargeInhibitDecisionHook(void) {
    if (origIssueChargeInhibit) return;
    Class cls = objc_getClass("ThermalDecisionTree");
    SEL selector = sel_registerName("issueChargeInhibit");
    ...
    const char *cursor = returnType; while (*cursor && strchr("rnNoORV", *cursor)) cursor++;
    if (*cursor != 'v') return;
    MSHookMessageEx(cls, selector, (IMP)HookedIssueChargeInhibit, (IMP *)&origIssueChargeInhibit);
}

static void (*origSetVirtualTemperature)(id, SEL, double) = NULL;

static void HookedSetVirtualTemperature(id self, SEL selector, double temperature) {
    double finalTemperature = temperature;
    if (runtimeEnabled() && finalTemperature > 8000.0) finalTemperature = 8000.0;
    if (origSetVirtualTemperature) origSetVirtualTemperature(self, selector, finalTemperature);
}

static void InstallVirtualTemperatureHook(void) {
    if (origSetVirtualTemperature) return;
    Class cls = objc_getClass("ThermalTemperatureManager");
    SEL selector = sel_registerName("setVirtualTemperature:");
    Method method = cls ? class_getInstanceMethod(cls, selector) : NULL;
    if (!method || method_getNumberOfArguments(method) != 3) return;
    char returnType[16] = {0}, argumentType[16] = {0};
    method_getReturnType(method, returnType, sizeof(returnType));
    method_getArgumentType(method, 2, argumentType, sizeof(argumentType));
    const char *r = returnType, *a = argumentType;
    while (*r && strchr("rnNoORV", *r)) r++;
    while (*a && strchr("rnNoORV", *a)) a++;
    if (*r != 'v' || *a != 'd') return;
    MSHookMessageEx(cls, selector, (IMP)HookedSetVirtualTemperature, (IMP *)&origSetVirtualTemperature);
}
...
InstallVirtualTemperatureHook();
InstallChargeInhibitDecisionHook();
...
// 从设备自身热配置中选择最大亮度/背光值（只针对 Brightness/Luminance/Nits）。
static void CPUthermalRecordMaximumBacklightValue(NSNumber *value) {
    if (![value isKindOfClass:[NSNumber class]]) return;
    if (!g_maxBacklightBrightnessValue || [value compare:g_maxBacklightBrightnessValue] == NSOrderedDescending)
        g_maxBacklightBrightnessValue = value;
}
    ...
    if ([node isKindOfClass:[NSDictionary class]]) {
        ...
        for (id rawKey in [(NSDictionary *)node allKeys]) {
            ...
            if (childContext && [value isKindOfClass:[NSNumber class]] && keyIsBacklightThermalLimit(key))
                CPUthermalRecordMaximumBacklightValue(value);
    ...
    if ([node isKindOfClass:[NSArray class]]) {
        ...
        if (brightnessArray && array.count > 0) {
            ...
            for (id value in array) {
                if ([value isKindOfClass:[NSNumber class]] && (maximum == nil || [value compare:maximum] == NSOrderedDescending))
                    maximum = value;
            }
            if (maximum) {
                CPUthermalRecordMaximumBacklightValue(maximum);
            }
        }
...
os_unfair_lock_unlock(&g_modeLock);
// 用户每次切换功率模式都立即应用并强制重建 thermalmonitord，
// 清除 PMGR/ApplePPM 旧对象与缓存；专用模式通知只由面板/CC发送。
(void)previous;
applyPowerModeToRuntime(NO);
scheduleThermalConfigurationReload();
