...
static BOOL g_hipLockScreenOnly = NO;
...
static void CPUthermalApplyHIPForCurrentScreen(void) {
BOOL target=g_hipLockedEnabled && (!g_hipLockScreenOnly || CPUthermalScreenIsBlanked());
...
// 纯 CPU 低功耗：全局 CPMS/PowerSave 会连带 DCP/显示功耗缓解，必须保持关闭。
...
if (!controller || !shouldApplyLowPowerLimit()) return;
trackPowerController(controller);
// 低功耗只限制 CPU；不再开启全局 CPMS/PowerSave，也不调用 Package 低功耗目标，
// 避免显示/背光被包级功耗缓解联动压低。
if ([controller respondsToSelector:@selector(setCPMSMitigationsEnabled:)])
((void (*)(id, SEL, BOOL))objc_msgSend)(controller, @selector(setCPMSMitigationsEnabled:), NO);
if ([controller respondsToSelector:@selector(setPowerSaveActive:)])
...
NSString *hipMode=[d[S("hipLockedMode")] isKindOfClass:[NSString class]]?d[S("hipLockedMode")]:S("global");
...
g_hipLockScreenOnly = [hipMode isEqualToString:S("lockScreen")];
...
// 两种模式都关闭全局 CPMS；低功耗只由 CPU Level/CPU 预算实现，显示维持解除温控策略。
- (void)setCPMSMitigationsEnabled:(BOOL)enabled {
...
// 直接套用运行状态，不再绕完整 settingsChanged 路径；进入目标 App 时立即施加预算，退出立即软恢复。
