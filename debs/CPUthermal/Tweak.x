...
// 温度保护引擎状态（SBCPUFloating 式）
static BOOL g_thermalEngineEnabled = YES;
static BOOL g_overheatAutoProtect = YES;
static BOOL g_nominalAutoRecover = YES;
static BOOL g_overheatActive = NO;
static __thread BOOL g_reassertingLowPower = NO;  // 防 updateCPU 钩子与 reassert 互相递归
static CFAbsoluteTime g_lastNormalSince = 0;
...
static BOOL isExtremeMode(void) {
os_unfair_lock_lock(&g_modeLock);
CPUthermalPowerMode m = g_powerMode;
os_unfair_lock_unlock(&g_modeLock);
return m == CPUthermalPowerModeExtreme;
}

static BOOL shouldApplyFullCPUProtection(void) {
return runtimeProtectionEnabled() && (isFullPowerMode() || isExtremeMode());
}
...
if (!g_overheatActive) g_powerMode = target;
os_unfair_lock_unlock(&g_modeLock);
...
if (isLowPowerMode()) {
// 只降频率与功耗，不触碰屏幕亮度：切档不再捕获/恢复亮度，也不触发亮度重提交。
...
if (isExtremeMode()) {
// 极限满频：在解除温控基础上强制满功率预算并立即重申，不等任何恢复脉冲。
CPUthermalForceNominalCombined();
applyFullPowerToCommonProduct();
restoreFullPowerToTrackedControllers();
applyCurrentModeToApplePPMCPU();
...
static int NotifyStateForName(const char *name) {
    static int pressureToken = 0, thermalToken = 0;
    static BOOL registered = NO;
    int *token = NULL;
    if (strcmp(name, "com.apple.system.thermalpressurelevel") == 0) token = &pressureToken;
    else if (strcmp(name, "com.apple.system.thermalnotification") == 0) token = &thermalToken;
    else return -1;
    if (!registered) {
        registered = YES;
        notify_register_check("com.apple.system.thermalpressurelevel", &pressureToken);
        notify_register_check("com.apple.system.thermalnotification", &thermalToken);
    }
    if (*token <= 0) return -1;
    uint64_t state = 0;
...
//   模式切换全部事件驱动、立即生效，无延迟。
// ============================================================================
static void CPUthermalApplyEffectiveMode(const char *reason) {
    os_unfair_lock_lock(&g_modeLock);
    CPUthermalPowerMode target = g_overheatActive ? CPUthermalPowerModeLow : g_userSelectedPowerMode;
    BOOL changed = (g_powerMode != target);
    g_powerMode = target;
    os_unfair_lock_unlock(&g_modeLock);
    if (changed) {
        applyPowerModeToRuntime(NO);
        CPUthermalPostThermalEngineState(g_overheatActive, (int)target);
        CPUthermalThrottleLog([NSString stringWithFormat:@"MODE -> %s (%s)",
            target == CPUthermalPowerModeLow ? "lowPower" : (target == CPUthermalPowerModeExtreme ? "extremeFull" : "fullPower"),
            reason ?: "change"]);
    }
}
...
static void CPUthermalEngineTick(void) {
    @try {
        if (g_thermalEngineEnabled && g_overheatAutoProtect) {
            int pressure = ThermalPressureLevel();
            int notif = ThermalNotificationLevel();
            double tempC = CPUthermalBatteryTempCelsius();
            BOOL hot = (pressure >= 2) || (notif >= 2) || (tempC >= 44.0);
            CFAbsoluteTime now = CFAbsoluteTimeGetCurrent();
            if (hot) {
                g_lastNormalSince = 0;
                if (!g_overheatActive) {
                    g_overheatActive = YES;
                    CPUthermalThrottleLog([NSString stringWithFormat:
                        @"HEAT protect engaged pressure=%d notif=%d temp=%.1fC -> lowPower", pressure, notif, tempC]);
                    CPUthermalApplyEffectiveMode("overheat");
                }
            } else if (g_overheatActive) {
                if (g_lastNormalSince == 0) g_lastNormalSince = now;
                double hold = g_nominalAutoRecover ? 5.0 : 0.0;
                if (now - g_lastNormalSince >= hold) {
                    g_overheatActive = NO;
                    g_lastNormalSince = 0;
                    CPUthermalThrottleLog([NSString stringWithFormat:
                        @"HEAT protect released pressure=%d notif=%d temp=%.1fC -> user mode", pressure, notif, tempC]);
                    CPUthermalApplyEffectiveMode("recovered");
                }
            }
        } else if (g_overheatActive) {
            g_overheatActive = NO;
            g_lastNormalSince = 0;
            CPUthermalApplyEffectiveMode("engine-off");
        }
        // 极限满频：每 2 秒重申一次满功率预算与 CPU 等级
        if (isExtremeMode()) {
            applyCurrentModeToApplePPMCPU();
            restoreFullPowerToTrackedControllers();
        }
...
static void CPUthermalEngineStart(void) {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)),
                   dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        CPUthermalThrottleLog(@"thermal engine started");
        CPUthermalApplyEffectiveMode("start");
        CPUthermalEngineTick();
    });
}
...
BOOL engineOn = [d[S("thermalEngineEnabled")] ?: @YES boolValue];
NSString *runMode = [d[S("powerMode")] isKindOfClass:[NSString class]] ? d[S("powerMode")] : nil;
if (!runMode) runMode = [d[S("thermalRunMode")] isKindOfClass:[NSString class]] ? d[S("thermalRunMode")] : S("fullPower");
CPUthermalPowerMode userMode = CPUthermalPowerModeFull;
if ([runMode isEqualToString:S("lowPower")]) userMode = CPUthermalPowerModeLow;
else if ([runMode isEqualToString:S("extremeFull")]) userMode = CPUthermalPowerModeExtreme;
if (!engineOn) userMode = CPUthermalPowerModeFull;   // 总开关关闭时固定解除温控
g_thermalEngineEnabled = engineOn;
g_overheatAutoProtect = [d[S("thermalPressureAutoProtectionEnabled")] ?: @YES boolValue];
g_nominalAutoRecover = [d[S("thermalNominalAutoRecoveryEnabled")] ?: @YES boolValue];
os_unfair_lock_lock(&g_modeLock);
g_userSelectedPowerMode = userMode;
if (!g_overheatActive) g_powerMode = userMode;
os_unfair_lock_unlock(&g_modeLock);
}
}
