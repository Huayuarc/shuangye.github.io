...
static dispatch_source_t g_foregroundStateMonitorTimer = NULL;
...
static void startForegroundStateMonitor(void) {
    if(g_foregroundStateMonitorTimer)return;
    dispatch_queue_t queue=dispatch_get_global_queue(QOS_CLASS_UTILITY,0);
    dispatch_source_t timer=dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER,0,0,queue);
    if(!timer)return;g_foregroundStateMonitorTimer=timer;
    dispatch_source_set_timer(timer,dispatch_time(DISPATCH_TIME_NOW,100ull*NSEC_PER_MSEC),250ull*NSEC_PER_MSEC,50ull*NSEC_PER_MSEC);
    dispatch_source_set_event_handler(timer,^{
        uint64_t hash=CPUthermalReadForegroundBundleHash();
        if(hash!=g_lastAppliedForegroundHash)onForegroundAppChanged(NULL,NULL,NULL,NULL,NULL);
    });
    dispatch_resume(timer);
}
