...
static void reconcileForegroundAppState(void) {
    ...
    BOOL appLowPower = foregroundAppShouldUseLowPower(prefs);
    BOOL blanked = CPUthermalScreenIsBlanked();
    CPUthermalPowerMode previous, target;
    os_unfair_lock_lock(&g_modeLock);
    previous = g_powerMode;
    g_foregroundAppForcesLowPower = appLowPower;
    ...
    if (previous != target && !g_appModeRestartScheduled) {
        g_appModeRestartScheduled = YES;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 50ull * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{
            kill(getpid(), SIGTERM);
        });
    }
}
