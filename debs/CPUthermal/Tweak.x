...
static BOOL g_appModeRestartScheduled = NO;
...
BOOL forced;
os_unfair_lock_lock(&g_modeLock); forced = g_foregroundAppForcesLowPower; os_unfair_lock_unlock(&g_modeLock);
...
NSString *savedMode = [prefs[S("powerMode")] isKindOfClass:[NSString class]]
    ? prefs[S("powerMode")] : S(kCPUthermalFullPowerModeC);
...
if (previous == CPUthermalPowerModeLow && target == CPUthermalPowerModeFull) {
    scheduleThermalMonitorReload();
} else if (target == CPUthermalPowerModeLow && source && strcmp(source, "unlock") == 0) {
    // 真正解锁时仅重建 thermalmonitord；新控制对象会按持久化低功耗初始化。
    scheduleThermalConfigurationReload();
}
NSLog(@"[CPUthermal] %s 状态恢复面板模式:%@", source ?: "wake",
...
static void registerThermalLevelResetObservers(void) {
    if (g_thermalPressureToken < 0) {
        notify_register_dispatch(kOSThermalNotificationPressureLevelName, &g_thermalPressureToken,
                                 dispatch_get_main_queue(), ^(int token) { handleThermalLevelNotification(token); });
    }
...
if (shouldApplyForegroundAppLowPowerLimit() && [controller respondsToSelector:@selector(setCPMSMitigationsEnabled:)])
...
if (shouldApplyForegroundAppLowPowerLimit()) {
if ([controller respondsToSelector:@selector(setCPULowPowerTarget:)]) ((void (*)(id, SEL, int))objc_msgSend)(controller, @selector(setCPULowPowerTarget:), kLowPowerPowerLimitMW);
if ([controller respondsToSelector:@selector(setMaxCPUPowerTarget:useLegacyPath:setProperty:)]) sendSetMaxCPUPowerTarget(controller, kLowPowerPowerLimitMW, NO);
if ([controller respondsToSelector:@selector(setCPUPowerZoneTarget:)]) ((void (*)(id, SEL, int))objc_msgSend)(controller, @selector(setCPUPowerZoneTarget:), kLowPowerPerformancePercent);
for (int source=0;source<kCPUDecisionSourceCount;source++) if ([controller respondsToSelector:@selector(setCPUPowerCeiling:fromDecisionSource:)]) sendTwoIntegerArguments(controller,@selector(setCPUPowerCeiling:fromDecisionSource:),kLowPowerPerformancePercent,(uintptr_t)source);
for (int contributor=0;contributor<kCPUDVD1ContributorCount;contributor++) if ([controller respondsToSelector:@selector(setCPUPowerCeiling:forDVD1Contributor:)]) sendTwoIntegerArguments(controller,@selector(setCPUPowerCeiling:forDVD1Contributor:),kLowPowerPerformancePercent,(uintptr_t)contributor);
}
...
if (shouldApplyForegroundAppLowPowerLimit() && [controller respondsToSelector:@selector(updatePackage)]) {
...
if (shouldApplyForegroundAppLowPowerLimit()) setCommonProductCeiling(product, @selector(setCPUPowerCeiling:fromDecisionSource:), kLowPowerPerformancePercent);
...
static void CPUthermalRecommitUserBrightness(void) {
    ...
    SEL copySelector=sel_registerName("copyPropertyForKey:");
    SEL setSelector=sel_registerName("setProperty:forKey:");
    if(![client respondsToSelector:copySelector]||![client respondsToSelector:setSelector])return;
    id display=((id(*)(id,SEL,id))objc_msgSend)(client,copySelector,S("DisplayBrightness"));
    id brightness=[display isKindOfClass:[NSDictionary class]]?display[S("Brightness")]:nil;
    ...
    ((void(*)(id,SEL,id,id))objc_msgSend)(client,setSelector,request,S("DisplayBrightness"));
}
...
static void CPUthermalRestoreExistingBacklightLimit(void) {
    if (!thermalDimmingPreventionEnabled() || !g_maxBacklightBrightnessValue || CPUthermalScreenIsBlanked()) return;
...
static void CPUthermalScheduleBacklightRecovery(void) {
    ...
    const double delays[] = {0.10,0.40,0.90,1.80,3.00};
    for (NSUInteger i=0;i<sizeof(delays)/sizeof(delays[0]);i++) {
        BOOL finalAttempt=(i+1==sizeof(delays)/sizeof(delays[0]));
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(delays[i]*NSEC_PER_SEC)),dispatch_get_global_queue(QOS_CLASS_USER_INITIATED,0),^{CPUthermalRestoreExistingBacklightLimit();if(finalAttempt)dispatch_async(dispatch_get_main_queue(),^{g_backlightRecoveryScheduled=NO;});});
    }
}
...
BOOL appLowPower = foregroundAppShouldUseLowPower(d);
os_unfair_lock_lock(&g_modeLock);
g_userSelectedPowerMode = selected;
g_foregroundAppForcesLowPower = appLowPower;
g_powerMode = (blanked || appLowPower) ? CPUthermalPowerModeLow : selected;
os_unfair_lock_unlock(&g_modeLock);
}
}
...
- (void)setCPULowPowerTarget:(int)target {
if (g_restoringFullPower) { %orig(target); return; }
if (shouldApplyForegroundAppLowPowerLimit()) { %orig(kLowPowerPowerLimitMW); return; }
if (shouldApplyLowPowerLimit()) { %orig(target); return; }
if (shouldApplyFullCPUProtection()) { %orig(kUnrestrictedPowerLimitMW); return; }
%orig(target);
}
...
if (shouldApplyForegroundAppLowPowerLimit()) { %orig; return; }
if (shouldApplyLowPowerLimit()) return;
if (shouldApplyFullCPUProtection()) {
// 解除温控模式: 阻断包级低功耗目标写入
return;
...
if (g_restoringFullPower) { %orig(target, legacy, propertyArg); return; }
if (shouldApplyForegroundAppLowPowerLimit()) { %orig(kLowPowerPowerLimitMW, NO, propertyArg); return; }
if (shouldApplyLowPowerLimit()) { %orig(target, legacy, propertyArg); return; }
if (shouldApplyFullCPUProtection()) { %orig(kUnrestrictedPowerLimitMW, NO, propertyArg); return; }
...
if (shouldApplyForegroundAppLowPowerLimit()) { %orig(kLowPowerPerformancePercent, source); return; }
if (shouldApplyLowPowerLimit()) {
%orig(ceiling, source);
return;
}
...
if (shouldApplyForegroundAppLowPowerLimit()) { %orig(kLowPowerPerformancePercent, contributor); return; }
if (shouldApplyLowPowerLimit()) {
%orig(ceiling, contributor);
return;
}
...
if (shouldApplyForegroundAppLowPowerLimit()) { %orig(kLowPowerPerformancePercent); return; }
if (shouldApplyLowPowerLimit()) {
%orig(target);
return;
}
...
static NSMutableArray *CPUthermalFillBacklightArrayFromFirst(NSArray *source) {
    ...
    if (![first isKindOfClass:[NSNumber class]]) return nil;
    NSMutableArray *result = [source mutableCopy];
    for (NSUInteger i = 1; i < result.count; i++) result[i] = first;
    return result;
}
...
BOOL blanked = CPUthermalScreenIsBlanked();
NSDictionary *runtimePrefs = readPrefsDictionary();
BOOL appLowPower = foregroundAppShouldUseLowPower(runtimePrefs);
CPUthermalPowerMode previous;
os_unfair_lock_lock(&g_modeLock);
previous = g_powerMode;
g_userSelectedPowerMode = selected;
...
os_unfair_lock_unlock(&g_modeLock);
// 用户每次切换功率模式都立即在当前进程应用；面板/CC同时通过 root
// MountClient立即重建 thermalmonitord，清除 PMGR/ApplePPM 旧对象与缓存。
(void)previous;
applyPowerModeToRuntime(NO);
...
CPUthermalPowerMode previousRuntimeMode;
os_unfair_lock_lock(&g_modeLock); previousRuntimeMode = g_powerMode; os_unfair_lock_unlock(&g_modeLock);
loadPrefs();
BOOL enabled = NO;
BOOL cpuProtection = NO;
BOOL blockPopup = NO;
...
if (previousPreventDimming != g_thermalPreventDimmingEnabled) scheduleThermalConfigurationReload();
    ...
    CPUthermalPowerMode currentRuntimeMode;
    os_unfair_lock_lock(&g_modeLock); currentRuntimeMode = g_powerMode; os_unfair_lock_unlock(&g_modeLock);
    if (previousRuntimeMode != currentRuntimeMode && !g_appModeRestartScheduled) {
        g_appModeRestartScheduled = YES;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 50ull * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{ kill(getpid(), SIGTERM); });
    }
}
...
NSDictionary *prefs = readPrefsDictionary();
BOOL appLowPower = foregroundAppShouldUseLowPower(prefs);
if (foregroundHash == g_lastAppliedForegroundHash && appLowPower == g_foregroundAppForcesLowPower) {
    if (appLowPower) applyPowerModeToRuntime(NO);
    return;
}
g_lastAppliedForegroundHash = foregroundHash;
onSettingsChanged(center, observer, name, object, userInfo);
}
