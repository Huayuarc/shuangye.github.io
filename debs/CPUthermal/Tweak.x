...
static const int kGameFramePacingCPUFloorPercent = 70;
...
if ([controller respondsToSelector:@selector(setCPUPowerFloor:fromDecisionSource:)]) {
sendTwoIntegerArguments(controller, @selector(setCPUPowerFloor:fromDecisionSource:), g_gameFramePacingEnabled ? kGameFramePacingCPUFloorPercent : kUnrestrictedPerformancePercent, (uintptr_t)source);
}
}
...
setCommonProductCeiling(product, @selector(setCPUPowerFloor:fromDecisionSource:), g_gameFramePacingEnabled ? kGameFramePacingCPUFloorPercent : kUnrestrictedPerformancePercent);
...
if (shouldApplyFullCPUProtection()) {
// 稳帧开启时保持 70% 高频下限，为 GPU/Package 预留持续功耗；关闭时使用激进 100%。
%orig(g_gameFramePacingEnabled ? kGameFramePacingCPUFloorPercent : kUnrestrictedPerformancePercent, source);
return;
}
%orig(floor, source);
}
