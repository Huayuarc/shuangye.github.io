...
static BOOL g_gameFramePacingEnabled = NO;
...
static BOOL shouldApplyGameEnhancement(void) {
return shouldApplyFullCPUProtection() && g_gameFramePacingEnabled;
}
...
if ([controller respondsToSelector:@selector(setCPUPowerFloor:fromDecisionSource:)]) {
sendTwoIntegerArguments(controller, @selector(setCPUPowerFloor:fromDecisionSource:), shouldApplyGameEnhancement() ? kUnrestrictedPerformancePercent : 0, (uintptr_t)source);
}
}
...
setCommonProductCeiling(product, @selector(setCPUPowerFloor:fromDecisionSource:), shouldApplyGameEnhancement() ? kUnrestrictedPerformancePercent : 0);
if (shouldApplyGameEnhancement()) {
...
if (shouldApplyGameEnhancement()) protectedComponent = protectedComponent || mentionsGPU || mentionsPackage;
if (protectedComponent) {
// 普通解除温控保护 CPU/thermal；增强开启后额外保护 GPU 与 Package 功率墙。
...
BOOL gameFramePacing = [d[S("gameFramePacingEnabled")] ?: @NO boolValue];
...
g_gameFramePacingEnabled = gameFramePacing;
os_unfair_lock_unlock(&g_stateLock);
...
if (shouldApplyFullCPUProtection()) {
%orig(shouldApplyGameEnhancement() ? kUnrestrictedPerformancePercent : 0, source);
return;
}
%orig(floor, source);
}
