...
static void scheduleThermalMonitorReload(void) {
// iOS 15 部分机型重建 thermalmonitord 后传感器不会健康签到，禁止运行时自退出。
...
// 仅处理明确 CPU/PPM 属性。GPU、Package、通用 ThermalState 与传感器状态必须放行，
// 否则 iOS 15 可能出现 Prs0 等传感器健康签到缺失。
BOOL mentionsGPU = NO;
BOOL mentionsPackage = NO;
BOOL mentionsThermal = NO;
...
- (void)tryTakeAction {
if (shouldApplyFullCPUProtection()) correctNominalStateIfNeeded();
// 保留原生健康循环；最终功率 setter 仍会拦截实际降频。
%orig;
}
...
- (void)handleTemperatureEvent:(int)arg1 service:(id)arg2 {
// 传感器事件必须放行，否则 iOS 15 watchdog 会报告 Missing sensor (如 Prs0)。
// 全屏温度计只在 ThermalManager 通知层屏蔽。
%orig;
}
...
- (void)evaluateDecisionTree {
if (shouldApplyFullCPUProtection()) correctNominalStateIfNeeded();
// 允许决策树完成传感器/健康状态更新，实际 mitigation 在最终 setter 层拦截。
%orig;
}
...
// readReleaseRateForAllComponents — 保留传感器和状态机健康更新
- (void)readReleaseRateForAllComponents {
%orig;
}
...
if (shouldApplyLowPowerLimit()) {
// 参考低频锁定实现：原生 update 前后都写 Level 2，防止锁屏唤醒恢复 Level 0。
...
if (shouldApplyFullCPUProtection()) {
// 解除温控模式: 拒绝任何 PowerSave token 降功耗指令
return;
}
%orig;
}
...
if (shouldApplyLowPowerLimit()) {
trackPowerController(self);
// 先锁低频，允许原生更新，再重复写 PowerSave/Level 2 覆盖唤醒恢复。
...
if ([self respondsToSelector:@selector(setPowerSaveActive:)]) {
((void (*)(id, SEL, BOOL))objc_msgSend)(self, @selector(setPowerSaveActive:), YES);
}
...
if (shouldApplyFullCPUProtection()) {
trackPowerController(self);
// Ceiling/PowerSave setter 仍会被逐项拦截；这里直接放行原生 DVFS 刷新。
%orig;
return;
}
%orig;
...
if (shouldApplyFullCPUProtection()) {
// 解除温控模式: 阻断 CPMS 低功耗目标写入（mW），维持最大组件功耗墙
return;
}
%orig;
}
...
if (shouldApplyFullCPUProtection()) {
// 解除温控模式: 阻断 CPMS 最大功耗目标写入（mW），阻止系统改写最大组件功耗墙
return;
}
%orig;
}
...
if (shouldApplyFullCPUProtection()) {
// 解除温控模式: 阻断 CPMS 功率上限写入，禁止系统下压功耗上限
return;
}
%orig;
}
