...
static void handleWakeStateToken(int token, const char *source) {
uint64_t state = UINT64_MAX;
if (token <= 0 || notify_get_state(token, &state) != NOTIFY_STATUS_OK) return;
// lockstate=0 表示解锁；hasBlankedScreen=0 表示屏幕点亮。
if (state == 0 && isLowPowerMode()) switchToFullPowerAfterWake(source);
}
...
handleWakeStateToken(token, "unlock");
});
}
...
handleWakeStateToken(token, "screen-on");
});
}
}
