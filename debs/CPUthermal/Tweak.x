...
static BOOL g_gameAntiThrottleEnabled = YES;
static dispatch_source_t g_gameCorrectionTimer = NULL;
static const uint64_t kGameCorrectionIntervalNsec = 2ull * NSEC_PER_SEC;
...
static void startGameCorrectionTimer(void);
static void stopGameCorrectionTimer(void);
static void applyGamePerformanceCorrection(void);
...
static void applyGamePerformanceCorrection(void) {
if (!g_gameAntiThrottleEnabled || !shouldApplyFullCPUProtection()) return;
BOOL previousRestoring = g_restoringFullPower;
@try {
g_restoringFullPower = YES;
CommonProduct *product = commonProductSnapshot();
...
if ([product respondsToSelector:@selector(setCPULevel:)]) {
((void (*)(id, SEL, int))objc_msgSend)(product, @selector(setCPULevel:), kFullPowerCPULevel);
}
setCommonProductCeiling(product, @selector(setCPUPowerCeiling:fromDecisionSource:), kUnrestrictedPerformancePercent);
setCommonProductCeiling(product, @selector(setCPUPowerFloor:fromDecisionSource:), kUnrestrictedPerformancePercent);
setCommonProductCeiling(product, @selector(setGPUPowerCeiling:fromDecisionSource:), kUnrestrictedPerformancePercent);
...
for (id ppm in trackedApplePPMInstancesSnapshot()) {
if ([ppm respondsToSelector:@selector(setCPULevel:)]) {
((void (*)(id, SEL, int))objc_msgSend)(ppm, @selector(setCPULevel:), kFullPowerCPULevel);
}
}
} @catch (NSException *exception) {
NSLog(@"[CPUthermal] 游戏性能校正失败: %@", exception);
} @finally {
g_restoringFullPower = previousRestoring;
}
}
...
static void startGameCorrectionTimer(void) {
if (!g_gameAntiThrottleEnabled || !shouldApplyFullCPUProtection()) {
stopGameCorrectionTimer();
return;
}
os_unfair_lock_lock(&g_runtimeLock);
if (g_gameCorrectionTimer) {
os_unfair_lock_unlock(&g_runtimeLock);
return;
}
...
g_gameCorrectionTimer = timer;
dispatch_source_set_timer(timer, dispatch_time(DISPATCH_TIME_NOW, 0), kGameCorrectionIntervalNsec, 250ull * NSEC_PER_MSEC);
dispatch_source_set_event_handler(timer, ^{
if (!g_gameAntiThrottleEnabled || !shouldApplyFullCPUProtection()) {
stopGameCorrectionTimer();
return;
}
applyGamePerformanceCorrection();
});
...
static void stopGameCorrectionTimer(void) {
os_unfair_lock_lock(&g_runtimeLock);
dispatch_source_t timer = g_gameCorrectionTimer;
g_gameCorrectionTimer = NULL;
if (timer) {
...
if (mentionsCPU || mentionsGPU || mentionsPackage || mentionsThermal) {
// 功耗上限 / 频率 / 速度 / 节流等级类属性
...
BOOL enabled = YES;
BOOL blockPopup = [d[S("thermalBlockNotifPopup")] ?: @YES boolValue];
BOOL preventDimming = [d[S("thermalPreventDimmingEnabled")] ?: @NO boolValue];
BOOL gameAntiThrottle = [d[S("gameAntiThrottleEnabled")] ?: @YES boolValue];
os_unfair_lock_lock(&g_stateLock);
g_enabled = enabled;
g_thermalBlockNotifPopup = blockPopup;
g_thermalPreventDimmingEnabled = preventDimming;
g_gameAntiThrottleEnabled = gameAntiThrottle;
os_unfair_lock_unlock(&g_stateLock);
...
if (shouldApplyFullCPUProtection()) {
// 高频保持：系统每次更新 Floor 时都维持 100，确保模式切换后立即升频。
%orig(kUnrestrictedPerformancePercent, source);
return;
}
%orig(floor, source);
...
NSLog(@"[CPUthermal] 已修补热配置: 防温控暗屏=1");
}
