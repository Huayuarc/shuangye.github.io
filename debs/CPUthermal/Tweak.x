...
static BOOL g_delayedThermalTrapTargetsEnabled = NO;
static BOOL g_highPerformanceModeEnabled = NO;
...
return enabled && blockNetwork && isFullPowerMode() && g_highPerformanceModeEnabled;
}
...
NSLog(@"[CPUthermal] %s 状态临时进入低功耗，保留用户模式:%@", source ?: "sleep",
...
if (previous == CPUthermalPowerModeLow && target == CPUthermalPowerModeFull)
    scheduleThermalMonitorReload();
}
NSLog(@"[CPUthermal] %s 状态恢复用户模式:%@", source ?: "wake",
      target == CPUthermalPowerModeLow ? S("低功耗") : S("解除温控"));
}
...
if (shouldApplyHighPerformanceMode()) {
...
if (shouldApplyHighPerformanceMode()) protectedComponent = protectedComponent || mentionsGPU || mentionsPackage;
if (protectedComponent) {
// 普通解除温控保护 CPU；高性能模式额外保护 GPU 与 Package 功率墙。
...
static float (*origSupervisor_Effort)(id, SEL, id, id) = NULL;
...
static float aliasSupervisor_Effort(id self, SEL cmd, id a, id b) {
if (shouldApplyFullCPUProtection()) return 0.0f;
return origSupervisor_Effort ? origSupervisor_Effort(self, cmd, a, b) : 0.0f;
}
...
if (!origTDT_GetRelease && CPUthermalMethodMatches(tree, release, 3, 'f')) MSHookMessageEx(tree, release, (IMP)aliasTDT_GetRelease, (IMP *)&origTDT_GetRelease);
}
Class supervisor = objc_getClass("SupervisorControl");
SEL effort = sel_registerName("calculateControlEffort:trigger:");
if (supervisor && !origSupervisor_Effort && CPUthermalMethodMatches(supervisor, effort, 4, 'f')) MSHookMessageEx(supervisor, effort, (IMP)aliasSupervisor_Effort, (IMP *)&origSupervisor_Effort);
Class component = objc_getClass("ComponentControl");
SEL cpms = sel_registerName("setCPMSMitigationState:");
if (component && !origComponent_CPMS && CPUthermalMethodMatches(component, cpms, 3, 'v')) MSHookMessageEx(component, cpms, (IMP)aliasComponent_CPMS, (IMP *)&origComponent_CPMS);
Class notification = objc_getClass("NotificationManager");
SEL update = sel_registerName("updateThermalNotification:");
if (notification && !origNotification_Thermal && CPUthermalMethodMatches(notification, update, 3, 'v')) MSHookMessageEx(notification, update, (IMP)aliasNotification_Thermal, (IMP *)&origNotification_Thermal);
}
...
SEL selector;
IMP original;
} CPUthermalSoCPowerHook;
static CPUthermalSoCPowerHook g_socPowerHooks[24];
static NSUInteger g_socPowerHookCount = 0;

static IMP CPUthermalOriginalSoCPowerIMP(id object, SEL selector) {
for (Class cls = [object class]; cls; cls = class_getSuperclass(cls)) {
for (NSUInteger i = 0; i < g_socPowerHookCount; i++)
if (g_socPowerHooks[i].cls == cls && g_socPowerHooks[i].selector == selector) return g_socPowerHooks[i].original;
}
return NULL;
}

static void CPUthermalSoCPowerReplacement(id self, SEL selector, int ceiling, uintptr_t source) {
IMP original = CPUthermalOriginalSoCPowerIMP(self, selector);
if (!original) return;
int patched = shouldApplyHighPerformanceMode() ? kUnrestrictedPerformancePercent : ceiling;
((void (*)(id, SEL, int, uintptr_t))original)(self, selector, patched, source);
}

static BOOL CPUthermalIntegerArgumentType(char type) {
return strchr("cCsSiIlLqQB", type) != NULL;
}

static void CPUthermalInstallSoCPowerHook(Class cls, SEL selector) {
if (!cls || !selector || g_socPowerHookCount >= 24) return;
for (NSUInteger i = 0; i < g_socPowerHookCount; i++)
if (g_socPowerHooks[i].cls == cls && g_socPowerHooks[i].selector == selector) return;
Method method = class_getInstanceMethod(cls, selector);
if (!method || method_getNumberOfArguments(method) != 4) return;
char ret[16]={0}, first[16]={0}, second[16]={0};
method_getReturnType(method, ret, sizeof(ret)); method_getArgumentType(method,2,first,sizeof(first)); method_getArgumentType(method,3,second,sizeof(second));
const char *r=ret,*a=first,*b=second; while(*r&&strchr("rnNoORV",*r))r++; while(*a&&strchr("rnNoORV",*a))a++; while(*b&&strchr("rnNoORV",*b))b++;
if (*r!='v' || !CPUthermalIntegerArgumentType(*a) || (!CPUthermalIntegerArgumentType(*b) && *b!='@')) return;
NSUInteger index=g_socPowerHookCount++;
g_socPowerHooks[index]=(CPUthermalSoCPowerHook){cls,selector,NULL};
MSHookMessageEx(cls,selector,(IMP)CPUthermalSoCPowerReplacement,&g_socPowerHooks[index].original);
}

static void installA15PlusSoCPowerHooks(void) {
static const char *classes[]={"MitigationController","PackagePowerCC","SoCPowerCC","DiePowerCC","ApplePPMSOC","ApplePPMCPU",NULL};
static const char *selectors[]={"setSoCPowerCeiling:fromDecisionSource:","setSystemPowerCeiling:fromDecisionSource:","setClusterPowerCeiling:fromDecisionSource:","setDiePowerCeiling:fromDecisionSource:","setPackagePowerCeiling:fromDecisionSource:",NULL};
for(NSUInteger c=0;classes[c];c++){
Class cls=objc_getClass(classes[c]); if(!cls)continue;
for(NSUInteger s=0;selectors[s];s++) CPUthermalInstallSoCPowerHook(cls,sel_registerName(selectors[s]));
}
}
...
BOOL delayedThermalTrapTargets = [d[S("delayedThermalTrapTargetsEnabled")] ?: @NO boolValue];
BOOL highPerformanceMode = [d[S("highPerformanceModeEnabled")] ?: @NO boolValue];
g_delayedThermalTrapTargetsEnabled = delayedThermalTrapTargets;
g_highPerformanceModeEnabled = highPerformanceMode;
...
CPUthermalPowerMode selected = [mode isEqualToString:S("lowPower")] ? CPUthermalPowerModeLow : CPUthermalPowerModeFull;
BOOL blanked = CPUthermalScreenIsBlanked();
os_unfair_lock_lock(&g_modeLock);
g_userSelectedPowerMode = selected;
g_powerMode = blanked ? CPUthermalPowerModeLow : selected;
os_unfair_lock_unlock(&g_modeLock);
}
}
}
...
if (shouldApplyFullCPUProtection()) {
%orig(shouldApplyHighPerformanceMode() ? kUnrestrictedPerformancePercent : 0, source);
return;
}
%orig(floor, source);
}
...
static id CPUthermalMaximumNumericValue(NSArray *values) {
...
static id CPUthermalPatchBacklightNode(id node, BOOL displayContext, NSString *parentKey) {
    if ([node isKindOfClass:[NSDictionary class]]) {
        ...
        for (id rawKey in [(NSDictionary *)node allKeys]) {
            ...
            if (childContext && ([lower isEqualToString:S("expectscpmssupport")] ||
                                 [lower isEqualToString:S("thermalmitigationenabled")])) {
                result[key] = @NO;
                continue;
            }
            if (childContext && ([lower isEqualToString:S("maxthermalpower")] ||
                                 [lower isEqualToString:S("minthermalpower")])) {
                [result removeObjectForKey:key];
                continue;
            }
            result[key] = CPUthermalPatchBacklightNode(value, childContext, key) ?: value;
        }
        return result;
    }
    ...
    if ([node isKindOfClass:[NSArray class]]) {
        ...
        BOOL brightnessArray = displayContext &&
            ([lowerKey containsString:S("brightness")] ||
             ...
             [lowerKey isEqualToString:S("backlightpower")]);
        if (brightnessArray && array.count > 0) {
            id maximum = CPUthermalMaximumNumericValue(array);
            if (maximum) {
                NSMutableArray *filled = [NSMutableArray arrayWithCapacity:array.count];
                for (NSUInteger i = 0; i < array.count; i++) [filled addObject:maximum];
                return filled;
            }
        }
        ...
        for (id value in array) {
            [result addObject:CPUthermalPatchBacklightNode(value, displayContext, parentKey) ?: value];
        }
        return result;
    }
    return node;
...
static id CPUthermalTrapTargetValueForKey(NSString *key, id originalValue) {
    if (![key isKindOfClass:[NSString class]]) return nil;
    NSInteger target = NSNotFound;
    static const char *targets99[] = {
        "ForcedThermalLevelTarget0",
        "ForcedThermalLevelTarget1",
        "ForcedThermalPressureLevelLightTarget",
        "THERMAL_TRAP_LOAD"
    };
    for (NSUInteger i = 0; i < sizeof(targets99)/sizeof(targets99[0]); i++) {
        if ([key caseInsensitiveCompare:S(targets99[i])] == NSOrderedSame) {
            target = 99;
            break;
        }
    }
    if ([key caseInsensitiveCompare:S("THERMAL_TRAP_SLEEP")] == NSOrderedSame) target = 100;
    if (target == NSNotFound) return nil;
    if ([originalValue isKindOfClass:[NSString class]])
        return [NSString stringWithFormat:S("%ld"), (long)target];
    return [NSNumber numberWithInteger:target];
}

static BOOL CPUthermalDictionaryHasTrapContext(NSDictionary *dict) {
    for (id rawKey in dict) {
        if (![rawKey isKindOfClass:[NSString class]]) continue;
        NSString *key = (NSString *)rawKey;
        if ([key caseInsensitiveCompare:S("ForcedThermalLevelTarget0")] == NSOrderedSame ||
            [key caseInsensitiveCompare:S("ForcedThermalLevelTarget1")] == NSOrderedSame ||
            [key caseInsensitiveCompare:S("ForcedThermalPressureLevelLightTarget")] == NSOrderedSame ||
            [key caseInsensitiveCompare:S("THERMAL_TRAP_LOAD")] == NSOrderedSame ||
            [key caseInsensitiveCompare:S("THERMAL_TRAP_SLEEP")] == NSOrderedSame) return YES;
    }
    return NO;
}

static id CPUthermalPatchTrapTargets(id node) {
    if ([node isKindOfClass:[NSDictionary class]]) {
        NSDictionary *source = (NSDictionary *)node;
        NSMutableDictionary *result = [source mutableCopy];
        BOOL trapContext = CPUthermalDictionaryHasTrapContext(source);
        for (id rawKey in [source allKeys]) {
            id value = [source objectForKey:rawKey];
            if ([rawKey isKindOfClass:[NSString class]]) {
                if (trapContext && [(NSString *)rawKey caseInsensitiveCompare:S("target")] == NSOrderedSame) {
                    result[rawKey] = [value isKindOfClass:[NSString class]] ? S("99") : [NSNumber numberWithInt:99];
                    continue;
                }
                id replacement = CPUthermalTrapTargetValueForKey(rawKey, value);
                if (replacement) {
                    result[rawKey] = replacement;
                    continue;
                }
            }
            result[rawKey] = CPUthermalPatchTrapTargets(value) ?: value;
        }
        return result;
    }
    if ([node isKindOfClass:[NSArray class]]) {
        NSMutableArray *result = [NSMutableArray arrayWithCapacity:[node count]];
        for (id value in node) [result addObject:CPUthermalPatchTrapTargets(value) ?: value];
        return result;
    }
    return node;
}
...
static NSDictionary *patchThermalPlist(NSDictionary *dict) {
    ...
    BOOL patchBacklight = thermalDimmingPreventionEnabled();
    BOOL patchTrapTargets = g_delayedThermalTrapTargetsEnabled;
    if (!patchBacklight && !patchTrapTargets) return dict;

    id patched = dict;
    if (patchBacklight) patched = CPUthermalPatchBacklightNode(patched, NO, nil);
    if (patchTrapTargets) patched = CPUthermalPatchTrapTargets(patched);
    if (patchBacklight) NSLog(@"[CPUthermal] 已按设备热配置最大值修补背光亮度/功率档位");
    if (patchTrapTargets) NSLog(@"[CPUthermal] 已在内存中延后 forced thermal/trap 目标");
...
id res = %orig(path);
if ((thermalDimmingPreventionEnabled() || g_delayedThermalTrapTargetsEnabled) &&
...
if (CPUthermalReadPostedPowerMode(&lowPower)) {
selected = lowPower ? CPUthermalPowerModeLow : CPUthermalPowerModeFull;
} else {
NSDictionary *prefs = readPrefsDictionary();
...
applyPowerModeToRuntime(NO);
if (previous == CPUthermalPowerModeLow && runtimeMode == CPUthermalPowerModeFull)
    scheduleThermalMonitorReload();
}
NSLog(@"[CPUthermal] 用户模式已选择:%@，当前屏幕:%@，运行模式:%@",
...
BOOL previousTrapTargets = g_delayedThermalTrapTargetsEnabled;
...
if (previousTrapTargets != g_delayedThermalTrapTargetsEnabled ||
    previousPreventDimming != g_thermalPreventDimmingEnabled) scheduleThermalConfigurationReload();
...
installA15PlusSoCPowerHooks();
applyCurrentPowerModeToRuntime();
NSLog(@"[CPUthermal] 启动完成，当前功率模式:%@，低功耗将在下次亮屏/解锁后自动结束", isLowPowerMode() ? S("低功耗") : S("解除温控"));
}
}
