...
// 前台指定应用使用可观察且可恢复的低功耗预算；退出后由 fullPower 恢复链还原 100%/65000mW。
...
static BOOL g_modeRestartScheduled = NO;
...
// thermalmonitord 启动前 8 秒保持原生初始化，避免 iOS15 传感器健康检查失败。
...
static void restartThermalMonitorForModeTransition(void);
...
return bootSettled() && enabled && blockNetwork && isFullPowerMode();
}
...
return bootSettled() && runtimeProtectionEnabled() && isFullPowerMode();
}
...
static BOOL shouldApplyLowPowerLimit(void) {
return bootSettled() && runtimeProtectionEnabled() && isLowPowerMode();
}

static CTRThermalMode CPUthermalRecoveredMode(void) {
// 低功耗交还系统；解除温控启用反汇编恢复的 Aggressive 行为：
// die=2500（25°C）、skin=0、跳过决策树，并将 SupervisorControl 输入固定为 25.0°C。
...
// 模式切换只软重推运行状态，不终止 thermalmonitord，避免亮屏黑屏。
...
static void restartThermalMonitorForModeTransition(void) {
// 手动模式和指定应用模式转换共用；标记新进程快速启动，防抖避免重复前台通知形成重启环。
os_unfair_lock_lock(&g_runtimeLock);
if(g_modeRestartScheduled){os_unfair_lock_unlock(&g_runtimeLock);return;}
g_modeRestartScheduled=YES;
os_unfair_lock_unlock(&g_runtimeLock);
CPUthermalMarkFastModeRestart();
...
NSString *tool=CPUthermalExistingExecutablePath("/usr/bin/killall",@[
S("/var/jb/usr/bin/killall"),S("/var/jb/bin/killall"),S("/usr/bin/killall"),S("/bin/killall")]);
if(!tool.length){os_unfair_lock_lock(&g_runtimeLock);g_modeRestartScheduled=NO;os_unfair_lock_unlock(&g_runtimeLock);return;}
pid_t pid=0;char *args[]={(char*)"killall",(char*)"-q",(char*)"thermalmonitord",NULL};
int result=posix_spawn(&pid,tool.fileSystemRepresentation,NULL,NULL,args,NULL);
if(result){os_unfair_lock_lock(&g_runtimeLock);g_modeRestartScheduled=NO;os_unfair_lock_unlock(&g_runtimeLock);}
});
}
...
target = g_foregroundAppForcesLowPower ? CPUthermalPowerModeLow : g_userSelectedPowerMode;
g_powerMode = target;
os_unfair_lock_unlock(&g_modeLock);
...
// 初始屏幕状态已由 loadPrefs() 的 CPUthermalScreenIsBlanked() 同步；
...
if (respectBootGuard && !bootSettled()) return;
if (isLowPowerMode()) {
...
// 切换后 100ms 立即重施，之后每秒保活；不等待旧版 5 秒首轮定时器。
dispatch_source_set_timer(timer, dispatch_time(DISPATCH_TIME_NOW, 100ull * NSEC_PER_MSEC),
                          1ull * NSEC_PER_SEC, 100ull * NSEC_PER_MSEC);
...
// 只识别明确的温控背光上限键。普通 brightness/luminance/nits 写入也承担
// 自动锁定前的正常渐暗；若按最大值替换，会导致自动锁定计时到期后屏幕仍长亮。
...
BOOL backlightValue = [lower containsString:S("brightness")] ||
                      [lower containsString:S("luminance")] ||
                      [lower containsString:S("nits")] ||
                      [lower containsString:S("backlight")];
return backlightValue && explicitCap && explicitThermal;
}
...
if (replacement) {
[filtered setObject:replacement forKey:key];
} else {
...
// 只处理明确的热背光上限键；普通亮度渐变和自动锁定生命周期写入原样放行。
if (replacement) return orig_IOServiceSetProperty(service, key, (__bridge CFTypeRef)replacement);
return KERN_SUCCESS;
}
...
- (void)evaluateDecisionTree {
// 全功率模式: 阻止决策树运行，避免温控降频；启动静默期内原样放行。
...
static void CPUthermalRecordMaximumBacklightValue(NSNumber *value, NSString *key) {
...
static id CPUthermalPatchBacklightNode(id node, BOOL displayContext, NSString *parentKey) {
    if ([node isKindOfClass:[NSDictionary class]]) {
        ...
        for (id rawKey in [(NSDictionary *)node allKeys]) {
            ...
            if ([value isKindOfClass:[NSNumber class]] && keyIsBacklightThermalLimit(key))
                CPUthermalRecordMaximumBacklightValue(value, key);
    ...
    if ([node isKindOfClass:[NSArray class]]) {
        ...
        if (brightnessArray && array.count > 0) {
            ...
            if (maximum) {
                CPUthermalRecordMaximumBacklightValue(maximum, parentKey);
...
static NSDictionary *patchThermalPlist(NSDictionary *dict) {
    ...
    id patched = CPUthermalPatchBacklightNode(dict, NO, nil);
    NSLog(@"[CPUthermal] 已按设备热配置最大值修补背光亮度/功率档位");
...
NSLog(@"[CPUthermal] plist 背光补丁异常，回退原始配置: %@", exception);
return res;
}
}
}
...
static NSDictionary *new_getConfigurationFor(NSString *key) {
    ...
    @try {
        ...
        id recovered = CTRTransformThermalConfiguration(backlightPatched, CPUthermalRecoveredMode());
        return [recovered isKindOfClass:[NSDictionary class]] ? recovered : backlightPatched;
    } @catch (NSException *exception) {
        NSLog(@"[CPUthermal] 配置补丁异常，回退原始配置: %@", exception);
        return config;
    }
}
...
os_unfair_lock_unlock(&g_modeLock);
if(previous!=runtimeMode)applyPowerModeToRuntime(NO);
// 手动模式切换由 Settings 通过 root MountTool 直接 killall，避免当前进程通知调度窗口和双重重启。
...
CPUthermalPowerMode previousMode;
os_unfair_lock_lock(&g_modeLock);
previousMode = g_powerMode;
os_unfair_lock_unlock(&g_modeLock);
loadPrefs();
CPUthermalPowerMode currentMode;
os_unfair_lock_lock(&g_modeLock);
...
if(previousMode!=currentMode)restartThermalMonitorForModeTransition();
...
static void reconcileForegroundAppState(void) {
uint64_t foregroundHash = CPUthermalReadForegroundBundleHash();
NSDictionary *prefs = readPrefsDictionary();
...
previous = g_powerMode;
g_foregroundAppForcesLowPower = appLowPower;
target = (blanked || appLowPower) ? CPUthermalPowerModeLow : g_userSelectedPowerMode;
g_powerMode = target;
os_unfair_lock_unlock(&g_modeLock);
g_lastAppliedForegroundHash = foregroundHash;
// 命中时即使模式未变化也重施 Level 2、2500mW、45%，覆盖随后到达的原生 update。
if (previous != target || appLowPower) applyPowerModeToRuntime(NO);
if(previous!=target)restartThermalMonitorForModeTransition();
}
...
if ([NSThread isMainThread]) reconcileForegroundAppState();
else dispatch_async(dispatch_get_main_queue(), ^{ reconcileForegroundAppState(); });
}
...
// 模式切换重启两个方向均150ms应用；普通冷启动仍保留8秒传感器健康检查。
BOOL fastModeRestart=CPUthermalConsumeFastModeRestart();
NSTimeInterval bootSettleDelay=(fastModeRestart||isLowPowerMode())?0.15:8.0;
dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(bootSettleDelay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
atomic_store_explicit(&g_bootSettled, true, memory_order_release);
CTRSetThermalModeProvider(CPUthermalRecoveredMode);
...
// Darwin 通知是主路径；250ms 读取 notify state 作为丢通知/场景切换兜底。
// 只有前台 hash 变化或当前命中指定应用时才执行，非目标应用不产生持续功率写入。
...
if(hash!=g_lastAppliedForegroundHash || g_foregroundAppForcesLowPower) reconcileForegroundAppState();
}];

registerScreenWakeObservers();
// CTRSetThermalModeProvider 延迟到 8 秒静默期结束，启动阶段保持原生读数。
...
// 运行功率状态已由 8 秒静默期 block 统一应用。
NSLog(@"[CPUthermal] 启动完成，等待静默期后应用功率模式");
}
}
