...
if ([controller respondsToSelector:@selector(setCPUPowerFloor:fromDecisionSource:)]) {
sendTwoIntegerArguments(controller, @selector(setCPUPowerFloor:fromDecisionSource:), kUnrestrictedPerformancePercent, (uintptr_t)source);
}
}
...
setCommonProductCeiling(product, @selector(setCPUPowerFloor:fromDecisionSource:), kUnrestrictedPerformancePercent);
...
// 解除温控模式统一保护 CPU、GPU 与 Package 功率墙。
...
if (previousTrapTargets != g_delayedThermalTrapTargetsEnabled) scheduleThermalConfigurationReload();
