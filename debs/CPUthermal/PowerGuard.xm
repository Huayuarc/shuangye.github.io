...
// 改为跑固定工作量：最快一次作为“满频基准”，其余与之比较得到等效频率百分比。
static uint64_t gBestPerfUs = 0;

static double PerfBurst(void) {
    ...
    double us = (double)(end.tv_sec - start.tv_sec) * 1e6 + (double)(end.tv_nsec - start.tv_nsec) / 1000.0;
    return us > 0.0 ? us : 0.001;
}

static void SampleFrame(const char *reason) {
    // 尽量跑在性能核上，否则会落到能效核导致读数虚低
    pthread_set_qos_class_self_np(QOS_CLASS_USER_INTERACTIVE, 0);
    PerfBurst();                                    // 第一趟让 DVFS 升档
    double us = PerfBurst();                        // 第二趟作为测量值
    if (gBestPerfUs == 0 || us < (double)gBestPerfUs) gBestPerfUs = (uint64_t)us;
    double ratio = gBestPerfUs > 0 ? (double)gBestPerfUs / us * 100.0 : 100.0;
    TLog(@"SAMPLE[%s] perf=%.0fus best=%lluus equiv=%.0f%% pressure=%d thermalState=%d",
         reason, us, (unsigned long long)gBestPerfUs, ratio, ThermalPressureLevel(), ThermalStateValue());
}
