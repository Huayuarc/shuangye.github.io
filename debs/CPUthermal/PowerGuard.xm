...
static int BatteryTempDeciCelsius(void);
...
// 算力采样（1.6.2-78 修正口径）
//
// 教训：之前用“固定墙钟窗口内的迭代数”，结果在负载下读数是 CPU 抢占比例，
// 不是频率 —— 现场出现 29%~100% 的大幅横跳，而峰值 4,840,000/12ms 在三段
// 日志里都出现过，说明峰值算力根本没被压。
//
// 现在改为“每单位 CPU 时间完成的迭代数”（CLOCK_THREAD_CPUTIME_ID）：
// 抢占不计入，读数只反映频率。
//   st = 单线程最快核（性能核）频率代理
//   mt = 全部核心聚合吞吐（多核持续频率代理，用来发现“只有多核被压”的情况）
// ===========================================================================
static uint64_t gBestStIterations = 0;
static uint64_t gBestMtIterations = 0;

static uint64_t PerfBurstCPU(double cpu_window_ms) {
    ...
    for (;;) {
        ...
        double cpu_us = (double)(now.tv_sec - start.tv_sec) * 1e6
                      + (double)(now.tv_nsec - start.tv_nsec) / 1000.0;
        if (cpu_us >= cpu_window_ms * 1000.0) break;
...
typedef struct { uint64_t iterations; } PerfSlot;

static void *PerfThreadMain(void *context) {
    PerfSlot *slot = (PerfSlot *)context;
    pthread_set_qos_class_self_np(QOS_CLASS_USER_INITIATED, 0);
    slot->iterations = PerfBurstCPU(10.0);
    return NULL;
}
...
static uint64_t PerfMultiThread(int threads) {
    if (threads < 1) threads = 1;
    pthread_t tids[16];
    PerfSlot slots[16];
    int created = 0;
    for (int i = 0; i < threads && i < 16; i++) {
        slots[i].iterations = 0;
        if (pthread_create(&tids[i], NULL, PerfThreadMain, &slots[i]) == 0) created++;
        else tids[i] = (pthread_t)0;
    }
    uint64_t total = 0;
    for (int i = 0; i < threads && i < 16; i++) {
        if (tids[i]) pthread_join(tids[i], NULL);
        total += slots[i].iterations;
    }
...
static void SampleFrame(const char *reason) {
    ...
    for (int round = 0; round < 2; round++) {
        uint64_t iterations = PerfBurstCPU(10.0);
        if (iterations > st) st = iterations;
    }
    if (st > gBestStIterations) gBestStIterations = st;
    double stPct = gBestStIterations > 0 ? (double)st / (double)gBestStIterations * 100.0 : 100.0;

    static int mtCounter = 0;
    double mtPct = -1.0;
    uint64_t mt = 0;
    if (++mtCounter % 3 == 0) {          // 多核聚合吞吐每 15 秒测一次，避免额外热源
        int cores = CPUCoreCount();
        mt = PerfMultiThread(cores);
        if (mt > gBestMtIterations) gBestMtIterations = mt;
        mtPct = gBestMtIterations > 0 ? (double)mt / (double)gBestMtIterations * 100.0 : 100.0;
    }
    ...
    int batteryTempDeci = BatteryTempDeciCelsius();

    if (mtPct >= 0.0) {
        TLog(@"SAMPLE[%s] st=%.0f%% mt=%.0f%% load=%.2f lpm=%d bat=%dmV/%dmA/%d%%/%d.%dC pressure=%d thermalState=%d",
             reason, stPct, mtPct, load, lpm ? 1 : 0, milliVolts, milliAmps, soc,
             batteryTempDeci / 10, batteryTempDeci % 10,
             ThermalPressureLevel(), ThermalStateValue());
    } else {
        TLog(@"SAMPLE[%s] st=%.0f%% load=%.2f lpm=%d bat=%dmV/%dmA/%d%%/%d.%dC pressure=%d thermalState=%d",
             reason, stPct, load, lpm ? 1 : 0, milliVolts, milliAmps, soc,
             batteryTempDeci / 10, batteryTempDeci % 10,
             ThermalPressureLevel(), ThermalStateValue());
    }
}
