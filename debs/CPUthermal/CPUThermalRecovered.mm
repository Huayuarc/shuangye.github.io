...
static void TransformDictionary(NSMutableDictionary *d, CTRThermalMode mode) {
    ...
    // 反汇编确认值：三个功率上限均为 0x1388(5000)，maxCPU/maxGPU 均为 5。
    ReplaceExisting(d,CTR_S("minPackagePower"),@5000);
    ReplaceExisting(d,CTR_S("CPUMaxPower"),@5000);
    ReplaceExisting(d,CTR_S("GPUMaxPower"),@5000);
    ReplaceExisting(d,CTR_S("PackageMaxPower"),@5000);
...
void CTRInstallRecoveredThermalHooks(void) {
    ...
    // 与宿主已有 ThermalManager/ComponentControl Hook 重叠的方法继续由宿主统一处理，
    // 本模块只补齐 1.6.3-27 未覆盖的 CommonProduct 私有压力路径。
}
