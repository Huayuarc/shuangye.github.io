...
static void NormalizeDecisionRows(id table) {
    ...
    for (id row in table) if ([row isKindOfClass:NSMutableDictionary.class]) {
        ...
        ReplaceExisting(d,CTR_S("target"),@25);
        ReplaceExisting(d,CTR_S("alternateTarget"),@25);
    }
}
static void TransformDictionary(NSMutableDictionary *d, CTRThermalMode mode) {
    ...
    // 只修改功率预算和决策树目标，不改传感器/上下文布尔开关、不删除原配置键，
    // 避免 iOS15 thermalmonitord 启动健康检查（Prs0 等）失败。
...
ORIG(int,dieTemp);
static int new_dieTemp(id self,SEL cmd) { return CurrentMode()==CTRThermalModeAggressive ? 2500 : orig_dieTemp(self,cmd); }
...
static float new_effort(id self,SEL cmd,float a,float b) {
    return CurrentMode()==CTRThermalModeAggressive ? orig_effort(self,cmd,25.0f,25.0f) : orig_effort(self,cmd,a,b);
}
