...
static inline void CPUthermalPostRefreshRateState(BOOL force120) {
    int token = 0;
...
static inline BOOL CPUthermalReadRefreshRateState(BOOL *force120) {
...
static inline NSString *CPUthermalRefreshInjectionNotification(NSString *bundleID) {
    return [S("com.huayuarc.cputhermal/refreshInject/") stringByAppendingFormat:S("%016llx"),(unsigned long long)CPUthermalRefreshBundleHash(bundleID)];
}
static inline void CPUthermalPostRefreshInjection(NSString *bundleID, BOOL selected) {
    NSString *name=CPUthermalRefreshInjectionNotification(bundleID); if(!name.length)return; int token=0;
    if(notify_register_check(name.UTF8String,&token)!=NOTIFY_STATUS_OK)return;notify_set_state(token,selected?1:0);notify_post(name.UTF8String);notify_cancel(token);
}
static inline BOOL CPUthermalRefreshAppSelected(NSString *bundleID) {
    if(!bundleID.length)return NO; NSString *name=CPUthermalRefreshInjectionNotification(bundleID);int token=0;uint64_t state=0;
    ...
    NSDictionary *prefs=CPUthermalReadPrefs();NSArray *selected=[prefs[S("refreshRateInjectedApps")] isKindOfClass:[NSArray class]]?prefs[S("refreshRateInjectedApps")]:nil;
