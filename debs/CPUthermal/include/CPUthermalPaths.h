...
static inline BOOL CPUthermalClearForegroundBundleIDIfCurrent(NSString *bundleID) {
    uint64_t ownHash = CPUthermalBundleIDHash(bundleID); if (!ownHash) return NO;
    int token=0;uint64_t current=0;
    ...
    if(result==NOTIFY_STATUS_OK&&current==ownHash){result=notify_set_state(token,0);if(result==NOTIFY_STATUS_OK)result=notify_post(kCPUthermalForegroundAppNotifC);}
    notify_cancel(token);return result==NOTIFY_STATUS_OK&&current==ownHash;
}
