...
static const char *kCPUthermalFastModeRestartNotifC = "com.huayuarc.cputhermal/fastModeRestart";
...
static inline void CPUthermalMarkFastModeRestart(void) {
    int token=0; if(notify_register_check(kCPUthermalFastModeRestartNotifC,&token)!=NOTIFY_STATUS_OK)return;
    notify_set_state(token,1); notify_post(kCPUthermalFastModeRestartNotifC); notify_cancel(token);
}

static inline BOOL CPUthermalConsumeFastModeRestart(void) {
    int token=0; uint64_t state=0;
    if(notify_register_check(kCPUthermalFastModeRestartNotifC,&token)!=NOTIFY_STATUS_OK)return NO;
    int result=notify_get_state(token,&state); if(result==NOTIFY_STATUS_OK&&state)notify_set_state(token,0);
    notify_cancel(token); return result==NOTIFY_STATUS_OK&&state!=0;
}
