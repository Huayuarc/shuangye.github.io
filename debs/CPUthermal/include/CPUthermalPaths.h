...
// ============================================================================
// 硬件层绝杀状态（MitigationHook.dylib）/ IORegistryEntrySetCFProperty 写边界
// 位打包：bit0 = 低功耗模式(1) / 解除温控(0)；bit8 = 拦截暗屏降亮度；bit9 = 强制满血快充。
// 由设置面板在开关/powerMode 变化时发布，MitigationHook 实时读取以驱动底层拦截。
// ============================================================================
#define MIT_CPU_MODE_LOW        (1ULL << 0)
#define MIT_BLOCK_DIMMING       (1ULL << 8)
#define MIT_FORCE_FAST_CHARGE   (1ULL << 9)

static inline void CPUthermalPostMitigationState(BOOL lowMode, BOOL blockDimming, BOOL forceFastCharge) {
    int token = 0;
    if (notify_register_check(kCPUthermalMitigationStateNotifC, &token) != NOTIFY_STATUS_OK) return;
    uint64_t state = 0;
    if (lowMode) state |= MIT_CPU_MODE_LOW;
    if (blockDimming) state |= MIT_BLOCK_DIMMING;
    if (forceFastCharge) state |= MIT_FORCE_FAST_CHARGE;
    notify_set_state(token, state);
    notify_post(kCPUthermalMitigationStateNotifC);
    notify_cancel(token);
}

static inline uint64_t CPUthermalReadMitigationState(void) {
    int token = 0;
    uint64_t state = 0;
    if (notify_register_check(kCPUthermalMitigationStateNotifC, &token) != NOTIFY_STATUS_OK) return 0;
    int result = notify_get_state(token, &state);
    notify_cancel(token);
    if (result != NOTIFY_STATUS_OK) return 0;
    return state;
}

static inline NSInteger CPUthermalMitigationMode(void) {
    return (CPUthermalReadMitigationState() & MIT_CPU_MODE_LOW) ? 1 : 0;
}
