...
static inline NSString *CPUthermalCurrentPrefPath(void) {
    // RootHide 的 jbroot 会指向当前 .jbroot-UUID。偏好文件首次尚未创建时也必须
    // 采用该路径，不能因 fileExists=NO 回退到真实 /var/mobile。
    ...
    if (convertedPath && strlen(convertedPath) > 0) {
        ...
        if ([converted containsString:S("/Containers/Shared/AppGroup/.jbroot-")]) {
            return converted;
        }
    }
