...
// 智能停充与 powerd 温度屏蔽模块的互斥状态：cutoff=1 时，温度模块必须停止
// 清除任何充电阻断位，确保阈值停充始终拥有最高优先级。
static inline int CPUthermalPostSmartChargeCutoffState(BOOL cutoff) {
    int token = 0;
    if (notify_register_check(kCPUthermalSmartChargeCutoffNotifC, &token) != NOTIFY_STATUS_OK) return -1;
    int result = notify_set_state(token, cutoff ? 1 : 0);
    if (result == NOTIFY_STATUS_OK) result = notify_post(kCPUthermalSmartChargeCutoffNotifC);
    notify_cancel(token);
    return result;
}
