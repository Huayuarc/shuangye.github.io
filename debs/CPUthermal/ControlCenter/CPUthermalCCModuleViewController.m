...
static const CGFloat kCPUthermalCCPreferredExpandedHeight = 340.0;   // 头部 + 3 行（标题+副标题）
...
static const CGFloat kCPUthermalCCHeaderHeight = 104.0;
static const CGFloat kCPUthermalCCMenuRowHeight = 78.0;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _modeValues = @[S(kCPUthermalLowPowerModeC), S(kCPUthermalFullPowerModeC), S(kCPUthermalExtremeModeC)];
        _modeTitles = @[S("低功耗"), S("稳定高性能"), S("极限满频")];
        _modeSubtitles = @[S("降频降功耗，不动亮度"), S("拦截热降频，原生调频"), S("强制满功率预算")];
...
- (void)viewDidLoad {
    ...
    if ([self respondsToSelector:@selector(setUseTallLayout:)]) {
        [self setUseTallLayout:YES];   // 宽行：标题+副标题都有位置，不再挤压
    }
...
- (CGFloat)preferredExpandedContentHeight {
    // 按实际模式数量计算：头部（图标+标题）+ 每行（标题+副标题），
    // 避免长按展开后被裁掉（此前固定 250 导致第三项被截断）。
    CGFloat rows = (CGFloat)MAX((NSUInteger)1, MIN(self.modeValues.count, self.modeTitles.count));
    CGFloat height = kCPUthermalCCHeaderHeight + rows * kCPUthermalCCMenuRowHeight;
    return MAX(kCPUthermalCCPreferredExpandedHeight, height);
}
