...
- (void)viewDidLayoutSubviews {
    ...
    if (compact) {
        ...
        self.modeOverlayLabel.frame = CGRectIntegral(CGRectMake(inset, MAX(0.0, CGRectGetHeight(bounds)-titleHeight-stateHeight-7.0), MAX(0.0, CGRectGetWidth(bounds)-inset*2.0), titleHeight));
        self.modeStateOverlayLabel.frame = CGRectIntegral(CGRectMake(inset, MAX(0.0, CGRectGetHeight(bounds)-stateHeight-5.0), MAX(0.0, CGRectGetWidth(bounds)-inset*2.0), stateHeight));
...
- (void)updateModuleSelectedState {
    ...
    if ([self respondsToSelector:@selector(setGlyphImage:)]) [self setGlyphImage:coloredGlyph];
    // 保留颜色配置供展开状态使用；紧凑状态由 AlwaysOriginal 图标确保不变白。
