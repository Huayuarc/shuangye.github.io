...
- (void)savePowerMode:(NSString *)mode {
    ...
    NSString *selectedMode = mode ?: S(kCPUthermalFullPowerModeC);
    prefs[S("powerMode")] = selectedMode;
    BOOL disabledHighPerformance = [selectedMode isEqualToString:S(kCPUthermalLowPowerModeC)];
    if (disabledHighPerformance) prefs[S("highPerformanceModeEnabled")] = [NSNumber numberWithBool:NO];

    CPUthermalWritePrefs(prefs);
    CPUthermalPostPowerMode(selectedMode);
    if (disabledHighPerformance) notify_post(kCPUthermalSettingsChangedNotifC);
}
...
- (void)viewDidLayoutSubviews {
    ...
    if (compact) {
        ...
        CGFloat inset = 4.0;
        CGFloat icon = MIN(30.0, MIN(CGRectGetWidth(bounds) * 0.34, CGRectGetHeight(bounds) * 0.38));
        CGFloat titleHeight = 15.0, gap = 3.0;
        CGFloat totalHeight = icon + gap + titleHeight;
        CGFloat iconY = MAX(5.0, (CGRectGetHeight(bounds)-totalHeight)/2.0);
        self.modeGlyphOverlay.frame = CGRectIntegral(CGRectMake((CGRectGetWidth(bounds)-icon)/2.0, iconY, icon, icon));
        self.modeOverlayLabel.frame = CGRectIntegral(CGRectMake(inset, iconY+icon+gap, MAX(0.0, CGRectGetWidth(bounds)-inset*2.0), titleHeight));
