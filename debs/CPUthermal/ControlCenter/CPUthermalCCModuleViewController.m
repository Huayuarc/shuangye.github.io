...
@property(nonatomic,strong) UIImageView *modeGlyphOverlay;
@property(nonatomic,strong) UILabel *modeOverlayLabel;
@property(nonatomic,strong) UILabel *modeStateOverlayLabel;
...
- (void)viewDidLoad {
    ...
    _modeGlyphOverlay = [UIImageView new];
    _modeGlyphOverlay.contentMode = UIViewContentModeScaleAspectFit;
    _modeGlyphOverlay.userInteractionEnabled = NO;
    [self.view addSubview:_modeGlyphOverlay];
    _modeOverlayLabel = [UILabel new];
    _modeOverlayLabel.textAlignment = NSTextAlignmentCenter;
    _modeOverlayLabel.adjustsFontSizeToFitWidth = YES;
    _modeOverlayLabel.minimumScaleFactor = 0.65;
    _modeOverlayLabel.font = [UIFont systemFontOfSize:9.0 weight:UIFontWeightSemibold];
    _modeOverlayLabel.textColor = [UIColor colorWithWhite:1.0 alpha:0.96];
    _modeOverlayLabel.layer.shadowColor = [UIColor blackColor].CGColor;
    _modeOverlayLabel.layer.shadowOpacity = 0.55;
    _modeOverlayLabel.layer.shadowRadius = 1.2;
    _modeOverlayLabel.layer.shadowOffset = CGSizeMake(0.0, 0.5);
    _modeOverlayLabel.userInteractionEnabled = NO;
    [self.view addSubview:_modeOverlayLabel];
    _modeStateOverlayLabel = [UILabel new];
    _modeStateOverlayLabel.text = S("已启用");
    _modeStateOverlayLabel.textAlignment = NSTextAlignmentCenter;
    _modeStateOverlayLabel.adjustsFontSizeToFitWidth = YES;
    _modeStateOverlayLabel.minimumScaleFactor = 0.68;
    _modeStateOverlayLabel.font = [UIFont systemFontOfSize:8.0 weight:UIFontWeightRegular];
    _modeStateOverlayLabel.textColor = [UIColor colorWithWhite:1.0 alpha:0.62];
    _modeStateOverlayLabel.layer.shadowColor = [UIColor blackColor].CGColor;
    _modeStateOverlayLabel.layer.shadowOpacity = 0.45;
    _modeStateOverlayLabel.layer.shadowRadius = 1.0;
    _modeStateOverlayLabel.layer.shadowOffset = CGSizeMake(0.0, 0.5);
    _modeStateOverlayLabel.userInteractionEnabled = NO;
    [self.view addSubview:_modeStateOverlayLabel];
...
- (void)viewDidLayoutSubviews {
    ...
    self.modeGlyphOverlay.hidden = !compact;
    self.modeOverlayLabel.hidden = !compact;
    self.modeStateOverlayLabel.hidden = !compact;
    if (compact) {
        ...
        CGFloat inset = 4.0;
        CGFloat icon = MIN(28.0, MIN(CGRectGetWidth(bounds) * 0.32, CGRectGetHeight(bounds) * 0.34));
        CGFloat titleHeight = 14.0, stateHeight = 11.0;
        CGFloat iconY = MAX(5.0, CGRectGetHeight(bounds) * 0.10);
        self.modeGlyphOverlay.frame = CGRectIntegral(CGRectMake((CGRectGetWidth(bounds)-icon)/2.0, iconY, icon, icon));
        self.modeOverlayLabel.frame = CGRectIntegral(CGRectMake(inset, MAX(iconY+icon+1.0, CGRectGetHeight(bounds)-titleHeight-stateHeight-7.0), MAX(0.0, CGRectGetWidth(bounds)-inset*2.0), titleHeight));
        self.modeStateOverlayLabel.frame = CGRectIntegral(CGRectMake(inset, MAX(0.0, CGRectGetHeight(bounds)-stateHeight-4.0), MAX(0.0, CGRectGetWidth(bounds)-inset*2.0), stateHeight));
        [self.view bringSubviewToFront:self.modeGlyphOverlay];
        [self.view bringSubviewToFront:self.modeOverlayLabel];
        [self.view bringSubviewToFront:self.modeStateOverlayLabel];
        ...
        UIImage *glyph = [UIImage systemImageNamed:S("thermometer.sun.fill") withConfiguration:configuration];
...
- (void)updateModuleSelectedState {
    NSString *currentMode = [self currentPowerMode];
    BOOL lowPower = [currentMode isEqualToString:S(kCPUthermalLowPowerModeC)];
    self.modeOverlayLabel.text = lowPower ? S("低功耗") : S("解除温控");
    self.modeStateOverlayLabel.text = S("已启用");
    self.title = self.modeOverlayLabel.text;

    // 功率模式模块始终使用与高性能模块一致的系统深色 platter；
    // 模式状态由温度计颜色和文字表示，不使用白色 selected 背景。
    if ([self respondsToSelector:@selector(setSelected:)]) {
        [self setSelected:NO];
    }

    UIColor *glyphColor = lowPower ? [UIColor systemGreenColor] : [UIColor systemOrangeColor];
    UIImageSymbolConfiguration *configuration = [UIImageSymbolConfiguration configurationWithPointSize:24 weight:UIImageSymbolWeightRegular];
    UIImage *baseGlyph = [UIImage systemImageNamed:S("thermometer.sun.fill") withConfiguration:configuration];
    UIImage *coloredGlyph = [baseGlyph imageWithTintColor:glyphColor renderingMode:UIImageRenderingModeAlwaysOriginal];
    self.modeGlyphOverlay.image = coloredGlyph;
    // 展开菜单仍使用系统 glyph；紧凑模式颜色由独立 UIImageView 保证。
    if ([self respondsToSelector:@selector(setSelectedGlyphColor:)]) [self setSelectedGlyphColor:glyphColor];
}
