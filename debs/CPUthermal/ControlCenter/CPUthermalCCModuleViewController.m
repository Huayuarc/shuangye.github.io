...
- (void)viewDidLayoutSubviews {
    ...
    if (compact) {
        ...
        // 原生白色 selected platter 承载单个大图标，不再绘制模式文字。
        CGFloat icon = MIN(48.0, MIN(CGRectGetWidth(bounds) * 0.55, CGRectGetHeight(bounds) * 0.55));
        self.modeGlyphOverlay.frame = CGRectIntegral(CGRectMake((CGRectGetWidth(bounds)-icon)/2.0,
                                                               (CGRectGetHeight(bounds)-icon)/2.0,
                                                               icon, icon));
        [self.view bringSubviewToFront:self.modeGlyphOverlay];
    } else if (self.compactGlyphApplied) {
        NSString *mode = [self currentPowerMode];
        self.title = [mode isEqualToString:S(kCPUthermalLowPowerModeC)] ? S("低功耗") : S("解除温控");
...
// iOS 15–17 不同 ControlCenterUIKit 版本对选中态查询名称不同。
// 低功耗使用原生未选中灰色 platter；解除温控维持原生白色 selected platter。
- (BOOL)isSelected {
    NSString *mode = [self currentPowerMode];
    return ![mode isEqualToString:S(kCPUthermalLowPowerModeC)];
}
...
- (void)updateModuleSelectedState {
    ...
    // 展开态保留菜单标题；紧凑态不显示任何文字。
    CGRect bounds = self.view.bounds;
    BOOL compact = CGRectGetWidth(bounds) < 150.0 && CGRectGetHeight(bounds) < 150.0;
    self.title = compact ? S("") : (lowPower ? S("低功耗") : S("解除温控"));

    // 低功耗：原生未选中灰色背景；解除温控：维持原生白色选中背景。
    if ([self respondsToSelector:@selector(setSelected:)]) {
        [self setSelected:!lowPower];
    }

    UIColor *glyphColor = lowPower ? [UIColor systemGreenColor] : [UIColor systemOrangeColor];
    UIImageSymbolConfiguration *configuration = [UIImageSymbolConfiguration configurationWithPointSize:42 weight:UIImageSymbolWeightSemibold];
