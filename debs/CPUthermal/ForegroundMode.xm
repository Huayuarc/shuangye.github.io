...
static uint64_t gApplicationLifecycleGeneration = 0;
...
static void CPUthermalRenewCurrentApplication(void) {
    UIApplication *application=[UIApplication sharedApplication];
    if(application.applicationState!=UIApplicationStateActive)return;
    CPUthermalPostForegroundBundleID(CPUthermalOwnBundleID());
}

static void CPUthermalBeginCurrentApplicationLease(void) {
    gApplicationLifecycleGeneration++;
    CPUthermalRenewCurrentApplication();
}

static void CPUthermalClearCurrentApplication(void) {
    NSString *bundleID=CPUthermalOwnBundleID();if(!bundleID.length)return;
    uint64_t generation=gApplicationLifecycleGeneration;
    CPUthermalClearForegroundBundleIDIfCurrent(bundleID);
    for(NSNumber *delay in @[@0.10,@0.35,@0.80])dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(delay.doubleValue*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
        if(generation!=gApplicationLifecycleGeneration)return;
        CPUthermalClearForegroundBundleIDIfCurrent(bundleID);
    });
}
...
static void CPUthermalStartSpringBoardMonitor(void) {
    ...
    void (^poll)(void) = ^{
        ...
        if(!bundleID.length||[bundleID isEqualToString:S("com.apple.springboard")])return;
    ...
    poll();
    [NSTimer scheduledTimerWithTimeInterval:0.5 repeats:YES block:^(__unused NSTimer *timer) { poll(); }];
}
...
- (void)viewDidAppear:(BOOL)animated {
    %orig;
    CPUthermalPostForegroundBundleID(S("com.apple.springboard"));
}
%end
...
- (void)didBecomeActive {
    %orig;
    if (![CPUthermalOwnBundleID() isEqualToString:S("com.apple.springboard")]) CPUthermalBeginCurrentApplicationLease();
}
...
%ctor {
    @autoreleasepool {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [center addObserverForName:UIApplicationDidBecomeActiveNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(__unused NSNotification *note) { CPUthermalBeginCurrentApplicationLease(); }];
            [center addObserverForName:UIApplicationWillEnterForegroundNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(__unused NSNotification *note) {
                dispatch_async(dispatch_get_main_queue(), ^{ CPUthermalBeginCurrentApplicationLease(); });
            }];
            ...
            [NSTimer scheduledTimerWithTimeInterval:0.5 repeats:YES block:^(__unused NSTimer *timer){ CPUthermalRenewCurrentApplication(); }];
            CPUthermalBeginCurrentApplicationLease();
        });
    }
}
