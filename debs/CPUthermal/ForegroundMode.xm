...
static BOOL gThirdPartyApplicationActive = NO;
...
static NSString *FrontmostBundleID(void) {
    ...
    id application=nil;
    // 第一优先级使用 SBApplicationController 的权威 frontmostApplication。
    const char *frontSelectors[]={"frontmostApplication","_frontmostApplication",NULL};
    for(int i=0;frontSelectors[i]&&!application;i++){
        SEL front=sel_registerName(frontSelectors[i]);
        if([controller respondsToSelector:front])application=((id(*)(id,SEL))objc_msgSend)(controller,front);
    }
    NSString *bundleID=BundleIDFromObject(application);
    if(bundleID.length && ![bundleID isEqualToString:S("com.apple.springboard")]) return bundleID;

    // runningApplications 中 SpringBoard 自身也可能报告 foreground；优先非 SpringBoard 项。
    ...
    NSString *springBoardFallback=nil;
    if([running isKindOfClass:[NSArray class]])for(id item in running){
        ...
        NSString *candidate=BundleIDFromObject(item);if(!candidate.length)continue;
        if(![candidate isEqualToString:S("com.apple.springboard")])return candidate;
        springBoardFallback=candidate;
    }
    ...
    if([workspace respondsToSelector:front]){
        NSString *workspaceBundle=BundleIDFromObject(((id(*)(id,SEL))objc_msgSend)(workspace,front));
        if(workspaceBundle.length && ![workspaceBundle isEqualToString:S("com.apple.springboard")])return workspaceBundle;
    }
    return springBoardFallback ?: bundleID;
}
...
- (void)activate {
    ...
    if(bundleID.length && ![bundleID isEqualToString:S("com.apple.springboard")]) gThirdPartyApplicationActive=YES;
...
- (void)viewDidAppear:(BOOL)animated {
    %orig;
    gThirdPartyApplicationActive=NO;
...
%ctor {
    @autoreleasepool { dispatch_async(dispatch_get_main_queue(), ^{
        ...
        void(^poll)(void)=^{
            ...
            if([bundleID isEqualToString:S("com.apple.springboard")] && gThirdPartyApplicationActive)return;
            if(![bundleID isEqualToString:S("com.apple.springboard")])gThirdPartyApplicationActive=YES;
