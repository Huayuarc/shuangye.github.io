...
static void CPUthermalStartSpringBoardMonitor(void) {
    ...
    __block NSUInteger heartbeatTicks = 0;
    void (^poll)(void) = ^{
        ...
        BOOL changed=hash!=lastHash;
        if(changed)lastHash=hash;
        // 变化时立即发布；未变化时每1秒权威重发，纠正第三方生命周期竞态覆盖。
        if(changed || (++heartbeatTicks%10u)==0u) CPUthermalPostForegroundBundleID(bundleID);
    };
    poll();
    ...
    @autoreleasepool {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [NSTimer scheduledTimerWithTimeInterval:0.5 repeats:YES block:^(__unused NSTimer *timer) {
                UIApplication *application=UIApplication.sharedApplication;
                if(application.applicationState==UIApplicationStateActive)CPUthermalPostForegroundBundleID(CPUthermalOwnBundleID());
            }];
