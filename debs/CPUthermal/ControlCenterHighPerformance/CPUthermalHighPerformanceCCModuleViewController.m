...
@property(nonatomic,assign) int settingsToken;
@property(nonatomic,assign) int powerModeToken;
@end
...
- (void)viewDidLoad {
    ...
    notify_register_dispatch(kCPUthermalSettingsChangedNotifC, &_settingsToken, dispatch_get_main_queue(), ^(int token) { [weakSelf refreshState]; });
    notify_register_dispatch(kCPUthermalPowerModeChangedNotifC, &_powerModeToken, dispatch_get_main_queue(), ^(int token) { [weakSelf refreshState]; });
...
- (void)toggle:(id)sender {
    if ([self isLowPowerMode]) { [self refreshState]; return; }
...
- (void)refreshState {
    BOOL lowPower = [self isLowPowerMode];
    BOOL enabled = lowPower ? NO : [self isEnabled];
    _button.enabled = !lowPower;
    ...
    _stateLabel.text = enabled ? S("已启用") : S("已关闭");
}
- (void)dealloc {
    if (_settingsToken > 0) notify_cancel(_settingsToken);
    if (_powerModeToken > 0) notify_cancel(_powerModeToken);
}
