...
static BOOL CPUthermalExternalCCSupportPresent(void) {
    ...
    for (NSString *root in roots) {
        ...
        NSString *support = [root stringByAppendingPathComponent:@"Library/Application Support/CCSupport"];
        if (CPUthermalPathExists(dylib) || CPUthermalPathExists(support)) return YES;
    }
    return NO;
}
