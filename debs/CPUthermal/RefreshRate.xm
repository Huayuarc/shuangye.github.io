...
static void CPUthermalApply(void){dispatch_async(dispatch_get_main_queue(),^{BOOL captured=atomic_load(&gScreenCaptured);BOOL high=CPUthermalHighAllowed();if(gPersistentLink){gPersistentLink.paused=captured;if(!captured){gPersistentLink.preferredFrameRateRange=high?CAFrameRateRangeMake(120,120,120):CAFrameRateRangeMake(10,60,60);SEL reason=NSSelectorFromString(S("setHighFrameRateReason:"));if(high&&[gPersistentLink respondsToSelector:reason])((void(*)(id,SEL,BOOL))objc_msgSend)(gPersistentLink,reason,YES);}}if(captured){[gDriverLayer removeAnimationForKey:S("CPUthermalProMotion120")];return;}if(high){CABasicAnimation*a=[CABasicAnimation animationWithKeyPath:S("opacity")];a.fromValue=@0.01;a.toValue=@0.011;a.duration=1;a.repeatCount=INFINITY;a.removedOnCompletion=NO;[gDriverLayer addAnimation:a forKey:S("CPUthermalProMotion120")];}else[gDriverLayer removeAnimationForKey:S("CPUthermalProMotion120")];CPUthermalApplyWindowServer(high);});}
...
%ctor{@autoreleasepool{
    ...
    // 仅普通第三方 .app 安装 App 层 Hook。SpringBoard、系统应用、Widget/ReplayKit 扩展均不安装。
    NSString*path=NSBundle.mainBundle.bundlePath?:S("");
    BOOL extension=[[path pathExtension]caseInsensitiveCompare:S("appex")]==NSOrderedSame;
    BOOL normalThirdPartyApp=thirdParty&&!extension&&[[path pathExtension]caseInsensitiveCompare:S("app")]==NSOrderedSame;
    if(normalThirdPartyApp)%init(CPUthermalAppRefreshHooks);
