...
%ctor { @autoreleasepool {
    ...
    // MobileSubstrate Filter 负责候选进程；这里按用户排除列表决定是否安装 Logos Hook。
    if(bundleID.length&&![bundleID hasPrefix:S("com.apple.")]&&CPUthermalRefreshAppExcluded(bundleID))return;
    %init(CPUthermalRefreshHooks);
