...
- (void)addApplicationAtPath:(NSString *)appPath toMap:(NSMutableDictionary *)map {
    NSString *plistPath = [appPath stringByAppendingPathComponent:S("Info.plist")];
...
- (NSArray<NSDictionary *> *)enumerateApplications {
    ...
    @try {
        ...
        id workspace = [workspaceClass respondsToSelector:NSSelectorFromString(S("defaultWorkspace"))]
            ? ((id (*)(id, SEL))objc_msgSend)(workspaceClass, NSSelectorFromString(S("defaultWorkspace"))) : nil;
        id applications = [workspace respondsToSelector:NSSelectorFromString(S("allApplications"))]
            ? ((id (*)(id, SEL))objc_msgSend)(workspace, NSSelectorFromString(S("allApplications"))) : nil;
...
- (NSArray<NSDictionary *> *)visibleApplications {
    ...
    NSString *term = [self.searchTerm lowercaseString] ?: S("");
...
- (NSArray *)specifiers {
    if (!_specifiers) {
        ...
        NSString *footer = apps.count ? [NSString stringWithFormat:S("已指定 %lu 个应用。开启后，该应用在前台时自动切换低功耗；离开后恢复插件当前模式。"), (unsigned long)enabled.count] : S("没有枚举到可用的第三方应用。");
        ...
        for (NSDictionary *app in apps) {
            ...
            [specifier setProperty:app[S("id")] forKey:S("bundleID")];
...
- (id)appEnabled:(PSSpecifier *)specifier {
    return @([[self enabledBundleIDs] containsObject:[specifier propertyForKey:S("bundleID")]]);
}

- (void)setAppEnabled:(id)value specifier:(PSSpecifier *)specifier {
    NSString *bundleID = [specifier propertyForKey:S("bundleID")];
    if (bundleID.length == 0) return;
    NSMutableSet *enabled = [self enabledBundleIDs];
    if ([value boolValue]) [enabled addObject:bundleID]; else [enabled removeObject:bundleID];
    ...
    NSString *term = self.searchController.searchBar.text ?: S("");
    _specifiers = nil;
