...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier *)spec {
    ...
    if ([key isEqualToString:S("hipLockedEnabled")] || [key isEqualToString:S("hipLockedMode")]) {
        BOOL enabled=[prefs[S("hipLockedEnabled")] ?: @NO boolValue];
        NSString *mode=[prefs[S("hipLockedMode")] isKindOfClass:[NSString class]]?prefs[S("hipLockedMode")]:S("global");
        // 设置页当前为亮屏：“全局”立即启用；“仅锁屏”先恢复原生，锁屏时由 thermalmonitord 自动启用。
        BOOL active=enabled && [mode isEqualToString:S("global")];
        [self runThermalToolCommand:"hip-lock" value:active hasValue:YES];
...
- (id)readPreferenceValue:(PSSpecifier *)spec {
    ...
    if (val) return val;
    if ([key isEqualToString:S("hipLockedMode")]) return S("global");
