...
- (NSString *)currentRunModeTitle {
    NSString *mode = [self prefs][S("thermalRunMode")];
...
- (void)showRunModeSheet {
    ...
    for (NSString *title in @[S("低功耗"), S("稳定高性能"), S("极限满频")]) {
        [a addAction:[UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *x){
            NSString *value = [title isEqualToString:S("低功耗")] ? S("lowPower") : ([title isEqualToString:S("极限满频")] ? S("extremeFull") : S("fullPower"));
            NSMutableDictionary *prefs = [self prefs];
            prefs[S("thermalRunMode")] = value;
            CPUthermalWritePrefs(prefs);
            notify_post(kCPUthermalSettingsChangedNotifC);
            [self updateProtectionStatus];
        }]];
    }
...
- (void)updateProtectionStatus {
    BOOL active = NO; int mode = 1;
    CPUthermalReadThermalEngineState(&active, &mode);
    NSString *modeTitle = mode == 0 ? S("低功耗") : (mode == 2 ? S("极限满频") : S("稳定高性能"));
    NSString *status = [NSString stringWithFormat:S("当前运行方式：%@\n过热保护：%@\n温度保护总开关：%@"),
        modeTitle, active ? S("已触发（降功耗中）") : S("未触发"),
        [[self prefs][S("thermalEngineEnabled")] ?: @YES boolValue] ? S("开") : S("关")];
    for (PSSpecifier *sp in self.specifiers) {
        if ([sp.name isEqualToString:S("温度保护当前状态")]) [sp setProperty:status forKey:S("footerText")];
    }
}
...
- (void)viewWillAppear:(BOOL)animated {[super viewWillAppear:animated];[self updateProtectionStatus];}
- (NSArray *)specifiers {
    if (!_specifiers) {
