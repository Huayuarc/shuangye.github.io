...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier *)spec {
    ...
    if ([key isEqualToString:S("hipLockedEnabled")])
        [self runThermalToolCommand:"hip-lock" value:[value boolValue] hasValue:YES];
    ...
    if ([key isEqualToString:S("powerMode")]) {
        ...
        [self restartThermalMonitorImmediately];
    } else if ([key isEqualToString:S("forceFastChargeEnabled")]) {
        // 强制满血快充（解除80%涓流/无视发热）→ bit9；并入原“禁止高温停充”职责
        // 关闭时清掉旧版遗留单键，保证单一开关语义
        if (![value boolValue]) [prefs removeObjectForKey:S("killThermalStopCharging")];
...
- (id)readPreferenceValue:(PSSpecifier *)spec {
    ...
    if ([key isEqualToString:S("forceFastChargeEnabled")]) {
        // 合并功能读取：新键 或 旧版遗留 killThermalStopCharging 任一开启都视为开
        if ([[self prefs] objectForKey:S("forceFastChargeEnabled")]) return [NSNumber numberWithBool:YES];
        if ([[[self prefs] objectForKey:S("killThermalStopCharging")] boolValue]) return [NSNumber numberWithBool:YES];
