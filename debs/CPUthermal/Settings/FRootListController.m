...
- (void)refreshHighPerformanceAvailability;
@end
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier *)spec {
    ...
    BOOL disabledHighPerformance = NO;
    ...
    if ([key isEqualToString:S("powerMode")]) {
        ...
        if ([value isEqualToString:S("lowPower")]) {
            prefs[S("highPerformanceModeEnabled")] = [NSNumber numberWithBool:NO];
            disabledHighPerformance = YES;
        }
    }
    if ([key isEqualToString:S("highPerformanceModeEnabled")] &&
        [prefs[S("powerMode")] isEqualToString:S("lowPower")]) value = [NSNumber numberWithBool:NO];
    ...
    if ([key isEqualToString:S("powerMode")]) [self refreshHighPerformanceAvailability];
}
...
- (id)readPreferenceValue:(PSSpecifier *)spec {
    ...
    // 所有功能开关默认关闭，仅用户主动开启后生效。
    return [NSNumber numberWithBool:NO];
}

- (void)refreshHighPerformanceAvailability {
    BOOL lowPower = [[[self prefs] objectForKey:S("powerMode")] isEqualToString:S("lowPower")];
    for (PSSpecifier *specifier in [self specifiers]) {
        NSString *key = [specifier propertyForKey:S("key")];
        if (![key isEqualToString:S("highPerformanceModeEnabled")]) continue;
        [specifier setProperty:[NSNumber numberWithBool:!lowPower] forKey:S("enabled")];
        if ([self respondsToSelector:@selector(reloadSpecifier:)]) [self reloadSpecifier:specifier];
        break;
    }
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self refreshHighPerformanceAvailability];
}
