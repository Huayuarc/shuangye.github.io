...
- (void)applyAntiDownclock {int r=[self run:"apply-anti-downclock" path:nil];[self alert:r?S("增强失败"):S("增强完成") message:r?[NSString stringWithFormat:S("返回代码 %d"),r]:S("防降频增强调度已写入 ThermalMonitor，并重启 thermalmonitord。")];}
- (void)replaceBundledD64 {int r=[self run:"replace-bundled-d64" path:nil];[self alert:r?S("替换失败"):S("替换完成") message:r?[NSString stringWithFormat:S("返回代码 %d"),r]:S("内置温控文件已写入后备目录。")];}
- (void)restoreThermalSchedule {int r=[self run:"restore-thermal-schedule" path:nil];[self alert:r?S("恢复失败"):S("恢复完成") message:r?[NSString stringWithFormat:S("返回代码 %d"),r]:S("已恢复 ThermalMonitor 原始配置。")];}
