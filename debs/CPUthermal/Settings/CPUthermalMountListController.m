...
- (void)performTitle:(NSString *)title work:(int(^)(void))work completion:(void(^)(int))completion {
 ...
 [self presentViewController:progress animated:YES completion:^{ dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED,0),^{ int result=work(); dispatch_async(dispatch_get_main_queue(),^{ self.operationRunning=NO; [progress dismissViewControllerAnimated:YES completion:^{ if(completion)completion(result); }]; }); }); }];
}
