...
// 重量级调度操作走后台线程执行，避免同步 waitpid 阻塞 Settings 主线程达 120 秒。
// 完成后在主线程弹窗汇报；期间显示等待框防止重复点击。
- (void)runScheduleAsync:(const char *)command path:(NSString *)path action:(NSString *)actionLabel completionTitle:(NSString *)title {
 UIAlertController *spinner=[UIAlertController alertControllerWithTitle:title message:[NSString stringWithFormat:S("%@ 进行中，请稍候…"),actionLabel] preferredStyle:UIAlertControllerStyleAlert];
 ...
 [spinner.view addSubview:ind]; ind.frame=CGRectMake(110,70,28,28); ind.autoresizingMask=(UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin|UIViewAutoresizingFlexibleTopMargin|UIViewAutoresizingFlexibleBottomMargin); [ind startAnimating];
 [self presentViewController:spinner animated:YES completion:nil];
 __weak typeof(self) weakSelf=self;
 dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT,0),^{
  int r=[self run:command path:path];
  dispatch_async(dispatch_get_main_queue(),^{
   [weakSelf dismissViewControllerAnimated:YES completion:^{
    NSString *mt=[self mountListMessage:r];
    UIAlertController *a=[UIAlertController alertControllerWithTitle:r?S("调度失败"):S("调度完成") message:[NSString stringWithFormat:S("%@\n%@"),mt,r?(NSString*)[NSString stringWithFormat:S("返回代码 %d"),r]:S("")] preferredStyle:UIAlertControllerStyleAlert];
    [a addAction:[UIAlertAction actionWithTitle:S("好的") style:UIAlertActionStyleDefault handler:nil]];
    [weakSelf presentViewController:a animated:YES completion:nil];
   }];
  });
 });
}
- (NSString *)mountListMessage:(int)code { return S("调度已执行；若返回 124 表示守护在 120 秒内未回复，请在稍后刷新状态确认。" ); }
...
- (void)addThermalMountPaths {
 NSArray *targets=@[S("/System/Library/LaunchDaemons"),S("/System/Library/ThermalMonitor")]; NSMutableArray *failures=[NSMutableArray array];
 for(NSString *path in targets){int r=[self run:"add" path:path];if(r)[failures addObject:[NSString stringWithFormat:S("%@（%d）"),path,r]];}
 [self reloadList];
 if(failures.count)[self alert:S("温控路径挂载未全部完成") message:[failures componentsJoinedByString:S("\n")]];
 else [self alert:S("温控路径挂载完成") message:S("LaunchDaemons 与 ThermalMonitor 已保存并挂载；后续用户空间启动会自动恢复。")];
}
