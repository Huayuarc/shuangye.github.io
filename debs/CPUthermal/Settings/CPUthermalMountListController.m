...
- (NSArray *)specifiers { if(!_specifiers){ NSMutableArray *s=[NSMutableArray array]; PSSpecifier *g=[PSSpecifier groupSpecifierWithName:S("RootHide 路径挂载")]; [g setProperty:S("将指定目录的当前内容复制到动态 .jbroot-* 存储，然后以只读 bindfs 挂载；保存的原始路径会在用户空间启动后自动恢复。仅 RootHide 环境启用。") forKey:S("footerText")]; [s addObject:g]; [s addObject:[PSSpecifier preferenceSpecifierNamed:S("添加挂载路径") target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil]]; s.lastObject->action=@selector(addMountPath); [s addObject:[PSSpecifier preferenceSpecifierNamed:S("重新挂载全部") target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil]]; s.lastObject->action=@selector(remountAll); self.paths=[self loadPaths]; [s addObject:[PSSpecifier groupSpecifierWithName:[NSString stringWithFormat:S("已保存路径（%lu）"),(unsigned long)self.paths.count]]]; for(NSString *p in self.paths){PSSpecifier *s1=[PSSpecifier preferenceSpecifierNamed:[NSString stringWithFormat:S("%@  %@"),[self isMounted:p]?S("●"):S("○"),p] target:self set:nil get:nil detail:nil cell:PSButtonCell edit:nil];s1->action=@selector(pathTapped:);[s1 setProperty:p forKey:S("mountPath")];[s addObject:s1];} if(!self.paths.count){PSSpecifier *e=[PSSpecifier groupSpecifierWithName:nil];[e setProperty:S("尚未添加路径。") forKey:S("footerText")];[s addObject:e];} _specifiers=[s copy]; } return _specifiers; }
@end
