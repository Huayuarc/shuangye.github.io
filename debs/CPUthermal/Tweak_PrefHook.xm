...
static void loadPrefs(void) {
    @autoreleasepool {
        ...
        id enabledValue = [preferences objectForKey:S("enabled")];
        id suppressValue = [preferences objectForKey:S("suppressThermalNotifications")];
        BOOL enabled = enabledValue ? [enabledValue boolValue] : YES;
        BOOL suppress = suppressValue ? [suppressValue boolValue] : YES;
...
static id filteredBatteryHealthSpecifiers(id result) {
    ...
    for (id specifier in specifiers) {
        if (specifierContainsBatteryWarning(specifier)) {
            removedWarning = YES;
            continue;
        }
