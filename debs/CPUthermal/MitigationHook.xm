...
static NSArray<NSString *> *prefCandidates(void){
    NSFileManager *fm=[NSFileManager defaultManager];
    NSMutableArray *list=[NSMutableArray array];
    // 1) RootHide 动态隐根
    NSString *ag=@"/var/mobile/Containers/Shared/AppGroup";
    NSArray *ents=[fm contentsOfDirectoryAtPath:ag error:nil] ?: @[];
    for (NSString *e in ents){
        if (![e hasPrefix:@".jbroot-"]) continue;
        [list addObject:[[[ag stringByAppendingPathComponent:e]
            stringByAppendingPathComponent:@"var/mobile/Library/Preferences"]
            stringByAppendingPathComponent:@"com.huayuarc.cputhermal.plist"]];
    }
    // 2) rootless /var/jb
    [list addObject:@"/var/jb/var/mobile/Library/Preferences/com.huayuarc.cputhermal.plist"];
    // 3) 传统隐根转换
    [list addObject:@"/var/mobile/Library/Preferences/com.huayuarc.cputhermal.plist"];
    return list;
}
static void refreshEff(void){
    uint64_t now=dispatch_time(DISPATCH_TIME_NOW,0);
    if (now-gPrefRefreshTS < (uint64_t)(2.0*NSEC_PER_SEC)) return; // 2s 节流
    gPrefRefreshTS=now;
    for (NSString *p in prefCandidates()){
        NSDictionary *d=[NSDictionary dictionaryWithContentsOfFile:p];
        if (!d) continue;
        BOOL v=NO;
        id f=[d objectForKey:@"forceFastChargeEnabled"];
        if ([f respondsToSelector:@selector(boolValue)]) v=[f boolValue];
        // 兼容旧版遗留禁停充键
        if (!v){ id k=[d objectForKey:@"killThermalStopCharging"];
                 if ([k respondsToSelector:@selector(boolValue)]) v=[k boolValue]; }
        if (v){ gPrefFast=YES; } else { /* 若该偏好文件存在但为关 → 覆盖 *、*/ gPrefFast=NO; }
        return; // 取到第一个有效文件即可
    }
    gPrefFast=NO;
}
...
static void periodicCleanup(void){
    ...
    if (orig_MultiProps(s,&cur,kCFAllocatorDefault,0)==KERN_SUCCESS && cur){
        ...
        double targetMV = 0; NSNumber *cvm=[cd objectForKey:@"ChargingVoltage"];
        if (cvm) targetMV = [cvm doubleValue];                    // 常见 mV
        if (targetMV<=0) targetMV = 4230.0;
...
static kern_return_t h_Close(io_connect_t c){ if (connBat(c)) logDiag(@"UIClose"); return o_Close?o_Close(c):KERN_FAILURE; }
static void traceSel(const char* kind,uint32_t sel,uint32_t iCn,uint32_t oCn,const void* outs,size_t sSz){
    if (!protectOn()) return;
    size_t n = sSz>40?40:sSz;
    char hx[160]; int hw=0; const unsigned char* p=(const unsigned char*)outs;
