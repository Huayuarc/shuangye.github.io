...
static _Atomic(bool) gForceFastCharge = false;
...
static BOOL CanForceOptimizedCharging(void);
...
// iOS 优化电池充电（OBC）单独维护的暂停状态；只有“强制充电”开关开启才处理。
static BOOL IsOptimizedChargingBlockKey(NSString *key) {
    if (![key isKindOfClass:[NSString class]]) return NO;
    // 对齐附件 MitigationHook 的 contains 语义，覆盖带厂商前后缀的 OBC/SmartCharge 字段。
    NSString *lower = [key lowercaseString];
    return [lower containsString:S("chargeinhibit")] ||
           [lower containsString:S("smartcharge")] ||
           [lower containsString:S("optimizedcharging")] ||
           [lower containsString:S("optimizedbatterycharging")] ||
           [lower containsString:S("obcinhibit")] ||
           [lower containsString:S("obcenabled")] ||
           [lower containsString:S("enforcedisableobc")] ||
           [lower containsString:S("predictivecharginginhibit")] ||
           [lower containsString:S("adaptivecharging")];
}

static BOOL CanForceOptimizedCharging(void) {
    return atomic_load_explicit(&gForceFastCharge, memory_order_acquire);
}

// 与附件 MitigationHook 保持一致：固定 40000 写入所有充电电流/上限字段。
static BOOL IsForceChargeCurrentKey(NSString *key) {
    if (![key isKindOfClass:[NSString class]]) return NO;
    NSString *lower = [key lowercaseString];
    return [lower containsString:S("chargecurrent")] ||
           [lower containsString:S("chargelimit")] ||
           [lower containsString:S("maxchargecurrent")] ||
           [lower containsString:S("chargerate")];
}
...
static NSDictionary *DictionaryByClearingThermalBlocks(NSDictionary *source) {
    ...
    BOOL changed = NO;
    BOOL forceCharge = CanForceOptimizedCharging();
    BOOL clearThermal = CanClearThermalChargeBlocks();
    for (id rawKey in source) {
        if (![rawKey isKindOfClass:[NSString class]]) continue;
        NSString *key = rawKey;
        id original = source[key];
        if (IsForceChargeCurrentKey(key) && [original respondsToSelector:@selector(longLongValue)]) {
            if (ChargeCurrentSlot(key)) CachePositiveChargeValue(key, original);
            if (forceCharge) {
                result[key] = IntegerMatchingValue(40000, original);
                changed = YES;
                continue;
            }
        ...
        if (forceCharge && IsOptimizedChargingBlockKey(key)) {
            result[key] = [NSNumber numberWithBool:NO];
            changed = YES;
            continue;
        }
        if ((clearThermal || forceCharge) && IsThermalChargeBlockKey(key)) {
...
static void ReloadPreferences(void) {
    ...
    BOOL forceFastCharge = [prefs[S("forceFastChargeIgnoreOptimizedCharging")] boolValue] &&
                           ![prefs[S("smartChargeEnabled")] boolValue];
    atomic_store_explicit(&gEnabled, enabled, memory_order_release);
    atomic_store_explicit(&gForceFastCharge, forceFastCharge, memory_order_release);
    if (enabled || forceFastCharge) RefreshBatteryServiceIDs();
}

static BOOL AnyChargeBypassEnabled(void) {
    return atomic_load_explicit(&gEnabled, memory_order_acquire) ||
           atomic_load_explicit(&gForceFastCharge, memory_order_acquire);
}
...
static void ClearThermalChargeBlocksOnClass(const char *className) {
    ...
    if (readResult == KERN_SUCCESS && raw) {
        ...
        if (CanForceOptimizedCharging()) {
            for (id rawKey in properties) {
                if (![rawKey isKindOfClass:[NSString class]]) continue;
                NSString *key = rawKey;
                id current = properties[key];
                if (IsForceChargeCurrentKey(key) && [current respondsToSelector:@selector(longLongValue)])
                    values[key] = IntegerMatchingValue(40000, current);
                else if (IsOptimizedChargingBlockKey(key) && [current respondsToSelector:@selector(boolValue)] && [current boolValue])
                    values[key] = [NSNumber numberWithBool:NO];
            }
        }
...
static void ApplyBypassState(void) {
    if (!CanClearThermalChargeBlocks() && !CanForceOptimizedCharging()) return;
...
static kern_return_t HookCreateProperties(io_registry_entry_t entry, CFMutableDictionaryRef *out, CFAllocatorRef allocator, IOOptionBits options) {
    ...
    // 批量读取温度伪装只由“屏蔽电池充电温度检测”开关控制；强制充电走写入边界。
...
static CFTypeRef HookCreateProperty(io_registry_entry_t entry, CFStringRef keyRef, CFAllocatorRef allocator, IOOptionBits options) {
    ...
    if (!original || !AnyChargeBypassEnabled()) return original;
    NSString *key = (__bridge NSString *)keyRef;
    id value = (__bridge id)original;

    if (atomic_load_explicit(&gEnabled, memory_order_acquire) && IsKnownBatteryEntry(entry) && IsTemperatureKey(key)) {
        id normal = NormalTemperatureMatchingValue(value);
        if (normal) { CFRelease(original); return CFBridgingRetain(normal); }
    }
    if (IsKnownChargeEntry(entry) && IsForceChargeCurrentKey(key) &&
        [value respondsToSelector:@selector(longLongValue)]) {
        if (ChargeCurrentSlot(key)) CachePositiveChargeValue(key, value);
        if (CanForceOptimizedCharging()) { CFRelease(original); return CFBridgingRetain(IntegerMatchingValue(40000, value)); }
    ...
    if (IsKnownChargeEntry(entry) && CanForceOptimizedCharging() && IsOptimizedChargingBlockKey(key) &&
        [value respondsToSelector:@selector(boolValue)] && [value boolValue]) {
        CFRelease(original); return CFBridgingRetain([NSNumber numberWithBool:NO]);
    }
...
static CFTypeRef HookSearchProperty(io_registry_entry_t entry, const io_name_t plane, CFStringRef keyRef, CFAllocatorRef allocator, IOOptionBits options) {
    ...
    if (atomic_load_explicit(&gEnabled, memory_order_acquire) &&
        IsKnownBatteryEntry(entry) && IsTemperatureKey(key)) {
        id normal = NormalTemperatureMatchingValue(value);
        if (normal) { CFRelease(original); return CFBridgingRetain(normal); }
    }
    if (IsKnownChargeEntry(entry) && CanForceOptimizedCharging() &&
        IsForceChargeCurrentKey(key) && [value respondsToSelector:@selector(longLongValue)]) {
        CFRelease(original); return CFBridgingRetain(IntegerMatchingValue(40000, value));
    }
    if (IsKnownChargeEntry(entry) && CanForceOptimizedCharging() && IsOptimizedChargingBlockKey(key) &&
        [value respondsToSelector:@selector(boolValue)] && [value boolValue]) {
        CFRelease(original); return CFBridgingRetain([NSNumber numberWithBool:NO]);
...
static kern_return_t HookSetProperty(io_registry_entry_t entry, CFStringRef keyRef, CFTypeRef value) {
    if ((!CanClearThermalChargeBlocks() && !CanForceOptimizedCharging()) || !IsKnownChargeEntry(entry))
        return origSetProperty(entry, keyRef, value);
    NSString *key = (__bridge NSString *)keyRef;
    id originalValue = (__bridge id)value;
    if (IsForceChargeCurrentKey(key) && [originalValue respondsToSelector:@selector(longLongValue)]) {
        if (ChargeCurrentSlot(key)) CachePositiveChargeValue(key, originalValue);
        if (CanForceOptimizedCharging())
            return origSetProperty(entry, keyRef, (__bridge CFTypeRef)IntegerMatchingValue(40000, originalValue));
    ...
    if (CanForceOptimizedCharging() && IsOptimizedChargingBlockKey(key))
        return origSetProperty(entry, keyRef, kCFBooleanFalse);
