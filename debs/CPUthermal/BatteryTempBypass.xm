...
static double (*origGetBatteryTemperatureMethod)(id, SEL) = NULL;

static BOOL CanForceChargeCurrent(void) {
    return CanClearThermalChargeBlocks() && !CPUthermalSmartChargeCutoffState();
}

static double HookedGetBatteryTemperature(id self, SEL selector) {
    double realTemperature = origGetBatteryTemperatureMethod ? origGetBatteryTemperatureMethod(self, selector) : 25.0;
    if (CanForceChargeCurrent() && realTemperature > 30.0) return 25.0;
...
static void HookedSetNotChargingReason(id self, SEL selector, int reason) {
    ...
    if (CanForceChargeCurrent() && (finalReason == 16 || finalReason == 256)) finalReason = 0;
...
static void InstallChargingManagerHooks(void) {
    Class monitor = objc_getClass("BatteryThermalMonitor");
    SEL temperatureSelector = sel_registerName("getBatteryTemperature");
    Method temperatureMethod = monitor ? class_getInstanceMethod(monitor, temperatureSelector) : NULL;
    if (!origGetBatteryTemperatureMethod && temperatureMethod && method_getNumberOfArguments(temperatureMethod) == 2) {
        char returnType[16] = {0}; method_getReturnType(temperatureMethod, returnType, sizeof(returnType));
        const char *cursor = returnType; while (*cursor && strchr("rnNoORV", *cursor)) cursor++;
        if (*cursor == 'd') MSHookMessageEx(monitor, temperatureSelector, (IMP)HookedGetBatteryTemperature, (IMP *)&origGetBatteryTemperatureMethod);
    }
