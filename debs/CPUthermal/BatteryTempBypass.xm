...
static BOOL IsChargeDisableKey(CFStringRef key) {
    return key&&(CFEqual(key,CFSTR("ChargeInhibit"))||CFEqual(key,CFSTR("ChargeBlocked"))||CFEqual(key,CFSTR("ChargingPaused"))||CFEqual(key,CFSTR("PredictiveChargingInhibit"))||CFEqual(key,CFSTR("SmartCharge")));
}
static BOOL ChargeBypassActive(void) {
    return atomic_load_explicit(&gTempBypassEnabled,memory_order_acquire)||atomic_load_explicit(&gForceFastChargeEnabled,memory_order_acquire);
}
static kern_return_t (*origSetProperty)(io_registry_entry_t,CFStringRef,CFTypeRef)=NULL;
static kern_return_t HookSetProperty(io_registry_entry_t entry,CFStringRef key,CFTypeRef value) {
    if(!ChargeBypassActive()||gInternalRawRead||!key)return origSetProperty(entry,key,value);
    if(IsChargeDisableKey(key))return origSetProperty(entry,key,kCFBooleanFalse);
    // EnforceDisableOBC=YES 表示关闭系统优化充电，字段语义与 ChargeInhibit 相反。
    if(CFEqual(key,CFSTR("EnforceDisableOBC")))return origSetProperty(entry,key,kCFBooleanTrue);
    return origSetProperty(entry,key,value);
}
static kern_return_t (*origSetProperties)(io_registry_entry_t,CFTypeRef)=NULL;
static kern_return_t HookSetProperties(io_registry_entry_t entry,CFTypeRef values) {
    if(!ChargeBypassActive()||gInternalRawRead||!values||CFGetTypeID(values)!=CFDictionaryGetTypeID())return origSetProperties(entry,values);
    CFMutableDictionaryRef patched=CFDictionaryCreateMutableCopy(kCFAllocatorDefault,0,(CFDictionaryRef)values);
    for(NSString *key in @[S("ChargeInhibit"),S("ChargeBlocked"),S("ChargingPaused"),S("PredictiveChargingInhibit"),S("SmartCharge")])
        if(CFDictionaryContainsKey(patched,(__bridge CFStringRef)key))CFDictionarySetValue(patched,(__bridge CFStringRef)key,kCFBooleanFalse);
    if(CFDictionaryContainsKey(patched,CFSTR("EnforceDisableOBC")))CFDictionarySetValue(patched,CFSTR("EnforceDisableOBC"),kCFBooleanTrue);
    kern_return_t result=origSetProperties(entry,patched);CFRelease(patched);return result;
}
...
%ctor { @autoreleasepool {
    ...
    if(iokit){
        ...
        void *setOne=dlsym(iokit,"IORegistryEntrySetCFProperty");
        if(setOne)MSHookFunction(setOne,(void *)HookSetProperty,(void **)&origSetProperty);
        void *setMany=dlsym(iokit,"IORegistryEntrySetCFProperties");
        if(setMany)MSHookFunction(setMany,(void *)HookSetProperties,(void **)&origSetProperties);
    }
