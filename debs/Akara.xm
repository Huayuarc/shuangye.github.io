...
- (void)dealloc {
    ...
    [_prefs release];
    [_connectivityRowOrderDictionary release];
    [_blurView release];
    [_blurBackgroundView release];
    [_blurBackgroundViewTopConstraint release];
    [super dealloc];
}
...
- (void)updateGlyphImage:(id)sender {}
- (void)updateSliderGlyphImage {}
- (void)updateSliderGlyphImage:(id)sender {}
...
- (void)handleAKRDismissSwipe:(UISwipeGestureRecognizer *)gesture { [self dismissAkara:gesture]; }
- (void)handleAKRDismissTap:(UITapGestureRecognizer *)gesture { [self dismissAkara:gesture]; }
- (void)handleAKRTopGesture:(UISwipeGestureRecognizer *)gesture { [self dismissAkara:gesture]; }
...
extern "C" void updateAkara(void) {
    [[AkaraSettings sharedSettings] updateAkaraWithNotification:AKRReloadNotification()];
}

extern "C" void reloadPrefValues(void) {
    [[AkaraSettings sharedSettings] load];
}

extern "C" void loadPrefs(void) {
    [[AkaraSettings sharedSettings] load];
}
...
extern "C" void setGlyphImageForRoundButton(id roundButton, UIImage *image) {
...
extern "C" void disableGlyphImageForRoundButton(id roundButton) {
    if (!roundButton) {
        return;
    }
    @try {
...
extern "C" void lockScreenStateChanged(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
}
