...
- (void)didMoveToWindow {
    %orig;
    if (!self.window || !isOnSpringBoard) return;
    // 避免重复添加捏合手势
    for (UIGestureRecognizer *gr in self.window.gestureRecognizers) {
        if ([gr isKindOfClass:[UIPinchGestureRecognizer class]]) return;
    }
    UIPinchGestureRecognizer *pinch = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(_sdrHandleDockPinch:)];
...
- (void)_sdrHandleDockPinch:(UIPinchGestureRecognizer *)recognizer {
    ...
    g_dockDefaultCount = newCount; // 保持全局变量同步
    // 持久化保存,重启后由 preferencesthings() 读回
    [[NSUserDefaults standardUserDefaults] setObject:@(newCount) forKey:@"dockIconCount"];
...
// ============================================================================
// iPhone 仿 iPad 网格后台（动画平滑度优化版）
// ============================================================================
%group GridSwitcherHooks

// 1. 强制启用 iPad 网格后台样式及参数
%hook SBAppSwitcherSettings

- (void)setSwitcherStyle:(NSInteger)style {
    if (!g_gridSwitcherEnabled) { %orig(style); return; }
    %orig(2); // 强制使用 Grid 样式
}

- (void)setGridSwitcherPageScale:(double)scale {
    if (!g_gridSwitcherEnabled) { %orig(scale); return; }
    %orig(g_gridScale);
}

- (void)setGridSwitcherHorizontalInterpageSpacingPortrait:(double)spacing {
    if (!g_gridSwitcherEnabled) { %orig(spacing); return; }
...
// 2. 核心优化：注入流畅的物理弹簧动画参数，解决生硬问题
%hook SBFluidSwitcherAnimationSettings

- (id)appStyleToGridCoexistenceAnimationSettings {
    id settings = %orig;
    if (g_gridSwitcherEnabled && [settings respondsToSelector:@selector(setResponse:)] && [settings respondsToSelector:@selector(setDampingRatio:)]) {
        // 降低 response 让进入后台的动画过渡更细腻柔和（标准流畅值为 0.35~0.42）
        [settings setResponse:0.38];
        // 调整阻尼比，赋予自然的轻微回弹（0.82~0.88 最佳）
        [settings setDampingRatio:0.85];
    }
    return settings;
}

- (id)gridToAppCoexistenceAnimationSettings {
    id settings = %orig;
    if (g_gridSwitcherEnabled && [settings respondsToSelector:@selector(setResponse:)] && [settings respondsToSelector:@selector(setDampingRatio:)]) {
        [settings setResponse:0.35];
        [settings setDampingRatio:0.88];
    }
    return settings;
}
...
- (instancetype)initWithNumberOfBands:(NSUInteger)numberOfBands {
    NSUInteger n = (g_audioEnabled && g_eq10Band) ? MAX(numberOfBands, 10) : numberOfBands;
...
%ctor { //More pref
    @autoreleasepool {
        ...
        if (NSClassFromString(@"SBIconListGridLayoutConfiguration")) {
