...
// 拦截添加子视图，禁止把占位图标加入 Dock
- (void)addSubview:(UIView *)view {
    if (g_disableAppLibrary && g_hideDockAppLibraryPlaceholder) {
        if ([NSStringFromClass(view.class) isEqualToString:@"SBDockAppLibraryIconView"]) {
            return; // 直接丢弃，不添加进 Dock
        }
    }
    %orig(view);
}
...
// 额外兜底：SBFloatingDockController 布局刷新拦截
%hook SBFloatingDockController
- (void)_layoutDockAnimated:(BOOL)arg1 {
    %orig;
    if (g_disableAppLibrary && g_hideDockAppLibraryPlaceholder) {
        UIView *dockView = [(id)self valueForKey:@"dockView"];
        if (!dockView) return;
