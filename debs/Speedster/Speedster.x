...
- (void)persistentTick:(CADisplayLink *)link {
    // 添加一点无害的代码，防止系统休眠这个 DisplayLink
    // volatile 防止编译器将此自增优化掉（避免 -Wunused-but-set-variable 触发 -Werror）
    static volatile NSUInteger tickCount = 0;
    tickCount++;
}
...
static BOOL pm_isTargetProcessForHooking(void) {
    // 只要是支持 120Hz 的设备，全部进程启用 Hook，不再做路径过滤
    return pm_deviceSupports120Hz();
}
...
%ctor { //More pref
    @autoreleasepool {
        ...
        if (g_proMotion120Enabled && pm_isTargetProcessForHooking()) {
            ...
            // 【修改点】：将 pm_isSpringBoardProcess 改为 isOnSpringBoard
            if (isOnSpringBoard && pm_deviceSupports120Hz()) {
