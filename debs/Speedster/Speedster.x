...
// 隐藏 Dock 下划线（iPad 浮动 Dock 背景板）
static BOOL g_hideDockLine = NO;
...
void preferencesthings(){ //pref starts to look THICC
    ...
    g_hideDockLine = (prefs && [prefs objectForKey:@"hideDockLine"] ? [[prefs valueForKey:@"hideDockLine"] boolValue] : NO );
...
- (void)setPreferredFramesPerSecond:(NSInteger)fps {
	if (deviceSupports120Hz() && g_proMotion120Enabled) { %orig(0); }
	else { %orig; }
}
- (void)setFrameInterval:(NSInteger)interval {
	if (deviceSupports120Hz() && g_proMotion120Enabled) { %orig(1); }
	else { %orig; }
}
%end
...
- (NSUInteger)maximumDrawableCount {
	...
	if (deviceSupports120Hz() && g_proMotion120Enabled && orig < 3) return 3;
	return orig;
}
%end
...
- (void)presentAfterMinimumDuration:(CFTimeInterval)duration {
	if (deviceSupports120Hz() && g_proMotion120Enabled) { %orig(1.0 / TARGET_FPS); }
	else { %orig; }
}
%end
...
- (void)presentDrawable:(id)drawable afterMinimumDuration:(CFTimeInterval)minimumDuration {
	if (deviceSupports120Hz() && g_proMotion120Enabled) { %orig(drawable, 1.0 / TARGET_FPS); }
...
// 隐藏 Dock 下划线（iPad 浮动 Dock 背景板）
// ============================================================================
%group HideDockLineHooks

%hook SBFloatingDockPlatterView
- (void)setAlpha:(CGFloat)alpha {
	if (g_hideDockLine) { %orig(0); return; }
	%orig(alpha);
}
- (void)setHidden:(BOOL)hidden {
	%orig(g_hideDockLine ? YES : hidden);
}
%end

// 备选：在浮动 Dock 布局时隐藏背景 platter
%hook SBFloatingDockView
- (void)layoutSubviews {
	%orig;
	if (!g_hideDockLine) return;
	for (UIView *v in [(UIView *)self subviews]) {
		if ([v isKindOfClass:NSClassFromString(@"SBFloatingDockPlatterView")]) {
			v.alpha = 0;
			v.hidden = YES;
		}
	}
...
// 当禁用 App 资源库时，隐藏 Dock 中的应用资源库图标
- (void)_setShowsAppLibrary:(BOOL)shows {
	%orig(g_disableAppLibrary ? NO : shows);
}
- (BOOL)_showsAppLibrary {
	return g_disableAppLibrary ? NO : %orig;
}
// 拦截 App Library 按钮视图，禁用时直接返回 nil 防止残留空白图标
- (id)_appLibraryButton {
	if (g_disableAppLibrary) return nil;
	return %orig;
}
- (void)_setAppLibraryButtonVisible:(BOOL)visible {
	if (g_disableAppLibrary) { %orig(NO); return; }
	%orig(visible);
}
%end
...
%ctor { //More pref
    @autoreleasepool {
        ...
        // 隐藏 Dock 下划线（浮动 Dock 背景板）
        if (NSClassFromString(@"SBFloatingDockPlatterView") || NSClassFromString(@"SBFloatingDockView")) {
            %init(HideDockLineHooks);
        }
