...
// ===== FiveIconDockXI 移植：Dock 图标数量布局配置前置声明 =====
...
// Dock 图标数量（FiveIconDockXI 移植）：默认 4 个图标，范围 4-10
static BOOL g_dockIconCountEnabled = NO;
...
void preferencesthings(){ //pref starts to look THICC
    ...
    // ========== Dock 图标数量（FiveIconDockXI 移植）==========
    g_dockIconCountEnabled = (prefs && [prefs objectForKey:@"dockIconCountEnabled"] ? [[prefs valueForKey:@"dockIconCountEnabled"] boolValue] : NO );
    g_dockIconCount = (prefs && [prefs objectForKey:@"dockIconCount"] ? [[prefs valueForKey:@"dockIconCount"] integerValue] : 4 );
    ...
    // Dock 图标数量范围兜底（4-10）
    if (g_dockIconCount < 4 || g_dockIconCount > 10) g_dockIconCount = 4;
...
// ============================================================================
// Dock 图标数量（FiveIconDockXI 移植）：默认 4 个图标，范围 4-10
// 通过 SBIconListGridLayoutConfiguration 控制 Dock（单行 rows==1）的列数，
// 使 Dock 可以容纳更多图标（+/- 调节，最大 10 个）
// ============================================================================
%group DockIconCountHooks

%hook SBIconListGridLayoutConfiguration
- (NSUInteger)numberOfPortraitColumns {
	if (g_dockIconCountEnabled) {
		if ([self numberOfPortraitRows] == 1) return (NSUInteger)g_dockIconCount;
	}
	return %orig;
}
...
%ctor { //More pref
    @autoreleasepool {
        ...
        // Dock 图标数量（FiveIconDockXI 移植）
        if (g_dockIconCountEnabled && NSClassFromString(@"SBIconListGridLayoutConfiguration")) {
            %init(DockIconCountHooks);
        }
