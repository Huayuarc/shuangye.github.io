...
static BOOL sdr_shouldBlockLockScreenWake(void) {
    if (!g_lsBlockEnabled) return NO;

    switch (g_lsBlockMode) {
        case 0:
...
static atomic_bool g_zombieReaperEnabled = false;
...
static void sdr_startZombieReaper(void) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            while (YES) {
                if (atomic_load_explicit(&g_zombieReaperEnabled, memory_order_acquire)) {
                    int status;
...
void preferencesthings(){ //pref starts to look THICC
    ...
    atomic_store_explicit(&g_zombieReaperEnabled, (prefs && [prefs objectForKey:@"zombieReaperEnabled"] ? [[prefs valueForKey:@"zombieReaperEnabled"] boolValue] : NO), memory_order_release);
...
- (bool)canTurnOnScreenForNotificationRequest:(id)arg1 {
	if (!sdr_shouldBlockLockScreenWake()) return %orig;
	return 0;
}
- (void)_turnOnScreen {
	if (!sdr_shouldBlockLockScreenWake()) { %orig; return; }
}
%end
...
%group AppLibraryTrailingPageHooks
...
%ctor { //More pref
    @autoreleasepool {
        ...
        Class rootFolderViewClass = NSClassFromString(@"SBRootFolderView");
        if (rootFolderViewClass &&
            class_getInstanceMethod(rootFolderViewClass, sel_registerName("_trailingCustomPageCount"))) {
            %init(AppLibraryTrailingPageHooks);
        }
