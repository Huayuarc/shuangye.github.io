...
%hook CASpringAnimation
    ...
    -(void)setMass:(double)arg1{ //in app speed
        if(inAppAnimationEnabled && !isOnSpringBoard && !isBackboardd){
    ...
    -(void)setDamping:(double)arg1{
        if((inAppAnimationEnabled && inAppAnimationBounceEnabled) && !isOnSpringBoard && !isBackboardd){
...
static BOOL pm_deviceSupports120Hz(void) {
    // 强制返回 YES，绕过 iOS 系统虚假的 availableModes 限制
    return YES;
}
...
- (void)startPersistentRefresh {
    ...
    // 延迟确保 Window 已就绪后再添加到屏幕
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        UIWindow *targetWindow = nil;
        NSArray *windows = [[UIApplication sharedApplication] valueForKey:@"windows"];

        // 遍历寻找任何一个可用且显示的窗口（兼容 SpringBoard）
        for (UIWindow *window in windows) {
            if (window.isKeyWindow || (!window.isHidden && window.bounds.size.width > 0)) {
                targetWindow = window;
                break; // 找到第一个可用窗口即可
            }
        }
        ...
        if (targetWindow) {
            [targetWindow.layer addSublayer:self.dummyLayer];
        }
    });
...
%ctor { //More pref
    @autoreleasepool {
        ...
        if (g_noRecordingCountdown && !isBackboardd) {
        ...
        // ===== ProMotion120 初始化 =====
        // backboardd 无 UI 显示循环，强制 120Hz hook 与常驻刷新源会干扰渲染服务，故跳过
        if (g_proMotion120Enabled && !isBackboardd) {
