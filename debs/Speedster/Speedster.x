...
static BOOL pm_isSpringBoardProcess = NO;
static BOOL pm_hostsNotificationUI = NO;
...
static BOOL pm_processHostsNotificationUI(void) {
    return pm_hostsNotificationUI;
}

static BOOL pm_shouldApplyGeneralHighFPSHooks(void) {
    return !pm_isSpringBoardProcess && !pm_processHostsNotificationUI();
}
...
static BOOL pm_bundleIDLooksLikeNotificationHost(NSString *bundleID) {
    if (!bundleID.length) return NO;
    return [bundleID isEqualToString:@"com.apple.springboard"] ||
           [bundleID isEqualToString:@"com.apple.UserNotificationsUIServer"] ||
           [bundleID rangeOfString:@"UserNotifications" options:NSCaseInsensitiveSearch].location != NSNotFound ||
           [bundleID rangeOfString:@"Notification" options:NSCaseInsensitiveSearch].location != NSNotFound;
}

static BOOL pm_isTargetProcessForHooking(void) {
    static BOOL checked = NO;
    static BOOL target = NO;
    if (!checked) {
        @autoreleasepool {
            NSString *bundleID = [[NSBundle mainBundle] bundleIdentifier];
            if ([bundleID isEqualToString:@"com.apple.springboard"] ||
                [bundleID isEqualToString:@"com.apple.WebKit.WebContent"] ||
                pm_bundleIDLooksLikeNotificationHost(bundleID)) {
                target = YES;
            } else {
                NSString *bundlePath = [[NSBundle mainBundle] bundlePath];
                if ([bundlePath rangeOfString:@"/Bundle/Application" options:NSCaseInsensitiveSearch].location != NSNotFound ||
                    [bundlePath rangeOfString:@"/Applications" options:NSCaseInsensitiveSearch].location != NSNotFound) {
                    target = YES;
                }
            }
        }
...
%ctor { //More pref
    @autoreleasepool {
        ...
        // ===== ProMotion120 初始化 =====
        if (g_proMotion120Enabled && pm_isTargetProcessForHooking()) {
            %init(ProMotion120Hooks);

            if (pm_isSpringBoardProcess && pm_deviceSupports120Hz()) {
