...
// 注：g_proMotion120Dynamic 已移除，以开源 ProMotion120 方案统一锁定 120Hz
...
// ProMotion 120Hz 前向声明（实现在文件后半部分）
...
void preferencesthings(){ //pref starts to look THICC
    ...
    // g_proMotion120Dynamic 已移除（以开源 ProMotion120 方案锁定 120Hz）
    ...
    // 每次首选项变更后同步 ProMotion kicker 状态
    if (deviceSupports120Hz()) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (g_proMotion120Enabled && !isCurrentAppIn120Blacklist()) {
                ...
                [[SpeedsterProMotionKicker sharedKicker] stopKicking];
            }
        });
    }
}
...
%hook SBFFluidBehaviorSettings
    -(void)setResponse:(double)arg1{ //App open and close speed
        // 120Hz 模式下的 Spring 校正因子：补偿更精确的物理积分导致的过快收敛
        double hz120Factor = (g_proMotion120Enabled && deviceSupports120Hz()) ? 1.25 : 1.0;
        double hz120FactorFast = (g_proMotion120Enabled && deviceSupports120Hz()) ? 1.3 : 1.0;
        ...
        if(isSpeedEnable){
            if(!isFineTuneSpeedEnable){
                ...
                switch (Speedvalue){
                case 1:
                    %orig(0.37 * hz120Factor);
                    SwitcherDismiss = (hz120Factor > 1.0) ? 0.15 : 0.2;
                    break;
                case 2:
                    %orig(0.25 * hz120Factor);
                    SwitcherDismiss = (hz120Factor > 1.0) ? 0.12 : 0.17;
                    break;
                case 3:
                    %orig(0.19 * hz120Factor);
                    SwitcherDismiss = (hz120Factor > 1.0) ? 0.10 : 0.15;
                    break;
                case 4:
                    %orig(0.1 * hz120FactorFast);
                    SwitcherDismiss = (hz120FactorFast > 1.0) ? 0.08 : 0.12;
                    break;
                case 5:
                    %orig(0.07 * hz120FactorFast);
                    SwitcherDismiss = (hz120FactorFast > 1.0) ? 0.07 : 0.1;
                    break;
                default:
                    %orig;
                    SwitcherDismiss = -1;
                ...
                double adjustedFT = ftValue * hz120Factor;
                %orig(adjustedFT);
                ...
                if (ftValue < 0.4 && ftValue >= 0.37){
                    ...
                    SwitcherDismiss = (hz120Factor > 1.0) ? 0.08 : 0.12;
                }else if(ftValue < 0.1){
                    SwitcherDismiss = (hz120Factor > 1.0) ? 0.07 : 0.1;
                }
            }
        }else{
            %orig;
...
// ============================================================================
// 120Hz ProMotion 强制高刷（基于 ProMotion120 开源方案 + Speedster 黑名单增强）
// ============================================================================
...
// --- ProMotion120 风格辅助函数 ---
static void applyHighFrameRateToDisplayLink(CADisplayLink *link) {
	if (!link) return;
	@try {
...
// 轻量级 CADisplayLink 保持 120Hz 管线活跃（ProMotion120 风格）
...
- (void)startKicking {
	...
	_displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(_kick:)];
	applyHighFrameRateToDisplayLink(_displayLink);
	[_displayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
	_displayLink.paused = NO;
}
- (void)_kick:(CADisplayLink *)dl {}
- (void)stopKicking {
	if (!_displayLink) return;
...
// 检查当前应用是否在 120Hz 排除列表中（保留 Speedster 黑名单）
static BOOL isCurrentAppIn120Blacklist(void) {
...
// ProMotion120 风格：统一条件判断
static BOOL shouldApplyProMotionHooks(void) {
	return deviceSupports120Hz() && g_proMotion120Enabled && !isCurrentAppIn120Blacklist();
}

%group ProMotion120Hooks

// === Level 1: 屏幕/设备级别（ProMotion120 方案）===
%hook UIScreen
- (NSInteger)maximumFramesPerSecond {
	if (shouldApplyProMotionHooks()) return TARGET_FPS;
	return %orig;
}
%end

// iOS 15+ UIWindowScene 帧率控制（保留 Speedster 增强）
%hook UIWindowScene
- (NSInteger)maximumFramesPerSecond {
	if (shouldApplyProMotionHooks()) return TARGET_FPS;
	return %orig;
...
// 应用级别帧率控制（保留 Speedster 增强）
%hook UIApplication
- (NSInteger)preferredAnimationFrameRate {
	if (shouldApplyProMotionHooks()) return TARGET_FPS;
	return %orig;
}
%end

// 场景级别帧率控制（ProMotion120 风格：全锁定 120Hz）
%hook UIScene
- (CAFrameRateRange)_preferredAnimationFrameRateRange {
	CAFrameRateRange range = %orig;
	if (shouldApplyProMotionHooks()) {
...
// === Level 2: DisplayLink 级别（ProMotion120 方案）===
%hook CADisplayLink
+ (CADisplayLink *)displayLinkWithTarget:(id)target selector:(SEL)sel {
	CADisplayLink *link = %orig;
	if (shouldApplyProMotionHooks()) {
		applyHighFrameRateToDisplayLink(link);
	}
	return link;
}
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
...
- (void)setPreferredFramesPerSecond:(NSInteger)fps {
	if (shouldApplyProMotionHooks()) { %orig(0); }
	else { %orig; }
}
- (void)setFrameInterval:(NSInteger)interval {
	if (shouldApplyProMotionHooks()) { %orig(1); }
	else { %orig; }
}
%end

// === Level 3: Metal 渲染管线（ProMotion120 方案：maxDrawableCount=3）===
%hook CAMetalLayer
- (NSUInteger)maximumDrawableCount {
	NSUInteger orig = %orig;
	if (shouldApplyProMotionHooks() && orig < 3) return 3;
	return orig;
}
%end
...
- (void)presentAfterMinimumDuration:(CFTimeInterval)duration {
	if (shouldApplyProMotionHooks()) { %orig(1.0 / TARGET_FPS); }
	else { %orig; }
}
%end
...
- (void)presentDrawable:(id)drawable afterMinimumDuration:(CFTimeInterval)minimumDuration {
	if (shouldApplyProMotionHooks()) { %orig(drawable, 1.0 / TARGET_FPS); }
	else { %orig(drawable, minimumDuration); }
}
%end

// === Level 4: 显示偏好（ProMotion120 方案）===
...
// === Level 5: CAContext（ProMotion120 方案）===
%hook CAContext
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
	if (shouldApplyProMotionHooks()) {
...
- (void)setPreferredFrameRate:(float)rate {
	if (shouldApplyProMotionHooks() && rate >= 60) { %orig(TARGET_FPS); }
	else { %orig(rate); }
}
%end

// === Level 6: 动态帧率仲裁（ProMotion120 方案：全部锁定 120Hz）===
...
// === Level 7: CAAnimation（ProMotion120 方案：锁定 120Hz）===
%hook CAAnimation
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
	if (shouldApplyProMotionHooks()) {
		CAFrameRateRange nr;
		nr.minimum = TARGET_FPS;
...
%ctor { //More pref
    @autoreleasepool {
        ...
        // ProMotion 120Hz 强制高刷（基于 ProMotion120 开源方案）
        if (deviceSupports120Hz()) {
            %init(ProMotion120Hooks);
            // CADisplayLink 轻量维持 120Hz 管线（ProMotion120 风格，vsync 对齐，无 GPU 空转）
            if (g_proMotion120Enabled && !isCurrentAppIn120Blacklist()) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[SpeedsterProMotionKicker sharedKicker] startKicking];
                });
        ...
        // 去除录屏和直播三秒倒计时（作者：LaYii-LE⚕️管理）
        if (g_noRecordingCountdown && NSClassFromString(@"RPControlCenterClient")) {
