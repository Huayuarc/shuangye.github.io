...
static BOOL g_proMotion120Dynamic = YES; // YES=动态80-120Hz, NO=锁定120Hz
...
void preferencesthings(){ //pref starts to look THICC
    ...
    g_proMotion120Dynamic = (prefs && [prefs objectForKey:@"proMotion120Dynamic"] ? [[prefs valueForKey:@"proMotion120Dynamic"] boolValue] : YES );
...
// ============================================================================
// 120Hz ProMotion 强制高刷（改进版 v4 — 基于 ProMotion120 增强方案）
// ============================================================================
...
// 轻量级 CADisplayLink 保持 120Hz 管线活跃
...
// 辅助函数：根据模式选择最小值
// 动态模式：仲裁层最低80Hz静止60Hz回退，组件层弹性降频
// 锁定模式：全程120Hz
static float arbitratorMinHz(void) {
    return g_proMotion120Dynamic ? 80.0f : 120.0f;
}
static float componentMinHz(void) {
    return g_proMotion120Dynamic ? 10.0f : 120.0f;
}
...
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
	if (deviceSupports120Hz() && g_proMotion120Enabled && !isCurrentAppIn120Blacklist()) {
		CAFrameRateRange nr;
		nr.minimum = g_proMotion120Dynamic ? ((range.minimum > 0 && range.minimum < 120) ? range.minimum : componentMinHz()) : TARGET_FPS;
...
// === Level 7: CAAnimation 动画帧率（iOS 15+） ===
%hook CAAnimation
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
	if (deviceSupports120Hz() && g_proMotion120Enabled && !isCurrentAppIn120Blacklist()) {
		CAFrameRateRange nr;
