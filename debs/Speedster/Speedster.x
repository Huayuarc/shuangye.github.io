...
static CAFrameRateRange makeHighFPSRange(CAFrameRateRange orig) {
	CAFrameRateRange r;
	r.minimum = (orig.minimum > 0 && orig.minimum < 120) ? MAX(orig.minimum, 60.0f) : 60.0f;
	r.maximum = TARGET_FPS;
	r.preferred = TARGET_FPS;
	return r;
}

// 轻量级 CADisplayLink 保持 120Hz 管线活跃（vsync 对齐版本）
// 空回调 + NSDefaultRunLoopMode 避免 GPU 空转发热，同时维持与 vsync 同步
// @interface 见文件头部前向声明
...
- (void)startKicking {
	...
	if ([_displayLink respondsToSelector:@selector(setPreferredFrameRateRange:)]) {
		...
		range.minimum = 30;    // 允许空闲时降频以省电
		range.maximum = TARGET_FPS;
		range.preferred = TARGET_FPS;
		[_displayLink setPreferredFrameRateRange:range];
	}
	// 仅加入 NSDefaultRunLoopMode：跟踪模式（滑动/过渡）下已有活跃 display link
	[_displayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSDefaultRunLoopMode];
	_displayLink.paused = NO;
}
- (void)_kick:(CADisplayLink *)dl {
	// 故意留空。不调用 [CATransaction flush] 以避免破坏 vsync 帧时序。
	// display link 的存在本身就能提示帧率仲裁器保持 120Hz 管线活跃。
}
- (void)stopKicking {
	if (!_displayLink) return;
...
// 应用级别帧率控制（iOS 12+，UIKit 内部大量使用此属性决定动画帧率）
%hook UIApplication
...
// 场景级别帧率控制（iOS 13+，系统通过此路径决定前台场景的实际渲染帧率）
%hook UIScene
...
- (NSUInteger)maximumDrawableCount {
	...
	if (deviceSupports120Hz() && g_proMotion120Enabled && !isCurrentAppIn120Blacklist() && orig < 4) return 4;
	return orig;
}
%end
...
// === Level 7: CAAnimation 动画帧率（iOS 15+，确保每个动画都以 120Hz 为目标） ===
%hook CAAnimation
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
	if (deviceSupports120Hz() && g_proMotion120Enabled && !isCurrentAppIn120Blacklist()) {
		CAFrameRateRange nr;
		nr.minimum = (range.minimum > 0) ? range.minimum : 30;
		nr.maximum = TARGET_FPS;
		nr.preferred = TARGET_FPS;
		%orig(nr);
	} else {
...
// iOS 13-14 回退：preferredFramesPerSecond 设为 0（显示器最大值）
...
- (NSInteger)medusaCapabilities {
	if (!g_iPadMultitask) return %orig;
	return 2;
}
%end
...
// ============================================================================
// iPhone Dock 手势启用（短滑呼出 Dock，实现拖拽分屏）
// ============================================================================
%group DockGestureHooks

// 降低 Dock 呼出阈值，让 iPhone 能用短滑显示 Dock
%hook SBHomeGestureSettings
- (double)dockPeekActivationHeight {
	if (!g_iPadMultitask) return %orig;
	return 10.0;
}
- (double)dockActivationHeight {
	if (!g_iPadMultitask) return %orig;
	return 30.0;
}
%end
...
%hook SBFloatingDockController
+ (BOOL)isFloatingDockSupported {
	if (g_iPadMultitask) return YES;
	return %orig;
}
- (BOOL)_shouldShowDockForGestureInApp {
	if (g_iPadMultitask) return YES;
	return %orig;
}
- (BOOL)_shouldShowDockInApp:(id)app {
	if (g_iPadMultitask) return YES;
	return %orig;
}
- (BOOL)shouldShowDockForGestureInApp {
	if (g_iPadMultitask) return YES;
	return %orig;
}
%end
...
+ (id)traitCollectionWithHorizontalSizeClass:(NSInteger)horizontalSizeClass {
	if (g_iPadMultitask && UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation)) {
		return %orig(2);
	}
	return %orig;
}
...
%ctor { //More pref
    @autoreleasepool {
        ...
        if (deviceSupports120Hz()) {
            ...
            // CADisplayLink 轻量维持 120Hz 管线（vsync 对齐，无 GPU 空转）
            if (g_proMotion120Enabled && !isCurrentAppIn120Blacklist()) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[SpeedsterProMotionKicker sharedKicker] startKicking];
                });
        ...
        // 分屏/侧拉 - iPhone Dock 手势启用
        if (g_iPadMultitask && NSClassFromString(@"SBHomeGestureSettings") && isOnSpringBoard) {
            %init(DockGestureHooks);
        }
