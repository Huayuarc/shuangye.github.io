...
// ========== ProMotion120 强制 120Hz ==========
static BOOL g_proMotion120Enabled = NO;
static NSInteger g_lastHighFrameRateReason = 0;   // 记录系统最近设置的 highFrameRateReason（模拟录屏用）
static BOOL g_pmDiagnostics = YES;                // ProMotion120 诊断日志开关
static NSString *pm_mainBundleID = nil;
...
// ============================================================================
// ===== ProMotion120 强制 120Hz 刷新率（完整移植） =====
// ============================================================================
...
// SpringBoard ProMotion 策略类（私有，逆向自 ProMotion120 by doimty）
// 关键：SpringBoard 主屏幕/多任务/控制中心的刷新率由这些方法决定。
// iOS 16 上系统 UI 被限制在 80Hz，必须 hook 这些方法才能解锁到 120Hz。
@interface SBProMotionPolicy : NSObject
- (NSInteger)maximumSupportedRefreshRate;
- (NSInteger)effectiveMaxRefreshRate;
- (NSInteger)policyRefreshRate;
- (BOOL)isLimitFrameRateEnabled;
- (BOOL)shouldLimitFrameRate;
@end

// SpringBoard 显示刷新率控制器（私有）
@interface SBDisplayRefreshRateController : NSObject
- (NSInteger)maximumRefreshRate;
- (NSInteger)defaultRefreshRate;
- (NSInteger)activeRefreshRate;
- (void)setActiveRefreshRate:(NSInteger)rate;
@end

static BOOL pm_deviceSupports120Hz(void) {
    // 既然用户开启了插件开关，我们优先假设需要尝试开启 120Hz
    // 去掉了 static checked 缓存，防止在进程极早期 CADisplay 未准备好时永久返回 NO
    BOOL supported = NO;
    BOOL displayReady = NO;
    @autoreleasepool {
    ...
    // 兜底策略（修复：原代码注释写了兜底但实际没实现，导致进程早期
    // CADisplay 未就绪时返回 NO，所有 ProMotion120 hooks 内部判断静默失效）：
    // - 检测到 120Hz 显示模式 → 支持
    // - 显示器已就绪但确实没有 120Hz 模式 → 不支持
    // - 显示器尚未就绪（进程早期/后台）→ 按支持处理返回 YES
    if (supported) return YES;
    if (displayReady) return NO;
    return YES;
}

static BOOL pm_shouldApplyGeneralHighFPSHooks(void) {
    return pm_deviceSupports120Hz();
}

static BOOL pm_shouldApplyDisplayLinkHighFPSHooks(void) {
    return pm_shouldApplyGeneralHighFPSHooks();
}

static void pm_applyHighFrameRateToDisplayLink(CADisplayLink *link) {
    if (!link) return;
    @try {
...
// CADisplayLink 私有高帧率原因 API（iOS 15/16 私有，用于模拟录屏强制 120Hz）
@interface CADisplayLink (SpeedsterProMotion120)
- (void)setHighFrameRateReason:(NSInteger)reason;
@end

@interface ProMotion120PersistentSource : NSObject
@property (nonatomic, strong) CADisplayLink *persistentLink;
@property (nonatomic, strong) CALayer *dummyLayer;
@property (nonatomic, strong) NSMutableArray *dummyLayers;
+ (instancetype)sharedInstance;
- (void)startPersistentRefresh;
- (NSInteger)pickHighFrameRateReason;
@end
...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        ...
        source.dummyLayers = [NSMutableArray array];
    });
    return source;
}
...
- (void)startPersistentRefresh {
    ...
    // 注入一个不可见的脏图层（脏图层法）：
    // backboardd 会在 CADisplayLink 回调没有任何实质图层变化时判定屏幕"静态"，
    // 从而把刷新率降回 60Hz/10Hz，使 120Hz 申请被系统丢弃。
    // 每帧微小改变 opacity 可强制渲染树提交，肉眼不可见但足以欺骗 CoreAnimation。
    self.dummyLayer = [CALayer layer];
    self.dummyLayer.frame = CGRectMake(0, 0, 1, 1);
    self.dummyLayer.backgroundColor = [[UIColor clearColor] CGColor];
    self.dummyLayer.opacity = 0.01;

    // 延迟确保 Window 已就绪后添加到所有可用 window（不只 KeyWindow）。
    // SpringBoard 的 UIWindow 状态可能与普通 App 不同，多窗口都挂上更可靠。
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        for (UIWindow *window in windows) {
            ...
            CALayer *clone = [CALayer layer];
            clone.frame = window.bounds;   // 撑满窗口，避免被裁剪
            clone.backgroundColor = [[UIColor clearColor] CGColor];
            clone.opacity = 0.01;
            [window.layer addSublayer:clone];
            if (self.dummyLayers) {
                [self.dummyLayers addObject:clone];
            }
        }
        if (g_pmDiagnostics) NSLog(@"[Speedster][PM] dummy layers attached: %lu", (unsigned long)self.dummyLayers.count);
    });

    self.persistentLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(persistentTick:)];
    pm_applyHighFrameRateToDisplayLink(self.persistentLink);

    // ★ 模拟录屏：给持久显示链路设置 highFrameRateReason，
    //   让 backboardd 认为存在"录屏/高帧率原因"，从而无条件保持最高刷新率。
    //   这对应你观察到的"开启录屏时全局 120Hz"——系统正是通过这个 reason 强制全速刷新。
    NSInteger reason = [self pickHighFrameRateReason];
    if ([self.persistentLink respondsToSelector:@selector(setHighFrameRateReason:)]) {
        [self.persistentLink setHighFrameRateReason:reason];
        if (g_pmDiagnostics) NSLog(@"[Speedster][PM] persistentLink setHighFrameRateReason=%ld", (long)reason);
    } else {
        if (g_pmDiagnostics) NSLog(@"[Speedster][PM] CADisplayLink 不支持 setHighFrameRateReason:");
    }

    [self.persistentLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
    if (g_pmDiagnostics) NSLog(@"[Speedster][PM] persistent refresh started in %@", pm_mainBundleID);
}

// 选择要注入的高帧率原因：
// 固定使用 6（CAHighFrameRateReasonScreenCapture，iOS 15/16 私有枚举，
// 对应系统录屏时强制全局最高刷新率的原因）。hook 记录到的系统值只用于诊断日志。
- (NSInteger)pickHighFrameRateReason {
    return 6;
}

- (void)persistentTick:(CADisplayLink *)link {
    // 强制每帧产生渲染树变化，打破系统的省电休眠机制
    static BOOL toggle = NO;
    toggle = !toggle;
    if (self.dummyLayer) {
        // 数值极小，完全不可见，但能触发 CoreAnimation 渲染
        self.dummyLayer.opacity = toggle ? 0.011 : 0.012;
    }
    for (CALayer *clone in self.dummyLayers) {
        clone.opacity = toggle ? 0.011 : 0.012;
    }
    // 立即提交本帧变更，避免 CA 合并/延迟，让 backboardd 每帧都看到渲染活动
    if (toggle) [CATransaction flush];
}

@end

// ============================================================
// ProMotion120 Hook Group
// ============================================================
%group ProMotion120Hooks

// SpringBoard ProMotion 策略强制 120Hz（关键！覆盖主屏幕/多任务/控制中心）
// 对齐 ProMotion120 by doimty 的实现：无条件硬编码返回，不依赖动态检测，
// 这样即使 CADisplay 早期未就绪，SpringBoard 策略层也始终请求 120Hz。
...
- (NSInteger)maximumFramesPerSecond {
    if (pm_deviceSupports120Hz() && pm_shouldApplyGeneralHighFPSHooks()) {
        return TARGET_FPS;
    }
    return %orig;
}
...
// App 级 CADisplayLink Hook
%hook CADisplayLink

+ (CADisplayLink *)displayLinkWithTarget:(id)target selector:(SEL)sel {
    CADisplayLink *link = %orig;
    if (pm_deviceSupports120Hz() && pm_shouldApplyDisplayLinkHighFPSHooks()) {
        pm_applyHighFrameRateToDisplayLink(link);
    }
    return link;
}
...
// ★ 记录系统设置的高帧率原因（模拟录屏的核心数据）
// 录屏时系统会通过这个方法把 CADisplayLink 的原因标记为"录屏"（ScreenCapture），
// 从而强制全局最高刷新率。我们记录这个值，persistent link 后续用它在主屏上模拟录屏状态。
- (void)setHighFrameRateReason:(NSInteger)reason {
    %orig(reason);
    if (reason > 0 && reason != g_lastHighFrameRateReason) {
        g_lastHighFrameRateReason = reason;
        if (g_pmDiagnostics) NSLog(@"[Speedster][PM] 捕获 setHighFrameRateReason=%ld", (long)reason);
    }
}
...
- (NSUInteger)maximumDrawableCount {
    ...
    if (pm_deviceSupports120Hz() && pm_shouldApplyGeneralHighFPSHooks() && orig < 3) {
        return 3;
    }
    return orig;
}
...
%hook CADisplay
- (void)setPreferences:(id)preferences {
    if (pm_deviceSupports120Hz() && preferences) {
        @try {
...
- (void)setPreferredFrameRate:(float)rate {
    if (pm_deviceSupports120Hz() && rate >= 60) {
        %orig(TARGET_FPS);
    } else {
        %orig(rate);
    }
...
// 接管所有隐式与显式动画的默认帧率（针对 SpringBoard）
// SpringBoard 内部大量使用 BaseBoard / UIView 块动画，默认采用省电帧率区间（60Hz 甚至更低）。
// 强制拦截 CAAnimation 的帧率设置，将区间直接锁死在 120Hz。
%hook CAAnimation
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
    if (g_proMotion120Enabled && pm_deviceSupports120Hz()) {
        CAFrameRateRange newRange;
...
// 突破 iOS 16 的 Info.plist 强制锁帧（针对"设置"等系统 App）
// iOS 16 强制要求 App（包括系统 App）在 Info.plist 中声明
// CADisableMinimumFrameDurationOnPhone = YES，否则底层 UIKit 会把刷新率上限锁死在 60Hz。
// 动态 Hook NSBundle，欺骗系统，让所有应用在读取这个配置时都返回 YES。
%hook NSBundle
...
- (NSDictionary *)infoDictionary {
    ...
    if (g_proMotion120Enabled && pm_deviceSupports120Hz() && dict) {
...
// 针对 iOS 16 的 UIView 块动画底层拦截
// 不显式指定时 UIView 块动画默认采用系统省电帧率区间，这里强制锁死 120Hz。
%hook UIView
+ (void)_setAnimationPreferredFrameRateRange:(CAFrameRateRange)range {
    if (g_proMotion120Enabled && pm_deviceSupports120Hz()) {
        CAFrameRateRange newRange;
        newRange.minimum = TARGET_FPS;
        newRange.maximum = TARGET_FPS;
...
// CAFrameRateArbitrator — iOS 15+ 底层帧率仲裁拦截
@interface CAFrameRateArbitrator : NSObject
- (CAFrameRateRange)arbitratedFrameRateRange;
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range;
@end

%hook CAFrameRateArbitrator
- (CAFrameRateRange)arbitratedFrameRateRange {
    CAFrameRateRange range = %orig;
    if (pm_deviceSupports120Hz()) {
        range.minimum = TARGET_FPS;
...
- (void)setPreferredFrameRateRange:(CAFrameRateRange)range {
    if (pm_deviceSupports120Hz()) {
        CAFrameRateRange fixed;
        fixed.minimum = TARGET_FPS;
        fixed.maximum = TARGET_FPS;
        fixed.preferred = TARGET_FPS;
        %orig(fixed);
    } else {
        %orig(range);
    }
...
%ctor { //More pref
    @autoreleasepool {
        ...
        // ===== ProMotion120 初始化 =====
        if (g_proMotion120Enabled) {
            // 记录当前进程 bundle id（诊断用）
            pm_mainBundleID = [[[NSBundle mainBundle] bundleIdentifier] copy];

            // 1. 无条件注册 Hook。真正的硬件支持判定留到运行时（Hook 内部）触发
            %init(ProMotion120Hooks);

            // 2. 常驻刷新源在所有注入进程都启动（SpringBoard 主屏幕/多任务、
            //    设置 App 等系统进程同样需要，防止静态画面被 backboardd 判定
            //    为"无内容变化"而降回 60Hz/10Hz）
            dispatch_async(dispatch_get_main_queue(), ^{
                BOOL supports = pm_deviceSupports120Hz();
                if (g_pmDiagnostics) {
                    NSLog(@"[Speedster][PM] %@ supports120=%d, lastReason=%ld",
                          pm_mainBundleID, supports, (long)g_lastHighFrameRateReason);
                }
                // 无论静态检测结果如何都启动 persistent source：
                // 检测函数在进程极早期可能拿不到 CADisplay 而误判为不支持，
                // 而 SpringBoard 主屏需要它持续申请 120Hz 刷新率。
