...
@property(nonatomic, assign) BOOL requireHardwareKeyboard, requirePointer;
@end

@interface UIScreen (TPPrivate)
- (BOOL)_isExternal;
@end
...
@interface UIView (TPPrivate)
- (UIScreen *)_screen;
@end

@interface UIDevice (TPPrivate)
+ (BOOL)_hasHomeButton;
@end
...
int hookedExtDisplayEnabledFunc(void);
...
// ========== TrollPad 移植功能全局变量 ==========
static uint16_t g_forcePadIdiom = 0;      // 强制 Pad idiom 计数器（调用点包裹）
static BOOL g_forcePadKBIdiom = NO;       // 键盘 Pad idiom
static BOOL g_tpEnabled = NO;             // iPad 化主开关
static BOOL g_tpLandscapeHomeScreen = NO;
static BOOL g_tpStageManager = NO;        // 台前调度/Medusa
static BOOL g_tpHideResizeCorners = NO;
static BOOL g_tpForceMedusaGames = NO;
static BOOL g_tpExternalDisplay = NO;
static BOOL g_tpUpsideDown = NO;
static BOOL g_tpIPadKeyboard = NO;
static BOOL g_tpShowShortcutButtons = NO;
static BOOL g_tpIPadSwitcherAnimation = NO;
...
void preferencesthings(){ //pref starts to look THICC
    ...
    // ========== TrollPad 移植功能偏好读取 ==========
    g_tpEnabled = (prefs && [prefs objectForKey:@"tpEnabled"] ? [[prefs valueForKey:@"tpEnabled"] boolValue] : NO );
    g_tpLandscapeHomeScreen = (prefs && [prefs objectForKey:@"tpLandscapeHomeScreen"] ? [[prefs valueForKey:@"tpLandscapeHomeScreen"] boolValue] : NO );
    g_tpStageManager = (prefs && [prefs objectForKey:@"tpStageManager"] ? [[prefs valueForKey:@"tpStageManager"] boolValue] : NO );
    g_tpHideResizeCorners = (prefs && [prefs objectForKey:@"tpHideResizeCorners"] ? [[prefs valueForKey:@"tpHideResizeCorners"] boolValue] : NO );
    g_tpForceMedusaGames = (prefs && [prefs objectForKey:@"tpForceMedusaGames"] ? [[prefs valueForKey:@"tpForceMedusaGames"] boolValue] : NO );
    g_tpExternalDisplay = (prefs && [prefs objectForKey:@"tpExternalDisplay"] ? [[prefs valueForKey:@"tpExternalDisplay"] boolValue] : NO );
    g_tpUpsideDown = (prefs && [prefs objectForKey:@"tpUpsideDown"] ? [[prefs valueForKey:@"tpUpsideDown"] boolValue] : NO );
    g_tpIPadKeyboard = (prefs && [prefs objectForKey:@"tpIPadKeyboard"] ? [[prefs valueForKey:@"tpIPadKeyboard"] boolValue] : NO );
    g_tpShowShortcutButtons = (prefs && [prefs objectForKey:@"tpShowShortcutButtons"] ? [[prefs valueForKey:@"tpShowShortcutButtons"] boolValue] : NO );
    g_tpIPadSwitcherAnimation = (prefs && [prefs objectForKey:@"tpIPadSwitcherAnimation"] ? [[prefs valueForKey:@"tpIPadSwitcherAnimation"] boolValue] : NO );
    g_forcePadKBIdiom = g_tpIPadKeyboard;
...
static Boolean hook_MGGetBoolAnswer(CFStringRef string) {
	...
	// TrollPad 移植：台前调度依赖 DeviceSupportsEnhancedMultitasking
	if (g_tpStageManager && CFEqual(string, CFSTR("DeviceSupportsEnhancedMultitasking"))) {
		return YES;
	}
...
- (unsigned long long)_defaultPresentationGesture {
    ...
    // TrollPad 移植：台前调度下主屏按钮机打开控制中心的工作区
    if (g_tpEnabled && g_tpStageManager) return 1;
    return %orig;
}
%end
...
// 解锁 MDC 版本外接显示器支持（clang 会遗忘 PAC 该函数，需此 hack）
int hookedExtDisplayEnabledFunc(void) {
...
// ============================================================================
// 强制 Pad idiom（基础，供台前调度/外接屏/上下颠倒等调用点使用）
// ============================================================================
%group TPIdiomHooks

%hook UIDevice
- (UIUserInterfaceIdiom)userInterfaceIdiom {
    if (g_forcePadIdiom > 0) {
...
// 横屏主屏
// ============================================================================
%group TPLandscapeHooks

%hook SpringBoard
- (NSInteger)homeScreenRotationStyle {
    return g_tpLandscapeHomeScreen ? 1 : %orig;
}
%end
...
// ============================================================================
// Medusa / 台前调度（Stage Manager）
// ============================================================================
%group TPStageManagerHooks

// 启用 Medusa 多任务（顶部三点按钮）
...
// 修复 iOS 16 多任务（分屏、Slide Over、台前调度）
...
// 强制启用可调整窗口（外接屏被 iOS 禁用）
%hook SBFluidSwitcherItemContainer
- (void)setAllowedTouchResizeCorners:(NSUInteger)cornerMask {
    if (((UIView *)self)._screen != UIScreen.mainScreen) {
        %orig(15);
        // 12 (binary 1100): 启用底部调整
        // 15 (binary 1111): 启用全部角调整
    } else {
        %orig;
    }
}
...
- (void)setAlpha:(CGFloat)alpha {
    %orig;
    ((UIView *)self).hidden = g_tpHideResizeCorners;
}
%end
...
- (void)setInputAssistantButtonItemsForResponder:(id)item {
    g_forcePadKBIdiom = g_tpShowShortcutButtons;
    %orig;
    g_forcePadKBIdiom = YES;
}
%end
...
// ============================================================================
// iPad 后台动画 / App Switcher 修正
// ============================================================================
%group TPSwitcherHooks

%hook SBFluidSwitcherViewController
// 使用 iPadOS 应用切换动画
- (BOOL)isDevicePad {
    return g_tpIPadSwitcherAnimation;
}
%end
...
%ctor { //More pref
    @autoreleasepool {
        ...
        // TrollPad 移植：台前调度也依赖 MGGetBoolAnswer(DeviceSupportsEnhancedMultitasking)
        if (g_pictureInPicture || g_tpStageManager) {
        ...
        // ===== TrollPad 移植功能初始化 =====
        if (g_tpEnabled) {
            // SpringBoard 进程：SB 侧功能（横屏/台前调度/外接屏/上下颠倒/后台动画）
            if (isOnSpringBoard) {
                // 强制 Pad idiom（基础，供各功能调用点使用）
                %init(TPIdiomHooks);

                // 横屏主屏
                if (g_tpLandscapeHomeScreen) {
                    %init(TPLandscapeHooks);
                }

                // Medusa / 台前调度
                if (g_tpStageManager && NSClassFromString(@"SBFullScreenSwitcherLiveContentOverlayCoordinator")) {
                    %init(TPStageManagerHooks);
                }

                // 外接显示器支持（含 MDC 版本解锁）
                if (g_tpExternalDisplay) {
                    %init(TPExternalDisplayHooks);
                ...
                // 允许上下颠倒
                if (g_tpUpsideDown) {
                    %init(TPUpsideDownHooks);
                }

                // iPad 后台动画
                if (g_tpIPadSwitcherAnimation) {
                    %init(TPSwitcherHooks);
                }
            }

            // iPad 键盘（UIKit 进程）
            if (g_tpIPadKeyboard && NSClassFromString(@"UIKeyboardImpl")) {
                %init(TPKeyboardHooks);
            }
        }
