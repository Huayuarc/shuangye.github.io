...
static void SpeedsterLoadDockPreferences(void) {
    ...
    NSInteger dockStyle = [preferences[@"dockStyle"] integerValue];
    gDockStyle = dockStyle == 1 || dockStyle == 3 ? 3 : 0;
...
- (void)setAppLibraryEnabled:(BOOL)enabled {
    %orig(gFloatingDockEnabled ? NO : enabled);
}

- (BOOL)appLibraryEnabled {
    return gFloatingDockEnabled ? NO : %orig;
}
%end
