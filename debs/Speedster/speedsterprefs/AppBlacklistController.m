...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 应用图标（使用 performSelector 避免私有 API 编译错误）
    UIImage *icon = nil;
    if ([UIImage respondsToSelector:@selector(_applicationIconImageForBundleIdentifier:)]) {
        icon = [UIImage performSelector:@selector(_applicationIconImageForBundleIdentifier:) withObject:bundleID];
    }
    if (icon) {
