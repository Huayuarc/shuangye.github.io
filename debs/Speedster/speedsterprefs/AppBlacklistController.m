...
- (void)loadInstalledApps {
    ...
    [self.allApps sortUsingComparator:^NSComparisonResult(id a, id b) {
        @try {
            NSString *bundleID_A = [a performSelector:@selector(applicationIdentifier)];
            NSString *bundleID_B = [b performSelector:@selector(applicationIdentifier)];
            BOOL aDisabled = [self.disabledSet containsObject:bundleID_A];
            BOOL bDisabled = [self.disabledSet containsObject:bundleID_B];
            if (aDisabled != bDisabled) {
                return aDisabled ? NSOrderedAscending : NSOrderedDescending;
            }
            NSString *nameA = [a performSelector:@selector(localizedName)] ?: bundleID_A;
            NSString *nameB = [b performSelector:@selector(localizedName)] ?: bundleID_B;
...
- (void)viewDidLoad {
    ...
    if ([self.tableView respondsToSelector:@selector(setSectionHeaderTopPadding:)]) {
        ((void (*)(id, SEL, CGFloat))[self.tableView methodForSelector:@selector(setSectionHeaderTopPadding:)])(self.tableView, @selector(setSectionHeaderTopPadding:), 0);
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    @try {
        ...
        displayName = [app performSelector:@selector(localizedName)] ?: bundleID;
    } @catch (NSException *exception) {
        bundleID = @"?";
        displayName = @"?";
    }
    ...
    UIImage *icon = nil;
    @try {
        icon = [UIImage _applicationIconImageForBundleIdentifier:bundleID];
    } @catch (NSException *exception) {
        icon = nil;
    }
    if (icon) {
...
// 重新按排除状态排序：已排除的应用靠前，未排除的靠后
- (void)sortAppsByBlacklist {
    [self.allApps sortUsingComparator:^NSComparisonResult(id a, id b) {
        @try {
            NSString *bundleID_A = [a performSelector:@selector(applicationIdentifier)];
            NSString *bundleID_B = [b performSelector:@selector(applicationIdentifier)];
