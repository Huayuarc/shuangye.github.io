...
- (NSArray *)specifiers {
	if (!_specifiers) {
		...
		NSNumber *dockStyle = preferences[@"dockStyle"];
		if (dockStyle.integerValue == 1) {
			preferences[@"dockStyle"] = @3;
			preferencesChanged = YES;
		} else if (dockStyle.integerValue == 2 || (dockStyle && dockStyle.integerValue != 0 && dockStyle.integerValue != 3)) {
			preferences[@"dockStyle"] = @0;
			preferencesChanged = YES;
		}
...
-(void)updateSpecifierVisibility:(BOOL)animated {
  ...
  PSSpecifier *blockModeSpecifier = self.savedSpecifiers[@"lsBlockMode"];
  if (![preferences[@"lsBlockEnabled"] boolValue]) {
    if (blockModeSpecifier && [self containsSpecifier:blockModeSpecifier]) {
      [self removeSpecifier:blockModeSpecifier animated:animated];
    }
  } else if (blockModeSpecifier && ![self containsSpecifier:blockModeSpecifier]) {
    PSSpecifier *masterSpecifier = self.savedSpecifiers[@"lsBlockEnabled"];
    NSUInteger masterIndex = [_specifiers indexOfObject:masterSpecifier];
    NSUInteger insertionIndex = masterIndex == NSNotFound ? _specifiers.count : masterIndex + 1;
    [self insertSpecifier:blockModeSpecifier atIndex:insertionIndex animated:animated];
  }
}
