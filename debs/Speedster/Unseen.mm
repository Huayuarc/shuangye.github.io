...
static NSString *const kUnseenPreferencesDomain = @"com.hoangdus.speedsterprefs";
...
static NSDictionary *load_preferences_dictionary(void) {
    ...
    NSString *plistPath = @(ROOT_PATH_VAR("/var/mobile/Library/Preferences/com.hoangdus.speedsterprefs.plist"));
...
static void load_preferences(void) {
    ...
    gUnseenEnabled = bool_preference(prefs, @"unseenEnabled", YES);
    gUnseenDisableUpdateMaskPatchEnabled = bool_preference(prefs, @"unseenDisableUpdateMaskPatchEnabled", YES);
    gUnseenScreenshotActionFilterEnabled = bool_preference(prefs, @"unseenScreenshotActionFilterEnabled", YES);
    gUnseenCaptureStateMaskEnabled = bool_preference(prefs, @"unseenCaptureStateMaskEnabled", YES);
...
static void unseen_preferences_updated(CFNotificationCenterRef center, void *observer,
...
__attribute__((constructor)) static void tweak_init(void) {
    ...
    CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL,
                                    (CFNotificationCallback)unseen_preferences_updated,
    ...
    // 仅 backboardd 执行渲染服务补丁与截屏捕获状态掩码
    if (process && strcmp(process, "backboardd") == 0) {
