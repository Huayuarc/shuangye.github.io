/**
 * AkaraModuleProvider.bundle - Complete Objective-C Header Reconstruction
 * Source binary: /tmp/akara_re/var/jb/Library/ControlCenter/CCSupport_Providers/AkaraModuleProvider.bundle/AkaraModuleProvider
 *
 * Extracted from: otool -ov, strings -a, otool -L, nm -an
 * Includes ALL classes found in the binary.
 *
 * NOTE: AkaraCCModuleViewController is IDENTICAL in structure to the one in
 * AkaraVerticalModuleProvider.bundle. It is re-included here for completeness.
 */
 ...
 * Reused across both AkaraModuleProvider and AkaraVerticalModuleProvider bundles.
 * Serves as the individual button-style module view controller for each provided module.
 */
...
@property (nonatomic, strong) CCUIButtonModuleViewController *buttonViewController;
...
#pragma mark - AkaraModuleProvider : NSObject <CCSModuleProvider>

/**
 * CCS Module Provider for Akara standard (non-vertical) modules.
 * Instance size: 16 bytes
 * Uses CCUIModuleInstanceManager to dynamically enumerate available modules.
 */
@interface AkaraModuleProvider : NSObject <CCSModuleProvider>
{
    NSMutableDictionary *_moduleInstancesByIdentifier;
}

@property (nonatomic, strong) NSMutableDictionary *moduleInstancesByIdentifier;
...
#pragma mark - AkaraProvidedModuleContentViewController : UIViewController <CCUIContentModuleContentViewController>

/**
 * Content view controller for the standard (non-vertical) Akara module.
 * Instance size: 72 bytes
 * Manages two AkaraCCModuleViewControllers with standard layout.
 * Uses NSLog for debug logging.
 */
@interface AkaraProvidedModuleContentViewController : UIViewController <CCUIContentModuleContentViewController>
{
...
@property (nonatomic, strong) NSString *firstModuleName;
@property (nonatomic, strong) NSString *secondModuleName;
@property (nonatomic, strong) UIVisualEffectView *blurView;
@property (nonatomic, strong) AkaraCCModuleViewController *firstModuleViewController;
@property (nonatomic, strong) AkaraCCModuleViewController *secondModuleViewController;
...
#pragma mark - ProvidedAkaraModule : NSObject <CCUIContentModule>

/**
 * CCUIContentModule implementation for the standard Akara module.
 * Instance size: 32 bytes
 */
@interface ProvidedAkaraModule : NSObject <CCUIContentModule>
{
    NSString *_settingsIdentifier;
    AkaraProvidedModuleContentViewController *_contentViewController;
    UIViewController<CCUIContentModuleBackgroundViewController> *_backgroundViewController;
}

@property (nonatomic, strong) NSString *settingsIdentifier;
@property (nonatomic, readonly, strong) AkaraProvidedModuleContentViewController *contentViewController;
@property (nonatomic, readonly, strong) UIViewController<CCUIContentModuleBackgroundViewController> *backgroundViewController;
...
#pragma mark - ProvidedAkaraModuleRootListController : PSListController

/**
 * Settings root list controller for the Akara standard module preferences.
 * Instance size: 48 bytes
 * Extends PSListController (Preferences framework).
 * Uses NSLog for debug logging of CC titles and values.
 */
@interface ProvidedAkaraModuleRootListController : PSListController
{
    NSString *_settingsIdentifier;
...
@property (nonatomic, strong) NSMutableArray *ccTitles;
@property (nonatomic, strong) NSMutableArray *ccValues;
