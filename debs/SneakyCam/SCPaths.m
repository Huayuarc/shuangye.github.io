...
NSString *SCCurrentJailbreakRoot(void) {
    ...
    for (NSString *name in items) {
        ...
        NSInteger score = 0;
        if (SCExists([root stringByAppendingPathComponent:@"Library/MobileSubstrate/DynamicLibraries/SneakyCam.dylib"])) score += 100;
...
// 只读访问：给注入到系统相机等应用的双摄模块使用，避免并发迁移/写入偏好。
NSDictionary *SCReadPreferencesNoMigrate(void) { return [NSDictionary dictionaryWithContentsOfFile:SCPreferencePath()] ?: @{}; }
