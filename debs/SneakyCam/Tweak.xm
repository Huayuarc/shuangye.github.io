...
static void SCTriggerAction(NSString *action) {
    ...
    else if([action isEqualToString:@"audio"]) [m toggleAudio];
...
static void SCPress(BOOL up, NSTimeInterval now) {
    ...
    if ([action isEqualToString:@"video"]||[action isEqualToString:@"audio"]||[action isEqualToString:@"photo"]) SCTriggerAction(action);
    // off 或未知值不执行
}
...
static void SCUpdateIndicator(void){
    ...
    NSDictionary *p=SCReadPreferences();BOOL enabled=[p[@"Enabled"]?:@NO boolValue];BOOL recording=[p[@"Recording"]?:@NO boolValue]||[p[@"AudioRecording"]?:@NO boolValue];BOOL allowed=[p[@"RecordingIndicator"]?:@YES boolValue];
...
static void SCEnabledNotification(CFNotificationCenterRef c, void*o, CFStringRef n, const void*obj, CFDictionaryRef u) {
    if(!SCEnabled()) { [[SCCaptureManager shared] terminateCapture]; [[SCCaptureManager shared] terminateAudio]; }
}
