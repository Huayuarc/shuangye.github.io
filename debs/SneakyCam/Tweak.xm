...
static void SCCleanupRemovedAudioPreferences(void) {
    NSDictionary *p=SCReadPreferences();
    if([p[@"VolumeUpAction"] isEqualToString:@"audio"])SCWritePreference(@"VolumeUpAction",@"video");
    if([p[@"VolumeDownAction"] isEqualToString:@"audio"])SCWritePreference(@"VolumeDownAction",@"photo");
    if(p[@"AudioEnabled"])SCWritePreference(@"AudioEnabled",nil);
    if(p[@"AudioRecording"])SCWritePreference(@"AudioRecording",nil);
}
static void SCImmediateHaptic(void) {
    id value=SCReadPreferences()[@"HapticFeedback"];
    if(value && ![value boolValue]) return;
    // PerfectGrabber 实测路径：动作确认后直接调用 1519，不再异步延后到 UI 状态切换之后。
    AudioServicesPlaySystemSound(1519);
}

static void SCTriggerAction(NSString *action) {
    if(!SCEnabled()) return;
    // 双击动作被接受时立即反馈，不等待设备会话或文件保存回调。
    SCImmediateHaptic();
    SCCaptureManager *m=[SCCaptureManager shared];
    if([action isEqualToString:@"video"]) [m toggleVideo];
    else if([action isEqualToString:@"photo"]) [m takePhoto];
}
static void SCTrigger(BOOL video) { SCTriggerAction(video?@"video":@"photo"); }
...
static void SCPress(BOOL up, NSTimeInterval now) {
    ...
    if ([action isEqualToString:@"video"]||[action isEqualToString:@"photo"]) SCTriggerAction(action);
    // off 或未知值不执行
}
...
static void SCEnabledNotification(CFNotificationCenterRef c, void*o, CFStringRef n, const void*obj, CFDictionaryRef u) {
    if(!SCEnabled()) [[SCCaptureManager shared] terminateCapture];
}

%ctor {
    @autoreleasepool {
        SCMigratePreferencesIfNeeded();
        if(SCIsSpringBoard()) SCCleanupRemovedAudioPreferences();
