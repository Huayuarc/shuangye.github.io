...
@property(nonatomic,assign) pid_t audioRecorderPID;
@property(nonatomic,copy) NSString *audioRecorderPath;
...
- (void)whenSessionReadyAttempt:(NSInteger)attempt action:(dispatch_block_t)action {
    ...
    // iOS 16 隐根 / RootHide 的 SpringBoard 宿主中，AVCaptureSession startRunning 完成后
    // isRunning 更新存在较长推迟。将原本 12×0.15s(1.8s) 的窗口放宽到 40×0.2s(8s)，
    // 并持续等待，避免误报「相机会话启动超时」。
    if(attempt>=40){ self.lastPrepareError=@"相机会话启动超时"; [self reportStatus:self.lastPrepareError hapticStyle:UIImpactFeedbackStyleLight repeats:0]; [self stopAndRelease]; return; }
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.2*NSEC_PER_SEC)),self.queue,^{ [self whenSessionReadyAttempt:attempt+1 action:action]; });
}
...
- (NSString *)resolvedAudioRecorderTool {
    NSString *root=SCCurrentJailbreakRoot();
    NSArray *paths=@[[root stringByAppendingPathComponent:@"/usr/local/bin/SneakyCamAudioRecorder"],@"/var/jb/usr/local/bin/SneakyCamAudioRecorder",@"/usr/local/bin/SneakyCamAudioRecorder"];
    for(NSString *p in paths)if(p.length&&[[NSFileManager defaultManager]isExecutableFileAtPath:p])return p;
    return nil;
}
- (void)setAudioRecordingState:(BOOL)recording status:(NSString *)status {
    SCWritePreference(@"AudioRecording",@(recording));
    if(status.length)[self reportStatus:status hapticStyle:recording?UIImpactFeedbackStyleHeavy:UIImpactFeedbackStyleMedium repeats:0];
    notify_post("com.spark.SneakyCam.recordingstatechanged");
}
- (void)waitAndForceStopAudioPID:(pid_t)pid completion:(dispatch_block_t)completion {
    if(pid<=0){if(completion)completion();return;}
    kill(pid,SIGTERM);
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY,0),^{
        // 给录音工具时间执行 AVAudioRecorder stop + AVAudioSession deactivate。
        BOOL exited=NO;int status=0;
        // AVAssetWriter 停止时需要等待 finishWriting 写完 M4A 尾部索引，最多保留 12 秒。
        for(NSInteger i=0;i<120;i++){pid_t r=waitpid(pid,&status,WNOHANG);if(r==pid||(r<0&&errno==ECHILD&&kill(pid,0)!=0)){exited=YES;break;}usleep(100000);}
        if(!exited&&kill(pid,0)==0){kill(pid,SIGKILL);for(NSInteger i=0;i<10;i++){pid_t r=waitpid(pid,&status,WNOHANG);if(r==pid||kill(pid,0)!=0)break;usleep(50000);}}
        dispatch_async(self.queue,^{if(completion)completion();});
    });
}
- (void)toggleAudio {
    dispatch_async(_queue,^{
        if(![self boolPref:@"AudioEnabled" def:YES]){[self reportStatus:@"录音功能已关闭" hapticStyle:UIImpactFeedbackStyleLight repeats:0];return;}
        if(self.audioRecorderPID>0&&kill(self.audioRecorderPID,0)==0){pid_t pid=self.audioRecorderPID;self.audioRecorderPID=0;[self setAudioRecordingState:NO status:@"正在停止录音…"];[self waitAndForceStopAudioPID:pid completion:^{[self setAudioRecordingState:NO status:@"录音已停止并释放麦克风"]; }];return;}
        self.audioRecorderPID=0;
        if(![self boolPref:@"Enabled" def:NO]){[self reportStatus:@"功能总开关已关闭" hapticStyle:UIImpactFeedbackStyleLight repeats:0];return;}
        NSString *tool=[self resolvedAudioRecorderTool];if(!tool){[self reportStatus:@"未找到录音工具" hapticStyle:UIImpactFeedbackStyleLight repeats:0];return;}
        NSString *dir=@"/var/mobile/Media/Camera";[[NSFileManager defaultManager]createDirectoryAtPath:dir withIntermediateDirectories:YES attributes:@{NSFilePosixPermissions:@0755} error:nil];
        NSDateFormatter *f=[NSDateFormatter new];f.dateFormat=@"yyyyMMdd-HHmmss";NSString *path=[dir stringByAppendingPathComponent:[NSString stringWithFormat:@"SneakyCam-Audio-%@.m4a",[f stringFromDate:NSDate.date]]];
        const char *argv[]={tool.fileSystemRepresentation,path.fileSystemRepresentation,NULL};pid_t pid=0;int rc=posix_spawn(&pid,tool.fileSystemRepresentation,NULL,NULL,(char *const *)argv,environ);
        if(rc!=0||pid<=0){[self reportStatus:[NSString stringWithFormat:@"录音启动失败：%d",rc] hapticStyle:UIImpactFeedbackStyleLight repeats:0];return;}
        self.audioRecorderPID=pid;self.audioRecorderPath=tool;self.audioOutputPath=path;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(.45*NSEC_PER_SEC)),self.queue,^{if(self.audioRecorderPID!=pid)return;if(kill(pid,0)!=0){int status=0;waitpid(pid,&status,WNOHANG);self.audioRecorderPID=0;[[NSFileManager defaultManager]removeItemAtPath:path error:nil];[self setAudioRecordingState:NO status:@"录音工具启动后退出，请检查麦克风权限"];return;}[self setAudioRecordingState:YES status:[NSString stringWithFormat:@"录音已开始：%@",path]];dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY,0),^{int status=0;waitpid(pid,&status,0);dispatch_async(self.queue,^{if(self.audioRecorderPID==pid){self.audioRecorderPID=0;[self setAudioRecordingState:NO status:[[NSFileManager defaultManager]fileExistsAtPath:path]?@"录音已保存":@"录音未生成有效文件"];}});});});
    });
}
- (void)terminateAudio {
    dispatch_async(_queue,^{pid_t pid=self.audioRecorderPID;self.audioRecorderPID=0;[self setAudioRecordingState:NO status:nil];if(pid>0&&kill(pid,0)==0)[self waitAndForceStopAudioPID:pid completion:^{[self setAudioRecordingState:NO status:@"录音已停止并释放麦克风"]; }];});
}

- (void)terminateCapture {
    dispatch_async(_queue, ^{
        self.videoDesired=NO;self.userStopping=YES;self.recoveryScheduled=NO;self.restartAttempts=0;
        [self setRecordingState:NO];
