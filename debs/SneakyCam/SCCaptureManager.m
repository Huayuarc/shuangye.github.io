...
@property(nonatomic,assign) NSInteger recoveryGeneration;
@property(nonatomic,assign) BOOL recordingStartPending;
...
- (NSInteger)sessionReadyAttempts {
    // 按运行时系统能力选等待窗：iOS 17 后台捕获状态传播比 iOS 15/16 更慢。
    NSOperatingSystemVersion v=NSProcessInfo.processInfo.operatingSystemVersion;
    if(v.majorVersion>=17)return 100; // 20 秒
    if(v.majorVersion>=16)return 60;  // 12 秒
    return 40;                       // 8 秒
}
- (NSTimeInterval)recoveryDelay {
    NSOperatingSystemVersion v=NSProcessInfo.processInfo.operatingSystemVersion;
    return v.majorVersion>=17?1.2:(v.majorVersion>=16?.75:.45);
}
...
- (BOOL)videoStabilizationEnabled { return [self boolPref:@"VideoStabilizationEnabled" def:YES]; }
- (void)applyVideoStabilization {
    if(![self videoStabilizationEnabled]||!self.movieOutput)return;
    AVCaptureConnection *connection=[self.movieOutput connectionWithMediaType:AVMediaTypeVideo];
    if(!connection||!connection.isVideoStabilizationSupported)return;
    if(connection.preferredVideoStabilizationMode!=AVCaptureVideoStabilizationModeOff)return;
    // 附件补丁策略：优先电影防抖；设备/格式拒绝时降级为标准防抖。
    @try { connection.preferredVideoStabilizationMode=AVCaptureVideoStabilizationModeCinematic; }
    @catch(NSException *e) { @try { connection.preferredVideoStabilizationMode=AVCaptureVideoStabilizationModeStandard; } @catch(NSException *ignored) {} }
}
...
- (BOOL)prepare:(BOOL)video {
    ...
    // 配置提交后 connection 才稳定生成，此时应用防抖；开始录像前还会再次兜底。
    if(video)[self applyVideoStabilization];
...
- (void)watchRecordingStartForGeneration:(NSInteger)generation {
    NSTimeInterval delay=NSProcessInfo.processInfo.operatingSystemVersion.majorVersion>=17?6.0:4.0;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(delay*NSEC_PER_SEC)),self.queue,^{
        if(generation!=self.recoveryGeneration||!self.videoDesired||self.userStopping)return;
        if(self.recordingStartPending&&!self.movieOutput.isRecording){self.recordingStartPending=NO;[self scheduleVideoRecovery:@"录像启动确认超时"];}
    });
}
- (void)startMovieSegmentRecovered:(BOOL)recovered {
    if(!self.videoDesired||self.userStopping||!self.movieOutput)return;
    ...
    [self applyVideoStabilization];self.recordingStartPending=YES;NSInteger generation=self.recoveryGeneration;
    [self.movieOutput startRecordingToOutputFileURL:self.movieURL recordingDelegate:self];
    [self watchRecordingStartForGeneration:generation];
    [self reportStatus:recovered?@"录像恢复请求已提交":@"录像启动请求已接受" hapticStyle:recovered?UIImpactFeedbackStyleMedium:UIImpactFeedbackStyleHeavy repeats:0];
}
- (void)scheduleVideoRecovery:(NSString *)reason {
    ...
    if(self.restartAttempts>=6){[self reportStatus:@"录像恢复失败：已达到重试上限" hapticStyle:UIImpactFeedbackStyleLight repeats:0];self.videoDesired=NO;self.recordingStartPending=NO;return;}
    self.restartAttempts++;self.recoveryScheduled=YES;NSInteger generation=++self.recoveryGeneration;
    [self reportStatus:[NSString stringWithFormat:@"录像被系统中断，正在恢复（%ld/6）：%@",(long)self.restartAttempts,reason?:@"屏幕状态变化"] hapticStyle:UIImpactFeedbackStyleLight repeats:0];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)([self recoveryDelay]*NSEC_PER_SEC)),self.queue,^{
        if(generation!=self.recoveryGeneration){self.recoveryScheduled=NO;return;}
        self.recoveryScheduled=NO;if(!self.videoDesired||self.userStopping)return;
        AVCaptureSession *old=self.session;if(old){[[NSNotificationCenter defaultCenter]removeObserver:self name:nil object:old];if(old.isRunning)[old stopRunning];}
        self.session=nil;self.movieOutput=nil;self.photoOutput=nil;self.videoInput=nil;self.audioInput=nil;self.recording=NO;self.recordingStartPending=NO;
        if(![self prepare:YES]){[self scheduleVideoRecovery:self.lastPrepareError?:@"会话重建失败"];return;}
        [self whenSessionReadyAttempt:0 action:^{if(generation==self.recoveryGeneration)[self startMovieSegmentRecovered:YES];}];
    });
}
...
- (void)sessionWasInterrupted:(NSNotification *)note {
    ...
    if(self.videoDesired&&!self.userStopping&&[self keepLockScreenRecording]){
        [self reportStatus:@"录像被屏幕状态中断，准备自动重建" hapticStyle:UIImpactFeedbackStyleLight repeats:0];
        // 部分 iOS 17 小版本不会及时发送 InterruptionEnded；主动延迟重建，不再无限等待通知。
        NSInteger generation=self.recoveryGeneration;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(1.5*NSEC_PER_SEC)),self.queue,^{if(generation==self.recoveryGeneration&&self.sessionInterrupted)[self scheduleVideoRecovery:@"屏幕状态中断"];} );
    }
}
...
- (void)whenSessionReadyAttempt:(NSInteger)attempt action:(dispatch_block_t)action {
    if(!self.videoDesired&&self.movieOutput){return;}
    if(self.session.isRunning){if(action)action();return;}
    NSInteger limit=[self sessionReadyAttempts];
    if(attempt>=limit){self.lastPrepareError=[NSString stringWithFormat:@"相机会话启动超时（iOS %ld，等待 %.0f 秒）",(long)NSProcessInfo.processInfo.operatingSystemVersion.majorVersion,limit*.2];[self reportStatus:self.lastPrepareError hapticStyle:UIImpactFeedbackStyleLight repeats:0];if(self.videoDesired&&!self.userStopping)[self scheduleVideoRecovery:self.lastPrepareError];else[self stopAndRelease];return;}
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(.2*NSEC_PER_SEC)),self.queue,^{[self whenSessionReadyAttempt:attempt+1 action:action];});
}
...
- (void)toggleVideo {
    dispatch_async(_queue, ^{
       ...
       self.videoDesired=YES;self.userStopping=NO;self.restartAttempts=0;self.sessionInterrupted=NO;self.recordingStartPending=NO;self.recoveryGeneration++;[self.videoSegments removeAllObjects];
        ...
        [self whenSessionReadyAttempt:0 action:^{
            ...
            [self startMovieSegmentRecovered:NO];
        }];
    });
}
...
- (void)captureOutput:(AVCaptureFileOutput *)o didFinishRecordingToOutputFileAtURL:(NSURL *)u fromConnections:(NSArray *)c error:(NSError *)e {
    ...
    self.videoDesired=NO;self.userStopping=NO;self.restartAttempts=0;self.recordingStartPending=NO;self.recoveryGeneration++;
    ...
    [self mergeVideoSegments:segments completion:^(NSURL *merged,NSError *mergeError){
        ...
        if([self boolPref:@"SaveToRoot" def:NO]){
            NSString *dir=@"/var/mobile/Media/Camera";[[NSFileManager defaultManager]createDirectoryAtPath:dir withIntermediateDirectories:YES attributes:nil error:nil];
...
- (void)terminateCapture {
    dispatch_async(_queue, ^{
        self.videoDesired=NO;self.userStopping=YES;self.recoveryScheduled=NO;self.restartAttempts=0;self.recordingStartPending=NO;self.recoveryGeneration++;
        [self setRecordingState:NO];
