...
- (void)toggleAudio {
    dispatch_async(_queue,^{
        ...
        if(self.audioRecorderPID>0&&kill(self.audioRecorderPID,0)==0){pid_t pid=self.audioRecorderPID;self.audioRecorderPID=0;kill(pid,SIGTERM);[self setAudioRecordingState:NO status:@"录音停止请求已接受"];return;}
...
- (void)terminateAudio {
    dispatch_async(_queue,^{pid_t pid=self.audioRecorderPID;self.audioRecorderPID=0;if(pid>0&&kill(pid,0)==0)kill(pid,SIGTERM);[self setAudioRecordingState:NO status:nil];});
}
