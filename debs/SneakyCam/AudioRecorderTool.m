...
int main(int argc,const char *argv[]){@autoreleasepool{
    ...
    [gRecorder stop];gRecorder=nil;
    // 显式撤销录音路由并重试停用，确保控制中心橙点及时消失。
    [session overrideOutputAudioPort:AVAudioSessionPortOverrideNone error:nil];
    for(NSInteger i=0;i<5;i++){NSError *deactivateError=nil;if([session setActive:NO withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation error:&deactivateError])break;usleep(100000);}
