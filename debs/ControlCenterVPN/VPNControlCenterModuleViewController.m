...
- (void)viewDidLayoutSubviews {
    ...
    if (!showExpanded) {
        CGFloat glyph = MIN(47.0, MAX(39.0, MIN(w, h) * .46));
        CGFloat stateH = 13.0, gap = 3.0;
        CGFloat total = glyph + gap + stateH;
        CGFloat y = floor(MAX(7.0, (h-total)/2.0));
        _iconPlate.frame = CGRectIntegral(CGRectMake((w-glyph)/2.0, y, glyph, glyph));
        _iconPlate.layer.cornerRadius = 0;
        _glyphView.frame = _iconPlate.bounds; y += glyph + gap;
        _titleLabel.frame = CGRectZero;
        _stateLabel.frame = CGRectIntegral(CGRectMake(5.0, y, MAX(0,w-10.0), stateH));
        _stateLabel.font = [UIFont systemFontOfSize:9.4 weight:UIFontWeightSemibold];
    } else {
        CGFloat pad = 16;
