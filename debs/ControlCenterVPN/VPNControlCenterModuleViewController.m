...
- (void)buildExpandedView {
    ...
    _sizeButton.accessibilityLabel = @"选择图标大小";
    _sizeButton.layer.cornerRadius = 13;
    _sizeButton.layer.borderWidth = 1.5;
    _sizeButton.layer.borderColor = [UIColor colorWithWhite:1 alpha:.78].CGColor;
    UIImageSymbolConfiguration *sizeCfg = [UIImageSymbolConfiguration configurationWithPointSize:14 weight:UIImageSymbolWeightBold];
    UIImage *sizeIcon = [UIImage systemImageNamed:@"arrow.up.left.and.arrow.down.right" withConfiguration:sizeCfg];
    [_sizeButton setImage:[sizeIcon imageWithTintColor:UIColor.whiteColor renderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
    [_sizeButton addTarget:self action:@selector(showSizePicker:) forControlEvents:UIControlEventTouchUpInside];
    [_expandedView addSubview:_sizeButton];
...
- (NSArray<NSDictionary *> *)glyphSizeOptions {
    return @[
        @{@"name":@"小", @"key":@"small", @"scale":@0.42},
        @{@"name":@"标准", @"key":@"medium", @"scale":@0.50},
        @{@"name":@"大", @"key":@"large", @"scale":@0.58}
    ];
}

- (NSString *)selectedGlyphSizeKey {
    NSString *key = [[NSUserDefaults standardUserDefaults] stringForKey:kVPNGlyphSizeKey];
    return key.length ? key : @"medium";
}

- (CGFloat)selectedGlyphScale {
    NSString *key = [self selectedGlyphSizeKey];
    for (NSDictionary *option in self.glyphSizeOptions)
        if ([option[@"key"] isEqualToString:key]) return [option[@"scale"] doubleValue];
    return .50;
}

- (void)showSizePicker:(id)sender {
    UIAlertController *sheet = [UIAlertController alertControllerWithTitle:@"选择图标大小" message:@"所选尺寸立即应用到控制中心火箭图标" preferredStyle:UIAlertControllerStyleActionSheet];
    NSString *current = [self selectedGlyphSizeKey];
    for (NSDictionary *option in self.glyphSizeOptions) {
        NSString *title = [NSString stringWithFormat:@"%@%@", [option[@"key"] isEqualToString:current] ? @"✓ " : @"", option[@"name"]];
        [sheet addAction:[UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
            [[NSUserDefaults standardUserDefaults] setObject:option[@"key"] forKey:kVPNGlyphSizeKey];
...
- (void)refreshState {
    ...
    UIColor *systemAccent = self.view.tintColor ?: UIColor.systemBlueColor;
    ...
    // 原生 Platter 的选中态最终在布局阶段按实时尺寸更新，避免扩展回调残留。
    VPNControlCenterModule *toggleModule = (VPNControlCenterModule *)self.module;
    toggleModule.selected = connected && !_expanded;
...
- (void)viewDidLayoutSubviews {
    ...
    if (!showExpanded) {
        CGFloat scale = [self selectedGlyphScale];
        CGFloat glyph = MIN(66.0, MAX(40.0, MIN(w, h) * scale));
        ...
        _sizeButton.frame = CGRectMake(CGRectGetMaxX(_expandedTitle.frame)+8, 13, 26, 26);
