...
- (void)viewDidLoad {
    ...
    _activeBackground.backgroundColor = UIColor.whiteColor;
...
- (UIColor *)selectedGlyphColor {
    NSString *key = [[NSUserDefaults standardUserDefaults] stringForKey:kVPNGlyphColorKey] ?: @"white";
...
- (void)showColorPicker:(id)sender {
    ...
    NSString *current = [[NSUserDefaults standardUserDefaults] stringForKey:kVPNGlyphColorKey] ?: @"white";
    for (NSDictionary *option in self.glyphColorOptions) {
...
- (void)refreshState {
    ...
    _activeBackground.hidden = !connected;
...
- (void)viewDidLayoutSubviews {
    ...
    if (!showExpanded) {
        // 用户指定紧凑模块只展示火箭 Logo：放大并绝对居中。
        CGFloat glyph = MIN(68.0, MAX(55.0, MIN(w, h) * .66));
