...
@interface UIControl (VPNControlCenterNativeTint)
- (UIColor *)_tintColorForSelectedState:(BOOL)selected;
@end

static NSString *const kVPNSelectedNodeKey = @"com.huayuarc.vpncontrolcenter.selectedNode";
static NSString *const kVPNGlyphSizeKey = @"com.huayuarc.vpncontrolcenter.glyphSize";
...
- (void)buildExpandedView {
    ...
    _expandedGlyph = [[UIImageView alloc] initWithImage:[self rocketImage]];
    _expandedGlyph.contentMode = UIViewContentModeScaleAspectFit;
    _expandedGlyph.tintColor = UIColor.whiteColor;
    ...
    _sizeSlider.minimumValue = .32;
    _sizeSlider.maximumValue = .68;
    _sizeSlider.continuous = YES;
    _sizeSlider.minimumTrackTintColor = UIColor.systemBlueColor;
    _sizeSlider.maximumTrackTintColor = [UIColor colorWithWhite:1 alpha:.24];
    [_sizeSlider addTarget:self action:@selector(glyphSizeSliderChanged:) forControlEvents:UIControlEventValueChanged];
    [_sizeSlider addTarget:self action:@selector(glyphSizeSliderFinished:) forControlEvents:UIControlEventTouchUpInside | UIControlEventTouchUpOutside | UIControlEventTouchCancel];
    [_expandedView addSubview:_sizeSlider];

    _sizeValueLabel = [UILabel new];
    _sizeValueLabel.font = [UIFont monospacedDigitSystemFontOfSize:11 weight:UIFontWeightSemibold];
    _sizeValueLabel.textColor = [UIColor colorWithWhite:1 alpha:.72];
    _sizeValueLabel.textAlignment = NSTextAlignmentRight;
    [_expandedView addSubview:_sizeValueLabel];
...
- (CGFloat)selectedGlyphScale {
    id value = [[NSUserDefaults standardUserDefaults] objectForKey:kVPNGlyphSizeKey];
    CGFloat scale = value ? [value doubleValue] : .50;
    return MIN(.68, MAX(.32, scale));
}

- (void)glyphSizeSliderChanged:(UISlider *)slider {
    _sizeValueLabel.text = [NSString stringWithFormat:@"%d%%", (int)lrint(slider.value * 100.0)];
    CGFloat preview = 18.0 + (slider.value-.32)/(.68-.32)*12.0;
    _expandedGlyph.transform = CGAffineTransformMakeScale(preview/30.0, preview/30.0);
    [self.view setNeedsLayout];
    [self.view layoutIfNeeded];
}

- (void)glyphSizeSliderFinished:(UISlider *)slider {
    [[NSUserDefaults standardUserDefaults] setDouble:slider.value forKey:kVPNGlyphSizeKey];
...
- (void)refreshState {
    ...
    // 颜色完全交由系统 Toggle 语义：关闭白色，开启使用系统原生强调色。
    UIColor *systemAccent = nil;
    if ([self.buttonView respondsToSelector:@selector(_tintColorForSelectedState:)])
        systemAccent = [self.buttonView _tintColorForSelectedState:YES];
    if (!systemAccent) systemAccent = self.view.tintColor ?: UIColor.systemBlueColor;
    UIColor *accent = connected ? systemAccent : UIColor.whiteColor;
    UIImage *glyph = [self rocketImage];
    _glyphView.image = glyph;
    _glyphView.tintColor = accent;
    _glyphView.layer.shadowOpacity = 0;
    // 同步模块与原生按钮控制器，确保选中背景和图标颜色同时出现。
    VPNControlCenterModule *toggleModule = (VPNControlCenterModule *)self.module;
    BOOL compactSelected = connected && !_expanded;
    toggleModule.selected = compactSelected;
    self.selected = compactSelected;
    self.glyphColor = UIColor.whiteColor;
    self.selectedGlyphColor = systemAccent;
    // 不直接调用控制器父类 refreshState：它会重建 Toggle 内部视图并干扰自定义扩展面板。
    _expandedGlyph.image = [self rocketImage];
    _expandedGlyph.tintColor = accent;
...
- (void)viewDidLayoutSubviews {
    ...
    // 唯一状态源：实时 bounds。扩展尺寸强制取消原生选中背景；
    // 缩回紧凑尺寸后即使系统漏掉返回回调，也恢复实际 VPN 连接态。
    VPNControlCenterModule *toggleModule = (VPNControlCenterModule *)self.module;
    BOOL shouldSelectNativePlatter = !roomyBounds && [self isConnected];
    if (toggleModule.isSelected != shouldSelectNativePlatter)
        toggleModule.selected = shouldSelectNativePlatter;
    if (self.isSelected != shouldSelectNativePlatter)
        self.selected = shouldSelectNativePlatter;

    if (!showExpanded) {
        CGFloat scale = _sizeSlider.isTracking ? _sizeSlider.value : [self selectedGlyphScale];
        CGFloat glyph = MIN(72.0, MAX(32.0, MIN(w, h) * scale));
        ...
        _sizeSlider.frame = CGRectMake(pad, 48, MAX(0,w-pad*2-48), 30);
        if (!_sizeSlider.isTracking) _sizeSlider.value = [self selectedGlyphScale];
        _sizeValueLabel.frame = CGRectMake(w-pad-44, 49, 44, 28);
        _sizeValueLabel.text = [NSString stringWithFormat:@"%d%%", (int)lrint(_sizeSlider.value*100.0)];
        _nodeScroll.frame = CGRectMake(pad, 82, w-pad*2, MAX(0, h-94));
...
- (void)willReturnToExpandedContentModule {
    ...
    VPNControlCenterModule *toggleModule = (VPNControlCenterModule *)self.module;
    toggleModule.selected = [self isConnected];
