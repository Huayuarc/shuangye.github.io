...
- (void)viewDidLayoutSubviews {
    ...
    if (!showExpanded) {
        CGFloat plate = MIN(34.0, MAX(29.0, MIN(w, h) * .34));
        CGFloat titleH = 14.0, stateH = 12.0, gap = 2.0;
        CGFloat total = plate + gap + titleH + stateH;
        CGFloat y = floor(MAX(5.0, (h-total)/2.0));
        _iconPlate.frame = CGRectIntegral(CGRectMake((w-plate)/2.0, y, plate, plate));
        _iconPlate.layer.cornerRadius = plate/2.0;
        _glyphView.frame = CGRectInset(_iconPlate.bounds, 6.0, 6.0); y += plate + gap;
        _titleLabel.frame = CGRectIntegral(CGRectMake(6.0, y, MAX(0,w-12.0), titleH)); y += titleH;
        _stateLabel.frame = CGRectIntegral(CGRectMake(5.0, y, MAX(0,w-10.0), stateH));
        _titleLabel.font = [UIFont systemFontOfSize:11.5 weight:UIFontWeightBold];
        _stateLabel.font = [UIFont systemFontOfSize:8.8 weight:UIFontWeightSemibold];
    } else {
        CGFloat pad = 16;
