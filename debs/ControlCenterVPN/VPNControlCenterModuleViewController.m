...
@property(nonatomic,strong) UIView *compactActiveBackground;
...
- (void)viewDidLoad {
    ...
    // 仅用于紧凑已连接态；扩展态由尺寸保护永久隐藏，避免覆盖节点面板。
    _compactActiveBackground = [UIView new];
    _compactActiveBackground.userInteractionEnabled = NO;
    _compactActiveBackground.backgroundColor = [UIColor colorWithWhite:1 alpha:.92];
    _compactActiveBackground.hidden = YES;
    if ([_compactActiveBackground.layer respondsToSelector:@selector(setCornerCurve:)])
        _compactActiveBackground.layer.cornerCurve = kCACornerCurveContinuous;
    [self.view addSubview:_compactActiveBackground];
...
- (NSArray<NSDictionary *> *)glyphColorOptions {
    return @[
        @{@"name":@"白色", @"key":@"white", @"color":UIColor.whiteColor},
...
- (void)refreshState {
    ...
    UIColor *chosenColor = [self selectedGlyphColor];
    UIColor *accent = connected ? chosenColor : UIColor.whiteColor;
    UIImage *glyph = [self rocketImage];
    _glyphView.image = glyph;
    _glyphView.tintColor = accent;
    _colorButton.backgroundColor = chosenColor;
    BOOL selectedWhite = [[self selectedGlyphColorKey] isEqualToString:@"white"];
    _glyphView.layer.shadowColor = UIColor.blackColor.CGColor;
    _glyphView.layer.shadowOpacity = connected && selectedWhite ? .28 : 0;
    _glyphView.layer.shadowRadius = connected && selectedWhite ? 2.0 : 0;
    _glyphView.layer.shadowOffset = CGSizeZero;
    // 紧凑态显示白底；扩展态根据实时尺寸隐藏，节点面板保持深色可读。
    VPNControlCenterModule *toggleModule = (VPNControlCenterModule *)self.module;
    toggleModule.selected = connected && !_expanded;
    _compactActiveBackground.hidden = !connected || _expanded;
...
- (void)viewDidLayoutSubviews {
    ...
    _compactActiveBackground.frame = b;
    _compactActiveBackground.layer.cornerRadius = MIN(24.0, MIN(w,h) * .23);
    ...
    // 不依赖回调：一旦尺寸属于扩展态，白色紧凑背景始终关闭。
    _compactActiveBackground.hidden = showExpanded || ![self isConnected];
