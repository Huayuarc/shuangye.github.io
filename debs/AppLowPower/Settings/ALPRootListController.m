...
// ============================================================
// 禁止使用 @"" 常量：roothide 重映射会破坏 __cfstring 内部指针。
// 所有字符串通过 S() 动态创建。
// ============================================================

@interface ALPRootListController : PSListController <UISearchResultsUpdating>
@property(nonatomic,strong) NSArray<NSDictionary *> *allApps;
@property(nonatomic,copy) NSString *searchTerm;
@property(nonatomic,strong) UISearchController *searchController;
@end

@implementation ALPRootListController
...
- (NSMutableDictionary *)prefs {
    return ALPReadMutablePrefs() ?: [NSMutableDictionary dictionary];
}

- (BOOL)masterEnabled {
    id value = [self prefs][S("enabled")];
    ...
    for (NSString *root in @[S("/var/mobile/Containers/Bundle/Application"), S("/var/containers/Bundle/Application")]) {
        for (NSString *uuid in [fm contentsOfDirectoryAtPath:root error:nil] ?: @[]) {
            ...
            for (NSString *entry in [fm contentsOfDirectoryAtPath:container error:nil] ?: @[])
                if ([[entry pathExtension] caseInsensitiveCompare:S("app")] == NSOrderedSame)
                    [self addApplicationAtPath:[container stringByAppendingPathComponent:entry] toMap:map];
        }
    }
...
- (NSMutableSet<NSString *> *)enabledBundleIDs {
    id value = [self prefs][S("lowPowerApps")];
...
- (void)rebuildSpecifiers {
    ...
    @try {
        PSSpecifier *master = [PSSpecifier emptyGroupSpecifier];
        [master setProperty:S("总开关关闭后立即恢复系统原生温控调度。") forKey:S("footerText")];
        [_specifiers addObject:master];
        PSSpecifier *toggle = [PSSpecifier preferenceSpecifierNamed:S("启用指定应用低功耗") target:self set:@selector(setMasterEnabled:specifier:) get:@selector(masterEnabledValue:) detail:nil cell:PSSwitchCell edit:nil];
        [_specifiers addObject:toggle];
        ...
        NSString *footer = self.searchTerm.length
            ? [NSString stringWithFormat:S("找到 %lu 个应用；已开启应用优先显示。"), (unsigned long)apps.count]
            : [NSString stringWithFormat:S("已指定 %lu 个应用。进入所选应用自动启用低功耗；退至后台后恢复系统原生调度。"), (unsigned long)enabledCount];
...
- (id)masterEnabledValue:(PSSpecifier *)specifier { return @([self masterEnabled]); }
- (void)setMasterEnabled:(id)value specifier:(PSSpecifier *)specifier {
    NSMutableDictionary *prefs = [self prefs];
    prefs[S("enabled")] = @([value boolValue]);
    ALPWritePrefs(prefs);
    notify_post(kALPSettingsChangedNotifC);
}

- (id)appEnabled:(PSSpecifier *)specifier {
    return @([[self enabledBundleIDs] containsObject:specifier.identifier ?: [specifier propertyForKey:S("id")]]);
}
