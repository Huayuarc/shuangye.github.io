...
static const char *kALPRestartStateNotifC = "com.huayuarc.applowpower/restartState";

// Darwin state 跨进程保存最近一次重建时间与待应用标记，
// thermalmonitord 被重建后仍可读取，避免重启风暴且能立即套用低功耗。
static inline void ALPPostRestartState(uint64_t state) {
    int token = 0;
    if (notify_register_check(kALPRestartStateNotifC, &token) != NOTIFY_STATUS_OK) return;
    notify_set_state(token, state);
    notify_post(kALPRestartStateNotifC);
    notify_cancel(token);
}

static inline uint64_t ALPReadRestartState(void) {
    int token = 0;
    uint64_t state = 0;
    if (notify_register_check(kALPRestartStateNotifC, &token) != NOTIFY_STATUS_OK) return 0;
    int result = notify_get_state(token, &state);
    notify_cancel(token);
    return result == NOTIFY_STATUS_OK ? state : 0;
}

// 低 1 位标记“重建后需立即套用低功耗”，高位保存重建时间戳（秒）。
static inline uint64_t ALPRestartTimestamp(uint64_t state) { return state >> 1; }
static inline BOOL ALPRestartWantsFastApply(uint64_t state) { return (state & 1ULL) != 0; }
static inline uint64_t ALPMakeRestartState(uint64_t timestamp, BOOL fastApply) {
    return (timestamp << 1) | (fastApply ? 1ULL : 0ULL);
}
