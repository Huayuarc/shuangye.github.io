...
static void applyLowPowerToRuntime(void) {
    ...
    applyLowPowerToApplePPM();
    startKeepAliveTimer();
    // 私有对象可能在 hook 之后才创建；短脉冲补齐首次进入应用的即时性。
    for (int pulse = 1; pulse <= 6; pulse++) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(pulse * 0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
static void reloadState(BOOL forceApply) {
    ...
    os_unfair_lock_unlock(&g_stateLock);
    if (shouldApply && (changed || forceApply)) applyLowPowerToRuntime();
    else if (!shouldApply && changed) restoreNativeRuntime();
    NSLog(@"[AppLowPower] 前台命中:%d 变化:%d", shouldApply, changed);
}
...
%ctor {
    @autoreleasepool {
        ...
        reloadState(NO);
        NSLog(@"[AppLowPower] thermalmonitord 调度已接入");
    }
}
