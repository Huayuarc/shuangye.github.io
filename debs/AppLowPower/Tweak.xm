...
static void restartThermalMonitorForNativeRestore(void);
...
static void restartThermalMonitorForFastApply(void) {
    uint64_t state = ALPReadRestartState();
    uint64_t now = (uint64_t)time(NULL);
    uint64_t previous = ALPRestartTimestamp(state);
    // 方向发生反转（原生→低功耗）时必须立即重建，不受冷却窗口限制。
    BOOL sameDirection = ALPRestartWantsFastApply(state);
    if (sameDirection && previous && now >= previous && (now - previous) < kALPRestartCooldownSeconds) return;
...
// 退出所选应用：CPMS/ApplePPM 可能仍缓存低功耗决策，仅 setter 归位不足以立刻升频。
// 重建守护让内核侧从原生配置重新初始化，硬件频率随即恢复。
static void restartThermalMonitorForNativeRestore(void) {
    uint64_t state = ALPReadRestartState();
    uint64_t now = (uint64_t)time(NULL);
    uint64_t previous = ALPRestartTimestamp(state);
    // 方向发生反转（低功耗→原生）时必须立即重建，不受冷却窗口限制。
    BOOL sameDirection = !ALPRestartWantsFastApply(state);
    if (sameDirection && previous && now >= previous && (now - previous) < kALPRestartCooldownSeconds) return;
    ...
    ALPPostRestartState(ALPMakeRestartState(now, NO));
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.08 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSLog(@"[AppLowPower] 重建 thermalmonitord 以立即恢复原生频率");
        kill(getpid(), SIGTERM);
    });
}
...
static void reloadState(BOOL forceApply) {
    ...
    if (shouldApply) {
        ...
        // 离开所选应用（含返回桌面、切到未选应用）：先就地解除限制，
        // 再重建守护清空 CPMS/ApplePPM 低功耗残留，使硬件频率立即回到原生。
        restoreNativeRuntime();
        if (!firstEvaluation) restartThermalMonitorForNativeRestore();
...
static NSString *springBoardFrontmostBundleID(void) {
    ...
    const char *frontSelectors[] = {"_accessibilityFrontMostApplication", "accessibilityFrontMostApplication", "frontmostApplication", "_frontmostApplication", NULL};
    ...
    if (!application) {
        ...
        if (!application) {
            // iOS 15/16 上 frontmostApplication 可能为空；用运行中应用的前台标记兜底。
            ...
            for (id candidate in [running isKindOfClass:[NSArray class]] ? running : @[]) {
                SEL foregroundSelector = sel_registerName("isProcessForeground");
                if ([candidate respondsToSelector:foregroundSelector] && ((BOOL (*)(id, SEL))objc_msgSend)(candidate, foregroundSelector)) { application = candidate; break; }
            }
        }
    }
    if (!application) {
        ...
        SEL sharedSelector = sel_registerName("sharedInstance");
        id workspace = [workspaceClass respondsToSelector:sharedSelector] ? ((id (*)(id, SEL))objc_msgSend)(workspaceClass, sharedSelector) : nil;
        const char *workspaceSelectors[] = {"currentForegroundApplication", "frontmostApplication", NULL};
        for (int i = 0; workspaceSelectors[i] && !application; i++) {
            SEL selector = sel_registerName(workspaceSelectors[i]);
            if ([workspace respondsToSelector:selector]) application = ((id (*)(id, SEL))objc_msgSend)(workspace, selector);
        }
    }
...
- (void)didDeactivateForEventsOnly:(BOOL)eventsOnly {
    %orig;
    // 应用退出前台后立即重新求值，返回桌面时无需等待轮询周期。
    dispatch_async(dispatch_get_main_queue(), ^{ reportFrontmostApplication(); });
}
%end

// 桌面回到前台即视为离开所选应用；避免依赖 0.5 秒轮询造成恢复延迟。
%hook SBHomeScreenViewController
- (void)viewDidAppear:(BOOL)animated {
    %orig;
    g_lastForegroundHash = 0;
    ALPPostForegroundBundleID(nil);
}
%end
    ...
    @autoreleasepool {
        ...
        if (isSpringBoard) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                // SpringBoard 作为系统级真值源高频核对前台应用，
                // 0.2 秒可让进入/退出应用在用户感知上即时生效。
