...
- (void)takePhoto { dispatch_async(_queue, ^{ NSDictionary *p=self.prefs; if(![p[@"Enabled"] ?: @YES boolValue]||![p[@"PhotoEnabled"] ?: @YES boolValue]||![self prepare:NO])return; AVCapturePhotoSettings *s=[AVCapturePhotoSettings photoSettings]; [self.photoOutput capturePhotoWithSettings:s delegate:self]; }); }
- (void)toggleVideo { dispatch_async(_queue, ^{ NSDictionary *p=self.prefs;if(![p[@"Enabled"] ?: @YES boolValue]||![p[@"VideoEnabled"] ?: @YES boolValue])return;if(self.movieOutput.isRecording){[self.movieOutput stopRecording];return;}if(![self prepare:YES])return;NSString *n=[NSString stringWithFormat:@"SneakyCam-%@.mov",NSUUID.UUID.UUIDString];self.movieURL=[NSURL fileURLWithPath:[NSTemporaryDirectory() stringByAppendingPathComponent:n]];[self.movieOutput startRecordingToOutputFileURL:self.movieURL recordingDelegate:self];self.recording=YES; }); }
