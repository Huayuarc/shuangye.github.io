...
static NSString *Root(void){NSString *p=@"/var/mobile/Library/PictureModule";if([[NSFileManager defaultManager] fileExistsAtPath:@"/var/jb"])p=@"/var/jb/var/mobile/Library/PictureModule";[[NSFileManager defaultManager] createDirectoryAtPath:p withIntermediateDirectories:YES attributes:nil error:nil];return p;}
static NSInteger Slot(NSString *identifier){NSInteger n=[identifier componentsSeparatedByString:@"."].lastObject.integerValue;return MAX(1,MIN(5,n));}
...
@property(nonatomic,strong) NSString *identifier;@property(nonatomic,strong) UIScrollView *scrollView;@property(nonatomic,strong) UIView *mediaView;@property(nonatomic,strong) UILabel *labelView;@property(nonatomic,strong) AVPlayer *player;@property BOOL expanded;@property int notifyToken;@property int prefsNotifyToken;
@end
...
- (void)unloadPicture{[_player pause];_player=nil;[_mediaView removeFromSuperview];_mediaView=nil;}
- (void)reloadPicture{[self unloadPicture];NSDictionary *p=Prefs(_identifier);NSInteger n=Slot(_identifier);NSString *type=p[@"type"];
 ...
 else{UIImageView *v=[[UIImageView alloc]init];v.image=[UIImage imageWithContentsOfFile:ImagePath(n)];_mediaView=v;}
...
- (void)layoutPicView{NSDictionary *p=Prefs(_identifier);CGFloat margin=[p[@"NoMargin"]boolValue]?0:4;CGRect b=CGRectInset(_scrollView.bounds,margin,margin);_mediaView.frame=b;_scrollView.contentSize=b.size;_labelView.hidden=(_labelView.text.length==0);[_labelView sizeToFit];_labelView.frame=CGRectMake(10,self.view.bounds.size.height-_labelView.bounds.size.height-8,_labelView.bounds.size.width,_labelView.bounds.size.height);}
...
- (void)doubleTap:(UITapGestureRecognizer *)g{[_scrollView setZoomScale:(_scrollView.zoomScale>1?1:2) animated:YES];}
