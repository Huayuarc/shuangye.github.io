...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier *)spec {
    ...
    // 与 Root.plist 的 PostNotification 保持一致：
    // 功率模式变更额外发送 powerModeChanged（普通开关走 settingsChanged）
    if ([key isEqualToString:S("powerMode")]) {
...
- (void)applyCPUFrequencyLock {
    ...
    if (mhz < 300 || mhz > 5000) {
        [self showSimpleAlertWithTitle:S("提示")
                              message:S("请输入 300 到 5000 之间的 CPU 目标频率（MHz）。")];
        return;
    }
...
- (void)commitCPUFrequencyLock:(int)mhz {
    ...
    [self showSimpleAlertWithTitle:S("已应用")
                          message:[NSString stringWithFormat:S("解除温控高频目标已设为 %d MHz；thermalmonitord 已重启并重新应用。"), mhz]];
}
