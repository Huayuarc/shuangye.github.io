...
// 功率模式选择：PSSegmentCell 分段控件
// 必须绑定自定义 set/get 走 jbroot 偏好路径（CPUthermalPaths.h），
// 若 set/get 为 NULL，Preferences 框架会通过 CFPreferences 读写真实路径
// /var/mobile/Library/Preferences/，与插件读写的 /var/jb/var/mobile/Library/Preferences/
// 不一致，导致控件呈灰色、无法选择且模式不生效。
PSSpecifier *spec = [PSSpecifier
...
[spec setProperty:S("fullPower") forKey:S("default")];
