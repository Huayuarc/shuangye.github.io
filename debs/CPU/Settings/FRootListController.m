...
- (void)savePowerMode:(NSString *)mode {
	...
	NSString *newMode = mode ?: S("fullPower");
	prefs[S("powerMode")] = newMode;
	if ([newMode isEqualToString:S("fullPower")]) {
		prefs[S(kCPUthermalThermalPressureKeyC)] = [NSNumber numberWithInteger:kCPUthermalThermalPressureLevelNominal];
	}
	[self savePrefs:prefs];
	if ([newMode isEqualToString:S("fullPower")]) {
		CPUthermalApplyThermalPressureLevelViaTool(kCPUthermalThermalPressureLevelNominal);
...
- (void)showThermalPressurePicker {
	if ([[self powerModeValue] isEqualToString:S("fullPower")]) {
		[self saveThermalPressure:kCPUthermalThermalPressureLevelNominal];
		[self showSimpleAlertWithTitle:S("温控等级") message:S("解除温控模式下热压级别固定为 Nominal（正常）。")];
		return;
	}

	UIAlertController *alert = [UIAlertController
	alertControllerWithTitle:S("温控等级")
	message:S("选择后会立即按 Battman 同步映射写入系统热压状态，无需等待重启。严重/锁定/睡眠可能触发系统高温警告。")
	preferredStyle:UIAlertControllerStyleActionSheet];
...
[group setProperty:S("默认解除温控，优先性能和帧率；仍保留 85°C 最后过热保护，防止异常高温损伤设备。") forKey:S("footerText")];
[specs addObject:group];
...
[group setProperty:S("热压级别与 Battman 同步：正常、轻微、中等、严重、锁定、睡眠。选择后立即执行并实时生效。") forKey:S("footerText")];
[specs addObject:group];
