...
// rootlessPath helper（透過 jbroot() 相容層）
...
static void lp_applyBatterySim(BOOL simulate) {
    @autoreleasepool {
        ...
        NSLog(@"[LowPower] 低电量模拟 prefs dict: %@", prefs);
        int result = CPUthermalApplyThermalStatusOverridesFromPrefs(prefs);
        if (result == kSCStatusOK) {
            // 驗證：嘗試讀回 OSThermalStatus.plist 確認寫入生效
            SCPreferencesRef verify = CPUthermalCreateOSThermalPrefs();
            if (verify) {
                CFNumberRef batterySOC = SCPreferencesGetValue(verify, CFSTR("kBatteryPercentRemainingKey"));
                int val = -1;
                if (batterySOC && CFNumberGetValue(batterySOC, kCFNumberIntType, &val)) {
                    NSLog(@"[LowPower] 低电量模拟驗證 kBatteryPercentRemainingKey=%d (期望:%d)", val, simulate ? 20 : -1);
                } else {
                    NSLog(@"[LowPower] 低电量模拟驗證: kBatteryPercentRemainingKey 不存在或非數值");
                }
                if (batterySOC) CFRelease(batterySOC);
                CFRelease(verify);
            }
...
static void lp_applyLowPowerToCommonProduct(void) {
    if (!g_lp_commonProduct) {
        // fallback: 重新查找 CommonProduct 實例
        Class cpClass = objc_getClass("CommonProduct");
        if (cpClass) {
            SEL sharedSel = @selector(sharedProduct);
            if (![cpClass respondsToSelector:sharedSel]) {
                sharedSel = @selector(sharedInstance);
            }
            if ([cpClass respondsToSelector:sharedSel]) {
                g_lp_commonProduct = ((id (*)(id, SEL))objc_msgSend)(cpClass, sharedSel);
                // 同步到 CPUthermalHelper
                Class helperClass = objc_getClass("CPUthermalHelper");
                if (helperClass && g_lp_commonProduct) {
                    id helper = ((id (*)(id, SEL))objc_msgSend)(helperClass, @selector(shared));
                    if (helper) {
                        ((void (*)(id, SEL, id))objc_msgSend)(helper, @selector(setCommonProductObject:), g_lp_commonProduct);
                    }
                }
                NSLog(@"[LowPower] fallback 重新獲取 CommonProduct: %@", g_lp_commonProduct);
            }
        }
        if (!g_lp_commonProduct) {
            NSLog(@"[LowPower] CommonProduct 仍為 nil，跳過");
            return;
        }
    }
    @try {
