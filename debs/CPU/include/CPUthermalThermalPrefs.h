...
// 适配: 检查 powerMode（CPUjiubb）和 thermalPowerMode（CPU 5.0.0）两种 key
static inline NSString *CPUthermalPowerModeFromPrefs(NSDictionary *prefs) {
    if (!prefs) return nil;
    NSString *mode = [prefs objectForKey:S("powerMode")];
    if (mode) return mode;
    mode = [prefs objectForKey:S("thermalPowerMode")];
    return mode;
}
...
static inline int CPUthermalApplyThermalStatusOverridesFromPrefs(NSDictionary *prefs) {
    ...
    NSString *powerMode = CPUthermalPowerModeFromPrefs(prefs);
