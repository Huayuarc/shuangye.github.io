...
static BOOL gPowerMode = 0;  // 0=正常 1=解除温控 2=低功耗
...
static void loadPrefs(void) {
    @autoreleasepool {
        ...
        if (d) {
            id val = [d objectForKey:S("enabled")];
            gEnabled = val ? [val boolValue] : NO;
            id modeVal = [d objectForKey:S("powerMode")];
            gPowerMode = modeVal ? [modeVal intValue] : 0;
        } else {
            gEnabled = NO;
            gPowerMode = 0;
        }
...
//
// 总开关开启 或 功率模式为非正常时 → 返回 nil (屏蔽系统散热建议)
// 关闭时 → 调用原实现
//
static id new_getBatteryServiceSuggestion(id self, SEL _cmd, id suggestion) {
    if (gEnabled || gPowerMode != 0) {
        // CPUthermal 激活: 返回 nil 屏蔽系统散热干预建议
        syslog(LOG_NOTICE, "[CPUthermalPrefHook] block ThermalManager suggestion (enabled:%d mode:%d)", gEnabled, gPowerMode);
        return nil;
    }
