...
- (int)currentLowPowerLockMHz {
    int requestedMHz = [[CPUthermalReadPrefs() objectForKey:S("lowPowerLockMHz")] intValue];
    switch (requestedMHz) {
        case 1380:
        case 1428:
        case 1584:
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _modeSubtitles = @[
            [NSString stringWithFormat:S("跨设备自适应，不高于 %d MHz"), [self currentLowPowerLockMHz]],
            S("动态锁定设置中的高频目标")
        ];
...
- (void)setupModeButtons {
    ...
    for (NSUInteger i = 0; i < itemCount; i++) {
        ...
        if (i == 0) {
            subtitle = [NSString stringWithFormat:S("跨设备自适应，不高于 %d MHz"), [self currentLowPowerLockMHz]];
        }
