...
- (int)currentLowPowerLockMHz {
    return CPUthermalLowPowerLockMHzFromValue(
        [CPUthermalReadPrefs() objectForKey:S("lowPowerLockMHz")]);
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _modeSubtitles = @[
            [NSString stringWithFormat:S("跨设备匹配最接近 %d MHz 的档位"), [self currentLowPowerLockMHz]],
            S("动态锁定设置中的高频目标")
        ];
...
- (void)setupModeButtons {
    ...
    for (NSUInteger i = 0; i < itemCount; i++) {
        ...
        if (i == 0) {
            subtitle = [NSString stringWithFormat:S("跨设备匹配最接近 %d MHz 的档位"), [self currentLowPowerLockMHz]];
        }
