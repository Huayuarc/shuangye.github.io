...
- (BOOL)isTweakEnabled {
    // 温控保护强制开启（默认启用），不再读取面板偏好；
    // off 模式为原生温控，不高亮；lowPower/fullPower 模式高亮表示干预中。
    NSString *mode = [self currentPowerMode];
    return ![mode isEqualToString:S(kCPUthermalOffModeC)];
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _modeValues = @[S(kCPUthermalOffModeC), S(kCPUthermalLowPowerModeC), S(kCPUthermalFullPowerModeC)];
        _modeTitles = @[S("原生温控"), S("低功耗"), S("解除温控")];
        _modeSubtitles = @[S("恢复系统原生温控行为"), S("模拟热压力降频，不限制 GPU"), S("解除全部温控降频限制")];
...
- (void)setupModeButtons {
    ...
    BOOL isLowPower = [currentMode isEqualToString:S(kCPUthermalLowPowerModeC)];
...
- (void)updateModuleSelectedState {
    ...
    if ([self respondsToSelector:@selector(setSelectedGlyphColor:)]) {
        NSString *currentMode = [self currentPowerMode];
        UIColor *glyphColor = [UIColor systemGrayColor];
        if ([currentMode isEqualToString:S(kCPUthermalLowPowerModeC)]) {
            glyphColor = [UIColor systemGreenColor];
        } else if ([currentMode isEqualToString:S(kCPUthermalFullPowerModeC)]) {
