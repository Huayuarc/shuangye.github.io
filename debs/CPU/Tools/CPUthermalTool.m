    ...
    @autoreleasepool {
        if (argc > 1) {
            ...
            if ([command isEqualToString:S("restore-fullpower")]) {
                ...
                // Step 2: 重启 thermalmonitord。
                // 关键修复：新进程以干净的「无残留状态」按 fullPower 初始化，
                // 根治旧实现遗留的 65000mW 功率状态卡死问题（原来仅发通知 + 等待 keep-alive）。
                notify_post(kCPUthermalPowerModeChangedNotifC);
                int rc = restartThermalmonitord();
                NSLog(@"[CPUthermalTool] 已重启 thermalmonitord (rc=%d)，新进程按 fullPower 初始化", rc);
                return rc;
            }
        }
