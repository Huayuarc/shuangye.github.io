...
// ============================================================
// 发送双通道通知给 thermalmonitord
// ============================================================
- (void)_postModeChangeNotification {
    // 通道1: CFNotificationCenter Darwin 通知
    CFNotificationCenterPostNotification(
        CFNotificationCenterGetDarwinNotifyCenter(),
        kCPUthermalReloadNotification,
        NULL, NULL, YES
    );
    // 通道2: notify.h 通知（更底层，更可靠）
    notify_post(kCPUthermalReloadNotifyName);
}
...
- (void)showPowerModePicker:(PSSpecifier *)specifier {
    ...
    for (NSUInteger i = 0; i < titles.count; i++) {
        UIAlertAction *action = [UIAlertAction actionWithTitle:titles[i]
            ...
            // 直接写入 plist（不依赖 specifier key，防止 PSButtonCell key 传递异常）
            NSMutableDictionary *prefs = [NSMutableDictionary dictionaryWithContentsOfFile:PREFS_PATH];
            if (!prefs) prefs = [NSMutableDictionary dictionary];
            prefs[@"thermalPowerMode"] = values[i];
            [prefs writeToFile:PREFS_PATH atomically:YES];

            // 发送双通道通知
            [self _postModeChangeNotification];

            // 用户反馈确认
            NSString *doneMsg = [values[i] isEqualToString:@"lowPower"] ? @"已切换至低功耗模式" : @"已切换至解除温控模式";
