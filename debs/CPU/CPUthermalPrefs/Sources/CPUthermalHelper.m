...
- (NSDictionary *)patchThermalPlist:(NSDictionary *)dict {
    ...
    if (self.thermalPreventDimmingEnabled) {
        ...
        NSDictionary *backlight = mutableDict[@"backlightComponentControl"];
        if ([backlight isKindOfClass:[NSDictionary class]]) {
            ...
            mutableDict[@"backlightComponentControl"] = mutableBacklight;
        }
    }
...
- (NSInteger)loadThermalMode {
    ...
    if (!prefs) return ThermalControlModeFullPower;
    NSNumber *mode = prefs[@"thermalControlMode"];
    return mode ? [mode integerValue] : ThermalControlModeFullPower;
}
...
- (void)saveThermalMode:(NSInteger)mode {
    ...
    dict[@"thermalControlMode"] = @(mode);
    [dict writeToFile:path atomically:YES];
    notify_post("com.huayuarc.cputhermal.reloadPrefs");
}
