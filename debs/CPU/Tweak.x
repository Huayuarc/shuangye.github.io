...
static const double kLowPowerKeepAliveInterval = 1.0;  // 每1秒重应用一次，大幅缩小窗口期
...
return 40;  // 与 ceiling 相同，形成功率硬锁定
}
...
// 同时锁定 floor = ceiling，防止 governor 跳回高频
if ([controller respondsToSelector:@selector(setCPUPowerFloor:fromDecisionSource:)]) {
((void (*)(id, SEL, int, uintptr_t))objc_msgSend)(controller, @selector(setCPUPowerFloor:fromDecisionSource:), lowPowerPowerFloorValue(), 0);
}
...
// ============================================================================
static void startLowPowerKeepAliveTimer(void) {
stopLowPowerKeepAliveTimer();  // 确保先停止已有定时器
...
dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kLowPowerKeepAliveInterval * NSEC_PER_SEC)),
(int64_t)(kLowPowerKeepAliveInterval * NSEC_PER_SEC),
(int64_t)(0.05 * NSEC_PER_SEC));  // 允许 50ms 延迟容差（与 0.2s 间隔匹配）
...
if (g_restoringFullPower || g_applyingLowPower) {
return %orig;
}

// 紧急保护 — 只有在全功率模式且非低功耗时才完全放行
// 低功耗模式下也拦截紧急 selector，防止系统通过高优通道绕过锁频
if (SELECTOR_IS_CRITICAL(selector)) {
    if (!shouldApplyLowPowerLimit()) {
        return %orig;
    }
    // 低功耗模式下紧急指令也拦截
    NSLog(@"[CPUthermal] 低功耗拦截紧急 selector: 0x%x", selector);
    return KERN_SUCCESS;
}
...
// ===================== 增强：低功耗模式下拦截频率属性写入 =====================
if (g_cpuProtection && shouldApplyLowPowerLimit()) {
static NSArray *freqWriteKeys;
static dispatch_once_t onceFreq;
dispatch_once(&onceFreq, ^{
freqWriteKeys = @[S("freq"),S("frequency"),S("MaxCPUPower"),S("CPUPowerTarget")];
});
for (NSString *wk in freqWriteKeys) {
if ([ks containsString:wk]) {
...
// 即使 replacement 失败也阻止写入原始值，防止 bypass
NSLog(@"[CPUthermal] 低功耗模式阻止 cpuKey 写入: %@", ks);
return KERN_SUCCESS;
}
break;
}
...
if (shouldApplyLowPowerLimit()) {
NSLog(@"[CPUthermal] 拦截ApplePPMCPU.updateCPU（低功耗锁频保护）");
return;
}
%orig;
}
...
// 低功耗模式：禁止原生刷新CPU参数，防止解锁覆盖限制
if (shouldApplyLowPowerLimit()) {
NSLog(@"[CPUthermal] 拦截MitigationController原生updateCPU（低功耗锁频保护）");
return;
}
if (shouldApplyFullCPUProtection()) {
return;
...
// 低功耗模式：禁止原生刷新Package功耗参数
if (shouldApplyLowPowerLimit()) {
NSLog(@"[CPUthermal] 拦截MitigationController原生updatePackage（低功耗锁频保护）");
return;
}
if (shouldApplyFullCPUProtection()) {
return;
