...
static void writeLowPowerIORegistryParams(void) {
    ...
    // 每 3 个 keep-alive 周期写一次（≈每 3 秒），匹配"每隔几秒钟"且降低内核写入频率
    static int writeCounter = 0;
    if (++writeCounter < 3) return;
    writeCounter = 0;
    ...
    CFNumberRef powerNum = CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &powerMW);
    if (!freqNum || !powerNum) {
        if (freqNum) CFRelease(freqNum);
