...
static BOOL g_thermalStateProtection= NO;  // 热状态封锁(Nominal/热压力/强制级别)
static BOOL g_blockHidEvents        = NO;  // 阻止 HID 温度事件
static BOOL g_keepCPSMAlive         = NO;  // 保留 CPMS 紧急保护(安全阀) 默认关闭
static BOOL g_suppressThermalNotifications = NO; // 屏蔽高温通知(Darwin/ObjC)
static BOOL g_blockNotifPopup = NO;              // 屏蔽高温温度计警告弹窗
...
g_brightnessProtection  = [d[S("brightnessProtection")] ?: [NSNumber numberWithBool:NO] boolValue];
...
// 热通知 — 可选择性阻断（尊重 suppressThermalNotifications + thermalBlockNotifPopup）
- (void)updateThermalNotification:(id)notification {
if (g_enabled && (g_suppressThermalNotifications || g_blockNotifPopup)) {
if (!isTemperatureAboveSafetyCeiling()) {
