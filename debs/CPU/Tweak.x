...
if (g_cpuProtection) {
    ...
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        // 使用C字符串构造，规避__CFString只读段潜在崩溃风险；移除大小写重复/宽泛易误伤关键词
        cpuKeys = @[
            S("cpu"),
            S("freq"),
            S("frequency"),
...
// 将每个阈值提高 500°C (500000 毫摄氏度)，彻底解除温控触发
int64_t raised = [val longLongValue] + 500000;
...
int64_t raised = [v longLongValue] + 500000;
newDict[k] = @(raised);
} else {
newDict[k] = v;
}
