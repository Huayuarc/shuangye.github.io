...
if (!mitigationClass || ![controller isKindOfClass:mitigationClass]) return;
...
if (!isFullPowerMode() || entry == MACH_PORT_NULL || !key || !value || CFGetTypeID(value) != CFNumberGetTypeID()) return;
...
if (!isFullPowerMode() || entry == MACH_PORT_NULL || !properties || CFGetTypeID(properties) != CFDictionaryGetTypeID()) return;
