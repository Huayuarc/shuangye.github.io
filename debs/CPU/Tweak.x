...
NSLog(@"[CPUthermal] 屏幕已熄灭，继续低功耗保活并取消旧唤醒脉冲");
}
...
NSLog(S("[CPUthermal] 已进入休眠状态，暂停保活并等待唤醒重建: %@"), reason ?: S("unknown"));
}
...
// 深睡后旧 dispatch source 可能仍有对象但不再按预期触发，唤醒时强制重建。
stopLowPowerKeepAliveTimer();
stopFullPowerKeepAliveTimer();
applyPowerModeToRuntime(NO);
reapplyTrackedCPUFrequencyTargets();
scheduleWakeRuntimeApply();
NSLog(S("[CPUthermal] 收到唤醒事件，已重建当前功率模式: %@"), reason ?: S("unknown"));
}
...
if (state != 0) handleRuntimeWakeEvent(S("display-on"));
else handleRuntimeDisplayOffEvent();
});
if (status != NOTIFY_STATUS_OK) g_displayStatusNotifyToken = -1;
}
...
handleRuntimeWakeEvent(S("device-unlocked"));
}
});
if (status != NOTIFY_STATUS_OK) g_lockStateNotifyToken = -1;
}
}
