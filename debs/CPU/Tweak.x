...
%ctor {
    ...
    // 注意：运行时模式切换完全由 LowPowerHooks.x 处理
    // Tweak.x 仅在 CommonProduct initProduct: 初始化路径中调用 CPUthermalApplyMode()
    // 避免在 g_lp_lowPowerActive 尚未更新时调用 tryTakeAction 导致评估循环过早完成
}
