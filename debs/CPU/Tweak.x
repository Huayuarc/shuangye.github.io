...
static const double kLowPowerKeepAliveInterval = 1.0;  // 每1.0s秒重应用一次，缩短周期防止频率漂移及锁屏反弹
...
// applyLowPowerLimitToController 已废弃 — 不再直接操作单个控制器，改用 applyLowPowerToCommonProduct + tryTakeAction
...
// ============================================================================
// 持久性低功耗保持定时器 — 每 1.0 秒重应用一次，防止决策树漂移、新控制器覆盖及锁屏频率反弹
// ============================================================================
static void startLowPowerKeepAliveTimer(void) {
...
(int64_t)(0.3 * NSEC_PER_SEC));  // 允许 0.3s 延迟容差
...
// 主动阻止系统决策树重新计算功耗策略（代替废弃的控制器缓存遍历）
if (g_commonProduct) {
    [g_commonProduct tryTakeAction];
}
});
...
if (shouldApplyLowPowerLimit()) {
// setCPULevel 钩子已确保 level 被钳制在 [0,2]
