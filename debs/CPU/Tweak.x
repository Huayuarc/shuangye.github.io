...
static void lp_startTimer(void) {
    ...
    NSLog(@"[LowPower] 保活定时器已启动 (%.1f 秒)", (double)kLPTimerIntervalMs / 1000.0);
}
...
- (void)setCPULevel:(int)level {
    if (g_restoringPower) { %orig(level); return; }
    if (shouldApplyLowPowerLimit()) {
        if (level < 0) level = 0;
        if (level > 2) level = 2;
        %orig(level);
...
static void CPUthermalApplyMode(void) {
    ...
    if ([helper.thermalPowerMode isEqualToString:S("lowPower")]) {
        [helper.commonProductObject putDeviceInThermalSimulationMode:S("moderate")];
