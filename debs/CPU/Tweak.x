...
static const double kLowPowerKeepAliveInterval = 2.0;  // 每2.0秒重应用一次（缩短刷新周期，防止解锁覆盖）
...
// ============================================================================
// 持久性低功耗保持定时器 — 每 5 秒重应用一次，防止决策树漂移
// ============================================================================
static void startLowPowerKeepAliveTimer(void) {
...
(int64_t)(0.5 * NSEC_PER_SEC));  // 允许 0.5s 延迟容差
...
NSLog(@"[CPUthermal] 低功耗保持定时器已启动 (每 %.1fs 重应用)", kLowPowerKeepAliveInterval);
}
