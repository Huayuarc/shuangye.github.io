...
// 低功耗模式期望目标（MHz）；运行时会按设备真实性能核 P-State 选择不高于目标的最近档位。
static const int kDefaultLowPowerRequestedFrequencyMHz = 1380;
static int g_lowPowerRequestedFrequencyMHz = kDefaultLowPowerRequestedFrequencyMHz;
...
static const int kFallbackLowPowerTargetMW = 2500;
...
static const double kLowPowerActiveKeepAliveInterval = 1.0;
static const double kLowPowerLockedKeepAliveInterval = 1.0;
static double g_lowPowerKeepAliveScheduledInterval = 0.0;
static IOPMAssertionID g_lowPowerSleepAssertionID = kIOPMNullAssertionID;
static os_unfair_lock g_lowPowerSleepAssertionLock = OS_UNFAIR_LOCK_INIT;
static NSHashTable *g_applePPMInstances = nil;           // 追踪 ApplePPMCPU 实例（弱引用，防止僵尸实例泄漏）
static int g_lastSystemLowPowerTargetMW = kFallbackLowPowerTargetMW;
static os_unfair_lock g_lowPowerTargetLock = OS_UNFAIR_LOCK_INIT;
...
static int normalizedLowPowerRequestedFrequencyMHz(int requestedMHz);
static int currentLowPowerPowerTargetMW(void);
static int captureLowPowerPowerTargetMW(int targetMW);
...
static void updateLowPowerKeepAliveTimerSchedule(void);
static void updateLowPowerSleepAssertion(void);
static void releaseLowPowerSleepAssertion(void);
...
static void reapplyLowPowerFrequencyLimitToServices(void);
...
static BOOL runtimeIsLocked(void) {
return g_deviceLocked || g_displayIsOff;
}
...
return requestedMHz;
default:
return kDefaultLowPowerRequestedFrequencyMHz;
}
}

static int currentLowPowerPowerTargetMW(void) {
os_unfair_lock_lock(&g_lowPowerTargetLock);
int targetMW = g_lastSystemLowPowerTargetMW;
os_unfair_lock_unlock(&g_lowPowerTargetLock);
return targetMW > 0 && targetMW < kUnrestrictedPowerLimitMW
? targetMW
: kFallbackLowPowerTargetMW;
}

static int captureLowPowerPowerTargetMW(int targetMW) {
if (targetMW > 0 && targetMW < kUnrestrictedPowerLimitMW) {
os_unfair_lock_lock(&g_lowPowerTargetLock);
g_lastSystemLowPowerTargetMW = targetMW;
os_unfair_lock_unlock(&g_lowPowerTargetLock);
return targetMW;
}
return currentLowPowerPowerTargetMW();
}
...
static int targetCPUFrequencyMHz(void) {
if (isLowPowerMode()) {
int requestedMHz = normalizedLowPowerRequestedFrequencyMHz(g_lowPowerRequestedFrequencyMHz);
int hardwareMaxMHz = hardwareMaxFrequencyMHz();
...
// 锁屏低功耗强行压制逻辑
// 修复①：低功耗模式下绝对禁止清空 PowerSaveToken 和 PowerSaveActive，
// 强制 PowerSaveActive=YES / PowerSaveToken=1 / CPULevel=kLowPowerCPULevel / DVD1Level=kLowPowerCPULevel，
// 防止系统把频率恢复为默认档。
static void enforceLowPowerLock(id controller) {
if (!controller || !isLowPowerMode()) return;
...
((void (*)(id, SEL, int))objc_msgSend)(controller, @selector(setDVD1Level:), kLowPowerCPULevel);
}
}
...
// 修复①：低功耗绝对禁止清空 PowerSaveActive/PowerSaveToken —— 先强制压制再走完整下发
enforceLowPowerLock(controller);
...
CPUthermalForceNominalCombined();
NSLog(@"[CPUthermal] 已主动套用低功耗 CommonProduct 状态（PowerSave/CPMS 已启用）");
} @catch (NSException *exception) {
...
static double currentLowPowerKeepAliveInterval(void) {
return runtimeIsLocked() ? kLowPowerLockedKeepAliveInterval : kLowPowerActiveKeepAliveInterval;
}

static void acquireLowPowerSleepAssertion(void) {
if (!g_enabled || !g_cpuProtection || !isLowPowerMode() || !runtimeIsLocked()) return;

os_unfair_lock_lock(&g_lowPowerSleepAssertionLock);
BOOL alreadyActive = (g_lowPowerSleepAssertionID != kIOPMNullAssertionID);
os_unfair_lock_unlock(&g_lowPowerSleepAssertionLock);
if (alreadyActive) return;

IOPMAssertionID assertionID = kIOPMNullAssertionID;
IOReturn result = IOPMAssertionCreateWithName(kIOPMAssertionTypePreventSystemSleep,
kIOPMAssertionLevelOn,
CFSTR("CPUthermal locked low-power keep-alive"),
&assertionID);
CFStringRef assertionType = kIOPMAssertionTypePreventSystemSleep;

if (result != kIOReturnSuccess || assertionID == kIOPMNullAssertionID) {
assertionID = kIOPMNullAssertionID;
result = IOPMAssertionCreateWithName(kIOPMAssertionTypePreventUserIdleSystemSleep,
kIOPMAssertionLevelOn,
CFSTR("CPUthermal locked low-power keep-alive"),
&assertionID);
...
if (result != kIOReturnSuccess || assertionID == kIOPMNullAssertionID) {
NSLog(@"[CPUthermal] 锁屏低功耗电源断言创建失败: 0x%x", result);
return;
}

BOOL keepAssertion = NO;
os_unfair_lock_lock(&g_lowPowerSleepAssertionLock);
if (g_lowPowerSleepAssertionID == kIOPMNullAssertionID &&
g_enabled && g_cpuProtection && isLowPowerMode() && runtimeIsLocked()) {
g_lowPowerSleepAssertionID = assertionID;
keepAssertion = YES;
}
os_unfair_lock_unlock(&g_lowPowerSleepAssertionLock);
...
NSLog(@"[CPUthermal] 锁屏低功耗电源断言已启用: %@", assertionType);
}

static void releaseLowPowerSleepAssertion(void) {
IOPMAssertionID assertionID = kIOPMNullAssertionID;
os_unfair_lock_lock(&g_lowPowerSleepAssertionLock);
assertionID = g_lowPowerSleepAssertionID;
g_lowPowerSleepAssertionID = kIOPMNullAssertionID;
os_unfair_lock_unlock(&g_lowPowerSleepAssertionLock);

if (assertionID == kIOPMNullAssertionID) return;
IOReturn result = IOPMAssertionRelease(assertionID);
NSLog(@"[CPUthermal] 锁屏低功耗电源断言已释放: 0x%x", result);
}

static void updateLowPowerSleepAssertion(void) {
if (g_enabled && g_cpuProtection && isLowPowerMode() && runtimeIsLocked()) {
acquireLowPowerSleepAssertion();
return;
}
releaseLowPowerSleepAssertion();
}

static void updateLowPowerKeepAliveTimerSchedule(void) {
if (!g_lowPowerKeepAliveTimer) return;
double interval = currentLowPowerKeepAliveInterval();
if (g_lowPowerKeepAliveScheduledInterval == interval) return;

g_lowPowerKeepAliveScheduledInterval = interval;
dispatch_source_set_timer(g_lowPowerKeepAliveTimer,
dispatch_walltime(NULL, (int64_t)(interval * NSEC_PER_SEC)),
(uint64_t)(interval * NSEC_PER_SEC),
0);
NSLog(@"[CPUthermal] 低功耗保活周期已更新: %.1fs (%@)",
interval, runtimeIsLocked() ? @"锁屏" : @"亮屏");
}
...
runWakeRuntimeApplyPulse(300, generation);
});
}
...
updateLowPowerSleepAssertion();
updateLowPowerKeepAliveTimerSchedule();
...
NSLog(@"[CPUthermal] 屏幕已熄灭，低功耗保活切换为每 %.1fs 执行", currentLowPowerKeepAliveInterval());
}
...
NSLog(@"[CPUthermal] 设备已锁定，低功耗保活切换为每 %.1fs 执行", currentLowPowerKeepAliveInterval());
}
...
updateLowPowerSleepAssertion();
updateLowPowerKeepAliveTimerSchedule();
NSLog(S("[CPUthermal] 收到系统休眠事件，低功耗保活定时器保持运行: %@"), reason ?: S("unknown"));
}
...
// 关键修复：如果定时器已经存在，直接更新 Schedule 和电源断言，绝不销毁重建！
...
g_lowPowerKeepAliveScheduledInterval = 0.0;
updateLowPowerSleepAssertion();
updateLowPowerKeepAliveTimerSchedule();
dispatch_resume(g_lowPowerKeepAliveTimer);
NSLog(@"[CPUthermal] 低功耗保持定时器已启动 (当前每 %.1fs 重应用)", currentLowPowerKeepAliveInterval());
}
...
if (!keyMatchesLowPowerLimit(keyString)) return;
...
static const char *g_lowPowerCPUServiceClasses[] = {
"AppleCLPC",
"ApplePPMCPMS",
"AppleARMCPU",
...
CFNumberRef targetNumber = CFNumberCreate(kCFAllocatorDefault,
kCFNumberIntType,
&targetMHz);
CFStringRef frequencyKey = maxOperatingFrequencyPropertyName();
if (!targetNumber || !frequencyKey) {
if (targetNumber) CFRelease(targetNumber);
return;
}

int successfulWrites = 0;
for (int classIndex = 0; g_lowPowerCPUServiceClasses[classIndex]; classIndex++) {
io_iterator_t iterator = MACH_PORT_NULL;
CFMutableDictionaryRef matching = IOServiceMatching(g_lowPowerCPUServiceClasses[classIndex]);
...
if (successfulWrites > 0 && lastLoggedTargetMHz != targetMHz) {
lastLoggedTargetMHz = targetMHz;
NSLog(S("[CPUthermal] 低功耗频率上限已跨设备写入: 请求=%dMHz 实际P-State=%dMHz 服务数=%d"),
g_lowPowerRequestedFrequencyMHz, targetMHz, successfulWrites);
}
}
...
reapplyLowPowerFrequencyLimitToServices();
}
...
static void loadPrefs(void) {
    @autoreleasepool {
        ...
        g_lowPowerRequestedFrequencyMHz = normalizedLowPowerRequestedFrequencyMHz(
...
NSString *keyString = (__bridge NSString *)key;
if (g_cpuProtection && keyMatchesLowPowerLimit(keyString)) {
...
if (shouldApplyLowPowerLimit()) {
%orig(1); // 强制锁定 token 为 1，禁止系统在锁屏时清零
return;
}
if (shouldApplyFullCPUProtection()) {
%orig(0);
...
if (shouldApplyLowPowerLimit()) {
// 修复①：低功耗 updateCPU 时先强制压制 PowerSaveActive/PowerSaveToken/CPULevel/DVD1Level
enforceLowPowerLock(self);
activateLowPowerStateOnController(self);
} else applyFullPowerBudgetsOnController(self);
%orig;
return;
}
%orig;
...
- (void)setCPULowPowerTarget:(int)target {
    ...
    if (shouldApplyLowPowerLimit()) {
        // 记录设备热配置提供的真实低功耗预算；系统试图写 0/65000 解锁时恢复该设备自己的预算。
        int safeTarget = captureLowPowerPowerTargetMW(target);
        %orig(safeTarget);
        return;
    }
    if (shouldApplyFullCPUProtection()) {
...
if (shouldApplyLowPowerLimit()) {
// 唤醒/解锁时系统常把 CPUMaxPower 恢复为 65000；低功耗模式必须继续使用设备自己的 LPM 预算。
int lowPowerTargetMW = currentLowPowerPowerTargetMW();
int safeTargetMW = (target > 0 && target < lowPowerTargetMW) ? target : lowPowerTargetMW;
%orig(safeTargetMW, legacy, propertyArg);
return;
}
if (shouldApplyFullCPUProtection()) {
...
// 如果决策源尝试下发 100% 或 65000 限制，强制钳制为低功耗 Level 百分比 (kLowPowerCPULevel)
int safeCeiling = (ceiling >= 100 || ceiling >= kUnrestrictedPowerLimitMW) ? kLowPowerCPULevel : ceiling;
%orig(safeCeiling, source);
return;
}
if (shouldApplyFullCPUProtection()) {
%orig(kUnrestrictedPowerLimitMW, source);
return;
}
%orig;
}
...
if (shouldApplyFullCPUProtection()) {
%orig(kUnrestrictedPowerLimitMW, contributor);
return;
}
%orig;
}
