...
static const double kLowPowerLockedKeepAliveInterval = 5.0;
...
if (!d) return; // 读取失败时维持当前内存状态，不修改 g_powerMode
...
// 仅在明确存在 powerMode key 时更新，避免配置缺失时默认回退到 fullPower
...
- (void)setCPULowPowerTarget:(int)target {
    ...
    if (shouldApplyLowPowerLimit()) {
        // 如果系统下发了无限制哨兵值(65000)，拦截并保留系统的低功耗预算或降至合适值
        // 若系统传入有效低功耗值（如 2000~5000），则遵循系统值；若系统试图解锁(65000)，将其压回
        int safeTarget = (target >= kUnrestrictedPowerLimitMW || target <= 0) ? 2500 : target;
        %orig(safeTarget);
        return;
    }
    if (shouldApplyFullCPUProtection()) {
