...
// 1. 如果用户在面板手动输入了自定义锁频值 (g_cpuMinPowerValue > 0)，优先使用用户值
if (g_cpuMinPowerValue > 0) {
return g_cpuMinPowerValue;
}

// 2. 如果用户没填，通过 sysctl 动态获取当前设备 CPU 的真实最大频率 (Hz -> MHz)
...
if (s_hardwareMaxMHz > 0) {
return s_hardwareMaxMHz;
}

// 3. 兜底逻辑：如果系统无法读取真实主频，则返回 0（不注入 MaxOperatingFrequency，让系统使用硬件默认最大值）
return 0;
}
...
if (targetFreq > 0) {
// 有有效频率（用户指定或动态读取成功）时才写入
modified[S("MaxOperatingFrequency")] = [NSNumber numberWithInt:targetFreq];
} else {
// 如果为 0，直接移除该限制节点，让系统默认跑满硬件最大值
[modified removeObjectForKey:S("MaxOperatingFrequency")];
}
// 清除所有功率限制参数
