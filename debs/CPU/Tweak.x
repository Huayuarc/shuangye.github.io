...
static void writeLowPowerIORegistryParams(BOOL force) {
    if (!shouldApplyLowPowerLimit()) return;

    // 防刷机制：仅在非 force 状态下，强制要求 3 秒冷却。
    // force=YES 用于亮屏/解锁/唤醒等关键节点，强行突破冷却时间，
    // 避免系统在解锁瞬间重置底层频率上限后错过压制时机导致频率失控。
    static CFAbsoluteTime lastWrite = 0;
    CFAbsoluteTime now = CFAbsoluteTimeGetCurrent();
    if (!force && lastWrite != 0 && (now - lastWrite) < 3.0) return;
    lastWrite = now;
...
writeLowPowerIORegistryParams(NO); // 常规循环遵守 3 秒冷却
});
...
static void screenStateChanged(CFNotificationCenterRef center, void *observer, CFNotificationName name, const void *object, CFDictionaryRef userInfo) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (locked) {
            ...
            // 锁屏瞬间强制压稳底层频率
            applyPowerModeToRuntime();
            writeLowPowerIORegistryParams(YES); // 锁屏瞬间强制写入，突破冷却
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if (g_enabled && isLowPowerMode()) {
                    applyLowPowerToCommonProduct();
                    applyLowPowerLimitsToTrackedControllers();
                    writeLowPowerIORegistryParams(YES); // 延时补刀也强制写入底层
                }
            });
            ...
            // 亮屏瞬间强制突破冷却，直接锁死底层频率
            applyPowerModeToRuntime();
            writeLowPowerIORegistryParams(YES);
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if (g_enabled && isLowPowerMode()) {
                    applyPowerModeToRuntime();
                    writeLowPowerIORegistryParams(YES); // 二次补刀强制写入底层
                }
            });
    ...
    if (messageType == kIOMessageSystemHasPoweredOn) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (g_enabled) {
                ...
                applyPowerModeToRuntime();
                // 低功耗下强制写入底层频率并确保保活定时器存活
                // （防止长时间睡眠期间定时器异常丢失 + 唤醒瞬间死锁 IOKit 频率）
                if (isLowPowerMode()) {
                    writeLowPowerIORegistryParams(YES); // 硬件唤醒瞬间强制死锁频率
                    if (!g_lowPowerKeepAliveTimer) {
