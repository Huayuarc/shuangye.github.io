...
// 低功耗频率限制 — 固定 1380 MHz（iPhone 13 Pro 已验证）
static NSInteger g_lp_lowTargetMHz = 0;
// IOKit 频率钳位：下限 800MHz（允许低至 1380 目标），上限由低功耗目标动态决定
static const int64_t kLP_MinCapMHz = 800;
...
// CPUthermalTool 看门狗守护进程 PID（独立进程，自主读取 plist + 压频）
// 该进程永久运行，不受 lowPower 开关影响，thermalmonitord 重启后重新生成
static pid_t g_lp_watchdogPID = -1;
...
static void lp_spawnWatchdog(void);
static BOOL lp_isWatchdogAlive(void);
static NSString *lp_thermalToolPath(void);
...
static NSInteger lp_lowTargetMHz(void) {
    // 使用固定的 1380 MHz 低功耗目标频率
    // 经实测，此频率在 iPhone 13 Pro 上能平衡性能与发热
    return 1380;
}
...
// ============================================================================
// CPUthermalTool — 独立看门狗守护进程管理
//
// 关键设计：
//   CPUthermalTool --watch-plist 作为独立进程运行，不受 thermalmonitord
//   沙箱限制。它自主读取偏好 plist，在 lowPower 模式下每 200ms 直接 IOKit
//   写入频率上限，对抗系统温控的循环恢复。非低功耗时每 500ms 检查 plist。
//
//   %ctor 中生成一次，永久运行。poll timer 做健康检查自动复活。
// ============================================================================
static NSString *lp_thermalToolPath(void) {
...
static BOOL lp_isWatchdogAlive(void) {
    if (g_lp_watchdogPID <= 0) return NO;
    return (kill(g_lp_watchdogPID, 0) == 0);
}

static void lp_spawnWatchdog(void) {
    NSString *toolPath = lp_thermalToolPath();
    if (!toolPath) {
        NSLog(@"[LowPower] CPUthermalTool 未安装，无法启动看门狗");
        return;
    }

    // 如果已有运行中的看门狗，不重复生成
    if (lp_isWatchdogAlive()) {
        return;
    }

    // 清理旧 PID（如果进程已死但 PID 残留）
    if (g_lp_watchdogPID > 0) {
        int status;
        waitpid(g_lp_watchdogPID, &status, WNOHANG);
        g_lp_watchdogPID = -1;
    }

    // 构建参数: CPUthermalTool --watch-plist <plist_path> <nativeMHz>
    char nativeMHzStr[16];
    snprintf(nativeMHzStr, sizeof(nativeMHzStr), "%ld", (long)lp_nativeMaxMHz());
    ...
    // 使用干净的环境变量（不继承 DYLD_INSERT_LIBRARIES，
    // 避免 substrate 注入影响独立进程）
    char *const env[] = {
    ...
    int ret = posix_spawn(&pid, [toolPath fileSystemRepresentation], NULL, &attr,
                          (char *const *)argv, env);
    posix_spawnattr_destroy(&attr);

    if (ret == 0 && pid > 0) {
        g_lp_watchdogPID = pid;
        NSLog(@"[LowPower] CPUthermalD 看门狗已启动 (PID=%d, path=%s)", pid, [toolPath UTF8String]);
        // 异步等待，不阻塞
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            int status;
            waitpid(pid, &status, 0);
            NSLog(@"[LowPower] CPUthermalD 看门狗已退出 (PID=%d, status=%d)", pid, status);
        });
    } else {
        NSLog(@"[LowPower] CPUthermalD 看门狗启动失败: ret=%d (path=%s)",
              ret, [toolPath UTF8String]);
    }
}
...
static void lp_forceDirectFrequencyCap(void) {
    ...
    @autoreleasepool {
        // ================================================================
        // 路径 A: 由独立看门狗进程（CPUthermalTool --watch-plist）处理
        // 该进程在 %ctor 中启动，自主读取 plist，不受 thermalmonitord
        // 沙箱限制，每 200ms 直接 IOKit 写入频率上限。
        // ================================================================
        if (!lp_isWatchdogAlive()) {
            // 看门狗不在运行 -> 尝试启动（兜底）
            lp_spawnWatchdog();
        ...
        // ================================================================
        // 路径 B: 直接 IOKit 写入（备用，可能被沙箱拦截但值得一试）
        // ================================================================
        ...
        for (int i = 0; serviceNames[i]; i++) {
            ...
            orig_IOServiceSetProperty(service, CFSTR("cpu-frequency-cap"), maxFreq);
        ...
        if (pmgr != IO_OBJECT_NULL) {
            ...
            orig_IOServiceSetProperty(pmgr, CFSTR("cpu-frequency-cap"), maxFreq);
        ...
        NSLog(@"[LowPower] 頻率上限已施加: %ldMHz (min: %lldMHz, watchdog: %@)",
              (long)g_lp_lowTargetMHz, (long long)kLP_MinCapMHz,
              lp_isWatchdogAlive() ? @"运行中" : @"未运行");
    }
}
...
static void lp_applyState(void) {
    if (lp_isLowPower()) {
        ...
        // 独立看门狗进程负责 IOKit 频率上限（不受沙箱限制）
        // 路径 A: 看门狗守护进程（自主 plist 监听 + 200ms 保活）
        // 路径 B: 直接 IOKit 写入（thermalmonitord 内兜底）
        lp_forceDirectFrequencyCap();
    } else {
        // 看门狗进程持续运行，自主检查 plist 后恢复频率
        // 不需要在此 kill 它
        ...
        // 恢复时尝试清除 IOKit 频率上限
        @autoreleasepool {
            int64_t restoreHz = (int64_t)lp_nativeMaxMHz() * 1000000LL;
            CFNumberRef restoreFreq = CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt64Type, &restoreHz);
            CFNumberRef zeroFreq = CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt64Type, &(int64_t){0});
            ...
            for (int i = 0; serviceNames[i]; i++) {
                io_service_t service = IOServiceGetMatchingService(kIOMasterPortDefault,
                    IOServiceMatching(serviceNames[i]));
                if (service == IO_OBJECT_NULL) continue;
                orig_IOServiceSetProperty(service, CFSTR("clpc-cpu-max-frequency"), restoreFreq);
                orig_IOServiceSetProperty(service, CFSTR("clpc-cpu-min-frequency"), zeroFreq);
                orig_IOServiceSetProperty(service, CFSTR("max-cpu-frequency"), restoreFreq);
                orig_IOServiceSetProperty(service, CFSTR("cpu-frequency-cap"), restoreFreq);
                IOObjectRelease(service);
            }
            if (restoreFreq) CFRelease(restoreFreq);
            if (zeroFreq) CFRelease(zeroFreq);
        }
    }
}
...
static void lp_startPollTimer(void) {
    ...
    dispatch_source_set_event_handler(g_lp_pollTimer, ^{
        @autoreleasepool {
            ...
            // 看门狗守护进程健康检查：如果异常退出则自动复活
            if (!lp_isWatchdogAlive()) {
                NSLog(@"[LowPower] 看门狗守护进程不在运行，重新启动...");
                lp_spawnWatchdog();
            }
        }
    });
...
%ctor {
    @autoreleasepool {
        ...
        // ── 第9步：启动独立看门狗守护进程 ──
        // 该进程在 thermalmonitord 外部运行，自主读取 plist，
        // 不受 thermalmonitord 沙箱限制，通过 IOKit 直接写入频率上限。
        // 即使看门狗异常退出，poll timer 也会在 1 秒内复活它。
        lp_spawnWatchdog();
