...
// 用户授权：无视温度安全阀。开启后解除温控模式持续满血高频率运行，
// isTemperatureAboveSafetyCeiling() 恒返回 NO（不因温度触发任何回退/降频）。
static BOOL g_ignoreTemperatureLimit       = YES;
...
// 5 秒间隔：原 1 秒间隔会让主线程每秒遍历所有 controller 并触发大量
// 被 hook 的 setter/IOKit 调用，是 thermalmonitord 主线程卡死的主因之一。
static const double kKeepAliveInterval      = 5.0;
...
static void loadPrefs(void) {
    @autoreleasepool {
        ...
        g_ignoreTemperatureLimit       = [d[S("ignoreTemperatureLimit")] ?: @YES boolValue];
...
// 安全温度读取 — 5 秒静态缓存。
// 该函数原本被所有 IOKit/热决策 hook 调用，每次调用都做一次完整的
// IOServiceMatching + IOServiceGetMatchingService + IORegistryEntryCreateCFProperty
// 查询，在高频调用下会拖死 thermalmonitord 主线程（崩溃日志: main-thread 无响应）。
// 缓存存在良性竞态（并发时最多多读一次真实温度），不会崩溃。
static BOOL isTemperatureAboveSafetyCeiling(void) {
    // 用户授权「无视温度」：解除温控模式持续满血，安全阀直接放行，
    // 不再触发任何温度回退/降频逻辑。
    if (g_ignoreTemperatureLimit) return NO;
    static double lastReadTime = 0;
    static BOOL cachedIsOver = NO;
    static BOOL cacheValid = NO;
    double now = CFAbsoluteTimeGetCurrent();
    if (cacheValid && (now - lastReadTime) < 5.0) {
        return cachedIsOver;
    }
    BOOL isOver = NO;
    ...
    if (matching) {
        ...
        if (service) {
            ...
            if (temp) {
                int64_t tempVal = 0;
                isOver = CFNumberGetValue((CFNumberRef)temp, kCFNumberSInt64Type, &tempVal) && (tempVal >= kSafetyTempThreshold);
                CFRelease(temp);
            }
        }
    }
...
static kern_return_t hooked_IOServiceSetProperty(io_service_t service, CFStringRef key, CFTypeRef value) {
    // 必须确认 key 是 CFString 再转 NSString 发消息，否则向 CFData/CFNumber 等
    // 非字符串对象发送 lowercaseString 会触发 unrecognized selector / 野指针 SIGSEGV。
    if (g_enabled && g_cpuProtection && !isOffMode() &&
        key && CFGetTypeID(key) == CFStringGetTypeID() &&
        !isTemperatureAboveSafetyCeiling()) {
...
static CFPropertyListRef fake_IORegistryEntryCreateCFProperty(io_registry_entry_t entry, CFStringRef key, CFAllocatorRef allocator, IOOptionBits options) {
    ...
    // 仅当原始返回值就是 CFNumber（温度/热等级通常如此）时才伪装，
    // 保持返回类型一致，避免调用者按 CFData/CFBoolean 等类型处理伪装值时
    // 发生类型错乱崩溃。同时 key 必须为 CFString。
