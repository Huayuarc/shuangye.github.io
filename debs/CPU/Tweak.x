...
// 低功耗模式固定目标（MHz）
static const int kLowPowerFrequencyMHz = 1380;
...
static void forceCPUPerformanceLevelOnController(id controller, BOOL lowPower);
...
// 芯片硬件主频查表 (MHz) — 仅作为 sysctl 动态读取失败时的兜底
static int getFallbackMaxFrequencyForMachine(NSString *machine) {
if ([machine hasPrefix:@"iPhone10,"]) return 2390; // A11 (iPhone 8 / 8P / X)
if ([machine hasPrefix:@"iPhone11,"]) return 2490; // A12 (iPhone XS / XR)
if ([machine hasPrefix:@"iPhone12,"]) return 2650; // A13 (iPhone 11 系列)
if ([machine hasPrefix:@"iPhone13,"]) return 3100; // A14 (iPhone 12 系列)
if ([machine hasPrefix:@"iPhone14,"]) return 3230; // A15 (iPhone 13 系列 / 14 / 14 Plus)
if ([machine hasPrefix:@"iPhone15,2"] || [machine hasPrefix:@"iPhone15,3"]) return 3460; // A16 (iPhone 14 Pro / 15)
if ([machine hasPrefix:@"iPhone15,4"] || [machine hasPrefix:@"iPhone15,5"]) return 3460; // A16 (iPhone 15 Plus)
if ([machine hasPrefix:@"iPhone16,"]) return 3780; // A17 Pro (iPhone 15 Pro 系列)
if ([machine hasPrefix:@"iPhone17,"]) return 4040; // A18 / A18 Pro (iPhone 16 系列)
...
// 读取不受限制的主频节点
if (sysctlbyname("hw.cpufrequnrestricted", &freqHz, &size, NULL, 0) != 0 || freqHz == 0) {
sysctlbyname("hw.cpufreq", &freqHz, &size, NULL, 0);
}

if (freqHz > 0) {
s_hardwareMaxMHz = (int)(freqHz / 1000000LL);
} else {
// 3. 兜底：若 sysctl 失败，尝试通过设备型号查表
...
return configuredMHz;
}

static int targetCPUFrequencyMHz(void) {
if (isLowPowerMode()) {
return kLowPowerFrequencyMHz;
}
...
// 修正.x 移植后 fullPowerTargetForController 直接返回 65W，不再读取 ivar。
// 保留该辅助函数（不删减），标记 unused 以通过 -Werror 编译检查。
__attribute__((unused)) static int intIvarValue(id object, const char *name, int fallback) {
...
// 满血/强锁频模式下的 CPMS 功率上限 (mW)
static int fullPowerTargetForController(id controller) {
(void)controller;
// 65W (65000mW) 足够覆盖所有 A 系列芯片的最大瞬时功耗，防止 CPU 被压频到 2580/2400
...
((void (*)(id, SEL, BOOL))objc_msgSend)(controller, @selector(setPowerSaveActive:), lowPower);
}
if ([controller respondsToSelector:@selector(setPowerSaveToken:)]) {
sendSetPowerSaveToken(controller, lowPower ? 1 : 0);
}
...
forceCPUPerformanceLevelOnController(controller, YES);
...
forceCPUPerformanceLevelOnController(controller, NO);
...
static NSNumber *lockedFrequencyNumberForKey(NSString *key, NSNumber *originalNumber, int64_t targetMHz) {
if (!keyMatchesLowPowerLimit(key)) return nil;
int64_t original = originalNumber ? [originalNumber longLongValue] : targetMHz;
int64_t replacement = lockedFrequencyValue(original, targetMHz);
return [NSNumber numberWithLongLong:replacement];
}

static id patchedFrequencyConfigObject(id object, NSString *keyHint, int64_t targetMHz) {
if ([object isKindOfClass:[NSNumber class]]) {
NSNumber *patched = lockedFrequencyNumberForKey(keyHint, (NSNumber *)object, targetMHz);
...
[patchedArray addObject:patchedFrequencyConfigObject(item, keyHint, targetMHz) ?: item];
}
return patchedArray;
}
...
id patchedValue = patchedFrequencyConfigObject(value, childKey, targetMHz);
...
if (g_cpuProtection && !g_restoringFullPower && keyMatchesLowPowerLimit(ks)) {
...
forceCPUPerformanceLevelOnController(self, shouldApplyLowPowerLimit());
%orig;
return;
}
%orig;
...
if (shouldApplyLowPowerLimit()) {
// 低功耗 mW 目标使用系统设备配置，频率锁定由 0~100 性能百分比路径负责。
...
NSDictionary *patched = patchedFrequencyConfigObject(modified, key, targetCPUFrequencyMHz());
NSLog(@"[CPUthermal] 已应用低功耗频率配置: %@ target:%dMHz level:%d", key, targetCPUFrequencyMHz(), targetCPUPerformanceLevel());
return [patched copy];
}
}

// 解除温控模式：仅改写真正存在的 CPU frequency 字段，功率由 MitigationController 管理。
if (isFullPowerMode()) {
@autoreleasepool {
NSMutableDictionary *modified = [config mutableCopy];
if (modified) {
int targetFreq = fullPowerFrequencyValue();
if (targetFreq > 0) {
NSDictionary *patched = patchedFrequencyConfigObject(modified, key, targetFreq);
NSLog(@"[CPUthermal] 已应用解除温控频率配置: %@ target:%dMHz level:%d", key, targetFreq, targetCPUPerformanceLevel());
return [patched copy];
} else {
NSLog(@"[CPUthermal] 警告: 未能获取到有效的最大 CPU 频率，保持系统默认配置");
}
}
}
return config;
...
loadPrefs();
if (g_enabled) {
// 修正4：同步直接执行一次，不要完全依赖异步 dispatch_async 的 0.1s 延时
applyPowerModeToRuntime(NO);
// 修正.x 第2条：如果保活定时器在深度睡眠中被清理，重新补发启动
