...
// --- DVFS 防休眠 Keep-Alive 线程 ---
// 【改进代码.x】满载 (解除温控) 模式下，每 5 毫秒做一次微小数学运算，
// 防止 CPU 进入深度休眠/低负载而由 DVFS 下探到低频 State，锁定最高性能 P-State。
static dispatch_source_t g_dvfsKeepAliveThread = NULL;

static void startDVFSKeepAlive(void) {
if (g_dvfsKeepAliveThread) return;

// 使用 HIGH / REALTIME 优先级队列，防止 GCD 将线程挂起
dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0);
g_dvfsKeepAliveThread = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, queue);

if (g_dvfsKeepAliveThread) {
// 每 5ms 激活一次，确保频率始终处于最高 State
dispatch_source_set_timer(g_dvfsKeepAliveThread, DISPATCH_TIME_NOW, 0.005 * NSEC_PER_SEC, 0.002 * NSEC_PER_SEC);
dispatch_source_set_event_handler(g_dvfsKeepAliveThread, ^{
volatile uint64_t dummy = 0;
for (int i = 0; i < 50000; i++) {
dummy += i;
}
});
dispatch_resume(g_dvfsKeepAliveThread);
NSLog(@"[CPUthermal] DVFS 高优先级性能锁定线程已启动");
}
}

static void stopDVFSKeepAlive(void) {
if (g_dvfsKeepAliveThread) {
dispatch_source_cancel(g_dvfsKeepAliveThread);
g_dvfsKeepAliveThread = NULL;
}
}
...
if (isLowPowerMode()) {
stopDVFSKeepAlive(); // 【改进代码.x】进入低功耗时关闭满载 DVFS 防休眠线程
applyLowPowerToCommonProduct();
applyLowPowerLimitsToTrackedControllers();
...
startDVFSKeepAlive(); // 【改进代码.x】满载模式启动 DVFS 防休眠线程，锁定最高性能 P-State
}
}
...
return orig_IOPMSetAggressiveness(fb, type, 1); // 锁定低功耗
}
} else if (isFullPowerMode()) {
// 解除温控模式：系统试图调低性能策略时，强行重定向至最高性能档位
if (type == kPMSetProcessorSpeed) {
return orig_IOPMSetAggressiveness(fb, type, 0); // 强行锁定为 0 (最高性能/最高主频)
}
}
}
...
if (shouldApplyFullCPUProtection()) {
// 【改进代码.x】每次 updateCPU 时重新写入 P-State 0，强刷最高频，禁用 DVFS 低频下探
[self setCPULevel:0];
return;
}
%orig;
...
if (shouldApplyFullCPUProtection()) {
// 取消温控压制，允许 CPU 跑满最高 P-State
[self setCPUPowerCeiling:100 fromDecisionSource:0];
return;
}
%orig;
}
...
if (shouldApplyFullCPUProtection()) {
// 放行 Floor，不要强制写死为 Ceiling，防止系统 PMGR 抛出异常退回 2016MHz
%orig(0, source);
return;
}
%orig;
