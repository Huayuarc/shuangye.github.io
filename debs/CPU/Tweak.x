...
// 要求仅当系统低功耗模式激活时才限频（从 pref requireLowPowerMode 读取）
static BOOL g_lp_requireLowPowerMode = NO;
...
static void lp_loadPrefs(void) {
    @autoreleasepool {
        ...
        g_lp_requireLowPowerMode = [d[@"requireLowPowerMode"] boolValue];
        g_lp_lowPowerActive = [mode isEqualToString:@"lowPower"];

        // 检测系统低功耗模式状态
        g_lp_systemLowPowerActive = lp_systemLowPowerActive();

        // 如果启用了"跟随系统低功耗模式"，则限频仅当系统 LPM 激活时才生效
        if (g_lp_lowPowerActive && g_lp_requireLowPowerMode) {
            g_lp_lowPowerActive = g_lp_systemLowPowerActive;
        }

        g_lp_lowTargetMHz = lp_lowTargetMHz();
        NSLog(@"[LowPower] prefs: mode=%@ requireLPM=%d sysLPM=%d active=%d target=%ldMHz",
              mode, g_lp_requireLowPowerMode, g_lp_systemLowPowerActive,
...
static void lp_onSystemLowPowerChanged(void) {
    BOOL wasActive = g_lp_lowPowerActive;
    g_lp_systemLowPowerActive = lp_systemLowPowerActive();
    NSLog(@"[LowPower] 系统低功耗模式变化: %d → %d",
          !g_lp_systemLowPowerActive, g_lp_systemLowPowerActive);

    // 仅在 requireLowPowerMode 模式下，系统 LPM 切换才影响限频状态
    if (!g_lp_requireLowPowerMode) return;

    BOOL shouldBeActive = lp_isLowPower() && g_lp_systemLowPowerActive;
    if (shouldBeActive != wasActive) {
        g_lp_lowPowerActive = shouldBeActive;
        if (shouldBeActive) {
            g_lp_lowTargetMHz = lp_lowTargetMHz();
            lp_applyState();
            lp_startTimer();
            NSLog(@"[LowPower] 系统 LPM 激活 → 低功耗限频启动");
        } else {
            lp_stopTimer();
            lp_applyState();
            NSLog(@"[LowPower] 系统 LPM 关闭 → 低功耗限频解除");
        }
    }
}
...
static void lp_onModeChanged(CFNotificationCenterRef center, void *observer,
    ...
    // 路径A: CPUthermalHelper/CommonProduct 模拟模式
    CPUthermalApplyMode();

    // 路径B: LowPower 状态机（hook 覆盖、电量模拟、保活定时器）
    lp_loadPrefs();
    g_lp_lastPollMode = g_lp_lowPowerActive ? @"lowPower" : @"off";
    if (lp_isLowPower()) {
        lp_applyState();
