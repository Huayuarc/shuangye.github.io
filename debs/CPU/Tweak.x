...
static const double kLowPowerKeepAliveInterval = 0.2;  // 每0.2s秒重应用一次，缩短周期防止频率漂移
static NSMutableSet *g_applePPMInstances = nil;          // 追踪 ApplePPMCPU 实例

// 满血模式保活定时器 — 每 1.5 秒重应用一次，防止系统温控恢复
static dispatch_source_t g_fullPowerKeepAliveTimer = NULL;
static const double kFullPowerKeepAliveInterval = 1.5;
...
return 3200;  // A15 大核最大 ~3230 MHz，用 3200 留余量
}
...
// 强制设置所有已知控制器
applyLowPowerLimitsToTrackedControllers();
// 修复：强制所有 ApplePPMCPU 实例 P-State 为最低档
...
dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kFullPowerKeepAliveInterval * NSEC_PER_SEC)),
(int64_t)(kFullPowerKeepAliveInterval * NSEC_PER_SEC),
(int64_t)(0.2 * NSEC_PER_SEC));  // 允许 0.2s 延迟容差
...
// 修复：追踪实例，确保 keep-alive 能强制重应用
- (id)init {
id res = %orig;
if (res) {
if (!g_applePPMInstances) g_applePPMInstances = [NSMutableSet set];
...
if (shouldApplyLowPowerLimit()) {
// setCPULevel 钩子已确保 level 被强制为 0
// 放行 updateCPU 让 clamped level 实际应用到硬件
%orig;
return;
}
%orig;
...
if (shouldApplyLowPowerLimit()) {
// setter 钩子已将所有功率值固定为低功耗限制值
// 放行 updateCPU 让 clamped 值实际应用到硬件
%orig;
return;
}
if (shouldApplyFullCPUProtection()) {
...
if (shouldApplyLowPowerLimit()) {
// setter 钩子已固定 PackageLowPowerTarget 等功率值
// 放行 updatePackage 让低功耗限制实际应用到硬件
%orig;
return;
}
if (shouldApplyFullCPUProtection()) {
...
modified[S("MaxOperatingFrequency")] = @3200;
// 清除所有功率限制参数
