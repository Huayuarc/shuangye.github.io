...
// CPUthermalTool 进程 PID（用于低功耗 IOKit 直接写入）
static pid_t g_lp_toolPID = -1;
static dispatch_source_t g_lp_toolWaitTimer = NULL;
...
static void lp_killCPUTool(void);
...
// ============================================================================
// CPUthermalTool — 独立 IOKit 频率上限工具管理
// 在 thermalmonitord 外部以 root 身份直接写入 IOKit 硬件属性，
// 绕过 sandbox 对 thermalmonitord 的 IOKit 访问限制
// ============================================================================
static NSString *lp_thermalToolPath(void) {
...
static void lp_killCPUTool(void) {
    if (g_lp_toolPID > 0) {
        kill(g_lp_toolPID, SIGTERM);
        // 等 50ms 让工具退出
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            int status;
            waitpid(g_lp_toolPID, &status, WNOHANG);
        });
        g_lp_toolPID = -1;
    }
    if (g_lp_toolWaitTimer) {
        dispatch_source_cancel(g_lp_toolWaitTimer);
        g_lp_toolWaitTimer = NULL;
    }
    NSLog(@"[LowPower] CPUthermalTool 已终止");
}

static void lp_spawnCPUTool(NSString *toolPath, int targetMHz) {
    // 先终止旧进程（如果有）
    lp_killCPUTool();

    // 构建参数: CPUthermalTool --low-power <MHz>
    char MHzStr[16];
    snprintf(MHzStr, sizeof(MHzStr), "%d", targetMHz);
    ...
    pid_t pid = 0;
    int ret = posix_spawn(&pid, [toolPath fileSystemRepresentation], NULL, &attr, (char *const *)argv, NULL);
    posix_spawnattr_destroy(&attr);

    if (ret == 0 && pid > 0) {
        g_lp_toolPID = pid;
        NSLog(@"[LowPower] CPUthermalTool 已生成 (PID=%d, target=%dMHz)", pid, targetMHz);
    } else {
        NSLog(@"[LowPower] CPUthermalTool 生成失败: ret=%d", ret);
        // 兜底：依然尝试直接 IOKit 写入
    }
}
...
static void lp_forceDirectFrequencyCap(void) {
    ...
    @autoreleasepool {
        // ================================================================
        // 路径 A: 通过 CPUthermalTool 独立进程写入（推荐）
        // 以 root 身份运行，绕过 thermalmonitord 的 IOKit 访问限制
        // ================================================================
        NSString *toolPath = lp_thermalToolPath();
        if (toolPath) {
            lp_spawnCPUTool(toolPath, (int)g_lp_lowTargetMHz);
        } else {
            NSLog(@"[LowPower] CPUthermalTool 未安装，回退到直接 IOKit 写入");
        }
        ...
        NSLog(@"[LowPower] 頻率上限已施加: %ldMHz (min: %lldMHz, tool: %@)",
...
static void lp_applyState(void) {
    if (lp_isLowPower()) {
        ...
        // 直接 IOKit 頻率上限 + 独立工具进程 — 绕过 thermalmonitord PID 循环
        lp_forceDirectFrequencyCap();
    } else {
        lp_killCPUTool();  // 终止独立工具进程
