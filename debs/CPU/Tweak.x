...
// ============================================================================
// 热压力屏蔽状态（移植自 Battman thermal.c）
// ============================================================================
static BOOL g_tp_pressureOverrideRunning = NO;  // 运行时: 定时器是否在运行
static dispatch_source_t g_tp_pressureTimer = NULL;
static const int64_t kTP_TimerIntervalMs = 500; // 500ms 保活（压力变化较慢）

// ============================================================================
// 热压力屏蔽前向声明
// ============================================================================
static void tp_applyPressureOverride(void);
static void tp_startPressureTimer(void);
static void tp_stopPressureTimer(void);
...
// ============================================================================
// 持续强制系统热压力为 Nominal，热通知级别为 Normal，
// 让所有系统组件都认为设备温度正常，不会触发任何温控降频。
// ============================================================================

// 立即强制压力 Nominal + 通知 Normal
static void tp_applyPressureOverride(void) {
    ...
    if (ret != 0 && ret != 2) {
        ...
        dispatch_once(&onceToken, ^{
            NSLog(@"[ThermalPressure] 强制 Nominal 失败: %d", ret);
        });
    }
}
...
// 启动热压力屏蔽保活定时器
static void tp_startPressureTimer(void) {
    if (g_tp_pressureTimer) return;

    g_tp_pressureTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue());
    dispatch_source_set_timer(g_tp_pressureTimer,
        dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kTP_TimerIntervalMs * NSEC_PER_MSEC)),
        (uint64_t)(kTP_TimerIntervalMs * NSEC_PER_MSEC),
        (uint64_t)(50 * NSEC_PER_MSEC));
    dispatch_source_set_event_handler(g_tp_pressureTimer, ^{
        @autoreleasepool {
            if (!g_tp_pressureOverrideRunning) {
                tp_stopPressureTimer();
                return;
            }
            // 持续覆盖热压力为 Nominal
            tp_applyPressureOverride();
        }
    });
    dispatch_resume(g_tp_pressureTimer);
    g_tp_pressureOverrideRunning = YES;
    NSLog(@"[ThermalPressure] 保活定时器已启动 (%.1fs)", (double)kTP_TimerIntervalMs / 1000.0);
}

static void tp_stopPressureTimer(void) {
    if (g_tp_pressureTimer) {
        dispatch_source_cancel(g_tp_pressureTimer);
        g_tp_pressureTimer = NULL;
        g_tp_pressureOverrideRunning = NO;
        NSLog(@"[ThermalPressure] 保活定时器已停止");
    }
}
...
static void lp_startPollTimer(void) {
    ...
    dispatch_source_set_event_handler(g_lp_pollTimer, ^{
        @autoreleasepool {
            ...
            // 无论何种 CPU 模式，始终强制热压力为 Nominal
            tp_applyPressureOverride();
            tp_startPressureTimer();
        }
    });
...
static void lp_onWakeEvent(CFNotificationCenterRef center, void *observer,
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // 无论何种模式，唤醒后均重新激活热压力屏蔽
        tp_applyPressureOverride();
        tp_startPressureTimer();
        NSLog(@"[ThermalPressure] 唤醒后重新激活压力屏蔽");
    });
}
...
%ctor {
    @autoreleasepool {
        ...
        // ── 第5b步：无论何种模式，始终启动热压力屏蔽 ──
        tp_applyPressureOverride();
        tp_startPressureTimer();
        NSLog(@"[ThermalPressure] 已激活 — 持续强制 Nominal 压力");
