...
// 锁屏时持续持有 Dark Wake 电源断言，避免深睡冻结 5 秒保活周期。
updateLowPowerSleepAssertion();
...
applyCurrentModeToApplePPMCPU();
// 最后直接重写已观察到的 CPU 频率属性，防止深睡恢复路径绕过 ObjC 控制器。
reapplyTrackedCPUFrequencyTargets();
});
...
NSLog(@"[CPUthermal] 低功耗保持定时器已启动 (当前每 %.1fs 重应用 + 强制Nominal压力)", currentLowPowerKeepAliveInterval());
}
...
if (shouldApplyLowPowerLimit()) activateLowPowerStateOnController(self);
