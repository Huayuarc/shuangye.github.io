...
static os_unfair_lock g_runtimeLock = OS_UNFAIR_LOCK_INIT;    // pulse 与 timer 生命周期
...
runtimeConfigSnapshot(NULL, NULL, NULL, NULL, &preventDimming);
return preventDimming;
}
...
sendTwoIntegerArguments(controller, @selector(setCPUPowerCeiling:fromDecisionSource:), kUnrestrictedPowerLimitMW, (uintptr_t)source);
}
}
...
sendTwoIntegerArguments(controller, @selector(setCPUPowerCeiling:forDVD1Contributor:), kUnrestrictedPowerLimitMW, (uintptr_t)contributor);
}
}
forceCPUPerformanceLevelOnController(controller);
}
...
static void runFullPowerRecoveryPulse(int remainingPulses) {
if (remainingPulses <= 0 || !shouldApplyFullCPUProtection()) {
os_unfair_lock_lock(&g_runtimeLock);
...
// ============================================================================
// 解除温控模式保活定时器 — 定期清除既有 mitigation 状态，不写固定频点
// ============================================================================
static void startFullPowerKeepAliveTimer(void) {
...
g_fullPowerKeepAliveTimer = timer;
...
static void stopFullPowerKeepAliveTimer(void) {
os_unfair_lock_lock(&g_runtimeLock);
dispatch_source_t timer = g_fullPowerKeepAliveTimer;
...
os_unfair_lock_unlock(&g_runtimeLock);
if (timer) NSLog(@"[CPUthermal] 满血保活定时器已停止");
}
...
BOOL preventDimming = [d[S("thermalPreventDimmingEnabled")] ?: @YES boolValue];
os_unfair_lock_lock(&g_stateLock);
g_enabled = YES;
