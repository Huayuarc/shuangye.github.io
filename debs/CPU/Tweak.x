...
// 【改进代码.x】满载 (解除温控) 模式下，每 10 毫秒做一次微小数学运算，
...
dispatch_source_set_timer(g_dvfsKeepAliveThread, DISPATCH_TIME_NOW, 0.01 * NSEC_PER_SEC, 0.005 * NSEC_PER_SEC);
...
NSLog(@"[CPUthermal] DVFS 防休眠 Keep-Alive 线程已启动");
}
}
...
if (shouldApplyFullCPUProtection()) {
// 【改进代码.x】将 CPU Floor (频率下限) 强行锁定为上限，彻底拉平 DVFS 的调频曲线
[self setCPUPowerFloor:fullPowerCeilingForController(self) fromDecisionSource:0];
return;
}
%orig;
}
...
if (shouldApplyFullCPUProtection()) {
// 【改进代码.x】满载模式：CPU Floor 强行锁定为上限 (Ceiling)，拉平 DVFS 调频曲线
%orig(fullPowerCeilingForController(self), source);
return;
}
%orig;
