...
static dispatch_source_t g_fullPowerKeepAliveTimer = NULL;  // 最高性能持久化 Keep-Alive 定时器 (完善.x 第 3 项)
...
// 完善.x: 在 restoreFullPowerToController 增加对 Floor 的硬性拉满
// 主动注入最高 P-State 守护 —— 直接返回绝对硬性值，彻底关停降频动作，防止 DVFS 空闲降频
...
// 2. 强行拉满 Power Floor / Ceiling / Zone Target / MaxCPUPowerTarget
if ([controller respondsToSelector:@selector(setCPULowPowerTarget:)]) {
((void (*)(id, SEL, int))objc_msgSend)(controller, @selector(setCPULowPowerTarget:), fullPowerPercentValue());
}
if ([controller respondsToSelector:@selector(setMaxCPUPowerTarget:useLegacyPath:setProperty:)]) {
sendSetMaxCPUPowerTarget(controller, 1000, NO);
}
if ([controller respondsToSelector:@selector(setCPUPowerCeiling:fromDecisionSource:)]) {
((void (*)(id, SEL, int, uintptr_t))objc_msgSend)(controller, @selector(setCPUPowerCeiling:fromDecisionSource:), 1000, 0);
}
if ([controller respondsToSelector:@selector(setCPUPowerFloor:fromDecisionSource:)]) {
((void (*)(id, SEL, int, uintptr_t))objc_msgSend)(controller, @selector(setCPUPowerFloor:fromDecisionSource:), 100, 0);
}
if ([controller respondsToSelector:@selector(setCPUPowerZoneTarget:)]) {
...
NSLog(@"[CPUthermal] 已强锁最高性能状态 (Floor:100 Ceiling:1000 Zone:100 Max:1000) controller:%@", controller);
} @catch (NSException *exception) {
NSLog(@"[CPUthermal] 强锁最高性能状态失败: %@", exception);
} @finally {
...
// --- 最高性能锁频持久化 Keep-Alive 定时器 (完善.x 第 3 项) ---
// 防止 iOS 内核在 CPU 空闲时主动降频 (DVFS 空闲降频)：
// 解除温控模式下每 1 秒硬性重新刷写一次最高性能指令到 MitigationController / CommonProduct，
// 持续下发 Power Floor:100，强制抵消 PowerD 的降频指令。
static void startFullPowerKeepAliveTimer(void) {
if (g_fullPowerKeepAliveTimer) return;
dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0);
g_fullPowerKeepAliveTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, DISPATCH_TIMER_STRICT, queue);
if (g_fullPowerKeepAliveTimer) {
dispatch_source_set_timer(g_fullPowerKeepAliveTimer, dispatch_time(DISPATCH_TIME_NOW, 0), 1.0 * NSEC_PER_SEC, 0.25 * NSEC_PER_SEC);
dispatch_source_set_event_handler(g_fullPowerKeepAliveTimer, ^{
if (g_enabled && g_cpuProtection && isFullPowerMode()) {
// 安全阀: 超过 100°C 时停止强锁，放行系统温控保护
...
NSLog(@"[CPUthermal] 最高性能持久化守护定时器已启动");
}
}
...
if (isLowPowerMode()) {
stopFullPowerKeepAliveTimer(); // 低功耗模式关闭最高性能守护
applyLowPowerToCommonProduct();
applyLowPowerLimitsToTrackedControllers();
...
startFullPowerKeepAliveTimer(); // 完善.x: 开启最高性能持久化守护，防止 DVFS 空闲降频
}
}
...
// 获取组件释放速率 — 完善.x: 彻底阻断释放速率节流，直接返回 0（取消"减半"软化）
- (float)getReleaseRateForComponent:(id)component {
if (shouldApplyFullCPUProtection()) {
if (!isTemperatureAboveSafetyCeiling()) {
NSLog(@"[CPUthermal] 完全阻断释放速率 (getReleaseRateForComponent)");
return 0.0f;
}
}
...
// 完善.x: 直接返回 0，表示不施加任何降频/控温力度（取消"减半"软化）
- (float)calculateControlEffort:(id)trigger trigger:(id)arg2 {
if (shouldApplyFullCPUProtection()) {
if (!isTemperatureAboveSafetyCeiling()) {
NSLog(@"[CPUthermal] 完全阻断控制力度 (calculateControlEffort)");
return 0.0f;
}
}
...
- (void)setCPUPowerFloor:(int)floor fromDecisionSource:(uintptr_t)source {
if (g_enabled && g_cpuProtection && isFullPowerMode()) {
// 将 Floor 强行拉到最大值 (100)，锁定频率下限，彻底防止 DVFS 空闲降频
%orig(100, source);
return;
}
%orig;
...
// 防暗屏补丁：热配置字典返回前应用（替代已移除的 NSDictionary 全局 Hook）
if (g_thermalPreventDimmingEnabled && [config isKindOfClass:[NSDictionary class]]) {
config = patchThermalPlist(config);
}
