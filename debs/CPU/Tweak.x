...
// HIP(口袋过热)模块：通过 SCPreferences 控制 thermalmonitord 原生 Hot-In-Pocket 保护
...
// 【完善.x 关键常量】低功耗百分比目标与功耗上限
// 关键修复：40% 功率对应 A15/A16 约 1.4GHz~2.0GHz 频段（传入百分比而非 MHz）
...
// ============================================================================
// HIP (Hot-In-Pocket) — 口袋过热保护 配置
//
// 移植自 Battman-1.0.3.3 (scprefs/wrapper.c + ThermalTunes 面板 TT_SECT_HIP)
//
// 原理说明：
//   thermalmonitord 原生支持「口袋过热 (Hot-In-Pocket)」保护：
//   当屏幕关闭且不播放任何媒体时，自动降低 CPU 与 GPU 活动，
//   避免设备在口袋中积热。Battman 通过 SCPreferences 写入
//   OSThermalStatus.plist 的以下键来启用/停用该原生功能：
//     - hipOverride                : HIP 强制开关 (BOOL)
//     - hipPersistentlyEnabled     : HIP 持久化标志 (BOOL)
//     - simulateHip                : 模拟 HIP (BOOL)，强制进入 HIP 状态用于测试
//
// 本模块在设置面板提供「单个」功能开关（口袋过热保护）：
//   开启 → hipOverride=YES + hipPersistentlyEnabled=YES（持久化跟随），并清除 simulateHip
//   关闭 → 全部写入 NO
// ============================================================================
static BOOL g_hipEnabled = NO;  // 口袋过热保护唯一功能开关（默认关闭）

static BOOL shouldApplyLowPowerLimit(void);
static BOOL hipActiveRequested(void);
...
// HIP(口袋过热)请求激活时让位：原生 HIP 需要自由降频，不得拦截
if (hipActiveRequested()) return NO;
return g_enabled && g_cpuProtection && isFullPowerMode() && !fullPowerBootGuardActive();
}

static BOOL shouldApplyLowPowerLimit(void) {
// HIP(口袋过热)请求激活时让位：不干预频率，交由原生 HIP 全权控制
if (hipActiveRequested()) return NO;
...
// HIP(口袋过热)请求激活时跳过功率模式主动下发：
// 满血解除温控会阻止原生 HIP 降频，低功耗主动下发又会覆盖原生 HIP 的频率控制，
// 因此 HIP 生效期间完全让位，交由 thermalmonitord 原生 Hot-In-Pocket 机制降频。
if (hipActiveRequested()) {
NSLog(@"[CPUthermal] HIP 已请求激活，跳过功率模式主动下发，交由原生 HIP 控制");
return;
}
if (isLowPowerMode()) {
applyLowPowerToCommonProduct();
...
// ============================================================================
// HIP (Hot-In-Pocket) — 口袋过热保护 控制核心
//
// 移植自 Battman-1.0.3.3 scprefs/wrapper.c (THERM_GETTER_SETTER 宏展开)
//
// thermalmonitord 原生监听的热状态 plist：
//   /Library/Preferences/OSThermalStatus.plist
// 关键键：
//   hipOverride             → HIP 强制开关（BOOL）
//   hipPersistentlyEnabled  → HIP 持久化标志（BOOL）
//   simulateHip             → 模拟 HIP（BOOL，强制进入 HIP 状态，用于测试）
// 写入后 SCPreferencesCommitChanges + SCPreferencesApplyChanges 会触发
// thermalmonitord 的 SCPreferences 监听回调，原生 HIP 立即生效（无需重启）。
// ============================================================================

// 打开 thermalmonitord 使用的热状态 plist（SCPreferences 句柄）
static SCPreferencesRef getHIPThermalPrefs(void) {
...
// 提交并应用 SCPreferences 修改（ret: kSCStatus）
static int saveHIPThermalPrefs(SCPreferencesRef prefs) {
...
int err = SCError();
NSLog(@"[CPUthermal] HIP: SCPreferencesCommitChanges 失败: %s", SCErrorString(err));
return err;
}

// 写入热状态 plist 键值（persistKey 非空时同时写入持久化标志）
static int setHIPThermPrefs(CFStringRef persistKey, CFStringRef key, CFTypeRef value, bool persist) {
SCPreferencesRef prefs = getHIPThermalPrefs();
...
return saveHIPThermalPrefs(prefs);
}

// 读取热状态 plist 键值
static bool getHIPThermPrefs(CFStringRef key, bool *value) {
SCPreferencesRef prefs = getHIPThermalPrefs();
if (!prefs) return false;
CFTypeRef ret = SCPreferencesGetValue(prefs, key);
*value = (ret == kCFBooleanTrue);
CFRelease(prefs);
return (ret != NULL);
}

// --- HIP 开关：hipOverride + hipPersistentlyEnabled ---
// (对应 Battman setHIPEnabled/getHIPEnabled)
static int setHIPEnabled(bool enable, bool persist) {
return setHIPThermPrefs((__bridge CFStringRef)S("hipPersistentlyEnabled"),
(__bridge CFStringRef)S("hipOverride"),
enable ? kCFBooleanTrue : kCFBooleanFalse, persist);
}

static bool getHIPEnabled(bool *enable, bool *persist) {
if (!getHIPThermPrefs((__bridge CFStringRef)S("hipOverride"), enable)) return false;
if (persist) {
bool p = false;
getHIPThermPrefs((__bridge CFStringRef)S("hipPersistentlyEnabled"), &p);
*persist = p;
}
return true;
}

// --- 模拟 HIP：simulateHip（用于测试，强制进入 HIP 状态）---
// (对应 Battman setSimulateHIPEnabled/getSimulateHIPEnabled)
static int setSimulateHIPEnabled(bool enable, bool persist) {
return setHIPThermPrefs(NULL, (__bridge CFStringRef)S("simulateHip"),
enable ? kCFBooleanTrue : kCFBooleanFalse, persist);
}

static bool getSimulateHIPEnabled(bool *enable, bool *persist) {
if (!getHIPThermPrefs((__bridge CFStringRef)S("simulateHip"), enable)) return false;
if (persist) *persist = false; // simulateHip 无持久化键，与 Battman 一致
return true;
}

// HIP 是否处于"请求激活"状态（唯一功能开关开启即需要系统原生 HIP 生效）
static BOOL hipActiveRequested(void) {
return g_hipEnabled;
}

// 将 HIP 配置写入系统（thermalmonitord 原生读取），带状态去重避免频繁写 plist
static void applyHIPStateToSystem(void) {
// 状态缓存：仅当唯一功能开关变化时才真正写入 SCPreferences
static BOOL lastEnabled = NO;
static BOOL initialized = NO;
if (initialized && lastEnabled == g_hipEnabled) {
return; // 状态未变化，跳过写入
}
initialized  = YES;
lastEnabled  = g_hipEnabled;

// 开启 → hipOverride=YES + hipPersistentlyEnabled=YES（持久化跟随），并清除 simulateHip
// 关闭 → 全部写入 NO
int hipRet = setHIPEnabled(g_hipEnabled, g_hipEnabled);
int simRet = setSimulateHIPEnabled(false, false);

// 写后核对：回读系统实际状态用于日志/调试（完整保留 Battman 的 getter 接口）
bool actualHIP = false, actualPersist = false, actualSim = false;
getHIPEnabled(&actualHIP, &actualPersist);
getSimulateHIPEnabled(&actualSim, NULL);
NSLog(@"[CPUthermal] HIP 状态应用: 目标(enabled=%d persist=%d simulate=%d) "
@"实际(enabled=%d persist=%d simulate=%d) (hipRet=%d simRet=%d)",
g_hipEnabled, g_hipEnabled, 0,
actualHIP, actualPersist, actualSim, hipRet, simRet);
}
...
// HIP 口袋过热保护偏好（由设置面板控制，唯一功能开关，默认关闭）
g_hipEnabled = [d[S("hipEnabled")] ?: [NSNumber numberWithBool:NO] boolValue];
...
// 将 HIP 配置同步到 thermalmonitord 原生热状态 plist
applyHIPStateToSystem();
}
}
...
// HIP(口袋过热)请求激活：放行原生 HIP 的 CPU/GPU 降频属性写入，
// 否则满血模式拦截会阻止 HIP 生效
if (hipActiveRequested()) {
return orig_IOServiceSetProperty(service, key, value);
}
...
static kern_return_t hooked_IOPMSetAggressiveness(io_connect_t fb, unsigned long type, unsigned long newLevel) {
    // HIP(口袋过热)请求激活：让位，允许原生 HIP 正常调整电源策略
    if (g_enabled && g_cpuProtection && isLowPowerMode() && !hipActiveRequested()) {
...
- (void)setCPULowPowerTarget:(int)target {
    // 完善.x: 低功耗模式强制覆盖为 40%，而非原始 MHz 频点
    if (isLowPowerMode()) {
        %orig(kLowPowerPercentTarget); // 强制覆盖为 40%
        return;
    }
    %orig;
}
...
- (void)setMaxCPUPowerTarget:(int)target useLegacyPath:(BOOL)legacy setProperty:(uintptr_t)property {
    // 完善.x: 低功耗模式强制覆盖为 40%；保留 property 归一化避免 CFString 崩溃
    uintptr_t propertyArg = normalizedSetMaxCPUPowerPropertyArgument(self, property);
    if (isLowPowerMode()) {
        %orig(kLowPowerPercentTarget, legacy, propertyArg);
        return;
    }
    %orig;
}
...
// HIP(口袋过热)请求激活：返回原始热配置，不提高触发阈值也不应用低功耗参数，
// 让 thermalmonitord 原生 HIP 基于出厂参数正常判断并降频
if (hipActiveRequested()) return config;
