...
__attribute__((unused)) static void rememberOriginalIntValue(id controller, const char *name, int value) {
...
NSLog(@"[CPUthermal] 低功耗热压力写入失败: %d (下次将重试)", result);
} else {
static BOOL g_lastThermalPressureApplied = NO;
if (!g_lastThermalPressureApplied) {
NSLog(@"[CPUthermal] 低功耗: 设为 Moderate (%d)", kCPUthermalPressureModerate);
