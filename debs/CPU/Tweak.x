...
static int64_t g_lowPowerMinMHz = 1380;
static int64_t g_lowPowerMaxMHz = 1428;

// 满血模式 CPU 最大频率（MHz）— 由偏好 fullPowerMaxMHz 配置，未配置时使用固定安全回退值
...
// 1. 优先使用用户在面板输入的自定义锁频值
if (g_cpuMinPowerValue > 0) {
return g_cpuMinPowerValue;
}
...
// 修正1：低功耗模式下放行系统限频限功率 selector，仅在【解除温控/满血模式】下拦截 Mitigation 指令
// 原逻辑：(shouldApplyFullCPUProtection() || shouldApplyLowPowerLimit()) 统统拦截
if (shouldApplyFullCPUProtection() && SELECTOR_IS_MITIGATION(selector)) {
NSLog(@"[CPUthermal] IOKit 拦截: selector=0x%x 解除温控", selector);
return KERN_SUCCESS;
}
return %orig;
}
...
if (shouldApplyFullCPUProtection() && SELECTOR_IS_MITIGATION(selector)) {
NSLog(@"[CPUthermal] IOKit 异步拦截: selector=0x%x 解除温控", selector);
return KERN_SUCCESS;
}
return %orig;
}
...
if (shouldApplyFullCPUProtection() && SELECTOR_IS_MITIGATION(selector)) {
NSLog(@"[CPUthermal] IOKit 结构体调用拦截: selector=0x%x 解除温控", selector);
return KERN_SUCCESS;
}
return %orig;
}
...
// 【修改】解除温控/锁频模式下，强行压在 0 档（最高频率档），拒绝系统升档（数字越大频率越低）
if (shouldApplyFullCPUProtection()) {
%orig(0);
return;
}
...
// 【修改】只有在明确处于低功耗模式且未指定强制锁频时才压低到 2500
if (shouldApplyLowPowerLimit()) {
rememberOriginalIntValue(self, "MaxCPUPowerTarget", target);
// 修正2：除了 setCPULevel:2，强行给一个低功率 Ceiling (例如 2500mW)
// 阻止系统在唤醒时将其拉高到 65000mW
%orig(MIN(target, 2500), legacy, propertyArg);
return;
}
// 【修改】在解除温控/锁定频率模式下，统一强制下发 65W 功率，绝不接受 2580/2400/2016 等低功率 Ceiling
if (shouldApplyFullCPUProtection()) {
...
// 修正2：强行把 Ceiling 压低，防止解锁时恢复高频
%orig(MIN(ceiling, 2500), source);
return;
}
// 【修改】彻底锁定 Ceiling 为 65W，拦截系统下发的 2580MHz(约3500mW)/2400MHz 降频指令
if (shouldApplyFullCPUProtection()) {
...
- (void)setCPUPowerFloor:(int)floor fromDecisionSource:(uintptr_t)source {
    ...
    if (shouldApplyFullCPUProtection()) {
        // 关键修复：满血模式下，强行将功率 Floor 顶到 65W (kUnrestrictedPowerLimitMW)
        // 只有把 Floor 顶住，系统才无法在空闲/温控时把 CPU 功率拉低到 2016MHz/2400MHz
        %orig(kUnrestrictedPowerLimitMW, source);
        return;
    }
    %orig;
...
// 补丁热配置字典：阻止系统因温控调暗屏幕 + 温控锁定CPU频率值（对齐 insulation）
...
if (targetFreq > 0) {
// 只有拿到有效主频时才写入
modified[S("MaxOperatingFrequency")] = [NSNumber numberWithInt:targetFreq];
} else {
// 没读出来时保留默认字典节点
