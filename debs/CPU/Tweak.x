...
// 解除温控模式下每 2 秒硬性重新刷写一次最高性能指令到 MitigationController / CommonProduct。
static void startFullPowerKeepAliveTimer(void) {
...
dispatch_source_set_timer(g_fullPowerKeepAliveTimer, dispatch_time(DISPATCH_TIME_NOW, 0), 2.0 * NSEC_PER_SEC, 0.5 * NSEC_PER_SEC);
...
if (shouldApplyFullCPUProtection()) {
// 完善.x: 将 Floor 强行拉到最大值 (100)，锁定下限，防止空闲时切回 Low P-State
%orig(100, source);
return;
}
%orig;
