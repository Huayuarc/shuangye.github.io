...
// ===== 温控等级调校（从 Battman Thermal Tunes 移植，默认始终启用） =====
...
// 温控等级控制始终启用，始终阻止 thermalmonitord 覆盖 notify state
// 我们自己的写入通过线程特定的 getOwnNotifyWrite() 标志放行
%hookf(uint32_t, notify_set_state, int token, uint64_t state) {
    // 温控等级控制始终启用: 始终阻止 thermalmonitord 覆盖热状态 notify state
    // 我们自己的写入通过线程特定的 getOwnNotifyWrite() 标志放行
    BOOL shouldBlock = g_enabled && !getOwnNotifyWrite();
    if (shouldBlock) {
        char name[256] = {0};
...
// 温控等级调校始终启用，由定时器统一管理 notify state
// init 阶段不强制设为 nominal，避免覆盖用户已设置的级别
applyCurrentPowerModeToRuntime();
...
NSLog(@"[CPUthermal] 温控防护已激活 — CPU性能:%d 亮度:%d 通知:%d 温控等级:始终启用",
g_cpuProtection, g_brightnessProtection,
g_suppressThermalNotifications);

// 温控等级调校始终启用 — 启动定时器并立即应用
applyThermalLevels();
startThermalLevelTimer();
