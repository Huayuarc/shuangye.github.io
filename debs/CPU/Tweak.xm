...
static void CPUthermalApplyMode(void) {
    ...
    if ([helper.thermalPowerMode isEqualToString:@"fullPower"]) {
        // 全性能：强制标称模式，阻止一切温控动作
        [helper.commonProductObject putDeviceInThermalSimulationMode:@"nominal"];
    } else if ([helper.thermalPowerMode isEqualToString:@"lowPower"]) {
...
- (void)tryTakeAction {
    if ([[CPUthermalHelper.shared thermalPowerMode] isEqualToString:@"fullPower"]) {
        // 全性能：阻止一切温控缓解动作
        return;
    }
    // 低电模式 / 关闭：允许原生温控动作
