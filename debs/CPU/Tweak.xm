...
static BOOL g_cpuProtection         = NO; // CPU温控拦截（默认关闭）
static BOOL g_blockNotifPopup       = NO; // 屏蔽高温温度计警告弹窗（默认关闭）
static BOOL g_preventDimming        = NO; // 防温控暗屏（默认关闭）
...
g_cpuProtection         = [d[S("cpuProtection")] ?: [NSNumber numberWithBool:NO] boolValue];
g_blockNotifPopup       = [d[S("thermalBlockNotifPopup")] ?: [NSNumber numberWithBool:NO] boolValue];
g_preventDimming        = [d[S("thermalPreventDimmingEnabled")] ?: [NSNumber numberWithBool:NO] boolValue];
...
// 阻断高温温度计警告弹窗（全屏"iPhone 需要冷却"）
- (void)updateThermalNotification:(id)notification {
if (g_blockNotifPopup) {
NSLog(@"[CPUthermal] 阻止高温警告弹窗: %@", notification);
return;
}
%orig;
}
...
// ============================================================================
// NSDictionary hook — 拦截 ThermalMonitor 配置 plist，用于防温控暗屏
// ============================================================================
%hook NSDictionary
