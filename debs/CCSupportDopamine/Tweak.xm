...
{
	%orig;
	CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("com.opa334.ccsupport/ReloadProviders"), NULL, NULL, YES);
}
...
void reloadModuleSizes(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo)
{
	...
	id collection = [vcClass respondsToSelector:@selector(_sharedCollectionViewController)] ? [vcClass _sharedCollectionViewController] : nil;
	if ([collection respondsToSelector:@selector(_refreshPositionProviders)]) {
		[collection _refreshPositionProviders];
	}
}

static void CCSRefreshRepositoryAndLayout(void)
{
	CCSModuleProviderManager *providerManager = [CCSModuleProviderManager sharedInstance];
	[providerManager reload];
	id manager = [%c(CCUIModuleInstanceManager) sharedInstance];
	CCSModuleRepository *repository = nil;
	@try { repository = [manager valueForKey:@"_repository"]; } @catch (__unused NSException *e) {}
	void (^finish)(void) = ^{
		id settings = [%c(CCSModuleSettingsProvider) sharedProvider];
		if ([settings respondsToSelector:@selector(_queue_loadSettings)]) {
			@try { [settings _queue_loadSettings]; } @catch (__unused NSException *e) {}
		}
		dispatch_async(dispatch_get_main_queue(), ^{ reloadModuleSizes(NULL,NULL,NULL,NULL,NULL); });
	};
	if ([repository respondsToSelector:@selector(_updateAllModuleMetadata)]) {
		[repository _updateAllModuleMetadata]; finish();
	} else if ([repository respondsToSelector:@selector(_queue_updateAllModuleMetadata)]) {
		NSObject<OS_dispatch_queue> *queue = nil;
		@try { queue = [repository valueForKey:@"_queue"]; } @catch (__unused NSException *e) {}
		if (queue) dispatch_async(queue, ^{ [repository _queue_updateAllModuleMetadata]; dispatch_async(dispatch_get_main_queue(),finish); });
		else { [repository _queue_updateAllModuleMetadata]; finish(); }
	} else finish();
}
