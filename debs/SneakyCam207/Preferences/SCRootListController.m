...
- (void)openDownloads {
    // Filza 的 view 语义始终是“进入父目录并高亮目标”。创建隐藏锚点文件，
    // 再定位该文件，可确保 Filza 进入 /var/mobile/Downloads 而不是停在 /var/mobile。
    NSString *directory=@"/var/mobile/Downloads";
    NSString *anchor=[directory stringByAppendingPathComponent:@".SneakyCam-OpenHere"];
    NSFileManager *fm=[NSFileManager defaultManager];
    [fm createDirectoryAtPath:directory withIntermediateDirectories:YES attributes:nil error:nil];
    if(![fm fileExistsAtPath:anchor]) [@"SneakyCam folder anchor" writeToFile:anchor atomically:YES encoding:NSUTF8StringEncoding error:nil];
    NSURL *primary=[NSURL URLWithString:@"filza://view/var/mobile/Downloads/.SneakyCam-OpenHere"];
    NSURL *fallback=[NSURL URLWithString:@"filza:///var/mobile/Downloads/"];
