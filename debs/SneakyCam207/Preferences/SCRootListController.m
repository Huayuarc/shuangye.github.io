...
static void SCPreferencesStateChanged(CFNotificationCenterRef c,void *o,CFStringRef n,const void *obj,CFDictionaryRef u){ SCRootListController *vc=(__bridge id)o; dispatch_async(dispatch_get_main_queue(),^{ [vc reloadSpecifierID:@"LastActionDate"];[vc reloadSpecifierID:@"LastActionStatus"]; }); }
...
- (void)openDownloads {
    NSURL *filza=[NSURL URLWithString:@"filza://view/var/mobile/Downloads"];
    UIApplication *app=[UIApplication sharedApplication];
    if([app canOpenURL:filza]) {
        [app openURL:filza options:@{} completionHandler:^(BOOL ok){
            SCWritePreference(@"LastActionStatus", ok?@"已打开 /var/mobile/Downloads":@"Filza 打开失败");
