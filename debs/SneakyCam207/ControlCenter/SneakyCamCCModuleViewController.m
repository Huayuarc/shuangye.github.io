...
- (void)toggle:(id)sender { BOOL enabled=![self isEnabled];SCWritePreference(@"Enabled",@(enabled));notify_post("com.spark.SneakyCam.enabledchanged");notify_post("com.spark.SneakyCam");[self refreshState];UIImpactFeedbackGenerator *g=[[UIImpactFeedbackGenerator alloc] initWithStyle:UIImpactFeedbackStyleMedium];[g prepare];[g impactOccurred]; }
- (void)refreshState {
