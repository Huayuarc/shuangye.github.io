...
- (void)tapped:(id)sender {
    ...
    if([p[@"HapticFeedback"]?:@YES boolValue]) { UIImpactFeedbackGenerator *g=[[UIImpactFeedbackGenerator alloc]initWithStyle:UIImpactFeedbackStyleMedium];[g prepare];[g impactOccurred]; }
    [self refreshState];
}
- (void)refreshState {
    BOOL enabled=[self enabled],recording=[SCReadPreferences()[@"Recording"] boolValue]; NSString *symbol,*title,*state;
    if(_mode==SneakyCamCCModeToggle){symbol=@"camera.fill";title=@"隐私相机";state=enabled?@"已开启":@"已关闭";}
    else if(_mode==SneakyCamCCModePhoto){symbol=@"camera.fill";title=@"隐私拍照";state=enabled?@"点击拍照":@"总开关关闭";}
    else {symbol=recording?@"video.fill":@"video";title=@"隐私录像";state=!enabled?@"总开关关闭":(recording?@"录制中":@"已停止");}
