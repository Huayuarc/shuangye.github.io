...
- (void)reportStatus:(NSString *)status hapticStyle:(UIImpactFeedbackStyle)style repeats:(NSInteger)repeats {
    ...
    if(repeats>0 && [self boolPref:@"HapticFeedback" def:YES]) dispatch_async(dispatch_get_main_queue(), ^{
        UIImpactFeedbackGenerator *g=[[UIImpactFeedbackGenerator alloc]initWithStyle:style]; [g prepare];
        for(NSInteger i=0;i<repeats;i++) { [g impactOccurred]; if(i+1<repeats) usleep(80000); }
    });
}
...
- (void)captureOutput:(AVCaptureFileOutput *)o didStartRecordingToOutputFileAtURL:(NSURL *)u fromConnections:(NSArray *)c { [self setRecordingState:YES]; [self reportStatus:@"录像已开始" hapticStyle:UIImpactFeedbackStyleHeavy repeats:1]; }
- (void)captureOutput:(AVCaptureFileOutput *)o didFinishRecordingToOutputFileAtURL:(NSURL *)u fromConnections:(NSArray *)c error:(NSError *)e {
    ...
    if(!e){
        ...
        if(saved) [self reportStatus:[NSString stringWithFormat:@"录像已停止：%@",saved] hapticStyle:UIImpactFeedbackStyleLight repeats:2];
