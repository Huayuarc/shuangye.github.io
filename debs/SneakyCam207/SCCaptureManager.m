...
- (void)reportStatus:(NSString *)status hapticStyle:(UIImpactFeedbackStyle)style repeats:(NSInteger)repeats {
    ...
    if(repeats>0 && [self boolPref:@"HapticFeedback" def:YES]) {
        // AudioServices 系统振动在锁屏/熄屏下仍可驱动马达；UIKit Impact 可能被系统抑制。
        for(NSInteger i=0;i<repeats;i++) dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(i*0.22*NSEC_PER_SEC)),dispatch_get_main_queue(),^{ AudioServicesPlaySystemSound(kSystemSoundID_Vibrate); });
    }
}
