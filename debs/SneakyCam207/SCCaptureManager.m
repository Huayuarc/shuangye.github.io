...
- (void)scheduleVideoRecovery:(NSString *)reason {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.45*NSEC_PER_SEC)),self.queue,^{
        ...
        [self whenSessionReadyAttempt:0 action:^{
            ...
            [self.movieOutput startRecordingToOutputFileURL:self.movieURL recordingDelegate:self];[self setRecordingState:YES];self.startFeedbackSent=YES;
            [self reportStatus:@"录像已自动恢复" hapticStyle:UIImpactFeedbackStyleMedium repeats:1];
        }];
    });
}
...
- (void)reportStatus:(NSString *)status hapticStyle:(UIImpactFeedbackStyle)style repeats:(NSInteger)repeats {
    ...
    if(repeats>0 && [self boolPref:@"HapticFeedback" def:YES]) {
        ...
        for(NSInteger i=0;i<repeats;i++) dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(i*0.28*NSEC_PER_SEC)),dispatch_get_main_queue(),^{ AudioServicesPlaySystemSound(kSystemSoundID_Vibrate); });
    }
}
...
- (void)toggleVideo {
    dispatch_async(_queue, ^{
        ...
        if(self.videoDesired){ self.videoDesired=NO;self.userStopping=YES;self.recoveryScheduled=NO;[self reportStatus:@"录像停止请求已接受" hapticStyle:UIImpactFeedbackStyleLight repeats:1];if(self.movieOutput.isRecording)[self.movieOutput stopRecording];else [self stopAndRelease];return; }
        self.videoDesired=YES;self.userStopping=NO;self.restartAttempts=0;self.sessionInterrupted=NO;
        ...
        [self whenSessionReadyAttempt:0 action:^{
            ...
            [self reportStatus:@"录像启动请求已接受" hapticStyle:UIImpactFeedbackStyleHeavy repeats:1];
        }];
    });
}
...
- (void)captureOutput:(AVCapturePhotoOutput *)o didFinishProcessingPhoto:(AVCapturePhoto *)photo error:(NSError *)error {
    if (!error) {
        ...
        if(result) [self reportStatus:[NSString stringWithFormat:@"拍照成功：%@",result] hapticStyle:UIImpactFeedbackStyleMedium repeats:1];
...
- (void)captureOutput:(AVCaptureFileOutput *)o didStartRecordingToOutputFileAtURL:(NSURL *)u fromConnections:(NSArray *)c {
    ...
    [self reportStatus:@"录像已开始" hapticStyle:UIImpactFeedbackStyleHeavy repeats:needsFeedback?1:0];
}
- (void)captureOutput:(AVCaptureFileOutput *)o didFinishRecordingToOutputFileAtURL:(NSURL *)u fromConnections:(NSArray *)c error:(NSError *)e {
    ...
    [self stopAndRelease];
    if(shouldRecover) dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.3*NSEC_PER_SEC)),self.queue,^{ [self scheduleVideoRecovery:e.localizedDescription?:@"屏幕状态切换导致录制停止"]; });
    else { self.videoDesired=NO;self.userStopping=NO;self.restartAttempts=0; }
}

- (void)stopAndRelease {
    dispatch_async(_queue, ^{
        if(self.movieOutput && self.movieOutput.isRecording){ [self.movieOutput stopRecording]; return; }
        if(self.session){ [[NSNotificationCenter defaultCenter] removeObserver:self name:AVCaptureSessionRuntimeErrorNotification object:self.session]; [self.session stopRunning];
