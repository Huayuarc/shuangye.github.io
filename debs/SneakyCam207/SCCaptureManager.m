...
- (BOOL)prepare:(BOOL)video {
    ...
    if(![session canAddInput:in]) { self.lastPrepareError=@"系统拒绝添加摄像头输入"; return NO; }
    ...
    if(video) {
        ...
        if(![session canAddOutput:movie]) { self.lastPrepareError=@"设备不支持当前录像输出"; return NO; }
        [session addOutput:movie]; _movieOutput=movie; _photoOutput=nil;
        NSInteger fps=[self.prefs[@"FrameRate"] integerValue]; if(fps<=0)fps=60; [self configureHardware:dev fps:fps];
        ...
        if(![session canAddOutput:photo]) { self.lastPrepareError=@"设备不支持当前拍照输出"; return NO; }
        [session addOutput:photo]; _photoOutput=photo; _movieOutput=nil;
        [self configureHardware:dev fps:30];
    }
    _session=session; [session startRunning];
    if(!session.isRunning) { self.lastPrepareError=@"相机会话启动失败"; _session=nil; return NO; }
    return YES;
}
...
- (void)toggleVideo {
    dispatch_async(_queue, ^{
        ...
        if(self.movieOutput && self.movieOutput.isRecording){ [self reportStatus:@"录像停止请求已接受" hapticStyle:UIImpactFeedbackStyleLight repeats:1]; [self.movieOutput stopRecording]; return; }
        if(![self prepare:YES]) { [self reportStatus:self.lastPrepareError?:@"相机启动失败" hapticStyle:UIImpactFeedbackStyleLight repeats:0]; return; }
