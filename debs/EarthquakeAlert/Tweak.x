...
// ============================================================
- (void)playAlertSound {
    BOOL soundEnabled = [EQPrefs boolForKey:@"soundEnabled" defaultValue:YES];
    BOOL vibrateEnabled = [EQPrefs boolForKey:@"vibrateEnabled" defaultValue:YES];
    if (!soundEnabled && !vibrateEnabled) return;

    if (soundEnabled && _soundID == 0) {
        NSArray *paths = @[
    ...
    if (soundEnabled) {
        SystemSoundID sound = _soundID ?: 1005;
    ...
    if (vibrateEnabled) AudioServicesPlaySystemSound(kSystemSoundID_Vibrate);
}
...
- (void)handleEarthquakeAlert:(NSDictionary *)eventData {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        vc.dismissHandler = ^{
            EQAlertManager *strongSelf = weakSelf;
            if (strongSelf) strongSelf->_alertShowing = NO;
        };
