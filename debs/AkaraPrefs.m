...
static NSString * const AKRRootlessSupportPath = @"/var/jb/Library/Application Support/Akara";
static NSString * const AKRRootlessMobilePrefix = @"/var/jb/var/mobile";
static NSString * const AKRRootfulSupportPath = @"/Library/Application Support/Akara";
static NSString * const AKRRootfulMobilePrefix = @"/var/mobile";

static BOOL AKRIsRootless(void) {
    return [[NSFileManager defaultManager] fileExistsAtPath:@"/var/jb"];
}

static NSString *AKRSupportPath(void) {
    return AKRIsRootless() ? AKRRootlessSupportPath : AKRRootfulSupportPath;
}

static NSString *AKRMobilePath(NSString *subpath) {
    NSString *prefix = AKRIsRootless() ? AKRRootlessMobilePrefix : AKRRootfulMobilePrefix;
    return [prefix stringByAppendingPathComponent:subpath];
}

static NSString *AKRPrefsPath(void) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<UIImagePickerControllerInfoKey,id> *)info {
    ...
    if (image) {
        ...
        NSMutableDictionary *preferences = [NSMutableDictionary dictionaryWithContentsOfFile:AKRPrefsPath()] ?: [NSMutableDictionary dictionary];
        preferences[@"akaraBackgroundImage"] = path;
        [preferences writeToFile:AKRPrefsPath() atomically:YES];
        AKRPostPrefsChanged();
    }
...
- (void)resetPreferencesToDefault {
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
    [alert addAction:[UIAlertAction actionWithTitle:@"重置" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *action) {
        [[NSFileManager defaultManager] removeItemAtPath:AKRPrefsPath() error:nil];
        AKRPostPrefsChanged();
...
- (void)copyDefaultLayoutFiles {
    ...
    NSString *supportPath = AKRSupportPath();
...
- (void)setAvatarImage:(UIImage *)avatarImage {
    if (_avatarImage != avatarImage) {
        [_avatarImage release];
        _avatarImage = [avatarImage retain];
...
- (void)setupWithSpecifier:(PSSpecifier *)specifier {
    ...
    self.iconView = [[[UIImageView alloc] initWithImage:AKRBundleImage([specifier propertyForKey:@"iconPath"] ? [[specifier propertyForKey:@"iconPath"] lastPathComponent] : @"akara.png")] autorelease];
    ...
    self.subtitleLabel = [[[UILabel alloc] initWithFrame:CGRectZero] autorelease];
    ...
    self.containerStackView = [[[UIStackView alloc] initWithArrangedSubviews:@[self.iconView, self.titleLabel, self.subtitleLabel]] autorelease];
...
+ (instancetype)cellWithSpecifier:(PSSpecifier *)specifier {
    return [[[self alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"AKRDuoTwitterCell" specifier:specifier] autorelease];
}

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier specifier:(PSSpecifier *)specifier {
    self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier specifier:specifier];
    if (self) {
        _user = [[specifier propertyForKey:@"firstAccount"] retain] ?: [@"FectaTr1" retain];
        _user2 = [[specifier propertyForKey:@"secondAccount"] retain] ?: [@"_Kennyroo" retain];
        self.selectionStyle = UITableViewCellSelectionStyleNone;
        ...
        [_leftImageView addGestureRecognizer:[[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleLeftTap:)] autorelease]];
        [_rightImageView addGestureRecognizer:[[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleRightTap:)] autorelease]];
