...
static const void *PGHUDWindowAssociationKey = &PGHUDWindowAssociationKey;
static __weak id PGActiveOverlayGrabber;
static __weak id PGActiveHUDGrabber;

@interface PGPassThroughWindow : UIWindow
@end

@implementation PGPassThroughWindow
...
static void PGHideHUD(id grabber) {
    ...
    PGPassThroughWindow *hudWindow = objc_getAssociatedObject(grabber, PGHUDWindowAssociationKey);
    hudWindow.hidden = YES;
    hudWindow.rootViewController = nil;
    objc_setAssociatedObject(grabber, PGHUDWindowAssociationKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
static void PGShowHUD(id grabber) {
    ...
    if (!hud) {
        ...
        hudWindow = scene ? [[PGPassThroughWindow alloc] initWithWindowScene:scene]
                          : [[PGPassThroughWindow alloc] initWithFrame:window.bounds];
        hudWindow.frame = window.bounds;
        hudWindow.backgroundColor = UIColor.clearColor;
        hudWindow.windowLevel = UIWindowLevelAlert + 120.0;
        UIViewController *controller = [UIViewController new];
        controller.view.backgroundColor = UIColor.clearColor;
        hudWindow.rootViewController = controller;
        hud = [[PGGrabberHUDView alloc] initWithFrame:controller.view.bounds];
        hud.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        hud.userInteractionEnabled = YES;
        [controller.view addSubview:hud];
        hudWindow.hidden = NO;
        objc_setAssociatedObject(grabber, PGHUDWindowAssociationKey, hudWindow, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        objc_setAssociatedObject(grabber, PGHUDAssociationKey, hud, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    hudWindow.frame = window.bounds;
    hudWindow.hidden = NO;
    hud.frame = hudWindow.rootViewController.view.bounds;
    [hud startUpdating];
    NSUInteger generation = [objc_getAssociatedObject(grabber, PGHUDGenerationAssociationKey) unsignedIntegerValue] + 1;
    objc_setAssociatedObject(grabber, PGHUDGenerationAssociationKey, @(generation), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    // The card outlives the grabber gesture. Only this presentation's timer may close it.
    NSTimeInterval autoCloseDelay = MAX(1.0, MIN(10.0, PerfectGrabberPreferences.sharedPreferences.autoCloseDelay));
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(autoCloseDelay * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        NSUInteger current = [objc_getAssociatedObject(grabber, PGHUDGenerationAssociationKey) unsignedIntegerValue];
        if (current == generation && objc_getAssociatedObject(grabber, PGHUDAssociationKey) == hud) {
            PGHideHUD(grabber);
        }
    });
}
...
- (void)_willDismiss {
    ...
    %orig;
    if (!swipeUp && hasMediaCard) {
        // Gesture dismissal must not destroy the detached interactive card.
        // Re-hide native chrome after SpringBoard restores its presentation state.
