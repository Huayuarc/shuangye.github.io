...
- (void)mediaInteractionDidOccur {
    if (self.interactionHandler) self.interactionHandler();
}
- (void)previousTrack { [self mediaInteractionDidOccur]; PGSendMediaAction(PGMediaActionPrevious); [self refreshMediaSoon]; }
- (void)togglePlayback { [self mediaInteractionDidOccur]; PGSendMediaAction(PGMediaActionToggle); [self refreshMediaSoon]; }
- (void)nextTrack { [self mediaInteractionDidOccur]; PGSendMediaAction(PGMediaActionNext); [self refreshMediaSoon]; }
...
- (void)updateContents {
    ...
    UIFontWeight titleWeight = preferences.fontStyle == 1 ? UIFontWeightBold : UIFontWeightSemibold;
    UIFontWeight bodyWeight = preferences.fontStyle == 1 ? UIFontWeightSemibold : UIFontWeightRegular;
    UIFont *titleFont = [UIFont systemFontOfSize:16 weight:titleWeight];
    UIFont *artistFont = [UIFont systemFontOfSize:13 weight:bodyWeight];
    CGFloat statusSize = preferences.statusFontSize;
    UIFont *statusFont = [UIFont monospacedDigitSystemFontOfSize:statusSize weight:bodyWeight];
    if (preferences.fontStyle == 2) {
        UIFontDescriptorSymbolicTraits italic = UIFontDescriptorTraitItalic;
        UIFontDescriptor *titleDescriptor = [titleFont.fontDescriptor fontDescriptorWithSymbolicTraits:italic];
        UIFontDescriptor *artistDescriptor = [artistFont.fontDescriptor fontDescriptorWithSymbolicTraits:italic];
        UIFontDescriptor *statusDescriptor = [statusFont.fontDescriptor fontDescriptorWithSymbolicTraits:italic];
        if (titleDescriptor) titleFont = [UIFont fontWithDescriptor:titleDescriptor size:16];
        if (artistDescriptor) artistFont = [UIFont fontWithDescriptor:artistDescriptor size:13];
        if (statusDescriptor) statusFont = [UIFont fontWithDescriptor:statusDescriptor size:statusSize];
    }
    self.titleLabel.font = titleFont;
    self.artistLabel.font = artistFont;
    self.valueLabel.font = statusFont;
    self.titleLabel.textColor = tint;
    self.artistLabel.textColor = [tint colorWithAlphaComponent:0.68];
...
- (void)layoutSubviews {
    ...
    if (window) {
        ...
        CGFloat angle = -atan2(axis.y-origin.y, axis.x-origin.x);
        self.panelView.transform = CGAffineTransformMakeRotation(angle);
        if (PerfectGrabberPreferences.sharedPreferences.grabberStyle == 1) {
            CGRect screenBounds = space.bounds;
            CGFloat visualHeight = fabs(sin(angle)) * 340.0 + fabs(cos(angle)) * 150.0;
            CGPoint screenCenter = CGPointMake(CGRectGetMidX(screenBounds),
                                               CGRectGetMinY(screenBounds) + 20.0 + visualHeight / 2.0);
            CGPoint windowCenter = [window convertPoint:screenCenter fromCoordinateSpace:space];
            self.panelView.center = [self convertPoint:windowCenter fromView:window];
        }
    }
    self.blurView.frame = self.panelView.bounds;
    PerfectGrabberPreferences *preferences = PerfectGrabberPreferences.sharedPreferences;
    if (preferences.statusPosition == 1) {
        self.valueLabel.frame = CGRectMake(14, 4, 312, 27);
        self.artworkView.frame = CGRectMake(14, 38, 76, 76);
        self.titleLabel.frame = CGRectMake(104, 37, 220, 24);
        self.artistLabel.frame = CGRectMake(104, 62, 220, 20);
        self.controlsView.frame = CGRectMake(100, 83, 228, 38);
    } else {
