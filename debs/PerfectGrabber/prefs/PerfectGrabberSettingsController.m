...
- (NSArray *)specifiers {
    ...
    NSMutableArray *items = loaded ? [loaded mutableCopy] : [NSMutableArray array];
    ...
    if (items.count == 0) {
        ...
        [group setProperty:@"Root.plist 加载失败，请重新安装插件并重启设置。" forKey:PSFooterTextGroupKey];
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier *)specifier {
    ...
    } else if ([key isEqualToString:@"StatusFontSize"]) {
        value = @(MAX(11.0, MIN(20.0, [value doubleValue])));
    }
