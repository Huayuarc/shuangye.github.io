...
@property(nonatomic,assign) BOOL retryingToggle;
@end
...
- (void)viewDidLoad {
    ...
    // 部分 Provider 不会向 SpringBoard 广播状态变更，低频轮询保证 App 内切换也能同步 CC。
    _statusTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(pollVPNStatus:) userInfo:nil repeats:YES];
    [self reloadManagers];
}
- (void)reloadManagers {
    [self loadManagersThen:nil];
}
- (void)loadManagersThen:(void (^)(void))completion {
    __weak typeof(self) weakSelf = self;
    [NEVPNManager loadAllFromPreferencesWithCompletionHandler:^(NSArray<NEVPNManager *> *items, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            weakSelf.managers = error ? @[] : items;
            [weakSelf refreshState]; [weakSelf rebuildNodes];
...
- (void)startManager:(NEVPNManager *)manager allowRetry:(BOOL)allowRetry {
    NSError *error=nil;
    ...
    if (error && allowRetry) {
        ...
        [manager loadFromPreferencesWithCompletionHandler:^(__unused NSError *loadError) {
            dispatch_async(dispatch_get_main_queue(), ^{
                // Provider 冷启动时先刷新其偏好，再自动发起一次连接，不依赖进入 App。
                [weakSelf loadManagersThen:^{
                    NEVPNManager *fresh = weakSelf.preferredManager;
                    if (fresh) [weakSelf startManager:fresh allowRetry:NO];
                }];
            });
        }];
    }
...
- (void)toggleVPN {
    ...
    if (!m) {
        // 控制中心首次打开时 managers 尚未返回：载入成功后在同一次点击中继续连接。
        __weak typeof(self) weakSelf = self;
        [self loadManagersThen:^{
            if (weakSelf.preferredManager) [weakSelf toggleVPN];
        }];
        return;
    }
    if (self.activeManager) [m.connection stopVPNTunnel];
    else [self startManager:m allowRetry:YES];
    [self refreshState];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(.6*NSEC_PER_SEC)),dispatch_get_main_queue(),^{[self reloadManagers];});
}
- (void)buttonTapped:(id)sender forEvent:(id)event { [self toggleVPN]; }
- (void)statusChanged:(NSNotification *)note { dispatch_async(dispatch_get_main_queue(), ^{ [self reloadManagers]; }); }
- (void)pollVPNStatus:(NSTimer *)timer { if (!self.expandedMode) [self reloadManagers]; }
- (void)refreshState {
...
- (void)dealloc { [_statusTimer invalidate]; [[NSNotificationCenter defaultCenter] removeObserver:self]; }
@end
