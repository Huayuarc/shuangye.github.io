...
static id AKRCreateSystemModule(NSString *identifier) {
    ...
    @try {
        ...
        if ([principalClass instancesRespondToSelector:@selector(initWithModuleIdentifier:options:)]) {
            ...
            return [(id<AKRModuleIdentifierInitializable>)allocatedModule initWithModuleIdentifier:identifier options:nil];
        }
...
- (instancetype)initWithModuleName:(NSString *)moduleName {
    ...
    if (self) {
        ...
        _module = AKRCreateSystemModule(moduleName);
    }
    return self;
}
...
- (void)setupModuleView {
    ...
    if (!contentViewController) {
        contentViewController = [[AKRProviderFallbackContentViewController alloc] initWithTitle:[self.moduleIdentifier componentsSeparatedByString:@"."].lastObject];
    }
...
- (void)setupModules {
    self.firstModuleViewController = [[AkaraCCModuleViewController alloc] initWithModuleName:self.firstModuleName];
    self.secondModuleViewController = [[AkaraCCModuleViewController alloc] initWithModuleName:self.secondModuleName];
...
@property (nonatomic, strong) AKR_CONTENT_CLASS *contentViewController;
...
- (id)moduleInstanceForModuleIdentifier:(NSString *)identifier {
    ...
    if (!module) {
        module = [[AKR_PROVIDED_MODULE_CLASS alloc] initWithModuleIdentifier:identifier contentModule:nil presentationContext:nil];
