...
static NSString *ASPPreferencesPath(void) {
    ...
    for (NSString *name in items) {
        ...
        BOOL marker = [fm fileExistsAtPath:[root stringByAppendingPathComponent:@"Library/MobileSubstrate"]] ||
                      [fm fileExistsAtPath:[root stringByAppendingPathComponent:@"usr/lib"]];
        if (!marker) continue;
        NSDate *date = [[fm attributesOfItemAtPath:root error:nil] fileModificationDate] ?: NSDate.distantPast;
        if (!best || [date compare:bestDate] == NSOrderedDescending) { best = root; bestDate = date; }
    }
