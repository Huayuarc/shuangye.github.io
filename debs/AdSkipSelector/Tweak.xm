...
static BOOL ASPSyntheticTouch(UIWindow *window, UIView *hit, CGPoint p) {
    ...
    @try {
        ...
        ((void(*)(id,SEL,CGPoint,BOOL))objc_msgSend)(touch, NSSelectorFromString(@"_setLocationInWindow:resetPrevious:"), p, YES);
        ((void(*)(id,SEL,NSUInteger))objc_msgSend)(touch, NSSelectorFromString(@"setTapCount:"), 1);
        ((void(*)(id,SEL,NSTimeInterval))objc_msgSend)(touch, NSSelectorFromString(@"setTimestamp:"), NSProcessInfo.processInfo.systemUptime);
        ...
        [hit touchesBegan:[NSSet setWithObject:touch] withEvent:event];
        ((void(*)(id,SEL,NSInteger))objc_msgSend)(touch, NSSelectorFromString(@"setPhase:"), UITouchPhaseEnded);
        [hit touchesEnded:[NSSet setWithObject:touch] withEvent:event];
        return YES;
...
static BOOL ASPClickPoint(UIWindow *window, CGPoint p) {
    ...
    if (!hit) return NO;
    if (ASPActivate(hit)) return YES;
    return ASPSyntheticTouch(window, hit, p);
}
