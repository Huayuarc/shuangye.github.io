...
static NSMutableArray *ASP_specifiers(id self, SEL _cmd) {
    @try {
        ...
        // 不按 applicationType 过滤系统应用——避免某些进程返回空串/非 "User" 导致整列空白；
        // 系统应用用户可自行关闭，且列表里通常也少量。
        NSArray *apps = [ASPAllUserApplications() filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(ASPAppInfo *app, NSDictionary *_) { return app.bundleID.length > 0 && ![app.bundleID hasPrefix:@"com.apple."]; }]];
