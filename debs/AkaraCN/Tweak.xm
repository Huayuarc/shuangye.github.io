...
static void AKRInstallTriggers(void) {
    if (!PBool(@"enabled", YES)) return;
    if (!AKRTriggers) AKRTriggers=[NSMutableArray array];
    for(UIScene *s in UIApplication.sharedApplication.connectedScenes){
        if(![s isKindOfClass:UIWindowScene.class] || s.activationState==UISceneActivationStateUnattached) continue;
        UIWindowScene *scene=(UIWindowScene*)s; BOOL exists=NO;
        for(UIWindow *old in AKRTriggers) if(old.windowScene==scene){exists=YES;break;}
        if(exists) continue;
        CGFloat w=scene.coordinateSpace.bounds.size.width, h=scene.coordinateSpace.bounds.size.height;
        for(NSNumber *x in @[@0,@(w-72)]){
            UIWindow *t=[[UIWindow alloc]initWithWindowScene:scene];
            t.frame=CGRectMake(x.doubleValue,h-44,72,44); t.windowLevel=UIWindowLevelAlert-2;
            t.backgroundColor=UIColor.clearColor; t.rootViewController=[AKRTriggerController new];
            t.userInteractionEnabled=YES; t.hidden=NO; [AKRTriggers addObject:t];
        }
    }
}
static void AKRScheduleTriggerCheck(void) {
    AKRInstallTriggers();
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 2*NSEC_PER_SEC), dispatch_get_main_queue(), ^{ AKRScheduleTriggerCheck(); });
}
%ctor {
    if(![NSBundle.mainBundle.bundleIdentifier isEqualToString:@"com.apple.springboard"])return;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, NSEC_PER_SEC), dispatch_get_main_queue(), ^{ AKRScheduleTriggerCheck(); });
}
