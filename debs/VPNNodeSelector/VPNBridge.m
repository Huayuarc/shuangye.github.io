...
NSArray<NSDictionary *> *VPNLegacyCopyNodes(void) {
    ...
    for (NSUInteger grade = 0; grade < 4; grade++) {
        @try {
            ...
            NSString *activeID = [store respondsToSelector:activeSelector] ? ((id (*)(id, SEL, NSUInteger))objc_msgSend)(store, activeSelector, grade) : nil;
            for (id service in services) {
                NSString *serviceID = [service isKindOfClass:NSString.class] ? service : nil;
                if (!serviceID.length && [service respondsToSelector:NSSelectorFromString(@"serviceID")]) {
                    serviceID = ((id (*)(id, SEL))objc_msgSend)(service, NSSelectorFromString(@"serviceID"));
                }
                if (!serviceID.length || [seen containsObject:serviceID]) continue;
                ...
                NSString *name = nil;
                if ([service respondsToSelector:NSSelectorFromString(@"displayName")]) {
                    name = ((id (*)(id, SEL))objc_msgSend)(service, NSSelectorFromString(@"displayName"));
                }
                ...
                if (!name.length) name = serviceID;
                [seen addObject:serviceID];
                [nodes addObject:@{@"id":serviceID, @"name":name, @"grade":@(grade), @"active":@([activeID isEqualToString:serviceID])}];
            }
        } @catch (__unused NSException *exception) {}
    }
    return nodes.copy;
