...
static id gBackgroundVPNController;
static id gBackgroundRootController;
...
static BOOL VPNConsumePendingRequest(id controller) {
    ...
    for (id specifier in specifiers) {
        ...
        @try {
            ...
            SEL serviceSelector = NSSelectorFromString(@"_setActiveVPNServiceID:");
            if (candidateID.length && [controller respondsToSelector:serviceSelector]) ((void (*)(id, SEL, id))objc_msgSend)(controller, serviceSelector, candidateID);
            SEL activateSelector = NSSelectorFromString(@"activateVPN:");
            if ([controller respondsToSelector:activateSelector]) ((void (*)(id, SEL, id))objc_msgSend)(controller, activateSelector, specifier);
            SEL reloadSelector = NSSelectorFromString(@"updateVPNConfigurationsList");
            if ([controller respondsToSelector:reloadSelector]) ((void (*)(id, SEL))objc_msgSend)(controller, reloadSelector);
        } @catch (__unused NSException *exception) {}
        gApplyingRequest = NO;
...
static id VPNEnsureBackgroundController(void) {
    id current = gVPNController ?: gBackgroundVPNController;
    if (current) return current;
    Class cls = objc_getClass("VPNController");
    if (!cls) return nil;
    @try {
        gBackgroundVPNController = ((id (*)(id, SEL))objc_msgSend)(((id (*)(id, SEL))objc_msgSend)(cls, @selector(alloc)), @selector(init));
        current = gBackgroundVPNController;
        ...
        if (rootClass && [rootClass instancesRespondToSelector:rootInit]) {
            id allocated = ((id (*)(id, SEL))objc_msgSend)(rootClass, @selector(alloc));
            gBackgroundRootController = ((id (*)(id, SEL, id, id))objc_msgSend)(allocated, rootInit, @"Preferences", @"com.apple.Preferences");
            for (NSString *selectorName in @[@"setRootController:", @"setParentController:"]) {
                SEL selector = NSSelectorFromString(selectorName);
                if ([current respondsToSelector:selector]) ((void (*)(id, SEL, id))objc_msgSend)(current, selector, gBackgroundRootController);
            }
        }
        SEL specifiers = NSSelectorFromString(@"specifiers");
        if ([current respondsToSelector:specifiers]) ((id (*)(id, SEL))objc_msgSend)(current, specifiers);
        SEL update = NSSelectorFromString(@"updateVPNConfigurationsList");
        if ([current respondsToSelector:update]) ((void (*)(id, SEL))objc_msgSend)(current, update);
        return current;
    } @catch (__unused NSException *exception) {
        gBackgroundVPNController = nil;
        gBackgroundRootController = nil;
        return nil;
    }
}
...
static void VPNScheduleConsume(id controller) {
    ...
    else controller = VPNEnsureBackgroundController();
    if (controller && !gVPNController) gVPNController = controller;
...
static void VPNHandleSelectionRequest(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
    dispatch_async(dispatch_get_main_queue(), ^{ VPNScheduleConsume(gVPNController ?: VPNEnsureBackgroundController()); });
}
...
__attribute__((constructor)) static void VPNControllerHookInit(void) {
    @autoreleasepool {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *pendingName = VPNSharedCopy(VPNRequestedNameKey);
            NSString *pendingID = VPNSharedCopy(VPNRequestedIdentifierKey);
            if (pendingName.length || pendingID.length) VPNScheduleConsume(VPNEnsureBackgroundController());
        });
