...
- (void)syncControllerVisualState {
    VPNNodeViewController *coordinator = VPNCoordinator(_controller);
    BOOL active = VPNLegacyIsActive() || [coordinator hasActiveManager];
    _selected = active;
    [super refreshState];
...
- (BOOL)isSelected {
    VPNNodeViewController *coordinator = VPNCoordinator(_controller);
    return VPNLegacyIsActive() || [coordinator hasActiveManager];
}
- (void)setSelected:(BOOL)value {
    VPNNodeViewController *coordinator = VPNCoordinator(_controller);
    if ([coordinator stopActiveManagerIfNeeded]) {
        _selected = NO;
    } else {
        BOOL current = VPNLegacyIsActive();
        _selected = !current;
        VPNLegacySetActive(!current);
    }
    [self syncControllerVisualState];
    [self scheduleRefreshBurst];
}
