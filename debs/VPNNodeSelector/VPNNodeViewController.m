...
@interface NETunnelProviderManager (VPNNodeEnumeration)
+ (void)loadAllFromPreferencesWithCompletionHandler:(void (^)(NSArray<NETunnelProviderManager *> *, NSError *))handler;
@end
...
- (void)viewDidLoad {
    ...
    // Startup must stay passive: no NetworkExtension calls while SpringBoard scans modules.
}
...
- (void)reloadManagers {
    [self beginObservingIfNeeded];
    NSUInteger generation = ++self.loadGeneration;
    __weak typeof(self) weakSelf = self;
    [NETunnelProviderManager loadAllFromPreferencesWithCompletionHandler:^(NSArray<NETunnelProviderManager *> *tunnels, NSError *tunnelError) {
        NEVPNManager *personal = NEVPNManager.sharedManager;
        [personal loadFromPreferencesWithCompletionHandler:^(NSError *personalError) {
            dispatch_async(dispatch_get_main_queue(), ^{
                typeof(self) selfRef = weakSelf;
                if (!selfRef || generation != selfRef.loadGeneration) return;
                NSMutableArray<NEVPNManager *> *combined = [NSMutableArray array];
                if (!tunnelError && tunnels.count) [combined addObjectsFromArray:tunnels];
                if (!personalError && personal.protocolConfiguration && personal.localizedDescription.length) {
                    BOOL duplicate = NO;
                    for (NEVPNManager *item in combined) if ([[selfRef nameForManager:item] isEqualToString:[selfRef nameForManager:personal]]) { duplicate = YES; break; }
                    if (!duplicate) [combined addObject:personal];
                }
                selfRef.managers = combined.copy;
