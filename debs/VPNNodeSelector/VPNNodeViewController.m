...
- (NSString *)nameForManager:(NEVPNManager *)manager { return manager.localizedDescription.length ? manager.localizedDescription : @"VPN"; }
...
- (void)nodeTapped:(UIButton *)sender {
    ...
    if (targetIdentifier && VPNSafeSetCurrentIdentifier(targetIdentifier, targetGrade)) {
        self.systemSelectedName = [self nameForManager:target];
    }
