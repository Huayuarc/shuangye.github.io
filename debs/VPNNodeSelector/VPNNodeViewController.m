...
static void VPNWakePreferencesInBackground(void) {
    void *image = dlopen("/System/Library/PrivateFrameworks/SpringBoardServices.framework/SpringBoardServices", RTLD_LAZY | RTLD_LOCAL);
    if (!image) return;
    typedef int (*LaunchWithOptionsFn)(CFStringRef, CFDictionaryRef);
    LaunchWithOptionsFn launch = (LaunchWithOptionsFn)dlsym(image, "SBSLaunchApplicationWithIdentifierAndLaunchOptions");
    if (!launch) return;
    NSMutableDictionary *options = [NSMutableDictionary dictionary];
    CFStringRef *activateSuspended = (CFStringRef *)dlsym(image, "FBSOpenApplicationOptionKeyActivateSuspended");
    CFStringRef *launchSuspended = (CFStringRef *)dlsym(image, "SBSApplicationLaunchOptionLaunchSuspendedKey");
    if (activateSuspended && *activateSuspended) options[(__bridge NSString *)*activateSuspended] = @YES;
    else options[@"__ActivateSuspended"] = @YES;
    if (launchSuspended && *launchSuspended) options[(__bridge NSString *)*launchSuspended] = @NO;
    launch(CFSTR("com.apple.Preferences"), (__bridge CFDictionaryRef)options);
}

static NSString *VPNRequestSystemSelection(NSString *name, NSString *identifier, NSInteger grade) {
    if (!name.length && !identifier.length) return nil;
    ...
    CFPreferencesSetAppValue((__bridge CFStringRef)VPNRequestedIdentifierKey, (__bridge CFStringRef)identifier, (__bridge CFStringRef)VPNSharedDomain);
    CFPreferencesSetAppValue((__bridge CFStringRef)VPNRequestedGradeKey, (__bridge CFNumberRef)@(grade), (__bridge CFStringRef)VPNSharedDomain);
    CFPreferencesSetAppValue((__bridge CFStringRef)VPNRequestNonceKey, (__bridge CFStringRef)nonce, (__bridge CFStringRef)VPNSharedDomain);
    CFPreferencesAppSynchronize((__bridge CFStringRef)VPNSharedDomain);
    VPNWakePreferencesInBackground();
...
@property(nonatomic,strong) NEVPNManager *pendingSelectionManager;
@property(nonatomic,copy) NSString *pendingSelectionNonce;
...
static void VPNControllerSelectionDidChange(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{ [controller handleControllerSelectionChanged]; });
}
...
- (void)beginTargetAfterSystemAck:(NEVPNManager *)target {
    if (!target) return;
    NEVPNManager *active = self.activeManager;
    ...
    if (active && active != target) {
        ...
        [active.connection stopVPNTunnel];
        __weak typeof(self) weakSelf = self;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{ [weakSelf startPendingManagerForGeneration:generation]; });
    } else if (!active) {
        self.pendingManager = target;
...
- (void)enableTargetConfiguration:(NEVPNManager *)target completion:(void (^)(BOOL success))completion {
    if (!target) { if (completion) completion(NO); return; }
    __weak typeof(self) weakSelf = self;
    [target loadFromPreferencesWithCompletionHandler:^(NSError *loadError) {
        dispatch_async(dispatch_get_main_queue(), ^{
            typeof(self) selfRef = weakSelf;
            if (!selfRef) { if (completion) completion(NO); return; }
            @try { target.enabled = YES; } @catch (__unused NSException *exception) {}
            [target saveToPreferencesWithCompletionHandler:^(NSError *saveError) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (saveError) { if (completion) completion(NO); return; }
                    [target loadFromPreferencesWithCompletionHandler:^(NSError *reloadError) {
                        dispatch_async(dispatch_get_main_queue(), ^{ if (completion) completion(reloadError == nil); });
                    }];
                });
            }];
        });
...
- (void)handleControllerSelectionChanged {
    CFPreferencesAppSynchronize((__bridge CFStringRef)VPNSharedDomain);
    NSString *ack = CFBridgingRelease(CFPreferencesCopyAppValue((__bridge CFStringRef)VPNAckNonceKey, (__bridge CFStringRef)VPNSharedDomain));
    NEVPNManager *target = self.pendingSelectionManager;
    BOOL matched = self.pendingSelectionNonce.length && [ack isEqualToString:self.pendingSelectionNonce];
    if (matched && target) {
        self.pendingSelectionManager = nil;
        self.pendingSelectionNonce = nil;
        self.systemSelectedName = VPNSharedSelectedName();
        self.statusLabel.text = @"系统已同步";
        __weak typeof(self) weakSelf = self;
        [target loadFromPreferencesWithCompletionHandler:^(NSError *error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                typeof(self) selfRef = weakSelf;
                if (!selfRef) return;
                if (error) { selfRef.statusLabel.text = @"配置刷新失败"; return; }
                [selfRef beginTargetAfterSystemAck:target];
            });
        }];
    }
...
- (void)nodeTapped:(UIButton *)sender {
    ...
    NSString *targetIdentifierString = [targetIdentifier respondsToSelector:@selector(UUIDString)] ? [targetIdentifier UUIDString] : [targetIdentifier description];
    NSInteger targetGrade = [self gradeForManager:target];
    self.pendingSelectionManager = target;
    self.pendingSelectionNonce = VPNRequestSystemSelection(targetName, targetIdentifierString, targetGrade);
    VPNSafeSetCurrentIdentifier(targetIdentifier, targetGrade);
    self.statusLabel.text = @"同步系统选择…";
    __weak typeof(self) weakSelfEnable = self;
    [self enableTargetConfiguration:target completion:^(BOOL success) {
        typeof(self) selfRef = weakSelfEnable;
        if (!selfRef) return;
        if (success) {
            selfRef.pendingSelectionManager = nil;
            selfRef.pendingSelectionNonce = nil;
            selfRef.systemSelectedName = targetName;
            selfRef.statusLabel.text = @"系统已同步";
            [selfRef beginTargetAfterSystemAck:target];
        } else {
            selfRef.statusLabel.text = @"等待设置同步…";
        }
        [selfRef rebuildRows];
    }];
    UISelectionFeedbackGenerator *feedback = [UISelectionFeedbackGenerator new];
    [feedback selectionChanged];
    NSString *expectedNonce = self.pendingSelectionNonce;
    __weak typeof(self) weakSelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(4.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        typeof(self) selfRef = weakSelf;
        if (!selfRef || ![selfRef.pendingSelectionNonce isEqualToString:expectedNonce]) return;
        selfRef.statusLabel.text = @"打开设置 VPN 后自动同步";
        [selfRef rebuildRows];
    });
    [self rebuildRows];
}
