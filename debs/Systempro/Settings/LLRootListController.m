...
@interface LLRootListController () <UIColorPickerViewControllerDelegate> {
    BOOL _isPickingVPNColor;
}
@end
...
- (void)saveAllPrefs {
	...
	UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"已保存"
		message:[NSString stringWithFormat:@"已写入 %lu 项设置", (unsigned long)defaults.count]
		preferredStyle:UIAlertControllerStyleAlert];
...
- (void)resetAllPrefs {
	...
	UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"恢复默认值？"
		message:@"仅会重置当前分类下的设置，不影响 Systempro 其它功能。"
		preferredStyle:UIAlertControllerStyleAlert];
	[alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
...
// ============================================================================
// UIColorPickerViewControllerDelegate — 用于颜色选择
// ============================================================================

@implementation LLRootListController (ColorPicker)

- (void)chooseVPNStatusBarColor {
    _isPickingVPNColor = YES;
    ...
    CGFloat r = [prefs[@"sb_vpnStatusBarColorRed"] floatValue] / 255.0;
    CGFloat g = [prefs[@"sb_vpnStatusBarColorGreen"] floatValue] / 255.0;
    CGFloat b = [prefs[@"sb_vpnStatusBarColorBlue"] floatValue] / 255.0;
    CGFloat a = [prefs[@"sb_vpnStatusBarColorAlpha"] floatValue];
    if (a == 0) a = 1.0;
    colorPicker.selectedColor = [UIColor colorWithRed:r green:g blue:b alpha:a];

    [self presentViewController:colorPicker animated:YES completion:nil];
}

- (void)chooseRunIndicatorColor {
    _isPickingVPNColor = NO;
    UIColorPickerViewController *colorPicker = [[UIColorPickerViewController alloc] init];
    colorPicker.delegate = self;

    NSDictionary *prefs = [NSDictionary dictionaryWithContentsOfFile:kPrefPath];
    CGFloat r = [prefs[@"sb_runIndicatorColorRed"] floatValue] / 255.0;
    CGFloat g = [prefs[@"sb_runIndicatorColorGreen"] floatValue] / 255.0;
    CGFloat b = [prefs[@"sb_runIndicatorColorBlue"] floatValue] / 255.0;
    CGFloat a = [prefs[@"sb_runIndicatorColorAlpha"] floatValue];
    if (a == 0) a = 1.0;
    colorPicker.selectedColor = [UIColor colorWithRed:r green:g blue:b alpha:a];
...
- (void)colorPickerViewControllerDidSelectColor:(UIColorPickerViewController *)viewController {
    ...
    if (_isPickingVPNColor) {
        prefs[@"sb_vpnStatusBarColorRed"] = @((int)(r * 255));
        prefs[@"sb_vpnStatusBarColorGreen"] = @((int)(g * 255));
        prefs[@"sb_vpnStatusBarColorBlue"] = @((int)(b * 255));
        prefs[@"sb_vpnStatusBarColorAlpha"] = @(a);
    } else {
        prefs[@"sb_runIndicatorColorRed"] = @((int)(r * 255));
        prefs[@"sb_runIndicatorColorGreen"] = @((int)(g * 255));
        prefs[@"sb_runIndicatorColorBlue"] = @((int)(b * 255));
        prefs[@"sb_runIndicatorColorAlpha"] = @(a);
    }
