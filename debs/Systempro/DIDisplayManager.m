...
#define kPrefsPath @"/var/mobile/Library/Preferences/com.huayuarc.systempro.plist"
...
- (void)reloadPrefs {
    ...
    self.enabled = prefs[@"islandEnabled"] ? [prefs[@"islandEnabled"] boolValue] : NO;
    self.reappearDelay = prefs[@"reappearDelay"] ? [prefs[@"reappearDelay"] floatValue] : 1.0;
    // 通知灵动岛偏好
    self.bannerEnabled = prefs[@"notificationEnabled"] ? [prefs[@"notificationEnabled"] boolValue] : NO;
    self.bannerCornerRadius = prefs[@"bannerCornerRadius"] ? [prefs[@"bannerCornerRadius"] floatValue] : 22.0;
    self.bannerDamping = prefs[@"bannerDamping"] ? [prefs[@"bannerDamping"] floatValue] : 0.7;
    self.bannerInitialScale = prefs[@"bannerInitialScale"] ? [prefs[@"bannerInitialScale"] floatValue] : 0.8;
    self.notifDuration = prefs[@"notifDuration"] ? [prefs[@"notifDuration"] floatValue] : 3.0;
    if (self.notifDuration < 1.0) self.notifDuration = 1.0;
    if (self.notifDuration > 30.0) self.notifDuration = 30.0;
    // 双圆角
    self.mediaCornerRadius = prefs[@"mediaCornerRadius"] ? [prefs[@"mediaCornerRadius"] floatValue] : 18.0;
    self.notifCornerRadius = prefs[@"notifCornerRadius"] ? [prefs[@"notifCornerRadius"] floatValue] : 22.0;
