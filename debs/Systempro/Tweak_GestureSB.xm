...
- (UIEdgeInsets)portraitLayoutInsets {
    ...
    NSUInteger const rows = [[(id)self valueForKey:@"_numberOfPortraitRows"] unsignedIntegerValue];
...
%group FixCC134
%hook CCUIStatusBar
-(NSUInteger)leadingState { return 1; }
-(NSUInteger)trailingState { return 1; }
%end
%end
...
%group NoCCStatusBar
%hook CCUIStatusBar
-(id)initWithFrame:(CGRect)arg1 { return nil; }
%end
...
- (void)dismissAnimated:(bool)arg1 withCompletionHandler:(id)arg2 {
    if(!isCCAnimation) arg1 = 0;
    %orig(arg1, arg2);
}
%end

// ===== CC StatusBar Padding =====
%group CCStatusBar
%hook CCUIHeaderPocketView
...
- (UIColor *)_batteryColor {
    if (batteryColorMode == 1) {
        ...
        if(chargePercent >= 0.95) return [UIColor colorWithRed:0.08 green:0.71 blue:1.00 alpha:1.0];
        else if(chargePercent >= 0.90) return [UIColor colorWithRed:0.43 green:0.78 blue:1.00 alpha:1.0];
        else if(chargePercent >= 0.85) return [UIColor colorWithRed:0.59 green:0.91 blue:1.00 alpha:1.0];
        else if(chargePercent >= 0.80) return [UIColor colorWithRed:0.47 green:1.00 blue:0.99 alpha:1.0];
        else if(chargePercent >= 0.75) return [UIColor colorWithRed:0.38 green:1.00 blue:0.79 alpha:1.0];
        else if(chargePercent >= 0.70) return [UIColor colorWithRed:0.40 green:0.90 blue:0.71 alpha:1.0];
        else if(chargePercent >= 0.65) return [UIColor colorWithRed:0.26 green:0.96 blue:0.54 alpha:1.0];
        else if(chargePercent >= 0.60) return [UIColor colorWithRed:0.26 green:1.00 blue:0.45 alpha:1.0];
        else if(chargePercent >= 0.55) return [UIColor colorWithRed:0.56 green:0.87 blue:0.21 alpha:1.0];
        else if(chargePercent >= 0.50) return [UIColor colorWithRed:0.84 green:0.95 blue:0.22 alpha:1.0];
        else if(chargePercent >= 0.45) return [UIColor colorWithRed:0.93 green:0.96 blue:0.21 alpha:1.0];
        else if(chargePercent >= 0.40) return [UIColor colorWithRed:0.97 green:0.92 blue:0.20 alpha:1.0];
        else if(chargePercent >= 0.35) return [UIColor colorWithRed:0.95 green:0.77 blue:0.19 alpha:1.0];
        else if(chargePercent >= 0.30) return [UIColor colorWithRed:1.00 green:0.76 blue:0.18 alpha:1.0];
        else if(chargePercent >= 0.25) return [UIColor colorWithRed:0.98 green:0.68 blue:0.19 alpha:1.0];
        else if(chargePercent >= 0.20) return [UIColor colorWithRed:0.96 green:0.57 blue:0.20 alpha:1.0];
        else if(chargePercent >= 0.15) return [UIColor colorWithRed:0.95 green:0.48 blue:0.20 alpha:1.0];
        else if(chargePercent >= 0.10) return [UIColor colorWithRed:0.93 green:0.39 blue:0.21 alpha:1.0];
        else if(chargePercent >= 0.5) return [UIColor colorWithRed:0.91 green:0.31 blue:0.22 alpha:1.0];
        else return [UIColor colorWithRed:0.89 green:0.24 blue:0.22 alpha:1.0];
    }
...
-(void)setShowsInlineChargingIndicator:(BOOL)arg1 {
    if(isHideChargingIndicator) arg1 = NO;
    %orig(arg1);
}
%end
...
- (BOOL)isEnabled { return isHomeBarSB; }
%end
%end

// ===== Edge Protect + HomeBar Auto Hide =====
...
%group NoHomeBarLS
%hook CSTeachableMomentsContainerView
-(void)setHomeAffordanceContainerView:(UIView *)arg1 { return; }
-(void)setHomeAffordanceView:(UIView *)arg1 { return; }
%end
%end
...
-(void)setHeight:(double)arg1 { return %orig(0); }
%end
%end
...
-(void)setBackNavigationEntry:(id)arg1 { return; }
%end
%end
...
%group HomeBarCustom
%hook MTLumaDodgePillSettings
-(void)setMinWidth:(double)arg1 { arg1 = HomeBarWidth; %orig; }
-(void)setMaxWidth:(double)arg1 { arg1 = HomeBarWidth; %orig; }
-(void)setCornerRadius:(double)arg1 { arg1 = HomeBarRadius; %orig; }
-(void)setHeight:(double)arg1 { arg1 = HomeBarHeight; %orig; }
%end
%end
...
+ (BOOL)isFloatingDockSupported { return YES; }
%end

%hook SBFloatingDockSuggestionsModel
-(void)_setRecentsEnabled:(BOOL)arg1 { return %orig(isRecentApp); }
%end
...
-(NSInteger)medusaCapabilities { return 2; }
%end

%hook SBMainWorkspace
-(BOOL)isMedusaEnabled { return YES; }
%end
...
-(BOOL)isMedusaCapable {
    ...
    if ([orientations indexOfObject:@"UIInterfaceOrientationPortrait"] == NSNotFound) return NO;
    return YES;
}
-(BOOL)mainSceneWantsFullscreen { return screenMode; }
%end
%end
...
-(void)setSwitcherStyle:(NSInteger)arg1 { return %orig(2); }
%end
%end

// ===== No icon Fly =====
%hook CSCoverSheetTransitionSettings
-(BOOL)iconsFlyIn { return !isNoIconsFly; }
%end

// ===== Fast Open App =====
%group FastOpenApp
%hook SBFFluidBehaviorSettings
-(void)setResponse:(double)arg1 { %orig(0.1); }
%end
%end
...
%group NoNomalDockBG
@interface SBDockView : UIView { UIView *_backgroundView; }
@end
%hook SBDockView
...
@interface SBFloatingDockPlatterView : UIView { UIView *_backgroundView; }
@end
...
- (NSUInteger)numberOfPortraitColumns {
    NSUInteger rows = [[(id)self valueForKey:@"_numberOfPortraitRows"] unsignedIntegerValue];
    if (rows==1) {
...
-(CGPoint)originForIconAtCoordinate:(struct SBIconCoordinate)arg1 numberOfIcons:(NSInteger)arg2 {
    if (arg2==5) {
        struct SBIconCoordinate cor1; cor1.row = 1; cor1.col = 1;
        struct SBIconCoordinate cor2; cor2.row = 1; cor2.col = 2;
        struct SBIconCoordinate cor3; cor3.row = 1; cor3.col = 3;
        struct SBIconCoordinate cor4; cor4.row = 1; cor4.col = 4;
        ...
        if (arg1.col == 1) return CGPointMake(newIcon1X, defaultY);
        else if (arg1.col == 2) return CGPointMake(newIcon2X, defaultY);
        else if (arg1.col == 3) return CGPointMake(newIcon3X, defaultY);
        else if (arg1.col == 4) return CGPointMake(newIcon4X, defaultY);
        else return CGPointMake(newIcon5X, defaultY);
    }
    return %orig;
}
%end
...
-(id)initWithFrame:(CGRect)arg1 {
    ...
    else if (screenWidth <= 375) offset = 35;
    else if (screenWidth <= 414) offset = 28;
    return %orig;
}

-(void)layoutSubviews {
    %orig;
    UIView* timeView = [(id)self valueForKey:@"_timeLabel"];
    UIView* dateSubtitleView = [(id)self valueForKey:@"_dateSubtitleView"];
    UIView* customSubtitleView = [(id)self valueForKey:@"_customSubtitleView"];
...
-(void)handleBiometricEvent:(NSInteger)arg1 {
    %orig;
    if (arg1 == 4) {
        SBDashBoardBiometricUnlockController* biometricUnlockController = [(id)self valueForKey:@"_biometricUnlockController"];
        SBDashBoardMesaUnlockBehaviorConfiguration* unlockBehavior = [(id)biometricUnlockController valueForKey:@"_biometricUnlockBehaviorConfiguration"];
...
-(id)init {
    id r = %orig;
    [r setValue:@NO forKey:@"_hasMesaHardware"];
    [r setValue:@YES forKey:@"_hasPearlHardware"];
    return r;
}
%end
%end
...
-(void)setTextColor:(id)arg1 { %orig([UIColor clearColor]); }
%end

%hook SBFolderBackgroundView
- (id)initWithFrame:(struct CGRect)arg1 { return NULL; }
%end
...
%ctor {
    @autoreleasepool {
        ...
        if (enabled) {
            ...
            // Home Bar
            if (isHomeBarAutoHide) isHomeBarSB = YES;
            if (!isHomeBarLS) {
                %init(NoHomeBarLS);
