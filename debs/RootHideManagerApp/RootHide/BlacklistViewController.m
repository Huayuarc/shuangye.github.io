...
static void RHSetChoicyModeForApp(NSString *bundleID, NSInteger mode) {
    ...
    if (mode == 2) {
        app[@"tweakInjectionDisabled"] = @YES;
    } else {
...
// ---- Andromeda integration: strict mode additionally drives Andromeda's ---- //
// per-app detection bypass so its comprehensive hooks (filesystem/dyld/       //
// antidebug/devicecheck/appattest/urlscheme/mobilegestalt/sandbox/syscall)   //
// apply only to strictly shielded apps, not globally.                        //
#define RH_ANDROMEDA_PREFS_JB "/var/jb/var/mobile/Library/Preferences/com.andromeda.bypass.plist"
#define RH_ANDROMEDA_PREFS_STD "/var/mobile/Library/Preferences/com.andromeda.bypass.plist"
#define RH_ANDROMEDA_NOTIFY "com.andromeda.bypass/settingsChanged"

static NSArray *RHAndromedaRecommendedKeys(NSString *bundleID) {
...
static NSString *RHAndromedaPrefsPath(void) {
    if ([NSFileManager.defaultManager fileExistsAtPath:@RH_ANDROMEDA_PREFS_JB]) return @RH_ANDROMEDA_PREFS_JB;
    if ([NSFileManager.defaultManager fileExistsAtPath:@RH_ANDROMEDA_PREFS_STD]) return @RH_ANDROMEDA_PREFS_STD;
    // Default to the /var/jb variant to match Andromeda's baked-in path; created when written.
    return @RH_ANDROMEDA_PREFS_JB;
}

static void RHSetAndromedaForApp(NSString *bundleID, BOOL enable) {
    if (!bundleID.length) return;
    NSString *path = RHAndromedaPrefsPath();
    NSMutableDictionary *prefs = [[NSDictionary dictionaryWithContentsOfFile:path] mutableCopy] ?: [NSMutableDictionary new];
    NSMutableDictionary *perApp = [prefs[@"PerApp"] mutableCopy] ?: [NSMutableDictionary new];
    if (enable) {
        NSMutableDictionary *cfg = [perApp[bundleID] mutableCopy] ?: [NSMutableDictionary new];
        cfg[@"enabled"] = @YES;
        for (NSString *key in RHAndromedaRecommendedKeys(bundleID)) cfg[key] = @YES;
        perApp[bundleID] = cfg;
    } else {
        [perApp removeObjectForKey:bundleID];
    }
    prefs[@"PerApp"] = perApp;
    [prefs writeToFile:path atomically:YES];
    CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR(RH_ANDROMEDA_NOTIFY), NULL, NULL, YES);
}
...
static void RHApplyShieldMode(AppInfo *app, NSInteger mode) {
    ...
    // Drive Andromeda's per-app bypass on the main bundle only (extension processes
    // share the container but Andromeda gates on the main bundle id).
    RHSetAndromedaForApp(app.bundleIdentifier, mode == 2);
    ...
    for (AppInfo *app in applications) {
        ...
        if (mode == 2) {
            for (NSString *identifier in RHIdentifiersForApp(app)) {
                ...
                if (!RHChoicyDisabledForApp(identifier)) RHSetChoicyModeForApp(identifier, 2);
            }
            RHSetAndromedaForApp(app.bundleIdentifier, YES);
        } else {
