...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if(![NSFileManager.defaultManager fileExistsAtPath:roothideDir]) {
        ...
        if(![NSFileManager.defaultManager createDirectoryAtPath:roothideDir withIntermediateDirectories:YES attributes:attr error:&startupError]) {
            NSLog(@"RootHide directory create failed: %@", startupError);
        }
    }

    NSString* jsonPath = [NSBundle.mainBundle pathForResource:@"varCleanRules" ofType:@"json"];
    if(!jsonPath) jsonPath = [NSBundle.mainBundle pathForResource:@"VarCleanRules" ofType:@"json"];
    NSLog(@"jsonPath=%@", jsonPath);
    NSData* jsonData = jsonPath ? [NSData dataWithContentsOfFile:jsonPath] : nil;
    NSDictionary *rules = nil;
    if(jsonData.length > 0) {
        uLong hash = crc32(0, jsonData.bytes, (uInt)jsonData.length);
        NSLog(@"hash=%lx expected=%lx", hash, (unsigned long)VARCLEANRULESHASH);
        NSError *jsonError = nil;
        rules = [NSJSONSerialization JSONObjectWithCommentedData:jsonData options:NSJSONReadingMutableContainers error:&jsonError];
        if(jsonError || ![rules isKindOfClass:NSDictionary.class]) {
            NSLog(@"json error=%@", jsonError);
            rules = nil;
        }
    } else {
        NSLog(@"RootHide rules resource is missing or unreadable");
    }
    ...
    if(rules) {
        if([NSFileManager.defaultManager fileExistsAtPath:rulesFilePath]) {
            [NSFileManager.defaultManager removeItemAtPath:rulesFilePath error:&startupError];
            if(startupError) NSLog(@"old rules removal failed: %@", startupError);
        }
        if(![rules writeToFile:rulesFilePath atomically:YES]) NSLog(@"default rules write failed: %@", rulesFilePath);
    } else if(![NSFileManager.defaultManager fileExistsAtPath:rulesFilePath]) {
        [@{} writeToFile:rulesFilePath atomically:YES];
    }
    ...
    if(![NSFileManager.defaultManager fileExistsAtPath:customedRulesFilePath]) {
        if(![@{} writeToFile:customedRulesFilePath atomically:YES]) NSLog(@"custom rules template write failed: %@", customedRulesFilePath);
    }
    ...
    DeepCleanController *deepCleanController = [DeepCleanController sharedInstance];
    ...
    deepCleanController.tabBarItem = [[UITabBarItem alloc] initWithTitle:NSLocalizedString(@"Deep Clean",@"") image:[UIImage systemImageNamed:@"sparkles"] tag:2];
    setController.tabBarItem = [[UITabBarItem alloc] initWithTitle:NSLocalizedString(@"Setting",@"") image:[UIImage systemImageNamed:@"gearshape"] tag:3];
    ...
    UINavigationController *deepCleanNavigationController = [[UINavigationController alloc] initWithRootViewController:deepCleanController];
    ...
    tabBarController.viewControllers = @[listNavigationController, cleanNavigationController, deepCleanNavigationController, setNavigationController];
